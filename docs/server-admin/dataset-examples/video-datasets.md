...
# Searching for and Displaying Videos in ERDDAP™

This document shows how to display videos in ERDDAP™, as well as
creating a dataset that provides for searching the video metadata to
find the videos that meet the selected search criteria.

The essential components needed to set this up are first, videos that
are in a format that can be displayed in a web browser (ERDDAP™ itself
does not display the video, it only provides a link), and second, a csv
file (or a similar table based format such as parquet) that contain the
metadata relevant to the videos.

Having an appropriate video format is key. In the example below the
videos originally were an old Window’s format that even Windows browsers
could not display out of the box, and required adding a plugin. Old
formats often are very large files. The Handbrake application can be
used to convert the videos, which is what was done.

## Setup

The first step is to create an “EDDTableFromFileNames” dataset so that
there is an URL associated with each video that is accessible through
the ERDDAP™ “files” command. While an example of this dataset is given
below, the best way to generate this xml for your videos is to use
GenerateDatasetsXml.sh and point it to the directory with the videos (in
general the best way to add new datasets to ERDDAP™ is to use
GenerateDatasetsXml.sh which will get you most of the way there, and
then edit that for the final result).

The second step is to create an EDDTableFromAsciiFiles (or similar)
dataset that contains the metadata that is to be used in the video
search. An example is given below. For this to work there must be one
column that contains a link to the video, in the example it is the
variable “videoFile”. This variable can just contain the full URL
created in the first step, but then the file is not portable, such as if
you want to test on one machine and then actually serve on another
machine.

You can make the file portable by making the entry in “videoFile” to be
the relative path to the video, in the example below one such path is:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Then you can add the “fileAccessBaseUrl” attribute which appends that
URL to the path in the column. In the example:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Then if you switch servers you only need to change that one attribute in
the xml snippet, you do not need to edit the csv file.

Once done, you have a list of the videos accessable as files:
...
a simple search set up by the “EDDTableFromFileNames” dataset:
...
The default options in the last URL produce the URL:
...
and that produces a table with the last column:
...
Each of the entries in the response provides a link to a video, such as
if you mouse over the first one you get:
...
and if you click on it in the htmlTable the video displays in your
browser.
