...
### **Development Environment** {#development-environment}
  ...
  - We currently use Adoptium's javac jdk-25.0.1+8 to compile gov.noaa.pfeg.coastwatch.TestAll (it has links to a few classes that wouldn't be compiled otherwise) and run the tests. For security reasons, it is almost always best to use the latest versions of Java 25 and Tomcat 10.
  ...
  - In a few cases, we use 3rd party source code instead of .jar files (notably for DODS) and have modified them slightly to avoid problems compiling with Java 25. We have often made other slight modifications (notably to DODS) for other reasons.
