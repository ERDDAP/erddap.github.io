...
The reason this produces an error is that 'xarray' uses OPeNDAP (https://www.opendap.org) as its protocol for remote access, and while the ERDDAP™ syntax is based on OPeNDAP syntax,  and an ERDDAP™ server can also act as an OPeNDAP server,  there are differences in how this is done for the two services.  (See for example https://coastwatch.pfeg.noaa.gov/erddap/griddap/documentation.html#opendapLibraries).
