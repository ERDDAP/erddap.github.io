...
Ano.
 ...
 ERDDAP™ je navržen tak, aby běžel v rámci Tomcat, který může být provozován lokálně nebo v cloudovém prostředí. Oficiální Docker obrázek je k dispozici na [Docker Hub](https://hub.docker.com/r/erddap/erddap) . The `erddap:alfa-latest` tag je postaven na nejnovějších změnách (něco jako 'noční' vydání, [Nejpozdější alfa podrobnosti](https://hub.docker.com/layers/erddap/erddap/alpha-latest/) ) , zatímco `erddap:poslední` je poslední testovaná verze ( [nejnovější podrobnosti](https://hub.docker.com/layers/erddap/erddap/latest/) ) . Můžete také procházet GitHub kontejnerový registr vydání na [GitHub balíčky](https://github.com/ERDDAP/erddap/pkgs/container/erddap) . Více o použití si můžete přečíst ERDDAP™ s [Docker](https://github.com/ERDDAP/erddap/blob/main/DOCKER.md) .

Pro nasazení Kubernetes viz nové nasazení-kubernetes [Dokumentace](https://erddap.github.io/docs/server-admin/admin-tips/deploy-kubernetes) .
