...
# Hledám a zobrazuji videa v ERDDAP™ 

Tento dokument ukazuje, jak zobrazit videa v ERDDAP™ , jakož i
vytvoření datového souboru, který poskytuje vyhledávání metadat videa do
najít videa, která splňují vybraná vyhledávací kritéria.

Základní komponenty potřebné pro nastavení jsou první, videa, že
jsou ve formátu, který lze zobrazit ve webovém prohlížeči ( ERDDAP™ sám
nezobrazuje video, poskytuje pouze odkaz), a za druhé, csv
soubor (nebo podobný stolní formát, jako je parket) které obsahují
metadata týkající se videí.

Klíčem je mít vhodný video formát. V následujícím příkladu
videa byla původně starým formátem Windows, který dokonce Windows prohlížeče
nemohl zobrazit z krabice, a požadované přidání plugin. Starý
formáty jsou často velmi velké soubory. Použití ruční brzdy může být
Používá se k převodu videa, což je to, co bylo provedeno.

## Nastavení

Prvním krokem je vytvoření datového souboru "EDDTableFromFileNames" tak, aby
existuje URL spojené s každým videem, které je přístupné prostřednictvím
vá ERDDAP™ Příkazy souborů. Zatímco příklad tohoto souboru je uveden
Níže, nejlepší způsob, jak vytvořit tento xml pro vaše videa je použít
Generovat soubory dat Xml.sh a nasměrovat ji do adresáře s videa (v
obecný nejlepší způsob, jak přidat nové soubory dat do ERDDAP™ s
Generovat soubory dat Xml.sh, který vás dostane většinu cesty tam, a
pak editovat, že pro konečný výsledek).

Druhým krokem je vytvořit EDDTableFromAsciiFiles (nebo podobné) 
Soubor údajů, který obsahuje metadata, která mají být použita ve videu
Hledat. Níže je uveden příklad. Aby to fungovalo, musí existovat jeden.
sloupec, který obsahuje odkaz na video, v příkladu je
proměnlivá  Tato proměnná může pouze obsahovat plnou URL
vytvořen v prvním kroku, ale pak soubor není přenosný, jako kdyby
Chcete testovat na jednom stroji a pak skutečně sloužit na jiném
Stroj.

Můžete udělat soubor přenosný tím, že se záznam vvideoFile bude
relativní cesta k videu, v příkladu pod jedním takovým způsobem je:

2010_Piggy_Bank_audio_strepped/0001_20100627_144631.mp4

Pak můžete přidat atribut AccessBaseUrl appends that
URL na cestu ve sloupci. V příkladu:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Pak pokud přepnete servery, stačí změnit jeden atribut v
xml snippet, nemusíte upravovat soubor csv.

Po dokončení máte seznam videí přístupných jako soubory:
...
jednoduché vyhledávání vytvořené datovým souborem "EDDTableFromFileNames":
...
a vyhledávání metadat na adrese:
...
Výchozí volby v poslední URL vytvoří URL:

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black Treska tmavá](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 

a která vytváří tabulku s posledním sloupcem:
...
Každý záznam v odpovědi poskytuje odkaz na video, jako je
Pokud si myš přes první dostanete:
...
a pokud na něj kliknete v html Tabulka video displeje ve vašem
prohlížeč.
