...
* Asigurați-vă că fișierele de comparație a imaginii sunt disponibile (Acest lucru ar putea însemna funcționare `mvn verifica` , dacă doriți să accelerați această restricție la doar grupul ImageComparison deși rețineți că încă necesită efectuarea de teste Jetty) 
...
* EDStatic.java a stabilit dezvoltarea Mod de falsificare, modificarea numărului versiunii și specificarea datei de lansare.
* Construieşte.
```
mvn clean
mvn compile
...
Include mesajul atunci când se spune despre o nouă versiune.

Procedura standard este:
* Încărcaţi fişierul .war la Coastwatch \\[ tomcat \\] /content/erddap/
...
* Actualizează numărul versiunii din fișierul docusaurus.config.ts (în secțiunea subsol) .
...
* Face o cerere de tragere și fuziona modificările.
* Desfășoară site-ul de documentare (a se vedea readme) .

## Asigură-te că alte repo sunt actualizate dacă este necesar
...
## Anunță utilizatorii
Prima notificare a utilizatorilor care au solicitat modificări (sau ale căror insecte au fost reparate) . Oferă-le timp pentru a verifica modificările și/sau ridica probleme.
...
Unele dintre schimbări sunt schimbări pe care le-ai sugerat. Vă mulţumesc foarte mult pentru sugestii. Caută numele tău în lista de modificări pentru a vedea detaliile. Ar fi minunat dacă ați putea încerca noile caracteristici în curând, înainte de a anunța această nouă versiune la un public mai larg.

Dacă sunteţi un ERDDAP administrator, instrucțiunile de modernizare sunt la
