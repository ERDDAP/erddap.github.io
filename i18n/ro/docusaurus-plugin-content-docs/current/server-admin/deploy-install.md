...
liber (ca în bere și vorbire) versiunea Java 25 care oferă suport pe termen lung (upgrade-uri gratuite pentru mulți ani trecut eliberarea inițială) .
...
Vă recomandăm să-l despachetaţi. `/usr/local` .
...
Dacă îl descărcați, [descărcare "Core" "tar" .gz " Distribuţia Tomcat](https://tomcat.apache.org/download-10.cgi) şi să-l despachetăm `/Library/Tomcat` .
     * Pe Windows, puteți [descarca distributia "Core" "zip" Tomcat](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Pentru dezvoltare, folosim distribuţia "Core" "zip." Facem o `/programuri` directorul și despachetați-l acolo.) 
Sau puteți descărca distribuția "Core" "64-bit Windows zip," care include mai multe caracteristici.
Dacă distribuția este un instalator Windows, va pune, probabil, Tomcat în, de exemplu, `/Program Files/apache-tomcat-10.0.23` .
             
### server.xml{#serverxml} 

*  [server.xml](#serverxml) - În `tomcat/conf/server.xml` fișier, există două modificări pe care ar trebui să facă pentru fiecare dintre cele două ` <Connector> ` etichete
...
Astfel, doar super-utilizatorul poate trece la acţiune ca utilizator `tomcat` . Acest lucru face imposibil pentru hackeri să se logheze la server ca utilizator `tomcat` .
Și, în orice caz, ar trebui să-l facă astfel încât `tomcat` utilizatorul are permisiuni foarte limitate pe sistemul de fişiere al serverului (citeste+write+execte privilegii
pentru `apache-tomcat` arbore director și ` <bigParentDirectory> ` și privilegii de citire numai pentru directoarele cu date care ERDDAP™ are acces la).
  ...
  * Puteți trece la lucrul ca utilizator `tomcat` prin utilizarea comenzii
    ...
    * Puteți opri lucrul ca Tomcat utilizator prin utilizarea comenzii
    ...
    exit
    ````
    * Face cele mai multe dintre restul Tomcat și ERDDAP™ instrucțiuni de configurare ca utilizator `tomcat` . Mai târziu, executați `Pornire.sh` şi `Oprire. sh` scripturi ca utilizator `tomcat` 
...
* Pe Linux şi Macs:
Creează un fișier `Tomcat/bin/setenv.sh`   (sau în Red Hat Enterprise Linux \\[ RHEL \\] Editează `~tomcat/conf/tomcat10.conf` ) pentru a seta variabilele de mediu Tomcat lui.
...
* Pe Windows:
Creează un fișier `tomcat\bin\\setenv.bat` pentru a seta variabilele de mediu Tomcat lui.
...
Cu 32 biți Java , chiar și cu o memorie fizică abundentă, Tomcat și Java nu va rula dacă încercați să setați `-Xmx` mult peste 1500M (1200M pe unele calculatoare) .
...
sau în `Big ParentDirectory/logs/ [log.txt](/docs/server-admin/additional-information#log) ` fișier ( `Big ParentDirectory` este specificat în [setup.xml](#setupxml) ) .
...
Colecţia pentru că a rămas fără spaţiu în Eden (secţiunea Java morman pentru obiecte foarte tinere) . De obicei, mesajul vă arată
   `memoryUsefore-&gt;memoryUseAfter` . Dacă cele două numere sunt apropiate, înseamnă că colecţia de gunoi nu a fost productivă.
...
*  [La Linux şi Macs, schimbaţi permisiunile.](#permissions) dintre toate `*.sh` fișiere în `Tomcat/bin/` să fie executabil de către proprietar:
  ```
  chmod +x *.sh
  ```
...
3.   [Configurați `Tomcat/content/erddap` fișiere de configurare.](#erddap-content) 
...
şi să-l desfac în `tomcat` director, creare `Tomcat/content/erddap` .
...
numit ` ERDDAP _Numele valorii` înainte de rulare ERDDAP™ . De exemplu, utilizarea ` ERDDAP _baseUrl` suprascrie ` <baseUrl> ` valoare.
...
*  (Puţin probabil.) Acum sau (puţin mai probabil) în viitor, dacă doriți să modificați fișierul CSS al lui Erddap, copiați
   `tomcat/content/erddap/images/erddapStart2.css` la `tomcat/content/erddap/images/erddap2.css` şi apoi să facă schimbări.
Modificări ale `erddap2.css` să aibă efect numai atunci când ERDDAP™ este repornit și adesea solicită utilizatorului să curețe fișierele cache ale browser-ului.
     
...
2. Modificare ` <VirtualHost> ` etichetă (dacă există una) , sau adăugați unul la sfârșitul fișierului:
```
<VirtualHost *:80>
...
On Linux, use `ps -ef | grep tomcat` înainte şi după `Oprire. sh` pentru a vă asigura că procesul Tomcat sa oprit.
...
* Schimbă bannerul care apare în vârful tuturor ERDDAP™ Pagini .html prin editarea ` <startBodyHtml5> ` etichetă în dumneavoastră ` datasets.xml ` Dosar.
...
* Modificarea informațiilor pe partea stângă a paginii de start prin editarea ` <theShortDescriptionHtml> ` etichetă în dumneavoastră ` datasets.xml ` Dosar.
(Dacă nu există unul, copiați implicit de la ERDDAP™ 's `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` fișier
în ` datasets.xml ` și editați-l.) De exemplu, ai putea:
