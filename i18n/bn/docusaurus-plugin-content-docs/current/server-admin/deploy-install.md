...
যে ডিস্কটি ডিজাইন করা হয়েছে Java ২১ বা নতুন। নীচে, টম্যাট ডিরেক্টরিকে 'অ্যাপ্যাট্যাক' হিসেবে উল্লেখ করা হবে ।
...
আমরা সুপারিশ করছি এটিকে 'প্রাচ্য/local/local/cu' তে পুনরায় চালু করতে।
     * ম্যাক এ টম্যাট সম্ভবত ইতোমধ্যে 'লিব্রিসি/টোমি্যাটসি' নামে ইন্সটল করা হয়েছে, কিন্তু টমকে ১০ এর সাম্প্রতিক সংস্করণে এটি আপডেট করা উচিত।
ডাউনলোড করলে, ["কোর" ডাউনলোড করুন .gz "ম্যাটক ডিস্ট্রিবিউশন](https://tomcat.apache.org/download-10.cgi) এবং এটি 'Liberary/tmccoice' এ সংরক্ষণ করুন।
...
ডিস্ট্রিবিউশনটি যদি একটি উইন্ডোজ ইনস্টলার হয়ে থাকে, তাহলে সম্ভবত এটি টমকে ব্যবহার করবে। উদাহরণস্বরূপ, 'Prog-topic/cotif-ex3.
             
### সার্ভারের সংস্করণ{#serverxml} 

*  [সার্ভারের সংস্করণ](#serverxml) - 'pidcat/conf/server/.xml ফাইলে দুটি পরিবর্তন রয়েছে, যে দুটো মান 's.conf' করা উচিত <Connector> ট্যাগ
   ('&lt;সংযোগ পোর্ট=৮০' এর জন্য একটি এবং 'crequerort পোর্ট=2e=28435 এর জন্য) . .
   1.  (প্রস্তাবিত) '%s' সংযোগটিকে প্রসারিত করুন, সম্ভবত ৩০,০০০ এর মধ্যে (মিলি-সেকেন্ড, ৫ মিনিট) . .
   2.  (প্রস্তাবিত) একটি নতুন পরামিতি যোগ করুন: 'reselfed'=[[[[]] | "অসি। এটি ঐচ্ছিক এবং একটু কম নিরাপদ,
...
* প্রসঙ্গ অনুযায়ী প্রদর্শন (_V) --- রিসোর্স ক্যাশে - 'gnome.xml.in/inter.xml' - এর পূর্বে </Context> ট্যাগ পরিবর্তন করুন
...
* লিনাক্স কম্পিউটারের মধ্যে, PACTE-র বৈশিষ্ট্য পরিবর্তন করুন
  ...
  * ঠিকানা তালিকা পরিবর্তন করো http dconf ফাইল (সাধারণত/etc/defaults-এ http dconf/conf 'আ) :
    * বর্তমান স্কিম পরিবর্তন করুন <Timeout> বৈশিষ্ট্য নির্ধারণ করুন (ফাইলের শেষে যোগ করুন অথবা সমাপ্ত করুন) ৩৬০০ থেকে (সেকেন্ড) ডিফল্ট ৬০ বা ১২০ সেকেন্ডের পরিবর্তে।
    * বর্তমান স্কিম পরিবর্তন করুন <ProxyTimeout> বৈশিষ্ট্য নির্ধারণ করুন (ফাইলের শেষে যোগ করুন অথবা সমাপ্ত করুন) ৩৬০০ থেকে (সেকেন্ড) ডিফল্ট ৬০ বা ১২০ সেকেন্ডের পরিবর্তে।
  * শিক্ষা/আনরিসেট - ভাল 'আ (কিন্তু কখনো কখনো এটা ভিন্ন ডিরেক্টরিতে) . .
...
আপনার টম্যাট ইনস্টলেশন, বিশেষ করে পাবলিক সার্ভার জন্য.
         
* পাবলিকের জন্য ERDDAP™ লিনাক্স এবং Macs-র ইনস্টলেশন ইনস্টল করাই সবচেয়ে ভাল (প্রোগ্রাম) ব্যবহারকারীর নামের মত 'আ
...
যাই হোক, আপনাকে এটা করতে হবে যাতে আপনার ব্যবহারকারী সার্ভারের ফাইল-সিস্টেমে খুবই সীমিত অনুমতি রয়েছে (পড়ুন++PECox বিশেষ অধিকার)
'papick-my ডিরেক্টরির জন্য ট্রি ও 'ectty' <bigParentDirectory> শুধুমাত্র তথ্য বিশিষ্ট ও DND-র ক্ষেত্রে প্রযোজ্য ERDDAP™ ব্যবহারের প্রয়োজন।
  * আপনি 'type' অ্যাকাউন্ট তৈরি করতে পারেন (পাসওয়ার্ড উল্লেখ করা হয়নি) কমান্ড ব্যবহার করে:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * ব্যবহারকারী রূপে কর্ম সঞ্চালন করতে ইচ্ছুক কমান্ড ব্যবহার করে
    ```
    sudo su - tomcat
    ```
    ...
    * কমান্ড ব্যবহার করে ব্যবহারকারীরূপে কাজ বন্ধ করতে পারবেন
    ...
    exit
    ````
    * বাকি সব টম্যাট আর বাকি ERDDAP™ ব্যবহারকারী যেভাবে নির্দেশাবলী তৈরি করো পরে 'startupd.s.sh এবং 'ssh' স্ক্রিপ্ট সঞ্চালন করা হবে 'আ
লগ-ইন করার উদ্দেশ্যে চিহ্নিত টম্যাট ফাইল লিখতে ইচ্ছুক।
    * '#একো-টমক্যাট' ডিরেক্টরি থেকে টমকে অপসারণ করার পর:
...
'filet/bin/interv.sh নির্মাণ করুন 'আ (বা রেড হ্যাট এন্টারপ্রাইজ লিনাক্সে \\[ লিল \\] সম্পাদনা/config.com/conf/default-এ ব্যবহৃত 'আ) টমকের এনভায়রনমেন্ট ভেরিয়েবল নির্ধারণ করতে.
এই ফাইলটি ব্যবহৃত হবে 'palt/bin/ smb. s.sh. ও 'ssh.' দ্বারা ব্যবহৃত ফাইল ব্যবহার করা হবে। ফাইলটিতে কিছু ফাইল থাকার কথা:
  ```
  ...
  ```
   ...
   (যদি আপনি পূর্বে 'Jeve_OSE' ধার্য করেন, আপনি তা সরিয়ে ফেলতে পারেন।) 
ম্যাকস এ, আপনাকে সম্ভবত ‘JaVASV_HOMEা' নির্ধারণ করতে হবে না।

* উইন্ডো:
টমকের এনভায়রনমেন্ট ভেরিয়েবল নির্ধারণ করার জন্য একটি 'ptcat.com\\ t\\ t\\ t
এই ফাইলটি ব্যবহার করবে 'qt.com\\ @ info &gt; shutdown.bat স্থগিত রাখো
...
৩২ বিট সহ Java এমনকি প্রচুর শারীরিক মেমরি, টমাট এবং Java যদি আপনি ১৫০০ এর উপরে mXXdir সেট করার চেষ্টা করেন তাহলে এটি সঞ্চালিত হবে না (কম্পিউটারের মধ্যে 1২০০ মিটার) . .
আপনার সার্ভার মেমরির ২জি সংস্করণের চেয়ে ২.জি কম হলে, দলের মান হ্রাস করা হবে ('মিগাবাইত') কম্পিউটারের শারীরিক মেমরির ১/২

* ৬৪-৩ Opera অপারেটিং সিস্টেম এবং ৬৪- বিট Java :
৬৪- বিট Java 64 বিট অপারেটিং সিস্টেমের উপর কাজ করবে।
  * সহ Java ৮) টমকে 'cttyE' 'cind_OISS পরামিতির সাথে যোগ করতে হবে।
  * সহ Java ২১, তুই ৬৪ বিট বেছে নে Java ডাউনলোড করা হলে Java চিহ্নিত '৬৪ বিট।

৬৪- বিট সহ Java ; টমাট এবং Java অত্যন্ত উচ্চ UnicodeCOL উইন্ডো এবং দলের বৈশিষ্ট্য ব্যবহার করতে পারবেন। সার্ভার সার্ভার এর চেয়ে আরো শারীরিক স্মৃতি।
একটি সহজ পরামর্শ হিসাবে: আমরা সুপারিশ করছি আপনি mXXএক্সCOLS এবং  হিসেবে নির্ধারিত ('মিগাবাইত') ১/২ (কম) কম্পিউটারের শারীরিক স্মৃতি.
...
ub/ughetd/logs-র মধ্যে অথবা অন্তর্ভুক্ত রয়েছে [লগ। gtx](/docs/server-admin/additional-information#log) ফাইল ('bergivde' উল্লেখকৃত অবস্থায় থাকা আবশ্যক [উন্নত ব্যবস্থা।](#setupxml) ) . .
...
'মেমরিফ ব্যবহার করো 'সাত্র পরবর্তী pmamawe. যদি এই দু'টি সংখ্যা একসাথে থাকে, তার মানে আবর্জনা সংগ্রহ কাজ করছে না।
...
* আপনি যদি 'javaa' শব্দটি দেখেন তাহলে 'java' -এ উপস্থিত থাকেন ERDDAP™ `%s' [লগ। gtx](/docs/server-admin/additional-information#log) ফাইল
...
আপনি যদি DejaVu ফন্টের ব্যবহার না করেন, তবে আপনার নির্ধারিত ফন্ট সংকলনটি পরিবর্তন করা প্রয়োজন। '...' <fontFamily> সানসারফ </fontFamily> ও.
...
আপনি যদি jajaV ফন্টের ব্যবহার করতে চান, অনুগ্রহ করে নিশ্চিত করুন যে 'njaV' টিপুন <fontFamily> '...' সেট করুন। <fontFamily> দেজাভো সানসাস </fontFamily> স্থগিত রাখো
...
অস্থায়ী ডিরেক্টরির মধ্যে কোনো ফাইল উপস্থিত নেই।
  ...
  * লিনাক্সে:
    ...
    * অন্যান্য সহ Java ডিস্ট্রিবিউশন ব্যবহারকারী হিসেবে 'অ্যাটমকিউসি' ব্যবহারকারীর ফাইল, '$java' - এ কপি করো Java ফন্ট খুঁজে পাওয়া যায় নি ।
...
* আপনার টম্যাট ইনস্টলেশন পরীক্ষা করুন.
  * লিনাক্স:
    * ব্যবহারকারী হিসেবে "টমক" হিসাবে সঞ্চালনের উদ্দেশ্যে সঞ্চালিত হবে ।
...
যদি কোন সমস্যা হয়, তাহলে টমকেট লগ ফাইল 'অ্যাপ্যাটো'.
...
এটা এমন একটা রেখায় ফিরে আসা উচিত:
  ...
  ```
   ...
   (যেখানে #JOAD কিছু সংখ্যা রয়েছে) এটি নির ্ গত হয় মেরুদন ্ ড , (সম্ভবত টম্যাট.) “টিসিপ” ট্রাফিকের জন্য পোর্ট“৮০” শোনা যাচ্ছে।
...
* টম্যাট লগ ফাইল '. আউটলুক.xml.log/cat.gz দেখুন। টমক্যাট সমস্যা এবং কিছু ERDDAP™ শুরু থেকেই সমস্যা দেখা দেয়।
...
'type' -এর ডিরেক্টরীতে এটি pigect, 'typect/internet' তৈরি করুন।
...
স্বয়ংক্রিয় ভাবে সংযোগ করুন .zip আরম্ভের উদ্দেশ্যে চিহ্নিত ডিরেক্টরি (শুধুমাত্র যে ব্যবহারকারী ব্যবহার করতে পারবেন) এবং সমুন ্ নত ছাদের , erddapContentDirectory সিস্টেমের বৈশিষ্ট্য
 (উদাহরণ দিন । erddapContentDirectory  =~tomcat/content/erddap 'আ) তাই ERDDAP™ এই নতুন বিষয়বস্তুর তথ্য খুঁজে পাওয়া যাবে।

### উন্নত ব্যবস্থা।{#setupxml} 

*  ['intcat/content/spedp' নামক ফাইলের মন্তব্য পড়ুন। 'আ](#setupxml) অনুরোধকৃত পরিবর্তন করতে। প্রস্তুত করা হয়েছে। নিম্নলিখিত বৈশিষ্ট্যের সাহায্যে সমস্ত বৈশিষ্ট্য প্রাপ্ত করা যাবে। ERDDAP™ আচরণ.

প্রাথমিক সেটআপের জন্য, আপনি অন্তত:
      * 'আ <bigParentDirectory> 'আ
      * 'আ <emailEverythingTo> 'আ
      * 'আ <baseUrl> 'আ
      * 'আ <email...> বৈশিষ্ট্য
      * 'আ <admin...> বৈশিষ্ট্য
      * 'আ <baseHttpsUrl> 'আ (যখন আপনি সেট https ) 
...
নাম ERDDAP প্রসেসের পূর্বে ধার্য করা হবে (_C) ERDDAP™ . . উদাহরণস্বরূপ, ব্যবহার করুন ERDDAP উপেক্ষা করার উদ্দেশ্যে চিহ্নিত ব্যক্তিদের তালিকা প্রদর্শন করা হবে (_b) <baseUrl> মান ।
...
প্রথমবারের মত (সাধারণত ডিফল্ট ডাটা সেট করা হয়) আপনি 'plature/ vint/ vp' - এ XML পরিবর্তন করবেন datasets.xml 'আ
...
'platt/ercdp/ছবিS2/dyp/dypcdot2-র সাথে সংযোগ স্থাপন এবং তারপর তা পরিবর্তন করে।
পরিবর্তন শুধুমাত্রcraps2.cd দ্বারা প্রয়োগ করা হবে ERDDAP™ পুনরারম্ভের ফলে হওয়া ব্যবহারকারীর দ্বারা ও স্বনির্ধারিত পরিবর্তনগুলি বর্জন করা আবশ্যক।
     
...
4. Linux, Mac ও Windows-র মধ্যে (_i) [সার্দ্দি।](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) এই ক্ষেত্রে/ অভিভাবক_username
...
1. নিশ্চিত করুন যে "proxy_proxy_proxy_fuse_proxy_proxy_use_proxy_fuse http লোড করা হয়েছে:
...
যদি ERDDAP™ 'pavina' - র অন্য একটি পথে এটি কাজ করা হয়, এছাড়াও 'X-Papharde-Pode-FPEdg-FPE তে 'X-PEdir' ধার্য করা হয়
পাথ অংশ পূর্ব/cpus/ ইন্টারফেসের পাথ পূর্বে (_p) এই ধরনের বৈশিষ্ট্য আবশ্যক ERDDAP™ সেবা করা
/usadradp:
...
3. তাহলে অ্যাপাচি পুনরায় আরম্ভ করুন: //usbin/ আবশ্যক - ভাল 'আ (কিন্তু কখনো কখনো এটা ভিন্ন ডিরেক্টরিতে) . .
         
#### নাক্স{#nginx} 
...
নিগক্স পাওয়ার জন্য ERDDAP™ সঠিকভাবে কাজ করছে https আপনার টম্যাট সার্ভারের মধ্যে নিম্নলিখিত স্নিপেটগুলি স্থাপন করতে হবে। .xml. xml'র মধ্যে উপস্থিত তথ্য মুছে ফেলা আবশ্যক <Host> ব্লক:
```
...
* যদি টম্যাট ইতোমধ্যে চলছে, টমকে বন্ধ করে দাও (লিনাক্স বা Mac OS-র মধ্যে) ` Stack/bin/auto
অথবা (উইন্ডোর মধ্যে) & মন্তব্য করো shutdown.bat 'আ

লিনাক্সে, ব্যবহৃত sps -f | `%s' চালু করার আগে ও পরে সঞ্চালনের ব্যবস্থা বন্ধ করা হবে।
...
খুন-৯ <processID> 'আ
* আরম্ভ করা হবে (লিনাক্স বা Mac OS-র মধ্যে) `Bin/bin/bin.hmus অথবা / (উইন্ডোর মধ্যে) `টমকেক\\ tstart\t 'আ
...
টমটাগ লগ ফাইল (চিহ্নিত ডিরেক্টরির মধ্যে উপস্থিত অন্যান্য ফাইলগুলি ও অন্যান্য ডিরেক্টরির তালিকা প্রদর্শন করা হবে) . .
*    [আপনি](/docs/server-admin/additional-information#log) কি আসে দেখতে পারেন ERDDAP বার্তা ERDDAP '%s'
বার্তা থেকে ত্রুটি ও ত্রুটি ERDDAP এর মধ্যে ERDDAP™ 'আ <bigParentDirectory> /log/us. দেখাতে ব্যবহৃত ফাইল।
...
* কখন ERDDAP™ শুরু হলে, পুরোনো নাম পরিবর্তন করা হয় ERDDAP™ লগ ফাইল ('লগ আর্কাইভ' চিহ্নিত সময়ে <CurrentTime> .বাটিকা) নতুন একটি লগ ফাইল তৈরি করুন।
যদি 'লোজি.এডি.এম.এল. পুরাতন হয়, এটি একটি চিহ্ন ERDDAP™ সম্প্রতি পুনরায় আরম্ভ করা হয়নি। ERDDAP™ বাফারের মধ্যে লগ তথ্য লেখা হচ্ছে
এবং শুধুমাত্র লগ ফাইলে বাফার লেখা, কিন্তু আপনি জোর করে বল দিতে পারেন ERDDAP™ ভ্রমণের মাধ্যমে লগ ফাইলে যে বাফার লেখা হবে
'আ /erddap/status.html স্থগিত রাখো
...
টমকে প্রথমবারের মত অনেক কাজ করতে হবে ERDDAP™ আরম্ভ করা হয়েছে; লক্ষণীয়ভাবে, এটি 'errapsp' ফাইল মুছে ফেলতে হবে।
...
* সব ডেস্কটপে যে ব্যানারটি দেখা যায় তা পরিবর্তন করুন ERDDAP™ লেখক কর্তৃক <startBodyHtml5> 'তোমার নাম' datasets.xml ফাইল.
( না থাকলে, পূর্বনির্ধারিত একটি কপি কপি করুন) ERDDAP™ 'nternet/wap/WEB/ WeF/dBUPL/ ক্লাসগুলি/ALav/padv/fflffl/s.decdi// Defardap/s/bin/amp/nerdus. ফাইল
...
* ওয়েব পেজের বাম পাশে তথ্য পরিবর্তন করুন সম্পাদনা করুন <theShortDescriptionHtml> 'তোমার নাম' datasets.xml ফাইল.
( না থাকলে, পূর্বনির্ধারিত একটি কপি কপি করুন) ERDDAP™ 'nternet/wap/WEB/ WeF/dBUPL/ ক্লাসগুলি/ALav/padv/fflffl/s.decdi// Defardap/s/bin/amp/nerdus. ফাইল
এর মধ্যে datasets.xml ' এবং সম্পাদনা করুন। উদাহরণস্বরূপ, আপনি পারেন:
