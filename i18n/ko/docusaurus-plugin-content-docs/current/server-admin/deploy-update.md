...
## 메시지.xml{#messagesxml} 
4. 
    * 일반: 당신이 졸업하면 ERDDAP™ 버전 1.46 (또는 위) 그리고 당신은 그냥 표준 메시지를 사용, 새로운 표준 message.xml 자동으로 설치됩니다 (erddap을 통해 .class 파일 중. 전쟁 전쟁) ·
         
    * 희귀 : 당신이 졸업하면 ERDDAP™ 버전 1.44 (또는 아래) ·
...
당신은 알아낼 필요가 있습니다 (diff를 통해) 새로운 erddap에 있는 default message.xml에 어떤 변경이 생성되었습니다. 전쟁으로
...
\\* 컴퓨터에서 실제 Tomcat 디렉토리 아래에 _tomcat_에 참조를 대체합니다.
     
...
rm -rf의 erddap
3. 오래된 erddap 삭제. 전쟁 파일: 에서 _tomcat_/webapps, 사용 rm erddap. 전쟁 전쟁
4. 새로운 erddap을 복사합니다. 임시 디렉토리의 전쟁 파일 _tomcat_/webapps
5. 톰캣과 ERDDAP : 사용 _tomcat_/bin/startup.sh
6. - 연혁 ERDDAP™ 브라우저에서 다시 시작이 성공했는지 확인하십시오.
     (종종, 당신은 몇 번 시도하고 당신이 볼 전에 분 기다립니다 ERDDAP™ ·)   
             
### 윈도우{#windows} 
1. 폐쇄 Tomcat: 명령줄에서 사용: _tomcat_\bin\\\\ shutdown.bat 
2. decompressed 제거 ERDDAP™ 임명: 에서 _tomcat_/webapps, 사용
델 /S / Q erddap
3. 오래된 erddap 삭제. 전쟁 파일: 에서 _tomcat_\\webapps, 사용 del erddap. 전쟁 전쟁
4. 새로운 erddap을 복사합니다. 임시 디렉토리의 전쟁 파일 _tomcat_\\webapps
