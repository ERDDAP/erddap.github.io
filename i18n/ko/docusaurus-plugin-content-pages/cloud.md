...
예.
 ...
 ERDDAP™ 로컬 또는 클라우드 환경에서 실행할 수 있는 Tomcat 내에서 실행하도록 설계되었습니다. 공식 Docker 이미지는 [Docker 허브](https://hub.docker.com/r/erddap/erddap) · 더 보기 `erddap:알파-latest` tag is a build based on the most 최근 변경 사항 ('nightly' 릴리스와 같은 뭔가, [alpha-latest 세부사항](https://hub.docker.com/layers/erddap/erddap/alpha-latest/) ) , 동안 `erddap:최신` 가장 최근의 테스트 릴리스 ( [관련 기사](https://hub.docker.com/layers/erddap/erddap/latest/) ) · GitHub 컨테이너 레지스트리 릴리스도 검색할 수 있습니다. [GitHub 패키지](https://github.com/ERDDAP/erddap/pkgs/container/erddap) · 더 많은 정보를 읽을 수 있습니다. ERDDAP™ 이름 * [팟캐스트](https://github.com/ERDDAP/erddap/blob/main/DOCKER.md) ·

쿠버네티스 배포를 위해 새로운 배포-kubernetes를 참조하십시오. [관련 기사](https://erddap.github.io/docs/server-admin/admin-tips/deploy-kubernetes) ·
