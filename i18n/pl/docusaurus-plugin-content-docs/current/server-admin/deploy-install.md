...
Jeśli go ściągniesz, [Pobierz "Core" "tar .gz "Dystrybucja Tomcat](https://tomcat.apache.org/download-10.cgi) i rozpakować go w '/ Biblioteka / Tomcat'.
     * W systemie Windows możesz [Pobierz dystrybucję "Core" "zip" Tomcat](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Dla rozwoju używamy dystrybucji "Core" "zip". Tworzymy katalog '/ programów' i rozpakujemy go tam.) 
...
*  [server.xml](#serverxml) - W pliku 'tomcat / conf / server.xml' są dwie zmiany, które należy wprowadzić do każdego z dwóch ' <Connector> 'tagi
   ...
   1.  (Zalecane) Zwiększ wartość parametru 'connectionTimeout' do 300000 (milisekundy, czyli 5 minut) .
...
* context.xml -- Cache Resources - In 'tomcat / conf / context.xml', tuż przed ' </Context> 'tag, zmień znacznik Surowców
...
I w każdym razie, powinieneś zrobić to tak, że użytkownik 'tomcat' ma bardzo ograniczone uprawnienia do systemu plików serwera (przeczytaj + write + execute przywileje
...
Plik ten będzie używany przez 'tomcat / bin / startup.sh' i 'shutdown.sh'. Plik powinien zawierać coś w rodzaju:
  ```
...
Na Maca, prawdopodobnie nie musisz ustawiać 'JAVA _ HOME'.
...
Z 32 bitami Java , nawet z obfitej pamięci fizycznej, Tomcat i Java nie uruchomi jeśli spróbujesz ustawić '-Xmx' znacznie powyżej 1500M (1200M na niektórych komputerach) .
...
lub w 'bigParentDirectory / logi / [log.txt](/docs/server-admin/additional-information#log) 'Plik ('bigParentDirectory' jest określony w [setup.xml](#setupxml) ) .
...
* Jeśli widzisz 'java.lang.OutOfMemoryError' s in ERDDAP™ jest [log.txt](/docs/server-admin/additional-information#log) plik,
...
* Sprawdź instalację Tomcat.
  * Linux:
    * Jako użytkownik "tomcat", uruchom 'tomcat / bin / startup.sh ".
...
Jeśli występują problemy, zobacz plik dziennika Tomcat w 'tombot / logs / catalina.out'.
...
To powinno zwrócić jedną linię z czymś takim:
  ...
  ```
   ...
   (gdzie '#' jest jakąś cyfrą) , wskazując, że proces "java" (Prawdopodobnie Tomcat) Słucha w porcie "8080" dla ruchu "tcp".
...
* Patrz plik dziennika Tomcat 'tomcat / logs / catalina.out'. Problemy z Tomcat i niektóre ERDDAP™ Problemy związane z startupem są tam niemal zawsze wskazywane.
...
'tomcat / content / erddap / images / erddapStart2.css' to 'tomcat / content / erddap / images / erddap2.css', a następnie dokonać zmian.
...
4. Na Linuksie, Mac i Windows, _ _ download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) _ _ into 'tomcat / webapps':
...
path segment _ before _ '/ erddap'. Ustawienie to byłoby właściwe dla ERDDAP™ podanie
"/ subpath / erddap":
...
Tomcat musi zrobić dużo pracy za pierwszym razem aplikacji jak ERDDAP™ jest uruchomiony; w szczególności, musi rozpakować plik 'erddap.war'
...
(Jeśli nie ma żadnego, skopiuj domyślny ERDDAP™ 'tomcat / webapps / erddap / WEB- INF / classes / gov / noaa / pfel / erddap / util / messages.xml" plik
