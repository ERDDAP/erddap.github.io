Zestawy danych wideo
= = = = = = = = = = = = = = = =
2026- 05- 27

# Poszukiwanie i wyświetlanie wideo w ERDDAP™ 

Ten dokument pokazuje jak wyświetlać filmy w ERDDAP™ oraz
tworzenie zbioru danych, który umożliwia wyszukiwanie metadanych wideo do
znaleźć filmy, które spełniają wybrane kryteria wyszukiwania.

Podstawowe elementy potrzebne do tego są najpierw, filmy, które
są w formacie, który może być wyświetlany w przeglądarce internetowej ( ERDDAP™ sam
nie wyświetla wideo, dostarcza tylko link), a po drugie, csv
plik (lub podobny format tabeli, taki jak parkiet) które zawierają
metadane istotne dla filmów.

Kluczem jest posiadanie odpowiedniego formatu wideo. W przykładzie poniżej
filmy pierwotnie były stary format Okna, że nawet przeglądarki Windows
nie mógł wyświetlić z pola, i wymagane dodanie wtyczki. Stary
formaty często są bardzo dużymi plikami. Hamulec ręczny może być
używane do konwersji filmów, co było zrobione.

## Ustawienia

Pierwszym krokiem jest stworzenie zbioru danych "EDDTableFromFileNames", tak aby
istnieje adres URL powiązany z każdym wideo, który jest dostępny przez
do ERDDAP™ Polecenie "pliki". Podczas gdy podany jest przykład tego zbioru danych
poniżej, najlepszym sposobem, aby wygenerować ten xml dla wideo jest użycie
GenerateDatasets Xml.sh i wskazać go do katalogu z wideo (w
ogólne najlepszy sposób na dodanie nowych zbiorów danych do ERDDAP™ do stosowania
GenerateDatasets Xml.sh, który zapewni Ci większość drogi tam, i
następnie edytować to dla końcowego wyniku).

Drugim krokiem jest utworzenie plików EDDTableFromAsciiFiles (lub podobne) 
zbiór danych zawierający metadane, które mają być użyte w filmie
Szukaj. Przykład podano poniżej. Żeby to zadziałało, musi być jeden.
kolumna, która zawiera link do wideo, w przykładzie jest to
zmienna "videoFile". Ta zmienna może po prostu zawierać pełny adres URL
utworzony w pierwszym kroku, ale wtedy plik nie jest przenośny, np. jeśli
chcesz przetestować na jednej maszynie, a następnie rzeczywiście służyć na innej
Maszyna.

Możesz zrobić plik przenośny, przez co wpis w "VideoFile" będzie
Ścieżka względna do filmu, w przykładzie poniżej jest:

2010 _ Piggy _ Bank _ audio _ naned / 0001 _ 20100627 _ 144631.mp4

Następnie można dodać atrybut "fileAccessBaseUrl", który dodaje, że
URL do ścieżki w kolumnie. W przykładzie:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Jeśli przełączysz serwery musisz tylko zmienić jeden atrybut w
Snippet xml, nie trzeba edytować pliku csv.

Po wykonaniu, masz listę filmów dostępnych jako pliki:
...
proste wyszukiwanie utworzone przez zbiór danych "EDDTableFromFileNames":
...
oraz wyszukiwania metadanych w:
...
Domyślne opcje w ostatnim URL produkują URL:
...
i która tworzy tabelę z ostatnią kolumną:
...
Każdy z wpisów w odpowiedzi zawiera link do wideo, takich jak
jeśli mysz nad pierwszym masz:
...
i jeśli klikniesz na to w html Table wyświetlania wideo w Twoim
przeglądarka.
