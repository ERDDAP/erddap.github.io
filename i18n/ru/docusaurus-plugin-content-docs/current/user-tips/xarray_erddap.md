...
Причина, по которой это приводит к ошибке, заключается в том, что «xarray» использует OPeNDAP   ( https://www.opendap.org ) в качестве протокола для удаленного доступа, а также ERDDAP™ Синтаксис основан на OPeNDAP синтаксис, а также ERDDAP™ Сервер также может выступать в качестве OPeNDAP Есть различия в том, как это делается для двух служб. (См., например https://coastwatch.pfeg.noaa.gov/erddap/griddap/documentation.html#opendapLibraries ) . Любой ERDDAP URL без нарезки или фильтров, только datasetID ведет себя как OPeNDAP URL совместим с xarray.
