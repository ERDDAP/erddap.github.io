...
Если вы хотите работать в облачных сервисах, вы, вероятно, предпочтете версию Docker.
...
В частности, ERDDAP™ Возможно, вы не сможете быстро удалить и/или переименовать файлы. Вероятно, это связано с антивирусным программным обеспечением.
...
Из соображений безопасности всегда лучше использовать последнюю версию. Java 21.
...
Для проверки установки запустите, например, "/javaJreBinDirectory/java-version".
/usr/local/jdk-21.0.3+9/jre/bin/java -версия.
...
который предназначен для работы с Java 21 или больше. Ниже каталог Tomcat будет называться «tomcat».

Предупреждение&#33;__ Если у вас уже есть Tomcat, работающий с другим веб-приложением (Особенно драки) Мы рекомендуем вам установить ERDDAP™ в
...
Если вы загрузите его, [Скачать «Core» «tar» .gz Распределение "Томкат"](https://tomcat.apache.org/download-10.cgi) и распаковать его в "/Библиотека/Томкат".
     * В Windows вы можете [Скачать дистрибутив «Core» «zip» Tomcat](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Для разработки мы используем дистрибутив «Core» «zip». Мы создаем каталог «/программ» и распаковываем его там.) 
Или вы можете скачать «Core» «64-битный Windows zip» дистрибутив, который включает в себя больше функций.
Если дистрибутив является установщиком Windows, он, вероятно, поместит Tomcat, например, в «/Program Files/apache-tomcat-10.0.23».
             
### server.xml{#serverxml} 

*  [server.xml](#serverxml) В файле «tomcat/conf/server.xml» есть два изменения, которые вы должны внести в каждый из двух файлов: <Connector> теги
   (Один для &lt;&lt;Connector port="8080" и один для &lt;&lt;Conector port="8443") .
   1.  (рекомендованный) Увеличить значение параметра «connectionTimeout», возможно, до 300000 (миллисекунды, что составляет 5 минут) .
   2.  (рекомендованный) Добавить новый параметр: "relaxedQueryChars=" | ". Это необязательно и немного менее безопасно.
...
* context.xml - Кэш ресурсов - в "tomcat/conf/context.xml", непосредственно перед " </Context> 'тег, изменить тег Ресурсы
...
* На компьютерах Linux измените настройки тайм-аута Apache, чтобы отнимающие много времени запросы пользователей не отставали.
  ...
  * Модифицировать Apache http d.conf файл (обычно в '/etc/ http d/conf/ пункт) :
    * Изменить существующую <Timeout> устанавливать (Добавить в конце файла) до 3600 (секунды) Вместо стандартных 60 или 120 секунд.
    * Изменить существующую <ProxyTimeout> устанавливать (Добавить в конце файла) до 3600 (секунды) Вместо стандартных 60 или 120 секунд.
  * Перезапуск Apache: `/usr/sbin/apachectl -к грациозный пункт (Но иногда он находится в другом каталоге.) .
...
Установка Tomcat, особенно для публичных серверов.
         
* для публики ERDDAP™ Установки на Linux и Mac, лучше всего настроить Tomcat (Программа) как принадлежащий пользователю 'tomcat пункт
...
Таким образом, только суперпользователь может переключаться на действия в качестве пользовательского «tomcat». Это делает невозможным для хакеров вход на ваш сервер в качестве пользовательского «tomcat».
И в любом случае вы должны сделать так, чтобы пользователь «tomcat» имел очень ограниченные разрешения на файловую систему сервера (привилегии чтения + записи + выполнения).
для дерева каталогов «apache-tomcat» и <bigParentDirectory> и только для чтения привилегий для каталогов с данными, которые ERDDAP™ Нужен доступ.
  * Вы можете создать учетную запись пользователя «tomcat» (который не имеет пароля) Используя команду:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * Вы можете перейти на работу в качестве пользовательского «tomcat» Используя команду
    ```
    sudo su - tomcat
    ```
    ...
    * Вы можете перестать работать как пользователь Tomcat, используя команду
    ...
    exit
    ````
    * Выполняйте большую часть работы и ERDDAP™ Настройка инструкций как пользователь «tomcat». Позже запустите скрипты «startup.sh» и «shutdown.sh» в качестве пользовательского «tomcat». пункт
Чтобы у Tomcat было разрешение писать в свои файлы журналов.
...
* Linux и Mac:
Создание файла 'tomcat/bin/setenv.sh пункт (В Red Hat Enterprise Linux \\[ Рейл \\] Отредактировать '~tomcat/conf/tomcat10.conf пункт) Установить переменные среды Tomcat.
Этот файл будет использоваться по адресу "tomcat/bin/startup.sh" и "shutdown.sh". Файл должен содержать что-то вроде:
  ```
...
* В Windows:
Создайте файл 'tomcat\bin\\setenv.bat' для установки переменных среды Tomcat.
Этот файл будет использоваться в 'tomcat\bin\\startup.bat' и '' shutdown.bat '.
...
Настройки памяти «-Xmx» и «-Xms» важны, потому что ERDDAP™ Лучше работать с большей памятью.
Всегда устанавливайте '-Xms' на то же значение, что и '-Xmx'.

* Для 32-битных операционных систем и 32-битных Java :
64 бит Java Гораздо лучше, чем 32 бит Java Но 32 бит Java Он будет работать до тех пор, пока сервер не будет занят.
...
32-битный Java Даже с обильной физической памятью, Tomcat и Java не будет работать, если вы попытаетесь установить «-Xmx» намного выше 1500M; (1200 М на некоторых компьютерах) .
Если ваш сервер имеет менее 2 ГБ памяти, уменьшите значение «-Xmx». (Исполнитель: 'M'egaBytes) 1/2 физической памяти компьютера.

* Для 64-битных операционных систем и 64-битных Java :
64 бит Java Работает только на 64-битной операционной системе.
  * с Java 8, вы должны добавить "-d64" к параметру Tomcat "CATALINA_OPTS" в "setenv.bat".
  * с Java 21, выберите 64 бит Java Когда вы загружаете версию Java Помечено "64 бит".

64-битный Java Томкэт и Java Можно использовать очень высокие настройки '-Xmx' и '-Xms'. Чем больше физической памяти на сервере, тем лучше.
В качестве упрощенного предложения: мы рекомендуем вам установить '-Xmx' и '-Xms' на (Исполнитель: 'M'egaBytes) до 1/2 (менее) Физическая память компьютера.
...
или в 'bigParentDirectory/logs/ [log.txt](/docs/server-admin/additional-information#log) файл («Дирекция родителей» указывается в [Настройка.xml](#setupxml) ) .
...
Обычно это не проблема. Это частое сообщение от нормально работающего человека. Java Говорят, что только что закончился небольшой мусор
Коллекция, потому что в Эдеме закончилось место (В этом разделе Java Куча очень молодых предметов) . Обычно сообщение показывает
"memoryUseBefore-&gt;memoryUseAfter". Если эти два числа близки друг к другу, это означает, что сбор мусора не был продуктивным.
...
Это может произойти во время стрессового периода, а затем уйти. Но если оно сохранится, это будет признаком беды.
* Если вы видите 'java.lang.OutOfMemoryError's in ERDDAP™ ? [log.txt](/docs/server-admin/additional-information#log) файл,
...
*  [На Linux и Mac измените разрешения](#permissions) из всех файлов '*.sh' в файле 'tomcat/bin/', исполняемых владельцем:
  ```
  chmod +x *.sh
  ```
...
который доступен всем Java распределения. Если вы установите <fontFamily> к названию шрифта, который недоступен, ERDDAP™ не будет загружаться
и распечатает список доступных шрифтов в файле log.txt. Вы должны использовать один из этих шрифтов.

Если вы решите использовать шрифты DejaVu, пожалуйста, убедитесь, что: <fontFamily> "установка setup.xml" <fontFamily> Дежаву Санс </fontFamily> '.
...
и распаковывать файлы шрифтов во временный каталог.
  ...
  * На Linux:
    ...
    * с другими Java распределения: Как пользователь «tomcat», скопируйте файлы шрифтов в «$JAVA_HOME/lib/fonts». Java Вы можете найти шрифты.
Запомните: если позже вы перейдете на новую версию Java Вам нужно переустановить эти шрифты.
  * На Mac: для каждого файла шрифта дважды нажмите на него, а затем нажмите «Установить шрифт».
  * В Windows 7 и 10: в проводнике Windows выберите все файлы шрифта. Прямой клик. Нажмите на Установка.
             
...
* Проверьте установку Tomcat.
  ...
  * Мак (Run Tomcat как пользователь системного администратора) :
    * Запустите 'tomcat/bin/startup.sh'.
    * Просмотр URL + ":8080/" в браузере (например, [http://coastwatch.pfeg.noaa.gov:8080/](http://coastwatch.pfeg.noaa.gov:8080/) ) .
...
Если есть проблемы, см. файл журнала Tomcat по адресу «tomcat/logs/catalina.out».
...
Это должно вернуть одну строку с чем-то вроде:
  ...
  ```
   ...
   (где "#" - некоторая цифра) Указывая, что процесс «джавы» (Предположительно Tomcat) Прослушивается на порту "8080" для "tcp" трафика.
...
* Файл журнала Tomcat "tomcat/logs/catalina.out". Проблемы с Tomcat и некоторые другие ERDDAP™ Проблемы с запуском почти всегда указываются там.
...
3.   [Настройка файлов конфигурации «tomcat/content/erddap».](#erddap-content) 
...
и распаковывать его в каталог «tomcat», создавая «tomcat/content/erddap».
...
unzip 'erddapКонтент .zip в желаемый каталог (К которому имеет доступ только пользователь «tomcat») и установить " erddapContentDirectory "системное имущество"
 (например, erddapContentDirectory  =~tomcat/content/erddap пункт) так ERDDAP™ Вы можете найти этот новый каталог контента.

### Настройка.xml{#setupxml} 

*  [Прочитайте комментарии в 'tomcat/content/erddap/setup.xml пункт](#setupxml) и внести запрошенные изменения. setup.xml - это файл со всеми настройками, которые определяют, как вы ERDDAP™ ведет себя.

Для первоначальной настройки вы должны по крайней мере изменить эти настройки:
      * пункт <bigParentDirectory> пункт
      * пункт <emailEverythingTo> пункт
      * пункт <baseUrl> пункт
      * пункт <email...> настройки
      * пункт <admin...> настройки
      * пункт <baseHttpsUrl> пункт (Когда вы устанавливаете https ) 

При создании BigParentDirectory из родительского каталога BigParentDirectory:

    * Сделать пользователя "tomcat" владельцем "bigParentDirectory":
      ```
...
названный ERDDAP _valueName перед запуском ERDDAP™ . Например, использовать " ERDDAP _baseUrl' перекрывает <baseUrl> ценность.
...
впервые (Обычно только с наборами данных по умолчанию) Вы будете изменять XML в 'tomcat/content/erddap/ datasets.xml пункт
указать все наборы данных, которые вы хотите получить ERDDAP™ служить. Именно здесь вы проведете большую часть своего времени.
...
Изменения в «erddap2.css» вступают в силу только тогда, когда ERDDAP™ Перезапускается и часто требует от пользователя очистить кэшированные файлы браузера.
     
...
Это хорошая идея, чтобы проверить, что результат хорошо сформирован XML, вставив текст XML в XML-проверку, как [xmlvalidation](https://www.xmlvalidation.com/) .
     
### Установите erddap. военный файл{#install-the-erddapwar-file} 

4. Linux, Mac и Windows, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.0/erddap.war) __ в "tomcat/webapps":

__Версия 2.28.0, 620 824 288 байт, MD5=f948b2ba603f65a83ac67af43da9e4c2, от 2025-08-29__
...
1. Убедитесь, что «mod_proxy» и «mod_proxy_» http "загружены:
...
2. Изменить существующую <VirtualHost> тег (Если есть один) или добавить в конце файла:
```
<VirtualHost *:80>
...
Если ERDDAP™ подается на пути, отличном от «/erddap», также установите заголовок «X-Forwarded-Prefix» на
Сегмент пути _before_erddap. Этот параметр будет уместным для ERDDAP™ служил в
"/subpath/erddap":
...
3. Затем перезапустить Apache: `/usr/sbin/apachectl -к грациозный пункт (Но иногда он находится в другом каталоге.) .
         
#### NGINX{#nginx} 
...
*  (В Linux или Mac OS, если вы создали специального пользователя для запуска Tomcat, например, Tomcat, не забудьте сделать следующие шаги в качестве этого пользователя.) 
* Если Tomcat уже работает, закройте Tomcat. (Linux или Mac OS) "tomcat/bin/shutdown.sh"
или (В Windows) 'omcat\bin' shutdown.bat пункт

Для Linux используйте 'ps-ef' | grep tomcat до и после shutdown.sh, чтобы убедиться, что процесс Tomcat остановился.
...
"убийство"9 <processID> пункт
* Начните с Tomcat (Linux или Mac OS) "tomcat/bin/startup.sh" или (В Windows) 'omcat\bin\\startup.bat пункт
...
сообщения об ошибках от ERDDAP в ERDDAP™ пункт <bigParentDirectory> /logs/log.txt' файл.
...
начинать ERDDAP™ или если имеется сообщение об ошибке, связанное с этой попыткой.
* Когда ERDDAP™ Начинается, переименовывает старое ERDDAP™ log.txt файл ("logArchived" в <CurrentTime> .txt) Создает новый файл log.txt.
Если файл log.txt старый, это признак того, что ERDDAP™ Недавно не перезапустили. ERDDAP™ Запись log info в буфер
...
Томкэт должен проделать большую работу в первый раз, когда приложение ERDDAP™ Запускается; в частности, он должен распаковать файл «erddap.war».
 (который подобен .zip файл) . На некоторых серверах первая попытка просмотра ERDDAP™ ларьки (30 секунд?) Пока эта работа не будет завершена.
...
* Измените баннер, который появляется в верхней части ERDDAP™ .html страницы путем редактирования <startBodyHtml5> "Тэг в твоем" datasets.xml Файл.
(Если нет, скопируйте по умолчанию из ERDDAP™ "s 'tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml" файл
...
* Изменить информацию на левой стороне домашней страницы, отредактировав <theShortDescriptionHtml> "Тэг в твоем" datasets.xml Файл.
(Если нет, скопируйте по умолчанию из ERDDAP™ "s 'tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml" файл
в datasets.xml и редактировать его. Например, вы можете:
  * Опишите, чем занимается ваша организация и/или группа.
  * Опишите, какие данные это ERDDAP™ Есть.
  * Чтобы изменить значок, который появляется на вкладках браузера, поместите favicon вашей организации. ico in 'tomcat/content/erddap/images/'.
