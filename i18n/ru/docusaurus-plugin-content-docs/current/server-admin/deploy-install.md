...
В качестве упрощенного предложения: мы рекомендуем вам установить «-Xmx» и «-Xms». (Исполнитель: 'M'egaBytes) до 1/2 (менее) Физическая память компьютера.
...
или в 'bigParentDirectory/logs/ [log.txt](/docs/server-admin/additional-information#log) файл («BigParentDirectory» указывается в [Настройка.xml](#setupxml) ) .
...
и распаковывать файлы шрифтов во временный каталог.
  ...
  * На Linux:
    ...
    * с другими Java Распределение: Как пользователь «tomcat», скопируйте файлы шрифтов в «$JAVA_HOME/lib/fonts». Java Вы можете найти шрифты.
...
4. Linux, Mac и Windows, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) __ в "tomcat/webapps":
...
Сегмент пути _before_erddap. Этот параметр будет уместным для ERDDAP™ служить в
"/subpath/erddap":
...
Ошибочные сообщения от ERDDAP в ERDDAP™ пункт <bigParentDirectory> /logs/log.txt' файл.
...
Tomcat должен сделать много работы в первый раз, когда приложение ERDDAP™ Запускается; в частности, он должен распаковать файл «erddap.war».
