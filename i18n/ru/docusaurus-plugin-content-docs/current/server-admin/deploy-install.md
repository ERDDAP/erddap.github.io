...
*    **Докер** - Axiom теперь предлагает[ERDDAP™В контейнере Docker](https://hub.docker.com/u/axiom/)IOOS теперь предлагает[Руководство для быстрого стартаERDDAP™В контейнере Docker](https://ioos.github.io/erddap-gold-standard/index.html).
Это стандартERDDAP™Установка, но Аксиом положил его в контейнер докера.
Если вы уже используете Docker, вы, вероятно, предпочтете версию Docker.
Если вы еще не используете Docker, мы обычно не рекомендуем это.
Если вы решили установитьERDDAP™Через Docker мы не предлагаем никакой поддержки процесса установки.
Мы еще не работали с Докером. Если вы работаете с этим, пожалуйста, отправьте нам свои комментарии.
...
4. На Linux, Mac и Windows, скачать[erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.26.0/erddap.war)_tomcat_/webapps.
