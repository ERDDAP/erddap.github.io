...
Из соображений безопасности всегда лучше использовать последнюю версию. Java 25.
...
Чтобы проверить установку, запустите `/javaJreBinDirector/java инверсия` Например,
...
Если дистрибутив является установщиком Windows, он, вероятно, поместит Tomcat в `/Программные файлы/apache-tomcat-10.0.23` .
             
### server.xml{#serverxml} 

*  [server.xml](#serverxml) - В `Tomcat/conf/server.xml` Есть два изменения, которые вы должны внести в каждый из двух. ` <Connector> ` метки
...
* context.xml - Ресурсы кэша - в `Tomcat/conf/context.xml` непосредственно перед ` </Context> ` Тег, изменить тег Ресурсы
...
* На компьютерах Linux измените настройки тайм-аута Apache, чтобы отнимающие много времени запросы пользователей не отставали.
  ...
  * Восстановление Apache: `/usr/sbin/apachectl -к грациозный`   (Но иногда он находится в другом каталоге.) .
...
для `апач-томкат` дерево каталогов и ` <bigParentDirectory> ` Привилегии только для чтения для каталогов с данными, которые ERDDAP™ Нужен доступ.
  * Вы можете создать `кот` учетная запись пользователя (который не имеет пароля) Используя команду:
    ...
    ```
  * Вы можете переключиться на работу в качестве пользователя `кот` Используя команду
    ```
    sudo su - tomcat
    ```
...
и распаковывать файлы шрифтов во временный каталог.
  ...
  * На Linux:
    ...
    * с другими Java распределения: Как `кот` Пользователь, скопируйте файлы шрифта в `$JAVA_HOME/lib/fonts` так Java Вы можете найти шрифты.
...
Если ERDDAP™ На другом пути, кроме `/erdap` Также установить `X-Forwarded-Prefix` заголовок к
Сегмент пути _before_ `/erdap` . Этот параметр будет уместным для ERDDAP™ служил в
 `/subpath/erddap` :
...
сообщения об ошибках от ERDDAP в ERDDAP™   ` <bigParentDirectory> /logs/log.txt` Файл.
...
Томкэт должен проделать большую работу в первый раз, когда приложение ERDDAP™ Начинается; в частности, он должен распаковать `erddap.war` файл
