...
Si la distribution est un installateur Windows, elle mettra probablement Tomcat dans, par exemple, `/Program Files/apache-tomcat-10.0.23`.
             
...
* contexte.xml -- Ressources Cache - Dans `tomcat/conf/context.xml`, juste avant le ` </Context> ` tag, modifier l'étiquette Ressources
...
Ainsi, seul le super utilisateur peut passer à agir en tant qu'utilisateur «tomcat». Cela rend impossible pour les pirates de se connecter à votre serveur comme utilisateur `tomcat`.
Et en tout cas, vous devez le faire de sorte que l`utilisateur `tomcat` ait des permissions très limitées sur le système de fichiers du serveur (read+write+execute privilèges
pour l'arborescence du répertoire `apache-tomcat` et ` <bigParentDirectory> ` et les privilèges en lecture seule pour les répertoires avec des données qui ERDDAP™ a besoin d'accès à).
...
* Sur Linux et Macs :
Créer un fichier `tomcat/bin/setenv.sh " (ou dans Red Hat Enterprise Linux \\[ RHEL \\] , éditer `~tomcat/conf/tomcat10.conf ") pour définir les variables d'environnement de Tomcat.
...
*  [Sur Linux et Macs, changez les permissions](#permissions) de tous les fichiers `*.sh` dans `tomcat/bin/` devant être exécutés par le propriétaire:
  ```
  chmod +x *.sh
  ```
...
Si vous choisissez d'utiliser les polices DejaVu, assurez-vous que le ` <fontFamily> ` setting in setup.xml est ` <fontFamily> DejaVu Sans </fontFamily> ".
...
et décompresser les fichiers de police vers un répertoire temporaire.
  ...
  * Sur Linux :
    ...
    * Avec d'autres Java distributions: En tant qu'utilisateur `tomcat`, copiez les fichiers de police dans `$JAVA_HOME/lib/fonts` Java peut trouver les polices.
...
* Testez votre installation Tomcat.
  ...
  * Mac (exécuter tomcat en tant qu'utilisateur administrateur système) :
    * Exécutez `tomcat/bin/startup.sh`.
    * Affichez votre URL + ":8080/" dans votre navigateur (Par exemple, [http://coastwatch.pfeg.noaa.gov:8080/](http://coastwatch.pfeg.noaa.gov:8080/) ) .
...
`tomcat/content/erddap/images/erddapStart2.css` vers `tomcat/content/erddap/images/erddap2.css` puis y apporter des modifications.
Les changements à `erddap2.css` ne prennent effet que lorsque ERDDAP™ est redémarré et nécessite souvent aussi l'utilisateur pour effacer les fichiers en cache du navigateur.
     
...
4. Sous Linux, Mac et Windows, _télécharger [Guerre](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) __ dans `tomcat/webapps`:
...
Si ERDDAP™ est servi sur un chemin autre que `/erddap`, aussi défini l`en-tête `X-Forwarded-Prefix` à
...
* Quand ERDDAP™ commence, il renomme l'ancien ERDDAP™ fichier log.txt (`logArchivé À <CurrentTime> .txt') et crée un nouveau fichier log.txt.
Donc si le fichier `log.txt` est vieux, c'est un signe que ERDDAP™ n'a pas redémarré récemment. ERDDAP™ écrit des informations de journal dans un tampon
