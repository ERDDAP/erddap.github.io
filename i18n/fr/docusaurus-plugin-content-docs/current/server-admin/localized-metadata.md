...
Cette fonctionnalité vous permet d'inclure des métadonnées localisées sur vos ensembles de données et variables. Tout attribut défini dans un addAttributes tag peut être localisé. Ceci est destiné à être utilisé pour les attributs de chaîne commune comme le titre, résumé, licence, instituion. Il n'est pas recommandé d'utiliser pour le calcul numérique (Par exemple "_FillValue") ou des valeurs normalisées (Par exemple, "catégorie_ioos") et la localisation de ces types de valeurs peut avoir un comportement surprenant.

### Instructions d'utilisation
Pour les activer et les utiliser, suivez les étapes suivantes :

1.  **Ajouter des étiquettes ` datasets.xml ` ** :
Dans ` datasets.xml ` fichier, ajoutez vos métadonnées localisées dans la section Ajouter des attributs :
   ```xml
   <addAttributes>
...
2.  **Comportement par défaut** :
   - Si aucune balise xml:lang n'est fournie, les informations fournies seront affichées pour toutes les langues. Cela correspond au comportement précédent.
