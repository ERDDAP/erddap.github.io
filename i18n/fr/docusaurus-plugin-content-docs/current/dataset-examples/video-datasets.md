...
# Rechercher et afficher des vidéos dans ERDDAP™ 

Ce document montre comment afficher des vidéos dans ERDDAP™ , ainsi que
création d'un ensemble de données permettant de rechercher les métadonnées vidéo
trouver les vidéos qui répondent aux critères de recherche sélectionnés.

Les composants essentiels nécessaires pour mettre en place ce sont d'abord, les vidéos qui
sont dans un format qui peut être affiché dans un navigateur Web ( ERDDAP™ lui-même
n'affiche pas la vidéo, il ne fournit qu'un lien), et ensuite, un csv
fichier (ou un format similaire basé sur une table comme le parquet) qui contiennent les
métadonnées pertinentes pour les vidéos.

Avoir un format vidéo approprié est essentiel. Dans l'exemple ci-dessous,
les vidéos étaient à l'origine un vieux format de Windows que même les navigateurs Windows
n'a pas pu s'afficher hors de la boîte, et a exigé l'ajout d'un plugin. Vieille
les formats sont souvent de très grands fichiers. L'application de frein à main peut être
utilisé pour convertir les vidéos, ce qui est ce qui a été fait.

## Configuration

La première étape consiste à créer un ensemble de données --EDDTableFromFileNames--
il y a une URL associée à chaque vidéo qui est accessible par
des ERDDAP™ Commande de fichiers. Bien qu'un exemple de cet ensemble de données soit donné
ci-dessous, la meilleure façon de générer ce xml pour vos vidéos est d'utiliser
Générer des ensembles de données Xml.sh et pointez-le vers le répertoire avec les vidéos (dans
général la meilleure façon d'ajouter de nouveaux ensembles de données à ERDDAP™ est à utiliser
Générer des ensembles de données Xml.sh qui vous donnera la plupart du chemin, et
puis éditer cela pour le résultat final).

La deuxième étape consiste à créer une table EDDFromAsciiFiles (ou similaires) 
dataset qui contient les métadonnées à utiliser dans la vidéo
Recherche. Un exemple est donné ci-dessous. Pour que cela fonctionne, il doit y en avoir une
colonne qui contient un lien vers la vidéo, dans l'exemple il est le
Variable de fichier vidéo. Cette variable peut juste contenir l'URL complète
créé dans la première étape, mais alors le fichier n'est pas portable, comme si
vous voulez tester sur une machine puis servir sur une autre
machine.

Vous pouvez rendre le fichier portable en faisant l'entrée dans le fichier vidéo
le chemin relatif vers la vidéo, dans l'exemple ci-dessous un tel chemin est:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Ensuite, vous pouvez ajouter l'attribut d'AccessBaseUrl.
URL vers le chemin dans la colonne. Dans l'exemple:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Si vous changez de serveur, vous n'avez qu'à changer cet attribut
l'extrait de xml, vous n'avez pas besoin de modifier le fichier csv.

Une fois terminé, vous avez une liste des vidéos accessibles sous forme de fichiers :
...
une recherche simple mise en place par l'ensemble de données de -EDDTableFromFileNames:
...
et la recherche de métadonnées à l'adresse suivante:
...
Les options par défaut dans la dernière URL produisent l'URL:

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black sébastes](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 

et qui produit un tableau avec la dernière colonne:
...
Chacune des entrées de la réponse fournit un lien vers une vidéo, comme
Si vous souris sur le premier que vous obtenez:
...
et si vous cliquez dessus dans le html Tableau les affichages vidéo dans votre
navigateur.
