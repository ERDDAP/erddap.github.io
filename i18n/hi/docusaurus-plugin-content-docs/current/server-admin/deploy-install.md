...
हम इसे `/usr/local` में अनपैक करने की सलाह देते हैं।
     * मैक पर, टॉमकैट शायद पहले से ही `/Library/Tomcat` में स्थापित है, लेकिन इसे टॉमकैट 10 के नवीनतम संस्करण में अद्यतन करना चाहिए।
...
*  [सर्वर xml](#serverxml) - 'tomcat/conf/server.xml' फ़ाइल में, वहाँ दो परिवर्तन है कि आप दो ` <Connector> टैग
   ...
   1.  (अनुशंसित) 'कनेक्शन टाइमआउट' पैरामीटर मान को बढ़ाएं, शायद 300000 तक (मिलीसेकंड, जो 5 मिनट है) ।
...
इस प्रकार, केवल सुपर उपयोगकर्ता उपयोगकर्ता उपयोगकर्ता `tomcat` के रूप में कार्य करने के लिए स्विच कर सकते हैं। यह हैकर्स के लिए उपयोगकर्ता `tomcat` के रूप में अपने सर्वर में लॉग इन करना असंभव बनाता है।
...
यदि आपके सर्वर में 2GB से कम मेमोरी है, तो `-Xmx` मान को कम करें ('M'egaBytes) कंप्यूटर की भौतिक मेमोरी का 1/2।

* 64 बिट ऑपरेटिंग सिस्टम और 64 बिट के लिए Java :
64 बिट Java केवल 64 बिट ऑपरेटिंग सिस्टम पर काम करेगा।
  * साथ Java 8, आपको टॉमकैट `CATALINA_OPTS` पैरामीटर को `setenv.bat` में `-d64` जोड़ने की आवश्यकता है।
  * साथ Java 21, आप 64 बिट चुन सकते हैं Java जब आप एक संस्करण को डाउनलोड करते हैं Java चिह्नित "64 बिट"।

64 बिट के साथ Java टॉमकैट Java बहुत उच्च `-Xmx` और `-Xms` सेटिंग्स का उपयोग कर सकते हैं। सर्वर में बेहतर भौतिक मेमोरी।
...
`memoryUseBefore-&gt; memoryUse after`। यदि उन दो संख्याओं को एक साथ बंद कर दिया जाता है तो इसका मतलब है कि कचरा संग्रह उत्पादक नहीं था।
...
और `log.txt` फ़ाइल में उपलब्ध फ़ॉन्ट्स की एक सूची मुद्रित करेगा। आपको उन फ़ॉन्ट्स में से एक का उपयोग करना चाहिए।
...
*  ['tomcat/content/erddap/setup.xml' में टिप्पणियां पढ़ें `](#setupxml) और अनुरोधित परिवर्तन करें। सेटअप.xml सभी सेटिंग्स के साथ फ़ाइल है जो निर्दिष्ट करता है कि कैसे आपका ERDDAP™ व्यवहार करना।
...
` ERDDAP दौड़ने से पहले Name ERDDAP™ । उदाहरण के लिए, `` का उपयोग करें ERDDAP _baseUrl <baseUrl> मूल्य
...
`tomcat/content/erddap/images/erddapStart2.css` to `tomcat/content/erddap/images/erddap2.css` and फिर इसे बदल देता है।
...
NGINX प्राप्त करने के लिए और ERDDAP™ ठीक से काम करना https , आपको टॉमकैट सर्वर के अंदर निम्नलिखित स्निपेट लगाने की आवश्यकता है। <Host> `ब्लॉक:
```
...
लिनक्स पर `ps-ef | पहले और बाद में `shutdown.sh` के बाद यह सुनिश्चित करने के लिए कि tomcat प्रक्रिया बंद हो गया है grep tomcat.
...
इसलिए यदि `log.txt` फ़ाइल पुरानी है, तो यह एक संकेत है कि ERDDAP™ हाल ही में पुनः आरंभ नहीं हुआ है। ERDDAP™ एक बफर को लॉग जानकारी लिखता है
...
* होम पेज के बाईं ओर की जानकारी को संपादित करके बदलें <theShortDescriptionHtml> `` में टैग datasets.xml फ़ाइल
(यदि कोई नहीं है, तो डिफ़ॉल्ट को से कॉपी करें ERDDAP™ 'tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/message.xml` फ़ाइल
` datasets.xml इसे संपादित करें उदाहरण के लिए, आप कर सकते हैं:
