...
यदि वितरण एक Windows इंस्टॉलर है, तो संभवतः इसे टॉमकैट रखा जाएगा, उदाहरण के लिए, `/Program Files/apache-tomcat-10.0.23` ।
             
### सर्वर xml{#serverxml} 

*  [सर्वर xml](#serverxml) - में `Tomcat/conf/server.xml` फ़ाइल, वहाँ दो परिवर्तन है कि आप दो में से प्रत्येक के लिए करना चाहिए कर रहे हैं ` <Connector> ` टैग
...
इस फ़ाइल का उपयोग करके किया जाएगा `Tomcat/bin/startup.sh` और `बंद करना। sh` । फ़ाइल में कुछ समान होना चाहिए:
  ```
...
The `-Xmx` और `-Xms` स्मृति सेटिंग्स महत्वपूर्ण हैं क्योंकि ERDDAP™ अधिक स्मृति के साथ बेहतर काम करता है।
...
संग्रह क्योंकि यह एडेन में कमरे से बाहर चला गया (अनुभाग Java बहुत युवा वस्तुओं के लिए ढेर) । आमतौर पर संदेश आपको दिखाता है
   `स्मृति UseBefore-&gt; स्मृति उपयोग के बाद` । यदि उन दो नंबर एक साथ बंद हैं, तो इसका मतलब है कि कचरा संग्रह उत्पादक नहीं था।
...
और में उपलब्ध फ़ॉन्ट्स की एक सूची मुद्रित करेगा `लॉग-इन` फ़ाइल आपको उन फ़ॉन्टों में से एक का उपयोग करना चाहिए।

यदि आप Dejavu fonts का उपयोग करना चुनते हैं, तो कृपया सुनिश्चित करें कि आप Dejavu fonts का उपयोग करें। ` <fontFamily> ` सेटअप में सेटिंग xml है ` <fontFamily> Dejavu Sans </fontFamily> ` ।
...
unzip `erddapContent .zip ` वांछित निर्देशिका में (केवल `tomcat` उपयोगकर्ता की पहुँच है) और सेट ` erddapContentDirectory ` सिस्टम संपत्ति
 (उदाहरण ` erddapContentDirectory  =~tomcat/content/erddap ` ) इसलिए ERDDAP™ इस नई सामग्री निर्देशिका को पा सकते हैं।

### साइटमैप{#setupxml} 

*  [में टिप्पणी पढ़ें `tomcat/content/erddap/setup.xml` ](#setupxml) और अनुरोधित परिवर्तन करें। सेटअप.xml सभी सेटिंग्स के साथ फ़ाइल है जो निर्दिष्ट करता है कि कैसे आपका ERDDAP™ व्यवहार करता है।
...
4. लिनक्स, मैक और विंडोज पर, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.30.0/ERDDAP-2.30.0.war) __ in `Tomcat/webapps` :

__Version 2.30.0, 706,939,121 बाइट्स, MD5=CDC4B3D82A20B33A6623B85312F6DC21, दिनांक 2026-04-06____
...
तो अगर `लॉग-इन` फ़ाइल पुरानी है, यह एक संकेत है कि ERDDAP™ हाल ही में पुनः आरंभ नहीं हुआ है। ERDDAP™ एक बफर के लिए लॉग जानकारी लिखता है
...
टॉमकैट को पहली बार एक आवेदन की तरह काम करने का एक बहुत कुछ करना है ERDDAP™ शुरू हो गया है; विशेष रूप से, इसे पैक नहीं करना चाहिए `erddap.war` फ़ाइल
