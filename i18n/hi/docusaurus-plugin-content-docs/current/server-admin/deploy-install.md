...
कैसे करने के लिए प्रारंभिक सेटअप ERDDAP™ अपने सर्वर पर
 ...
 ERDDAP™ किसी भी सर्वर पर चलाया जा सकता है जो समर्थन करता है Java टॉमकैट (और अन्य अनुप्रयोग सर्वर जैसे जेटी, लेकिन हम उन्हें समर्थन नहीं देते) ।
...
IOOS अब प्रदान करता है [क्विक स्टार्ट गाइड ERDDAP™ एक डोकर कंटेनर में](https://ioos.github.io/erddap-gold-standard/index.html) ।
...
स्थापना को सत्यापित करने के लिए, उदाहरण के लिए `/javaJreBinDirectory/java -version` चलाएं
...
सुरक्षा कारणों के लिए कृपया अपना अद्यतन करें ERDDAP का संस्करण Java समय-समय पर नए संस्करणों के रूप में Java 21 अडॉप्टियम से उपलब्ध हो जाते हैं।
...
सुरक्षा कारणों से, टॉमकैट 10 के नवीनतम संस्करण का उपयोग करना लगभग हमेशा सबसे अच्छा होता है (संस्करण 9 और नीचे स्वीकार्य नहीं हैं) 
जिसके साथ काम करने के लिए डिज़ाइन किया गया है Java 21 या नया। नीचे, टॉमकैट निर्देशिका को 'tomcat' कहा जाएगा।
...
हम इसे `/usr/local` में अनपैक करने की सलाह देते हैं।
     * मैक पर, टॉमकैट शायद पहले से ही `/Library/Tomcat` में स्थापित है, लेकिन इसे टॉमकैट 10 के नवीनतम संस्करण में अपडेट करना चाहिए।
अगर आप इसे डाउनलोड करते हैं, ["कोर" "tar" डाउनलोड करें .gz टॉमकैट वितरण](https://tomcat.apache.org/download-10.cgi) और इसे `/Library/Tomcat` में अनपैक करें।
     * विंडोज पर, आप कर सकते हैं ["कोर" "zip" टॉमकैट वितरण डाउनलोड करें](https://tomcat.apache.org/download-10.cgi) 
        ...
        (विकास के लिए, हम "कोर" "zip" वितरण का उपयोग करते हैं। हम `/programs` डाइरेक्टरी बनाते हैं और इसे वहाँ अनपैक करते हैं।) 
या आप "कोर" "64-bit Windows zip" वितरण डाउनलोड कर सकते हैं, जिसमें अधिक विशेषताएं शामिल हैं।
यदि वितरण एक Windows इंस्टॉलर है, तो संभवतः इसे टॉमकैट डाल दिया जाएगा, उदाहरण के लिए, `/Program Files/apache-tomcat-10.0.23`।
             
### सर्वर xml{#serverxml} 

*  [सर्वर xml](#serverxml) - 'tomcat/conf/server.xml' फ़ाइल में, वहाँ दो परिवर्तन है कि आप दो ` <Connector> टैग
   (`&lt;Connector port="8080"` के लिए एक और `&lt;Conector port="8443"` के लिए एक) ।
   1.  (अनुशंसित) 'कनेक्शन टाइमआउट' पैरामीटर मान में वृद्धि, शायद 300000 करने के लिए (मिलीसेकंड, जो 5 मिनट है) ।
...
* संदर्भ.xml -- संसाधन कैश - `tomcat/conf/context.xml` में, `tomcat/conf/context.xml` से ठीक पहले </Context> ` टैग, संसाधन टैग बदलें
...
* लिनक्स कंप्यूटर पर, अपाचे टाइमआउट सेटिंग्स को बदल दें ताकि समय लेने वाले उपयोगकर्ता अनुरोध टाइमआउट नहीं करते
  ...
  * अपाचे को संशोधित करें ` http फ़ाइल (आमतौर पर `/etc/ में http d/conf/ `) :
    * मौजूदा ` बदलें <Timeout> सेटिंग (या फ़ाइल के अंत में एक जोड़ें) 3600 (सेकंड) , डिफ़ॉल्ट 60 या 120 सेकंड के बजाय।
    * मौजूदा ` बदलें <ProxyTimeout> सेटिंग (या फ़ाइल के अंत में एक जोड़ें) 3600 (सेकंड) , डिफ़ॉल्ट 60 या 120 सेकंड के बजाय।
...
विशेष रूप से सार्वजनिक सर्वरों के लिए आपका टॉमकैट इंस्टॉलेशन।
         
* जनता के लिए ERDDAP™ लिनक्स और मैक पर इंस्टॉलेशन, टॉमकैट स्थापित करना सबसे अच्छा है (कार्यक्रम) उपयोगकर्ता `tomcat `
...
इस प्रकार, केवल सुपर उपयोगकर्ता उपयोगकर्ता 'tomcat' के रूप में कार्य करने के लिए स्विच कर सकते हैं। यह हैकर्स के लिए उपयोगकर्ता `tomcat` के रूप में अपने सर्वर पर लॉग इन करना असंभव बनाता है।
और किसी भी मामले में, आपको ऐसा करना चाहिए कि सर्वर के फ़ाइल सिस्टम (read+write+execute विशेषाधिकार) पर `tomcat` उपयोगकर्ता की बहुत सीमित अनुमति है।
'apache-tomcat' निर्देशिका पेड़ और `` के लिए <bigParentDirectory> डेटा के साथ निर्देशिकाओं के लिए केवल विशेषाधिकार ERDDAP™ पहुँच की जरूरत है).
  * आप 'tomcat' उपयोगकर्ता खाता बना सकते हैं (जो पासवर्ड नहीं है) आदेश का उपयोग करके:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * आप उपयोगकर्ता `tomcat` के रूप में काम करने के लिए स्विच कर सकते हैं आदेश का उपयोग करके
    ```
    sudo su - tomcat
    ```
    ...
    * आप कमांड का उपयोग करके उपयोगकर्ता tomcat के रूप में काम करना बंद कर सकते हैं
    ...
    exit
    ````
    * टॉमकैट के बाकी हिस्सों में से अधिकांश और ERDDAP™ उपयोगकर्ता `tomcat` के रूप में सेटअप निर्देश। बाद में, उपयोगकर्ता `tomcat' के रूप में `startup.sh` और `shutdown.sh` स्क्रिप्ट चलाएं `
ताकि टॉमकैट अपनी लॉग फ़ाइलों को लिखने की अनुमति दे।
    * अनपैकिंग के बाद, 'apache-tomcat' निर्देशिका के माता-पिता से:
      ...
      * Tomcat, आपका उपयोगकर्ता नाम, या एक छोटे समूह का नाम जिसमें टॉमकैट और टॉमकैट के सभी प्रशासक शामिल हैं। ERDDAP :
        ```
...
इस फ़ाइल का उपयोग 'tomcat/bin/startup.sh` और `shutdown.sh` द्वारा किया जाएगा। फ़ाइल में कुछ समान होना चाहिए:
  ```
  ...
  ```
   ...
   (यदि आप पहले `JRE_HOME` सेट करते हैं, तो आप इसे हटा सकते हैं।) 
मैक पर, आपको शायद 'JAVA_HOME' सेट करने की आवश्यकता नहीं है।

* विंडोज पर:
टॉमकैट के पर्यावरण चर सेट करने के लिए एक फ़ाइल `tomcat \bin \\setenv.bat` बनाएं।
इस फ़ाइल का उपयोग 'tomcat \bin \\ startup.bat` और `` द्वारा किया जाएगा। shutdown.bat `।
फ़ाइल में कुछ समान होना चाहिए:
  ```
...
`-Xmx` और `-Xms` मेमोरी सेटिंग्स महत्वपूर्ण हैं क्योंकि ERDDAP™ अधिक स्मृति के साथ बेहतर काम करता है।
हमेशा `-Xms` को समान मूल्य के रूप में सेट करें।

* 32 बिट ऑपरेटिंग सिस्टम और 32 बिट के लिए Java :
64 बिट Java 32 बिट से ज्यादा बेहतर है Java लेकिन 32 बिट Java जब तक सर्वर वास्तव में व्यस्त नहीं है तब तक काम करेगा।
सर्वर में अधिक भौतिक मेमोरी बेहतर: 4 + जीबी वास्तव में अच्छा है, 2 जीबी ठीक है, कम अनुशंसित नहीं है।
32 बिट Java यहां तक कि प्रचुर मात्रा में भौतिक स्मृति, टॉमकैट और Java यदि आप 1500M से अधिक `-Xmx` सेट करने की कोशिश करते हैं तो नहीं चलेंगे (कुछ कंप्यूटरों पर 1200M) ।
यदि आपके सर्वर में 2GB मेमोरी से कम है, तो `-Xmx` मान को कम करें ('M'egaBytes) कंप्यूटर की भौतिक मेमोरी का 1/2।

* 64 बिट ऑपरेटिंग सिस्टम और 64 बिट के लिए Java :
64 बिट Java केवल 64 बिट ऑपरेटिंग सिस्टम पर काम करेगा।
  * साथ Java 8, आपको 'setenv.bat' में 'CATALINA_OPTS' पैरामीटर में 'D64' जोड़ने की आवश्यकता है।
  * साथ Java 21, आप 64 बिट चुन सकते हैं Java जब आप एक संस्करण डाउनलोड करते हैं Java चिह्नित "64 बिट"।

64 बिट के साथ Java टॉमकैट Java बहुत अधिक `-Xmx` और `-Xms` सेटिंग्स का उपयोग कर सकते हैं। सर्वर में बेहतर भौतिक मेमोरी।
एक सरल सुझाव के रूप में: हम अनुशंसा करते हैं कि आप `-Xmx` और `-Xms` सेट करें ('M'egaBytes) 1/2 (या कम) कंप्यूटर की भौतिक मेमोरी की।
...
या `bigParentDirectory/logs/ में [लॉग-इन](/docs/server-admin/additional-information#log) फ़ाइल (`bigParentDirectory` में निर्दिष्ट है [साइटमैप](#setupxml) ) ।
...
संग्रह क्योंकि यह एडेन में कमरे से बाहर चला गया (अनुभाग Java बहुत युवा वस्तुओं के लिए ढेर) । आमतौर पर संदेश आपको दिखाता है
`memoryUseBefore-&gt; memoryUse after`। यदि उन दो नंबर एक साथ बंद हैं, तो इसका मतलब है कि कचरा संग्रह उत्पादक नहीं था।
...
* यदि आप `java.lang.OutOfMemoryError`s in ERDDAP™ ' [लॉग-इन](/docs/server-admin/additional-information#log) फ़ाइल,
...
*  [लिनक्स और मैक पर, अनुमतियों को बदल दें](#permissions) मालिक द्वारा निष्पादन योग्य होने के लिए 'tomcat/bin/` में सभी `*.sh` फ़ाइलों की संख्या:
  ```
  chmod +x *.sh
  ```
...
*  [छवियों के लिए फ़ॉन्ट्स:](#fonts) हम दृढ़ता से मुक्त पसंद करते हैं [Dejavu fonts](https://dejavu-fonts.github.io/) अन्य Java फोंट
इन फोंटों का उपयोग दृढ़ता से अनुशंसित है लेकिन आवश्यक नहीं है।

यदि आप DejaVu फ़ॉन्ट्स का उपयोग नहीं करते हैं, तो आपको सेटअप में फ़ॉन्टपरिवार सेटिंग को बदलने की आवश्यकता है। <fontFamily> Sanserif </fontFamily> `,
जो सभी के साथ उपलब्ध है Java वितरण। यदि आप `` सेट करते हैं <fontFamily> ` एक फ़ॉन्ट का नाम जो उपलब्ध नहीं है, ERDDAP™ भार नहीं
और `log.txt` फ़ाइल में उपलब्ध फ़ॉन्ट्स की एक सूची मुद्रित करेगा। आपको उन फ़ॉन्टों में से एक का उपयोग करना चाहिए।

यदि आप DejaVu फ़ॉन्ट्स का उपयोग करना चुनते हैं, तो कृपया सुनिश्चित करें कि ````` <fontFamily> सेटअप में सेटिंग.xml है ` <fontFamily> Dejavu Sans </fontFamily> `।
...
और एक अस्थायी निर्देशिका के लिए फ़ॉन्ट फ़ाइलों को खोलना।
  ...
  * लिनक्स पर:
    ...
    * अन्य Java वितरण: `tomcat` उपयोगकर्ता के रूप में, फ़ॉन्ट फ़ाइलों को `$JAVA_HOME/lib/fonts` में कॉपी करें, इसलिए Java फ़ॉन्ट्स पा सकते हैं।
...
* अपने टॉमकैट इंस्टॉलेशन का परीक्षण करें।
  * लिनक्स:
    * उपयोगकर्ता "tomcat" के रूप में, `tomcat/bin/startup.sh` चलाते हैं।
...
यदि कोई समस्या है, तो टॉमकैट लॉग फ़ाइल को 'tomcat/logs/catalina.out` पर देखें।
...
यही कारण है कि कुछ के साथ एक लाइन वापस आना चाहिए:

  ```
  tcp 0 0 :::8080 :::* LISTEN ## ##### ####/java
  ```

   (जहाँ `#` कुछ अंक हैं) , यह दर्शाता है कि 'जावा' प्रक्रिया (संभवतः टॉमकैट) "tcp" यातायात के लिए बंदरगाह "8080" पर सुन रहा है।
यदि कोई रेखा वापस नहीं हुई थी, तो यदि लाइन लौटी तो काफी अलग है, या यदि दो या दो से अधिक लाइन वापस हो गई तो पोर्ट सेटिंग के साथ समस्या हो सकती है।
...
3.   ['tomcat/content/erddap' विन्यास फ़ाइलों को सेट करें।](#erddap-content) 
...
इसे 'tomcat' निर्देशिका में खोलना, 'tomcat/content/erddap' बनाना।
...
unzip .zip वांछित निर्देशिका में (जिसके लिए केवल 'tomcat' उपयोगकर्ता का उपयोग होता है) और सेट ` erddapContentDirectory ` प्रणाली संपत्ति
 (E.g. ` erddapContentDirectory  =~tomcat/content/erddap `) इसलिए ERDDAP™ इस नई सामग्री निर्देशिका को पा सकते हैं।

### साइटमैप{#setupxml} 

*  ['tomcat/content/erddap/setup.xml' में टिप्पणियां पढ़ें `](#setupxml) और अनुरोधित परिवर्तन करें। सेटअप.xml सभी सेटिंग्स के साथ फ़ाइल है जो निर्दिष्ट करता है कि कैसे आपका ERDDAP™ व्यवहार करता है।
...
जब आप बिगParentDirectory बनाते हैं, तो बिगParentDirectory की मूल निर्देशिका से:
    ...
    * 'tomcat' उपयोगकर्ता को 'bigParentDirectory' का मालिक बनाएं:
      ```
...
` ERDDAP दौड़ने से पहले ERDDAP™ । उदाहरण के लिए, ` ERDDAP _baseUrl <baseUrl> मूल्य
...
यह सुनिश्चित करने के लिए कि सूचना गुप्त रहेगी। ERDDAP™ केवल स्टार्टअप प्रति एक बार पर्यावरण चर पढ़ा जाता है,
...
पहली बार (आमतौर पर सिर्फ डिफ़ॉल्ट डेटासेट के साथ) , आप XML को 'tomcat/content/erddap/ में बदल देंगे datasets.xml `
...
आप एक उदाहरण देख सकते हैं [ datasets.xml गिटहब](https://github.com/ERDDAP/erddap/blob/main/development/jetty/config/datasets.xml) ।
     
*  (विपरीत) अब या (थोड़ा अधिक संभावना) भविष्य में, यदि आप erddap की CSS फ़ाइल को संशोधित करना चाहते हैं, तो कॉपी करें
`tomcat/content/erddap/images/erddapStart2.css` to `tomcat/content/erddap/images/erddap2.css`. और फिर इसमें बदलाव करते हैं।
`erddap2.css` में परिवर्तन केवल तभी प्रभावी होता है जब ERDDAP™ पुनः आरंभ किया जाता है और अक्सर उपयोगकर्ता को ब्राउज़र की कैश्ड फ़ाइलों को साफ़ करने की भी आवश्यकता होती है।
     
...
यह सत्यापित करने के लिए एक अच्छा विचार है कि परिणाम XML टेक्स्ट को XML चेकर में पेस्ट करके अच्छी तरह से निर्मित XML है। [xmlvalidation](https://www.xmlvalidation.com/) ।
     
### erddap स्थापित करें। युद्ध{#install-the-erddapwar-file} 

4. लिनक्स, मैक और विंडोज पर, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.0/erddap.war) __ in 'tomcat/webapps':

__Version 2.28.0, 620,824,288 बाइट्स, MD5=f948b2ba603f65a83ac67af43da9e4c2, दिनांक 2025-08-29___
...
एसएसएल / टीएलएस समाप्ति अक्सर वेबसर्वर प्रॉक्सी परत के साथ-साथ हल हो जाती है। विशिष्ट प्रत्येक तैनाती की आवश्यकताओं पर निर्भर करते हैं।

#### अपाचे{#apache} 

1. सुनिश्चित करें कि `mod_proxy` और `mod_proxy_ http ` लोड हो रहा है:
...
अगर ERDDAP™ `/erddap` के अलावा एक पथ पर सेवा की जाती है, जिसे `X-Forwarded-Prefix` शीर्षक भी सेट किया जाता है।
पथ खंड यह सेटिंग एक के लिए उपयुक्त होगी ERDDAP™ परोसा
`/subpath/erddap`:
...
nginx विन्यास फ़ाइल में, इन शीर्षलेखों को सेट करें:
```
...
या (Windows में) `tomcat\bin\' shutdown.bat `

लिनक्स पर `ps-ef | पहले और बाद में `shutdown.sh` के बाद यह सुनिश्चित करने के लिए कि tomcat प्रक्रिया बंद हो गई है।
प्रक्रिया को बंद होने से पहले सूचीबद्ध किया जाना चाहिए और अंततः बंद होने के बाद सूचीबद्ध नहीं किया जाना चाहिए।
यह एक मिनट या दो के लिए ले सकता है ERDDAP™ पूरी तरह बंद करने के लिए। रोगी बनें या अगर ऐसा लगता है कि यह अपने आप को बंद नहीं करेगा, तो इसका उपयोग करें:
-9 <processID> `
* साथ में टॉमकैट शुरू (लिनक्स या मैक ओएस) 'tomcat/bin/startup.sh' or (Windows में) `tomcat \bin \\ startup.bat `
...
टॉमकैट लॉग फ़ाइलों में (`tomcat/logs/catalina.out` और उस निर्देशिका में अन्य फाइलें) ।
...
* कब ERDDAP™ शुरू होता है, यह पुराने नाम बदलता है ERDDAP™ log.txt फ़ाइल (`logArchived पर <CurrentTime> .txt`) और एक नई log.txt फ़ाइल बनाता है।
इसलिए यदि `log.txt` फ़ाइल पुरानी है, तो यह एक संकेत है कि ERDDAP™ हाल ही में पुनः आरंभ नहीं हुआ है। ERDDAP™ एक बफर के लिए लॉग जानकारी लिखता है
...
### मुसीबत: धीमी गति से स्टार्टअप पहली बार{#trouble-slow-startup-first-time} 

टॉमकैट को पहली बार एक आवेदन की तरह काम करने का एक बहुत कुछ करना है ERDDAP™ शुरू किया गया है; विशेष रूप से, इसे `erddap.war` फ़ाइल को अनपैक करना होगा।
...
भविष्य में बंद करने के लिए (फिर से शुरू)   ERDDAP™ , देखें [कैसे बंद करें और टोमकैट को फिर से शुरू करें ERDDAP ](/docs/server-admin/additional-information#shut-down-and-restart) ।
...
यदि आप एक ईमेल प्राप्त करना चाहते हैं, तो एक नया संस्करण ERDDAP™ उपलब्ध है या अन्य महत्वपूर्ण ERDDAP™ घोषणा
...
* सभी के शीर्ष पर दिखाई देने वाले बैनर को बदलें ERDDAP™ HTML पृष्ठों को संपादित करके <startBodyHtml5> `` में टैग datasets.xml फ़ाइल
(यदि कोई नहीं है, तो डिफ़ॉल्ट को से कॉपी करें ERDDAP™ 'tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/message.xml` फ़ाइल
` datasets.xml इसे संपादित करें उदाहरण के लिए, आप कर सकते हैं:
  * एक अलग छवि का उपयोग करें (अर्थात्, आपका संगठन का लोगो) ।
  * पृष्ठभूमि रंग बदलें।
...
* `` संपादन द्वारा होम पेज के बाईं ओर की जानकारी बदलें <theShortDescriptionHtml> `` में टैग datasets.xml फ़ाइल
(यदि कोई नहीं है, तो डिफ़ॉल्ट को से कॉपी करें ERDDAP™ 'tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/message.xml` फ़ाइल
` datasets.xml इसे संपादित करें उदाहरण के लिए, आप कर सकते हैं:
  * अपने संगठन और/या समूह के बारे में बताएं।
  * इस बात का वर्णन करें कि किस तरह का डेटा इस तरह है ERDDAP™ है।
  * ब्राउज़र टैब पर दिखाई देने वाले आइकन को बदलने के लिए, अपने संगठन के फ़ैविकॉन को डाल दें। 'tomcat/content/erddap/images/` में ico।
