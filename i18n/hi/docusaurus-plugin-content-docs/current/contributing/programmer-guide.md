...
हाल के सार्वजनिक संस्करणों और विकास संस्करणों के लिए स्रोत कोड भी उपलब्ध है [गिटहब](https://github.com/ERDDAP) । कृपया पढ़िए [विकी](https://github.com/ERDDAP/erddap/wiki) उस परियोजना के लिए। यदि आप स्रोत कोड को संशोधित करना चाहते हैं (और संभवतः परिवर्तन मानक में शामिल किया गया है ERDDAP™ वितरण) , यह अनुशंसित दृष्टिकोण है।
...
आप उपयोग कर सकते हैं `Mvn compile` और यह निर्भरता और रेफरी फ़ाइलों को प्राप्त करेगा। आप भी उपयोग कर सकते हैं `mvn पैकेज` एक युद्ध फ़ाइल उत्पन्न करने के लिए।
आप मैन्युअल रूप से रेफरी फ़ाइलों को डाउनलोड कर सकते हैं:
  ...
  -  [erddapContent .zip ](https://github.com/ERDDAP/erddapContent/releases/download/content1.0.1/erddapContent.zip)   (संस्करण 1.0.0, 20333 बाइट्स, MD5=2B8D2A5AE5ED73E3A42B529C168C60B5, दिनांक 2024-10-14) To unzip it in _tomcat_. _tomcat_/content/erddap ।
...
-  ERDDAP™ और इसके उपमहाद्वीप बहुत उदार, खुला स्रोत हैं [लाइसेंस](/license) इसलिए आप किसी भी उद्देश्य के लिए स्रोत कोड का उपयोग और संशोधित कर सकते हैं। ध्यान दें कि ERDDAP™ और कई उप-घटकों में लाइसेंस होते हैं जिनकी आवश्यकता होती है कि आप उस कोड के स्रोत को स्वीकार करते हैं जिसका आप उपयोग कर रहे हैं। देखें [ऋण](/credits) । चाहे आवश्यकता हो या नहीं, इन सभी योगदानकर्ताओं को स्वीकार करने के लिए यह सिर्फ अच्छा रूप है।
...
###  **विकास वातावरण**  {#development-environment} 
  ...
  -  **वैकल्पिक** : सेट अप ERDDAP™ टॉमकैट में
चूंकि ERDDAP™ मुख्य रूप से टॉमकैट में चल रहे एक servlet होने का इरादा है, हम दृढ़ता से अनुशंसा करते हैं कि आप मानक का पालन करें [स्थापना निर्देश](/docs/server-admin/deploy-install) टॉमकैट स्थापित करने के लिए और फिर स्थापित करने के लिए ERDDAP™ टॉमकैट की वेबअप डायरेक्टरी में। अन्य बातों के अलावा, ERDDAP™ टॉमकैट की निर्देशिका संरचना में स्थापित किया गया था और टॉमकैट को कुछ .jar फ़ाइलों को प्रदान करने की उम्मीद थी।
  ...
  - हम वर्तमान में एडॉप्टियम के javac jdk-25.0.1+8 का उपयोग करते हैं ताकि gov.noaa.pfeg.coastwatch.Testall (इसमें कुछ वर्गों के लिंक होते हैं जिन्हें अन्यथा संकलित नहीं किया जाएगा) और परीक्षण चलाते हैं। सुरक्षा कारणों के लिए, नवीनतम संस्करणों का उपयोग करना लगभग हमेशा सबसे अच्छा होता है Java 25 और टॉमकैट 10।

    - जब हम जावा चलाते हैं, तो वर्तमान निर्देशिका है _tomcat_/webapps/erddap/WEB-INF.

    - हमारे javac और java classpath है
       `कक्षाएं;./../.. /lib/servlet-api.jar;lib/*` 
...
`java-cp class; ../../.. /lib/servlet-api.jar;lib/* -Xmx4000M -Xms4000M कक्षाएं/सरकारी/noaa/pfel/coastwatch/Testall
       `वैकल्पिक: आप जोड़ सकते हैं` -verbose:gc`, जो बताता है Java कचरा संग्रह आंकड़े प्रिंट करने के लिए।
  ...
  - कुछ मामलों में, हम इसके बजाय तीसरे पक्ष के स्रोत कोड का उपयोग करते हैं। (विशेष रूप से DODS ) और उन्हें थोड़ा संशोधित करने के लिए समस्याओं से बचने के लिए Java 25. हमने अक्सर अन्य मामूली संशोधन किए हैं (विशेष रूप से DODS ) अन्य कारणों से।

  - अधिकांश कक्षाओं में उनके संबद्ध src/test फ़ाइल में परीक्षण विधियां हैं। आप JUnit परीक्षण कर सकते हैं `Mvn परीक्षण` आदेश यह डेटा की कई ज़िप फ़ाइलों को डाउनलोड करेगा जो परीक्षण की नवीनतम रिलीज से निर्भर करता है [ ERDDAP /erddap टेस्ट](https://github.com/ERDDAP/erddapTest/releases/) .
     
नोट: मैवेन कैश डाउनलोड लेकिन प्रत्येक निष्पादन पर डाउनलोड किए गए अभिलेखागार को खोलना होगा, जो समय लेता है। डाउनलोड करना
और unzipping परीक्षण डेटा संग्रह, आप निर्दिष्ट कर सकते हैं `डाउनलोड` Maven संपत्ति (उदाहरण `Mvn -DskipTestResourceडाउनलोड पैकेज` ) ।
...
यदि कोड आपके संगठन के लिए विशिष्ट है, तो शायद इसकी एक अलग शाखा को बनाए रखना सबसे अच्छा है। ERDDAP™ अपने उपयोग के लिए। Axiom यह करता है। दुर्भाग्यवश, गिट ऐसा करना आसान बनाता है। NATD के लिए एक सुसंगत दृष्टि बनाए रखना चाहता है ERDDAP , यह एक रसोई सिंक परियोजना बनने की अनुमति नहीं है जहां हर कोई अपनी परियोजना के लिए एक कस्टम सुविधा जोड़ता है।
  ...
  - पालन करें Java कोड कन्वेंशन।
सामान्य तौर पर, आपका कोड अच्छी गुणवत्ता होना चाहिए और मूल का पालन करना चाहिए [ Java कोड कन्वेंशन](https://www.oracle.com/technetwork/java/codeconventions-150003.pdf) : निर्देशिका संरचना में उचित स्थान पर .class फ़ाइलों को डालें, .class फ़ाइलों को एक उचित नाम दें, उचित रूप से शामिल करें Java डॉक टिप्पणियां, कोड के प्रत्येक पैराग्राफ के प्रारंभ में शामिल //टिप्पणियां, 4 स्पेस के साथ इंडेंट (टैब नहीं) , रेखाओं से बचें &gt; 80 वर्ण आदि। सम्मेलनों में परिवर्तन और स्रोत कोड हमेशा पूरी तरह से तारीख तक नहीं है। जब संदेह में, सम्मेलनों के लिए मैच कोड और मौजूदा कोड नहीं।
...
लंबे समय तक, यह सबसे अच्छा है यदि आप अपने कोड को बनाए रखने और उसके बारे में सवालों का जवाब देने के लिए चल रहे दायित्व को मानते हैं। (उदाहरण के लिए, में ERDDAP™ गूगल ग्रुप) । कुछ लेखकों के नोट के रूप में, कोड एक दायित्व के साथ-साथ परिसंपत्ति भी है। यदि भविष्य में एक बग खोजा जाता है, तो यह सबसे अच्छा है यदि आप इसे ठीक करते हैं क्योंकि कोई आपके कोड को बेहतर नहीं जानता है (यह भी कि पहले स्थान पर बग से बचने के लिए एक प्रोत्साहन है) । NATD जारी रखरखाव प्रदान करने के लिए एक फर्म प्रतिबद्धता के लिए नहीं पूछ रहा है। NATD सिर्फ यह कह रहा है कि रखरखाव बहुत सराहना की जाएगी।
