...
Wenn Sie es herunterladen, [den "Core" "Tar" herunterladen .gz " Tomcat Verteilung](https://tomcat.apache.org/download-10.cgi) und in `/Library/Tomcat` auspacken.
     * Unter Windows können Sie [die "Core" "zip" Tomcat Distribution herunterladen](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Für die Entwicklung verwenden wir die "Core" "zip" Distribution. Wir erstellen ein `/programs`-Verzeichnis und entpacken es dort.) 
Oder Sie können die "Core" "64-bit Windows zip" Distribution herunterladen, die mehr Funktionen beinhaltet.
Wenn die Distribution ein Windows-Installer ist, wird es wahrscheinlich Tomcat in, z.B. `/Programme/apache-tomcat-10.0.23`.
             
...
* Ändern Sie auf Linux-Computern die Apache-Timeout-Einstellungen, so dass zeitraubende Benutzeranfragen nicht Timeout
  ...
  * Ändern des Apache ` http d.conf-Datei (in der Regel `/etc/ http d/conf/ `) :
    * Ändern Sie die bestehende ` <Timeout> ` Einstellung (oder eine am Ende der Datei hinzufügen) bis 3600 (Sekunden) , anstatt der Standard 60 oder 120 Sekunden.
    * Ändern Sie die bestehende ` <ProxyTimeout> ` Einstellung (oder eine am Ende der Datei hinzufügen) bis 3600 (Sekunden) , anstatt der Standard 60 oder 120 Sekunden.
...
für den `apache-tomcat` Verzeichnisbaum und `` <bigParentDirectory> ` und lesen Sie nur Privilegien für Verzeichnisse mit Daten, die ERDDAP™ müssen Zugang zu).
  ...
  * Sie können als Benutzer wechseln `tomcat` mit dem Befehl
    ...
    * Sie können die Arbeit als Benutzer-Tomcat durch den Befehl stoppen
    ...
    exit
    ````
    * Den größten Teil des restlichen Tomcat und ERDDAP™ Setupanleitung als Benutzer `tomcat`. Später führen Sie die `startup.sh` und `shutdown.sh` Skripte als Benutzer `tomcat `
...
* Auf Linux und Macs:
Erstellen einer Datei `tomcat/bin/setenv.sh ` (oder in Red Hat Enterprise Linux \\[ RHEL \\] , edit `~tomcat/conf/tomcat10.conf `) Tomcats Umgebungsvariablen einstellen.
Diese Datei wird von `tomcat/bin/startup.sh` und `shutdown.sh` verwendet. Die Datei sollte etwas wie:
  ...
  ```
   ...
   (Wenn Sie vorher `JRE_HOME` gesetzt haben, können Sie das entfernen.) 
...
Diese Datei wird von `tomcat\bin\\startup.bat` verwendet und ` shutdown.bat `.
...
Als einfacher Vorschlag: Wir empfehlen Ihnen, `-Xmx` und `-Xms` zu setzen (in 'M'egaBytes) bis 1/2 (oder weniger) der physischen Erinnerung des Computers.
...
Wenn Sie die DejaVu-Schriften nicht verwenden möchten, müssen Sie die SchriftartFamilieneinstellung in setup.xml zu ` ändern <fontFamily> SansSerif </fontFamily> `,
...
Das sollte eine Zeile mit etwas wie:
  ...
  ```
   ...
   (wo `#` eine Ziffer ist) , angeben, dass ein `java` Prozess (Vermutlich Tomcat) hört auf Port "8080" für "tcp" Verkehr.
...
und entpacken Sie es in das `tomcat`-Verzeichnis und erstellen Sie `tomcat/content/erdap`.
...
Unzip `erddapContent .zip ` in das gewünschte Verzeichnis (nur der `tomcat` Benutzer hat Zugriff) und ` erddapContentDirectory ` Systemimmobilie
 (z.B. ` erddapContentDirectory  =~tomcat/content/erddap `) also ERDDAP™ finden Sie dieses neue Content-Verzeichnis.

### Setup.xml{#setupxml} 

*  [Lesen Sie die Kommentare in `tomcat/content/erddap/setup.xml `](#setupxml) und die angeforderten Änderungen vornehmen. setup.xml ist die Datei mit allen Einstellungen, die festlegen, wie Ihr ERDDAP™ verhält sich.
...
4. Auf Linux, Mac und Windows, __download [Erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) __ in `tomcat/webapps`:
...
wenn ERDDAP™ wird auf einem anderen Weg als `/erddap` serviert, auch das `X-Forwarded-Prefix` Header auf dem
...
* Wann ERDDAP™ beginnt, es umbenannt die alten ERDDAP™ log.txt Datei (`logArchiv Im <CurrentTime> .txt) und erstellt eine neue log.txt-Datei.
...
(Wenn es keinen gibt, kopieren Sie den Standard aus ERDDAP™ 's `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` Datei
...
` datasets.xml ` und bearbeiten.) Zum Beispiel:
  ...
  * Um das Symbol zu ändern, das auf Browser-Tabs erscheint, setzen Sie Ihr Unternehmen favicon. ico in `tomcat/content/erddap/images/`.
