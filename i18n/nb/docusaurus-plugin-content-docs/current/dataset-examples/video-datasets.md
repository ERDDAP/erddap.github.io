...
# Søke etter og vise videoer i ERDDAP™ 

Dette dokumentet viser hvordan du viser videoer i ERDDAP™ I tillegg til
å opprette et datasett som gir for å søke videometadata til
Finn videoene som oppfyller de valgte søkekriteriene.

De viktigste komponentene som trengs for å konfigurere dette er først, videoer som
er i et format som kan vises i en nettleser ( ERDDAP™ seg selv
viser ikke videoen, det gir bare en lenke), og andre, en csv
fil (eller lignende tabellbasert format som parkett) som inneholder
Metadata som er relevante for videoene.

Å ha et passende videoformat er nøkkelen. I eksemplet under
Videoer opprinnelig var et gammelt Vinduets format som selv Windows-nettlesere
kunne ikke vise ut av boksen, og kreves å legge til en plugin. Gamle
Formater er ofte svært store filer. Håndbrekk-applikasjonen kan være
brukes til å konvertere videoene, som er det som ble gjort.

## Oppsett

Det første trinnet er å opprette et datasett for \"EDDTableFromFileNames\" slik at
Det er en URL tilknyttet hver video som er tilgjengelig gjennom
den ERDDAP™ \"Filer\" kommando. Et eksempel på dette datasettet er gitt
Nedenfor, den beste måten å generere denne xml for videoene er å bruke
Opprett datasett Xml.sh og peke det til katalogen med videoene (i
generell den beste måten å legge til nye datasett på ERDDAP™ er å bruke
Opprett datasett Xml.sh som vil få deg det meste av veien dit, og
så rediger det for det endelige resultatet).

Det andre trinnet er å lage en EDDTableFromAsciiFiles (eller lignende) 
datasett som inneholder metadata som skal brukes i videoen
Søk. Et eksempel er gitt nedenfor. For at dette skal fungere må det være én
kolonne som inneholder en link til videoen, i eksemplet det er
Variabel \"videofil\". Denne variabelen kan bare inneholde hele URL-en
opprettet i det første trinnet, men så filen er ikke bærbar, som om
Du vil teste på en maskin og deretter faktisk tjene på en annen
maskin.

Du kan gjøre filen bærbar ved å gjøre oppføringen i \"videofil\" å være
relativ bane til videoen, i eksemplet under en slik bane er:

2010_Piggy_Bank_audio_striped/0001_20100627_144631.mp4

Deretter kan du legge til \"filAccessBaseUrl\"-attributten som legger til at
URL til banen i kolonnen. I eksemplet:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Hvis du bytter servere, må du bare endre den ene egenskapen i
xml snutt, du trenger ikke å redigere csv-filen.

Når du er ferdig, har du en liste over videoene som kan brukes som filer:
...
Et enkelt søk satt opp av datasettet \"EDDTableFromFileNames\":
...
Standardalternativene i den siste URL-adressen gir URL-en:

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black steinfisk»](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 

som gir en tabell med den siste kolonnen:
...
Hver av oppføringene i svaret gir en lenke til en video, som
Hvis du museer over den første du får:
...
Hvis du klikker på det i HTML Tabell videoen vises i din
Nettleser.
