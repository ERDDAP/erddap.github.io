...
For å bekrefte installasjonen, kjør `/javaJreBinDirectory/java -versjon` For eksempel
    `/usr/local/jdk-25.0.1+8/jre/bin/java -versjon` ..

    ERDDAP™ jobber med Java fra andre kilder, men vi anbefaler Adoptium fordi det er det viktigste, samfunnsstøttede,
gratis (som i øl og tale) versjon av Java 25 som tilbyr langsiktig støtte (gratis oppgraderinger i mange år tidligere den første utgivelsen) ..
...
Vi anbefaler å pakke det ut i `/usr/lokal` ..
     * På en Mac er Tomcat sannsynligvis allerede installert i `/Library/Tomcat` , men bør oppdatere det til den siste versjonen av Tomcat 10.
Hvis du laster det ned, [Last ned "Core" "tar .gz " Tomcat-distribusjon](https://tomcat.apache.org/download-10.cgi) og pakke den i `/Library/Tomcat` ..
...
*  [server.xml](#serverxml) - I `tomcat/conf/server.xml` fil, det er to endringer du bør gjøre til hver av de to ` <Connector> ` tagger
   (En til `&lt;Connector port="8080"` En for `&lt;Kontorport=-8443"` ) ..
...
* context.xml -- Resources Cache - Inn `tomcat/conf/context.xml` rett før ` </Context> ` Tagg, endre ressurstaggen
...
Således kan bare superbrukeren bytte til å fungere som bruker `tomcat` .. Dette gjør det umulig for hackere å logge på serveren din som bruker `tomcat` ..
...
for `apache-tomcat` katalogtre og ` <bigParentDirectory> ` og skrivebare privilegier for mapper med data som ERDDAP™ trenger tilgang til).
  ...
  * Du kan bytte til å jobbe som bruker `tomcat` Ved å bruke kommandoen
    ...
    * Du kan slutte å jobbe som brukertomcat ved å bruke kommandoen
    ...
    exit
    ````
    * Gjør det meste av resten av Tomcat og ERDDAP™ installasjonsinstruksjoner som bruker `tomcat` .. Senere, løp `startup.sh` og `Nedlegg. sh` skript som bruker `tomcat` 
slik at Tomcat har tillatelse til å skrive til sine loggfiler.
    * Etter å ha pakket ut Tomcat, fra foreldrene til `apache-tomcat` mappe:
...
Denne filen vil bli brukt av `tomcat/bin/startup.sh` og `Nedlegg. sh` .. Filen skal inneholde noe som:
  ```
...
* På Windows:
Opprett en fil `Tomcat\bin\\setenv.bat` For å sette Tomcats miljøvariabler.
...
Hvis serveren har mindre enn 2GB minne, redusere `-Xmx` verdi (i 'M'egabytes) 1/2 av datamaskinens fysiske minne.
...
eller i `bigParentDirectory/logs/ [Logg.txt](/docs/server-admin/additional-information#log) ` fil ( `bigParentDirectory` er angitt i [setup.xml](#setupxml) ) ..
...
samling fordi det gikk tom for rom i Eden (delen av Java haug for svært unge objekter) .. Vanligvis viser meldingen deg
   `minnebruk før-&gt;minnebruk` .. Hvis disse to tallene er nært sammen, betyr det at søppelsamlingen ikke var produktiv.
...
*  [På Linux og Mac endre tillatelser](#permissions) av alle `*.sh` Filer i `tomcat/bin/` å være kjørbar av eieren:
  ```
  chmod +x *.sh
  ```
...
* Test Tomcat-installasjonen.
  * Linux:
    * Som bruker "tomcat", kjøre `tomcat/bin/startup.sh` ..
...
og slupp det ut i `tomcat` mappe, opprette `tomcat/innhold/erddap` ..

__Versjon 1.0.1, 20683 bytes, MD(2005)98a8099e7e674da59fe35e9c96efa7b5, datert 2025-06-02__
...
Når du oppretter bigParentDirectory, fra foreldrekatalogen til bigParentDirectory:
    ...
    * Gjøre `tomcat` brukeren eier av `bigParentDirectory` :)
      ```
...
Endringer i `erddap2.css` Bare tre i kraft når ERDDAP™ er omstartet og krever ofte også at brukeren å rydde nettleserens cachede filer.
     
...
4. På Linux, Mac og Windows, _ Last ned [Erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.30.0/ERDDAP-2.30.0.war) _I `tomcat/webapps` :)

_Versjon 2.30.0, 706.939,130 bytes, MD(2005)CDC4B3D82A20B33A6623B85312F6DC21, datert 2026-04-06_
...
Noen tidligere versjoner er også tilgjengelige.
   ...
   *  [2.28.1](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war)   (622 676 238 bytes, MD(2005)48b4226045f950c8a8d69ef9521b9bc9, datert 2025-09-05) 
   *  [2.29.0](https://github.com/ERDDAP/erddap/releases/download/v2.29.0/erddap.war)   (706.788.135 bytes, MD(2005)A5ED0DCC8D46CA26640F [30]8CE4A8560, datert 2025-12-15) 
...
1. Pass på at `mod_proxy` og `mod_proxy_ http ` er lastet:
...
2. Endre eksisterende ` <VirtualHost> ` Merke (Hvis det er en) , eller legg til en på slutten av filen:
```
<VirtualHost *:80>
...
banesegment _før_ `/erddap` .. Denne innstillingen vil være egnet for en ERDDAP™ serveres på
 `/subpath/erddap` :)
...
* Når ERDDAP™ starter opp, den omdøper den gamle ERDDAP™ log.txt-fil ( `loggArchivedAt <CurrentTime> .txt` ) og oppretter en ny log.txt-fil.
Hvis `Logg.txt` Filen er gammel, det er et tegn på at ERDDAP™ Har ikke nylig startet på nytt. ERDDAP™ Skriv logginformasjon til en buffer
...
* Endre banneren som vises øverst på alle ERDDAP™ .html sider ved å redigere ` <startBodyHtml5> ` Tagg i din ` datasets.xml ` fil.
...
* Endre informasjonen på venstre side av forsiden ved å redigere ` <theShortDescriptionHtml> ` Tagg i din ` datasets.xml ` fil.
(Hvis det ikke er én, kopier standard fra ERDDAP™ 's `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` fil
i ` datasets.xml ` og rediger det.) For eksempel kan du:
  * Beskriv hva din organisasjon og/eller gruppe gjør.
  * Beskriv hvilken type data dette ERDDAP™ Det har jeg.
  * Hvis du vil endre ikonet som vises på nettleserfanene, legger du organisasjonens favicon. ico i `tomcat/innhold/erddap/bilder/` ..
