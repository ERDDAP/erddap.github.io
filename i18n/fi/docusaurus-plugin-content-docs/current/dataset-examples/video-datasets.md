...
# Videoiden etsiminen ja näyttäminen ERDDAP™ 

Tämä dokumentti osoittaa, miten videoita voidaan näyttää ERDDAP™ sekä
aineiston luominen, joka mahdollistaa videometadan etsimisen
Katso videot, jotka täyttävät valitut hakukriteerit.

Tärkeät komponentit, joita tarvitaan, ovat ensin videoita, jotka
on muodossa, joka voidaan näyttää selaimessa ( ERDDAP™ itsessään
ei näytä videota, se tarjoaa vain linkin, ja toinen, csv
tiedostotiedosto (tai vastaavaan taulukkoon perustuvaan muotoon, kuten) joka sisältää
videoiden kannalta merkitykselliset metatiedot.

Asianmukainen videomuoto on tärkeä. Esimerkki alla
Videot olivat alunperin vanha ikkunan muoto, jossa jopa Windows-selaimet
Ei voinut näkyä laatikosta ja tarvitsi lisäosan. Vanha vanha
Muotoilu on usein hyvin suuria tiedostoja. Käsijarrutus voi olla
videoita, jotka on tehty.

## Asetukset

Ensimmäinen askel on luoda "EDDTableFromFileNames" -tietoaineisto, jotta
jokaiseen videoon liittyvä URL-osoite on saatavilla
The ERDDAP™ "Files" -komento. Esimerkkinä tästä aineistosta on
Paras tapa tuottaa tämä xml videoille on käyttää
GenerateDatasets Xml.sh ja osoita se hakemistoon videoiden kanssa
Paras tapa lisätä uusia tietoaineistoja ERDDAP™ on käyttää
GenerateDatasets Xml.sh, joka saa sinut suurimman osan matkasta
Lopputulokseen).

Toinen askel on luoda EDDTableFromAsciiFiles (tai vastaava) 
aineisto, joka sisältää videossa käytettävän metatiedot
etsintä. Esimerkki on esitetty alla. Jotta tämä toimisi, täytyy olla yksi
sarake, joka sisältää linkin videoon, esimerkissä se on
Vaihtoehtoinen "videofiili". Tämä muuttuja voi sisältää koko URL-osoitteen.
ensimmäinen vaihe, mutta sitten tiedosto ei ole kannettava, kuten
haluat testata yhden koneen ja sitten todella palvella toisessa
kone.

Voit tehdä tiedoston kannettavaksi tekemällä merkinnän "videofiili"
suhteellinen reitti videon alla olevassa esimerkissä on:

2010_Piggy_Bank_audio_stripped/0001_20627_144631

Sitten voit lisätä "fileAccessBaseUrl"-ominaisuuden, joka liittää
URL-osoite sarakkeeseen. Esimerkissä:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Jos vaihdat palvelimia, sinun tarvitsee vain muuttaa tätä ominaisuutta.
XML Snippet, sinun ei tarvitse muokata Csv-tiedostoa.

Kun olet valmis, sinulla on luettelo tiedostoina käytettävissä olevista videoista:
...
EDDTableFileNames -tietoaineiston luoma yksinkertainen haku:
...
Viimeisen URL-osoitteen oletusvaihtoehdot tuottavat URL-osoitteen:
...
Tämä on pöytä, jossa on viimeinen sarake:
...
Jokainen vastaus sisältää linkin videoon, kuten
Jos hiirtät ensimmäisen, jonka saat:
...
Jos napsautat sitä html Tallenna video näkyy omassa
Selain.
