...
# Пошук і відображення відео в ERDDAP™ 

Цей документ показує, як відобразити відео в ERDDAP™ й
створення метаданих для пошуку відео
знайти відео, які відповідають критеріям пошуку.

Основні компоненти, необхідні для встановлення цього вгору, відео, які
у форматі, який можна відобразити у веб-переглядачі ERDDAP™ сам
не відображається відео, він надає посилання, а другий, csv
Головна (або аналогічний формат таблиці, таких як паркет) що містять
метадані, що відносяться до відео.

За допомогою відповідного відеоформату є ключ. На прикладі нижче
відео спочатку були застарілим форматом вікна, який навіть браузери Windows
не може відображатися з коробки, і потрібно додати плагін. Старий
формати часто є дуже великими файлами. Додаток Handbrake може бути
використовується для перетворення відео, що це зроблено.

## Комплекти

Першим кроком є створення «EDDTableЗ альбомуFileNames», тому що
є URL, пов'язаний з кожним відео, який доступний через
Про нас ERDDAP™ Команда «Файли». В той час як наведено приклад даних
нижче, найкращий спосіб створення цього xml для ваших відео, щоб використовувати
ГенераціяДатасети Xml.sh і вказуйте його до каталогу з відео (в
Найкращий спосіб додавати нові дані ERDDAP™ - використовувати
ГенераціяДатасети Xml.sh, який отримає вам більшу частину шляху, і
потім відредагувати, що для кінцевого результату.

Другий крок для створення EDDTableЗ альбомуAsciiFiles (або схожий) 
Dataset, який містить метадані, які слід використовувати в відео
пошук. Приклад наведено нижче. Для цього необхідно мати одну
стовпчик, який містить посилання на відео, в прикладі це
змінна “відеоФайл”. Ця змінна може містити повну URL-адресу
створюється в першому кроці, але потім файл не портативний, наприклад, якщо
ви хочете перевірити на одній машині, а потім насправді служити на іншому
машина.

Ви можете зробити файл портативним, зробивши запис у “відеоФайл”
відносний шлях до відео, в прикладі нижче одного такого шляху:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Потім можна додати атрибут «fileAccessBaseUrl», який додає, що
URL на шлях у стовпці. У прикладі:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Потім якщо ви переключите сервери, вам потрібно лише змінити цей атрибут в
JavaScript licenses API Веб-сайт Go1.13.8

Після того, як ви зробили, у вас є список доступних відеофайлів:
...
простий набір пошуку за допомогою Dataset «EDDTableFromFileNames»:
...
Параметри за замовчуванням в останньому URL виробляють URL:

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black рокапи](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 

і що виробляє таблицю з останнього стовпчика:
...
Кожен з записів у відповідь надає посилання на відео, такі як
якщо ви миша над першою ви отримуєте:
...
і якщо ви натискаєте на неї в html Стіл відеопоказ у вашому відео
веб-сайт
