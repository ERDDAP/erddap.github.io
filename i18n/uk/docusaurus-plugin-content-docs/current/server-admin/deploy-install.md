...
Будь ласка, завантажте та встановіть останню версію
    [Адоптиум OpenJDK (Темурін) 25 мар (ЛТС) ](https://adoptium.net/temurin/releases/?version=25) й
...
З причин безпеки, будь ласка, поновіть свій ERDDAP - версія Java періодично як нові версії Java 25 стане доступним з Adoptium.

    ERDDAP™ Випробувано і використовується в основному з 25, а не іншими версіями. З різних причин ми не тестуємо за допомогою інших версій Java й
     
## Кошик{#tomcat} 
...
який призначений для роботи з Java 25 або новачок. Нижче наведено каталог Tomcat як `доматор` й
...
Якщо дистрибуція є Windows-інсталятором, він напевно помістить Tomcat, наприклад, `/Програма Файли/apache-tomcat-10.0.23` й
             
...
*  [Сервер.xml](#serverxml) - У `JavaScript licenses API Веб-сайт` файл, є два зміни, які ви повинні зробити до кожного з двох ` <Connector> ` мітки
   (один для `&lt;Connector порт="8080"` і один `&lt;Conector port="8443"` ) й
...
* JavaScript licenses API Веб-сайт Go1.13.8 --- Ресурси кеш - В `JavaScript licenses API Веб-сайт Go1.13.8` , прямо до ` </Context> ` тег, зміна тегів ресурсів
...
Встановлення Tomcat, особливо для публічних серверів.
         
* Для громадськості ERDDAP™ Налаштування на Linux і Macs, краще налаштувати Tomcat (Програма) як належність Користувача `доматор` 
...
Таким чином, тільки суперкористувач може переключатися в дію як користувача `доматор` й Це робить його неможливим для хакерів, щоб увійти на сервер як користувач `доматор` й
І в будь-якому випадку, ви повинні зробити це так, щоб `доматор` Користувач має дуже обмежені дозволи на файлову систему сервера (читати+write+execute привілеї
для `apache-tomcat` каталог дерево і ` <bigParentDirectory> ` та ознайомчі привілеї для каталогів з даними, які ERDDAP™ необхідний доступ.
  * Ви можете створити `доматор` обліковий запис користувача (не має пароля) за допомогою команди:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * Ви можете перейти до роботи користувача `доматор` за допомогою команди
    ```
    sudo su - tomcat
    ```
    ...
    * Ви можете зупинити роботу як користувацький томатор за допомогою команди
    ...
    exit
    ````
    * До більшості решти Томата і ERDDAP™ інструкції налаштування користувача `доматор` й Далі запустіть `стартап.sh` і `відключення. шемале` скрипти як користувач `доматор` 
...
Про нас `-Хмкс` і `-Хм` Параметри пам'яті важливі, тому ERDDAP™ працює краще з пам'яттю.
Завжди набір `-Хм` до того ж значення, як `-Хмкс` й
...
З 32 біт Java , навіть з рясною фізичною пам'яттю, Tomcat і Java не запустіть, якщо ви спробуєте встановити `-Хмкс` до 1500M (1200М на деяких комп'ютерах) й
Якщо сервер має менше 2 Гб пам'яті, знижуйте `-Хмкс` вартість (в 'M'egaBytes) до 1/2 фізичної пам'яті комп'ютера.
...
Ми рекомендуємо вам встановити `-Хмкс` і `-Хм` до (в 'M'egaBytes) до 1/2 (або менше) фізичної пам'яті комп'ютера.
...
* Якщо ви бачите `Türkçe, English, Українська...` й ERDDAP™ Р [JavaScript licenses API Веб-сайт Go1.13.8](/docs/server-admin/additional-information#log) файл,
...
*  [На Linux і Macs змініть дозволи](#permissions) Всі `Ім'я *` Файли в `tomcat/bin/` бути виконавцем:
  ```
  chmod +x *.sh
  ```
...
і буде друкувати список доступних шрифтів `JavaScript licenses API Веб-сайт Go1.13.8` файл. Ви повинні використовувати один з цих шрифтів.
...
і розпакування файлів шрифтів до тимчасового каталогу.
  ...
  * На Linux:
    ...
    * З іншими Java розподіли: Як `доматор` користувач, копіювання файлів шрифту в `$JAVA_HOME/lib/fonts` до Java може знайти шрифти.
...
* Переглянути файл Tomcat `JavaScript licenses API Веб-сайт Go1.13.8` й Проблеми Tomcat і деякі ERDDAP™ проблеми стартапів практично завжди вказуються.
...
і розпакувати його в `доматор` каталог, створення `JavaScript licenses API Веб-сайт` й
...
верстка `ерддапКонтент .zip ` в бажаний каталог (до якого тільки `доматор` Користувач має доступ) і встановити ` erddapContentDirectory ` системне майно
...
Коли ви створюєте bigParentDirectory, з батьківського каталогу bigParentDirectory:
    ...
    * Зроби `доматор` користувачу власнику власника `bigParentПублічний` :
      ```
...
Зміни до `erddap2.css` тільки приймати ефект при ERDDAP™ Працюють і часто зажадають користувачу очистити файли кешування браузера.
     
...
2. Змінення існуючої ` <VirtualHost> ` мітки (якщо є один) , або додати один в кінці файлу:
```
<VirtualHost *:80>
...
і повідомлення про помилки ERDDAP , у ERDDAP™   ` <bigParentDirectory> English, Українська, Français...` файл.
...
Так, якщо `JavaScript licenses API Веб-сайт Go1.13.8` файл старий, це знак, що ERDDAP™ не нещодавно перезавантажено. ERDDAP™ напишіть інформацію про журнал на буфер
