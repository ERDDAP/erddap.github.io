...
* JavaScript licenses API Веб-сайт Go1.13.8 --- Ресурси кешу - У `tomcat/conf/context.xml`, прямо перед ` </Context> ` тег, зміна тегу ресурсів
...
* На комп'ютерах Linux змініть налаштування часу Apache, щоб часові запити користувачів не витрачали часу
  ...
  * Змінення Apache ` http d.conf` файл (зазвичай в `/etc/ http до/conf/ й) :
...
Таким чином, тільки суперкористувач може переключатися до дії користувача `tomcat`. Це неможливе для хакерів, щоб увійти на сервер як користувач `tomcat`.
...
для каталогу `apache-tomcat` і ` <bigParentDirectory> ` і прочитано-тільки привілеї для каталогів з даними ERDDAP™ необхідний доступ.
...
На Macs, можливо, не потрібно встановлювати `JAVA_HOME`.
...
З 32 біт Java , навіть з рясною фізичною пам'яттю, Tomcat і Java не запустіть, якщо ви хочете встановити `-Xmx` набагато вище 1500M (1200M на деяких комп'ютерах) й
Якщо сервер має менше 2 Гб пам'яті, знижуйте значення `-Xmx` (в 'M'egaBytes) до 1/2 фізичної пам'яті комп'ютера.

* Для 64 біт Операційні системи та 64 біт Java :
64 біт Java працюватиме лише на 64 біт операційній системі.
  * З Java 8, Вам необхідно додати `-d64` до параметра Tomcat `CATALINA_OPTS у `setenv.bat`.
...
`-Xmx` і `-Xms` (в 'M'egaBytes) до 1/2 (або менше) Фізична пам'ять комп'ютера.
...
Якщо ви не використовуєте шрифти DejaVu, вам потрібно змінити налаштування шрифту.xml до `` <fontFamily> Сансеріф </fontFamily> `,
...
і розпакування файлів шрифтів до тимчасового каталогу.
  ...
  * На Linux:
    ...
    * З іншого боку Java розподіли: Як користувач `tomcat копіювати файли шрифтів у `$JAVA_HOME/lib/fonts` так Java може знайти шрифти.
...
* Переглянути файл Tomcat `tomcat/logs/catalina.out` Проблеми Tomcat і деякі ERDDAP™ проблеми запуску практично завжди вказані.
...
JavaScript licenses API Веб-сайт Go1.13.8 .zip ` в бажаний каталог (до якого тільки користувач `tomcat має доступ) і встановити ` erddapContentDirectory ` системне майно
...
Зміни до `erddap2.css` беруть участь тільки при ERDDAP™ Працював і часто зажадає користувачу очистити файли кешування браузера.
     
...
4. На Linux, Mac та Windows, __download [ерддап.воєнний](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) __ в `tomcat/webapps`:
...
Якщо ERDDAP™ `X-Forwarded-Prefix` головоломка
...
або (у Windows) `tomcat\bin\bin\\ shutdown.bat й

На Linux `ps -ef | Після того, як `shutdown.sh`, щоб переконатися, що процес tomcat зупинився.
...
* Коли ERDDAP™ початок, він перейменує старий ERDDAP™ Файл: Log.txt (`logАрхів Афіша <CurrentTime> .txt``) і створює новий файл log.txt.
Так, якщо файл `log.txt’ старий, це знак, який ERDDAP™ не нещодавно перезавантажений. ERDDAP™ напишіть інформацію про журнал на буфер
...
до ` datasets.xml ` і редагувати його.) Наприклад, ви можете:
