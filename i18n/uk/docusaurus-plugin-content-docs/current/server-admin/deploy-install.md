...
Як зробити початковий набір ERDDAP™ на сервері
 ...
 ERDDAP™ може працювати на будь-якому сервері, який підтримує Java і Томат (і інших серверів додатків, таких як Jetty, але ми не підтримуємо їх) й
...
Якщо ви шукаєте працювати на хмарних сервісах, ви, ймовірно, віддаєте перевагу версії Docker.
...
але ми не рекомендуємо використовувати його для публічного ERDDAP™ розгортання. Пробіг ERDDAP™ на Windows може виникнути проблеми:
...
(який можна побачити повідомлення про помилки в [JavaScript licenses API Веб-сайт Go1.13.8](/docs/server-admin/additional-information#log) Головна
"Неможливо видалити ...", зміни налаштувань антивірусного програмного забезпечення може частково полегшити проблему. Або розглянути за допомогою сервера Linux або Mac.
...
Будь ласка, завантажте та встановіть останню версію
    [Адоптиум OpenJDK (Темурін) 21 мар (ЛТС) ](https://adoptium.net/temurin/releases/?version=21) й
Для перевірки установки `/javaJreBinDirectory/java -version`, наприклад
`/usr/local/jdk-21.0.3+9/jre/bin/java -версія.

    ERDDAP™ роботи з Java з інших джерел, але ми рекомендуємо Adoptium, оскільки це основний, громадський супровід,
...
З причин безпеки, будь ласка, поновіть свій ERDDAP - версія Java періодично як нові версії Java 21 стає доступним з Адоптиуму.
...
2.  [Налаштування](#tomcat)   [Кошик](https://tomcat.apache.org) й Tomcat є найбільш популярним Java Сервер додатків,
...
який призначений для роботи з Java 21 або новий. Нижче наведено каталог Tomcat `tomcat`.

__Варення&#33;__ Якщо у вас вже є Tomcat, яка працює на веб-додатку (особливо THREDDS) , ми рекомендуємо вам встановити ERDDAP™ в
      [другий Tomcat](/docs/server-admin/additional-information#second-tomcat) , бо ERDDAP™ потребує різних налаштувань Tomcat
...
Ми рекомендуємо розпакувати його в `/usr/local`.
     * На Mac Tomcat вже встановлена в `/Library/Tomcat`, але слід оновити її до останньої версії Tomcat 10.
Якщо завантажити його, [завантажити "Core" "tar" .gz "Розширення томатів"](https://tomcat.apache.org/download-10.cgi) і розпакуйте його в `/Library/Tomcat`.
     * На Windows ви можете [завантажити "Core" "zip" Tomcat дистриб'ютор](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Для розробки ми використовуємо "Core" "zip". Ми робимо каталог `/programs` і розпакуємо його там.) 
Або ви можете завантажити "Core" "64-bit Windows zip" дистрибуція, яка включає більше функцій.
Якщо дистрибуція є інсталятором Windows, вона, ймовірно, поставить Tomcat, наприклад, `/Program Files/apache-tomcat-10.0.23`.
             
### Сервер.xml{#serverxml} 

*  [Сервер.xml](#serverxml) - У файлі `tomcat/conf/server.xml є два зміни, які слід зробити кожному з двох ` <Connector> ` теги
   (один для `&lt;Connector port="8080"` і один для `&lt;Conector port="8443"`) й
   1.  (Рекомендовані) Збільшення значення параметра `connectionTimeout`, можливо, до 300000 (мілісекунди, які 5 хвилин) й
   2.  (Рекомендовані) Додати новий параметр: `relaxedQueryChars="[] | ````. Це необов'язково і трохи менш надійний,
...
* JavaScript licenses API Веб-сайт Go1.13.8 --- Ресурси кешу - У `tomcat/conf/context.xml`, прямо перед ` </Context> ` тег, зміна тегів ресурсів
...
* На комп'ютерах Linux змініть налаштування часу Apache, щоб часові запити користувачів не витрачали часу
  ...
  * Змінення Apache ` http файл d.conf` (зазвичай в `/etc/ http до/conf/ й) :
    * Зміна існуючого ` <Timeout> Налаштування ` (або додати один в кінці файлу) до 3600 (час) , замість за замовчуванням 60 або 120 секунд.
    * Зміна існуючого ` <ProxyTimeout> Налаштування ` (або додати один в кінці файлу) до 3600 (час) , замість за замовчуванням 60 або 120 секунд.
  * Статус на сервери -k витончений й (але іноді це в іншому каталозі) й
...
Встановлення Tomcat, особливо для публічних серверів.
         
* Для громадськості ERDDAP™ Налаштування на Linux і Macs, краще налаштувати Tomcat (Програма) як належність користувача `tomcat й
...
Таким чином, тільки супер-користувач може переключатися в дію як користувача `tomcat`. Це неможливе для хакерів, щоб увійти на сервер як користувач `tomcat`.
І в будь-якому випадку, ви повинні зробити це таким чином, що користувач `tomcat має дуже обмежені дозволи на файлову систему сервера (читати+write+execute привілеї
для каталогу `apache-tomcat` і ` <bigParentDirectory> ` і прочитано-тільки привілеї для каталогів ERDDAP™ необхідний доступ.
  * Ви можете створити обліковий запис користувача `tomcat (не має пароля) за допомогою команди:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * Ви можете перейти до роботи користувача `tomcat` за допомогою команди
    ```
    sudo su - tomcat
    ```
     (Вам запрошують пароль суперкористувача для цього.) 
    * Ви можете зупинити роботу як користувацький томатор за допомогою команди
    ```
    exit
    ````
    * До більшості решти Томата і ERDDAP™ інструкції з налаштування користувача `tomcat`. Пізніше запустіть скрипти `startup.sh` і `shutdown.sh` як користувач `tomcat й
Так що Tomcat має дозвіл на написання своїх файлів журналу.
    * Після розпакування Tomcat від батьків каталогу `apache-tomcat`:
...
* На Linux і Macs:
Створіть файл `tomcat/bin/setenv.sh й (або в Red Hat Enterprise Linux \\[ РХЕЛ \\] , редагування `~tomcat/conf/tomcat10.conf й) для встановлення змінних середовища Tomcat.
Цей файл буде використовуватися `tomcat/bin/startup.sh` і `shutdown.sh`. Файл повинен містити щось схоже:
  ```
  ...
  ```
   ...
   (Якщо ви раніше встановили `JRE_HOME`, ви можете видалити це.) 
На Macs, ймовірно, не потрібно встановлювати `JAVA_HOME`.

* На Windows:
Створіть файл `tomcat\bin\\setenv.bat` для встановлення змін середовища Tomcat.
Цей файл буде використовуватися `tomcat\bin\\startup.bat` shutdown.bat й
...
Важливі параметри пам'яті `-Xmx` та `-Xms` ERDDAP™ працює краще з пам'яттю.
Завжди встановіть `-Xms` до того ж значення, як `-Xmx`.
...
З 32 біт Java , навіть з рясною фізичною пам'яттю, Tomcat і Java не запустіть, якщо ви хочете встановити `-Xmx` набагато вище 1500M (1200М на деяких комп'ютерах) й
Якщо Ваш сервер має менше 2 Гб пам'яті, знижуйте значення `-Xmx` (в 'M'egaBytes) до 1/2 фізичної пам'яті комп'ютера.

* Для 64 біт Операційні системи та 64 біт Java :
64 біт Java працюватиме лише на 64 біт операційній системі.
  * З Java 8, потрібно додати `-d64` до параметра Tomcat `CATALINA_OPTS у `setenv.bat`.
  * З Java 21 вибираємо 64 біт Java коли ви завантажуєте версію Java позначений "64 біт".

З 64 біт Java Твитнуть Java `-Xmx` і `-Xms` налаштування. Більша фізична пам'ять в сервері краще.
`-Xmx` і `-Xms` (в 'M'egaBytes) до 1/2 (або менше) фізичної пам'яті комп'ютера.
...
або в `bigParentDirectory/logs/ [JavaScript licenses API Веб-сайт Go1.13.8](/docs/server-admin/additional-information#log) ` файл (`bigParentDirectory` вказаний в [Налаштування.xml](#setupxml) ) й
...
збірка, оскільки вона вибігла з кімнати в Едені (секція розділу Java Капіталізація для молодих об'єктів) й Зазвичай повідомлення показує вам
`memoryUseBefore-&gt;mmoryUseAfter`. Якщо у двох числах закриті разом, значить, збір сміття не було продуктивним.
...
Це може статися під час стресового часу, після чого піти. Але якщо він псує, то це ознака неприємностей.
* Якщо ви бачите `java.lang.OutOfMemoryError`s в ERDDAP™ Р [JavaScript licenses API Веб-сайт Go1.13.8](/docs/server-admin/additional-information#log) файл,
...
*  [На Linux і Macs змініть дозволи](#permissions) Файли `*.sh` у файлах `tomcat/bin/` будуть виконуватися власником:
  ```
  chmod +x *.sh
  ```
...
Якщо ви не хочете використовувати шрифти DejaVu, вам потрібно змінити налаштування шрифту.xml до ``` <fontFamily> Сансеріф </fontFamily> `,
який доступний з усіма Java розподіли. Якщо ви вказали ` <fontFamily> ` на ім’я шрифту, який не доступний, ERDDAP™ не завантажується
і буде друкувати список доступних шрифтів у файлі `log.txt. Ви повинні використовувати один з цих шрифтів.

Якщо ви хочете використовувати шрифти DejaVu, будь ласка, переконайтеся, що `` <fontFamily> Налаштування ```````` <fontFamily> ДеяВу Санс </fontFamily> й

Щоб встановити шрифти DejaVu, будь ласка, завантажте [ДеяВуФонтс .zip ](/DejaVuFonts.zip)   (5,522,795 bytes, MD5=33E1E61FAB06A547851ED308B4FFEF42) 
і розпакування файлів шрифтів до тимчасового каталогу.
  ...
  * На Linux:
    ...
    * З іншими Java розподіли: Як користувач `tomcat копіювати файли шрифтів у `$JAVA_HOME/lib/fonts` так Java може знайти шрифти.
...
* Тестування установки Tomcat.
  ...
  * Мапа (запустити tomcat як адміністратор системи) :
    ...
    * Перегляд URL + ":8080/" у вашому браузері (Наприклад: [http://coastwatch.pfeg.noaa.gov:8080/](http://coastwatch.pfeg.noaa.gov:8080/) ) й
Зверніть увагу, що за замовчуванням, ваш Tomcat доступний тільки за допомогою вас. Недоступно.
  * Windows localhost:
    * Клацніть правою кнопкою миші на піктограму Tomcat в системному лотку, і виберіть "Почати послугу".
    * Переглянути [http://127.0.0.1:8080/](http://127.0.0.1:8080/) або, можливо, [http://localhost:8080/](http://localhost:8080/) , у вашому браузері. Зверніть увагу, що за замовчуванням, ваш Tomcat доступний тільки за допомогою вас. Недоступно.

Ви повинні бачити сторінку Tomcat.

Якщо є проблеми, див. файл Tomcat на `tomcat/logs/catalina.out`.
...
Щоб повернути один рядок з чимось схожим:
  ...
  ```
   ...
   (де `#```) , вказавши, що процес `java` (Томат) Прослуховування на порту "8080" для "tcp" трафіку.
...
* Переглянути файл Tomcat `tomcat/logs/catalina.out` Проблеми Tomcat і деякі ERDDAP™ проблеми стартапів практично завжди вказуються.
Це поширено, коли ви спочатку налаштовані ERDDAP™ й

* Дивитися [Кошик](https://tomcat.apache.org/) Якщо ви хочете, щоб ви зрозуміли, що це було і рішення, які ви знайшли.

* Про нас [розділ про отримання додаткової підтримки](/docs/intro#support) й
             
###  ERDDAP™ Зміст{#erddap-content} 
3.   [Налаштування файлів `tomcat/content/erddap`](#erddap-content) 
...
Деякі попередні версії також доступні:
    ...
    *  [2.21](https://github.com/ERDDAP/erddap/releases/download/v2.21/erddapContent.zip)   (1E26F62E7A06191EE6868C40B9A29362, від 2022-10-09) 
    *  [2.22 км](https://github.com/ERDDAP/erddap/releases/download/v2.22/erddapContent.zip)   (1E26F62E7A06191EE6868C40B9A29362, від 2022-12-08) 
...
JavaScript licenses API Веб-сайт Go1.13.8 .zip ` в бажаний каталог (до якого тільки користувач `tomcat має доступ) і встановити ` erddapContentDirectory ` система нерухомості
 (Наприклад, ` erddapContentDirectory  =~tomcat/content/erddap й) до ERDDAP™ може знайти цей новий каталог контенту.

### Налаштування.xml{#setupxml} 

*  [Читати коментарі в `tomcat/content/erddap/setup.xml й](#setupxml) і змусити запитані зміни. Налаштування.xml є файлом з усіма налаштуваннями, які вказують на те, як ваш ERDDAP™ бджіл.

Для початкової настройки вам MUST принаймні змініть ці налаштування:
      * й <bigParentDirectory> й
      * й <emailEverythingTo> й
      * й <baseUrl> й
      * й <email...> Налаштування ``
      * й <admin...> Налаштування ``
      * й <baseHttpsUrl> й (коли ви встановили https ) 

Коли ви створюєте bigParentDirectory, з батьківського каталогу bigParentDirectory:

    * Зробимо користувача `tomcat` власник `bigParentDirectory`:
      ```
...
Ім'я ERDDAP _valueName` до запуску ERDDAP™ й Наприклад, ``` ERDDAP _baseUrl` перенадає ` <baseUrl> ` значення.
...
до ERDDAP™ Починається, після чого зміщується змінна середовища.

###  datasets.xml  {#datasetsxml} 

* Читати коментарі [ **Робота з datasets.xml Головна** ](/docs/server-admin/datasets) й Після того, як ви отримаєте ERDDAP™ Пробіг
вперше (як правило, з просто даними за замовчуванням) Ви зміните XML в `tomcat/content/erddap/ datasets.xml й
щоб вказати всі дані, які ви хочете ERDDAP™ подавати. Це те, де ви будете витрачати об'єм вашого часу
...
`tomcat/content/erddap/images/erddapStart2.css` to `tomcat/content/erddap/images/erddap2.css`, а потім вносити зміни до нього.
Зміни до `erddap2.css` беруть участь тільки при ERDDAP™ Працюють і часто зажадають користувачу очистити файли кешування браузера.
     
...
4. На Linux, Mac та Windows, __download [ерддап.воєнний](https://github.com/ERDDAP/erddap/releases/download/v2.28.0/erddap.war) __ в `tomcat/webapps`:

__Version 2.28.0, 620,824,288 байтів, MD5=f948b2ba603f65a83ac67af43da9e4c2, від 2025-08-29__
...
SSL/TLS розірвання часто занурюється на вебсервері проксі-шару, а також. Особливості залежать від вимог кожного розгортання.

#### Редакція{#apache} 

1. Забезпечити `mod_proxy` і `mod_proxy_ http Завантажено:
...
2. Змінення існуючої ` <VirtualHost> ` тег (якщо є один) , або додати один в кінці файлу:
```
<VirtualHost *:80>
...
Якщо ERDDAP™ `X-Forwarded-Prefix` головоломка `X-Forwarded-Prefix`
сегмент шляху _before_ `/erddap`. Цей параметр буде доречним для ERDDAP™ подаються
`/subpath/erddap`:
...
3. Далі перезавантажити Apache: `/usr/sbin/apachectl -k витончений й (але іноді це в іншому каталозі) й
         
#### NGINX{#nginx} 
...
*  (Я не рекомендую використовувати Tomcat Web Application Manager. Якщо ви не повністю вимкнено і запустили Tomcat, рано чи пізно маєте проблеми пам'яті PermGen.) 
...
або (у Windows) `tomcat\bin\bin\n shutdown.bat й

На Linux використовуйте `ps -ef | Після того, як `shutdown.sh`, щоб переконатися, що процес tomcat припинено.
...
`кваліфікація -9 <processID> й
* Почати Tomcat (в Linux або Mac OS) `tomcat/bin/startup.sh` або (у Windows) `tomcat\bin\\startup.bat й
...
*    [Ви](/docs/server-admin/additional-information#tomcat-logs) можна побачити, що приходить до Tomcat (і пов'язані помилки) 
у файлах журналу Tomcat (`tomcat/logs/catalina.out` та інші файли в каталозі) й
...
і повідомлення про помилки ERDDAP , у ERDDAP™ й <bigParentDirectory> /logs/log.txt` файл.
...
* Коли ERDDAP™ початок, він перейменує старий ERDDAP™ Файл: Log.txt (`logАрхів Афіша <CurrentTime> Українська) і створює новий файл log.txt.
Так, якщо файл `log.txt’ старий, це знак, який ERDDAP™ не нещодавно перезавантажено. ERDDAP™ напишіть інформацію про журнал на буфер
і тільки записує буфер до файлу журналу періодично, але ви можете ERDDAP™ писати буфер до файлу журналу, відвідавши
й /erddap/status.html й
...
Tomcat має зробити багато роботи в перший раз додаток, як ERDDAP™ Починається, незважаючи на те, що файл `erddap.war`
...
Ви можете приєднатися до ERDDAP™ Оголошення [Головна](https://groups.google.com/g/erddap-announce) й Цей список в середньому приблизно один лист кожні три місяці.

## Налаштування{#customize} 

*  [Налаштування ERDDAP™ виділити Вашу організацію (нема NOAA   ERD ) й](#customize) 
* Зміна банера, що з'являється вгорі всіх ERDDAP™ .html сторінки редагування ` <startBodyHtml5> ` тег у вашому `` datasets.xml ` файл.
(Якщо не є одним, скопіюйте за замовчуванням з ERDDAP™ `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` Головна
до ` datasets.xml ` і редагування.) Наприклад, ви можете:
...
до ` datasets.xml ` і редагування.) Наприклад, ви можете:
  ...
  * Щоб змінити іконку, яка з'являється на вкладках браузера, поставте свою організацію favicon. ico в `tomcat/content/erddap/images/`.
