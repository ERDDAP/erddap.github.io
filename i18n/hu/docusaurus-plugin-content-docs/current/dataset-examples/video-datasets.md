...
# Videók keresése és megjelenítése ERDDAP™ 

Ez a dokumentum bemutatja, hogyan jelenítse meg a videókat ERDDAP™ , valamint
olyan adatkészlet létrehozása, amely a videó metaadatának keresését biztosítja
találjon olyan videókat, amelyek megfelelnek a kiválasztott keresési kritériumoknak.

Az ehhez szükséges alapvető összetevők az első, videók, amelyek
olyan formátumban van, amelyet egy webes böngészőben lehet megjeleníteni ( ERDDAP™ önmagát
nem jeleníti meg a videót, csak egy linket biztosít, másodszor pedig egy csv
fájl (vagy hasonló táblázat alapú formátum, mint például a parquet) Ez tartalmazza a
metaadata releváns a videók.

A megfelelő video formátum kulcsfontosságú. Az alábbi példában
videók eredetileg egy régi Window formátuma, hogy még a Windows böngészők
nem jelenhet meg a dobozból, és plugin hozzáadása szükséges. öreg
A formátumok gyakran nagyon nagy fájlok. A Handbrake alkalmazás lehet
a videók konvertálására használtak, ami az, ami történt.

## Beállítás

Az első lépés egy „EDDTableFromFileNames” adatkészlet létrehozása, hogy
van egy URL kapcsolódó minden videó, amely elérhető keresztül
a ERDDAP™ "files" parancs. Míg egy példa erre az adatkészletre
alább, a legjobb módja annak, hogy ezt az xml-t hozza létre a videókhoz, hogy használja
GenerateDatasets Xml.sh és rámutat a könyvtárra a videókkal (a
általános a legjobb módja annak, hogy új adatkészleteket hozzáadjunk ERDDAP™ használni
GenerateDatasets Xml.sh, amely a legtöbb utat fogja elérni, és
Ezután szerkesztse ezt a végső eredményt).

A második lépés az EDDTableFromAsciiFiles létrehozása (vagy hasonló) 
adatkészlet, amely tartalmazza a metaadatot, amelyet a videóban kell használni
keresés. Egy példa az alábbiakban. Ehhez a munkához egynek kell lennie
oszlop, amely linket tartalmaz a videóhoz, például az
változó "videoFile". Ez a változó csak a teljes URL-t tartalmazza
az első lépésben létrehozott, de a fájl nem hordozható, például ha
tesztelni akar egy gépen, majd ténylegesen szolgálni egy másik
gép.

A fájl hordozhatóvá teheti a "videoFile" bejegyzést
a videóhoz való viszonylagos út, az egyik ilyen út alatt:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Ezután hozzáadhatja a "fileAccessBaseUrl" tulajdonságot, amely azokat módosítja
URL az út az oszlopban. Például:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Ezután, ha kiszolgálókat kapcsol, akkor csak azt kell megváltoztatnia, hogy egy tulajdonság
az xml snippet, nem kell szerkeszteni a csv fájlt.

Miután elvégezte, van egy lista a videók hozzáférhető, mint fájlok:
...
az „EDDTableFromFileNames” adatkészlet egyszerű keresése:
...
Az utolsó URL alapértelmezett lehetőségei az URL-t termelik:
...
és ez egy asztalt hoz létre az utolsó oszloptal:
...
A válasz minden bejegyzése linket biztosít egy videóhoz, például
ha egér az első, akit kapsz:
...
és ha rákattint a html-be táblázat a videó megjelenítések az Önben
böngésző.
