...
DID นี้สร้างทั้งหมดของdatasets.xmlชิ้นส่วนที่จําเป็นเพื่อให้[DDTable from Edardp](#eddfromerddap)ชุดข้อมูลจากชุดข้อมูล EDTable ในระยะไกลERDDAP. คุณจะมีทางเลือกในการเก็บต้นฉบับdatasetIDวินาที (ซึ่งอาจจะซ้ําบางdatasetIDในชุดของคุณERDDAP) หรือสร้างชื่อใหม่ที่จะพิเศษ (แต่ปกติแล้วมันอ่านยาก) .
     
##### EDDGridจากสมุดที่อยู่{#eddgridfromthreddscatalog} 
...
*   ERDDAP™แสดงรายการเป็นเซิร์ฟเวอร์ข้อมูลตัวหนึ่งที่ขอแนะนําให้ใช้ภายในNOAAใน[NOAAการเข้าถึงข้อมูล](https://www.ngdc.noaa.gov/wiki/index.php/Data_Access_Technical_Recommendations#Software_implementations)ไม่เหมือนกับซอฟท์แวร์อื่นที่ถูกพิจารณา
...
นอกจากสองหลัก[เครื่องมือ](#tools).
    ...
    * เดอะ[หน้าสถานะ](/docs/server-admin/additional-information#status-page)เป็นวิธีที่รวดเร็วที่จะตรวจสอบERDDAPสถานะของเว็บเบราว์เซอร์ใด ๆ มันรวมรายการชุดข้อมูลที่ไม่ได้โหลด (แม้ว่าจะไม่ใช่ข้อยกเว้น) และงานที่อ่านสถิติ (แสดงความก้าวหน้าของ[EDDGridคัดลอก](#eddgridcopy)ถึง[โปรแกรมคัดลอก DDTable](#eddtablecopy)ชุดข้อมูลและอื่น ๆ[EDDGridจากแฟ้ม](#eddgridfromfiles)หรือ[เลือกแฟ้ม](#eddtablefromfiles)ชุดข้อมูลที่จะใช้[แคชจากUrl](#cachefromurl)  (แต่ไม่ใช่แคช ขนาด GB) ) .
    * ถ้าคุณติดอยู่ เห็นเรา[ส่วนของการได้รับการสนับสนุนเพิ่มเติม](/docs/intro#support).
         
### ตัวแปรพิเศษ{#special-variables} 
*    **[ลองจิจูดละติจูด ระดับความสูง (ความลึก) และเวลา (แลป) ตัวแปร](#destinationname) [destinationName](#destinationname)เอสมันพิเศษ** 
    * โดยทั่วไปแล้ว:
        * ตัวแปร LLAT ถูกทําให้รู้จักERDDAP™หากตัวแปรแกน (สําหรับEDDGridชุดข้อมูล) หรือตัวแปรข้อมูล (สําหรับชุดข้อมูล EDTable)  [destinationName](#destinationname)คือ "ลองติจูด", "เส้นผ่าศูนย์กลาง", "เส้นรุ้ง", หรือ"time".
...
[&lt;at name="scale\\_factor"ประเภท=" Int"- 1&lt;at] (# ตัวประกอบ #) แปลงค่าความลึกเป็นค่าระดับความสูง
        ...
        * สําหรับชื่อตัวแปรพวกนี้[หน่วย](#units)ต้องเป็น "m", "เมตร", หรือ "เมตร" ถ้าหน่วยแตกต่างกัน (ยกตัวอย่างเช่น ความเข้าใจ) คุณสามารถใช้
[&lt;at name="scale\\_factor""-- *บาง ค่า* &lt;at] (# ตัวประกอบ #) และ&lt;ชื่อ AT= "หน่วย"&lt;at] (หน่วย) เพื่อแปลงหน่วยเป็นเมตร.
    ...
    * สําหรับ"time"ตัวแปร:
        ...
        * ดู[ยังไงERDDAPจัดการกับเวลา](https://coastwatch.pfeg.noaa.gov/erddap/convert/time.html#erddap).
...
นี่เรียกว่า "ไทนินท์" โดย เอสคิวแอล และ คาสเซนดร้า
    ERDDAP™แปลง[บูเลียน](#boolean-data)จากบางแหล่ง (อี. จี., เอสคิวแอล และ คาสเซนดร้า) ไบต์เข้าERDDAP™ด้วยค่า 0=เท็จ, 1 = true, และ 127=missing\\_value.
#### ubyte{#ubyte} 
...
ผู้ใช้สามารถใช้ตัวแปร char เพื่อวาดกราฟได้ERDDAP™จะแปลงอักขระเป็นเลขรหัสยูนิโค้ด ซึ่งสามารถใช้เป็นข้อมูลตัวเลขได้
#### ข้อความ{#string} 
*    **ข้อความ** เป็นลําดับของ 0 หรือมากกว่า, 2 คูณ (16 บิต)  [อักขระ UCS-2 ยูนิโค้ด](https://en.wikipedia.org/wiki/UTF-16).
    ERDDAP™ใช้/ แปลค่าข้อความ 0 เป็นค่าที่สูญหายERDDAP™ไม่สนับสนุนข้อความว่าง
ความยาวสูงสุดทางทฤษฎีคือ 2147483647 ตัวอักษร แต่อาจจะมีปัญหาหลายอย่างในหลายสถานที่ แม้จะมีเส้นเชือกที่สั้นกว่า
ใช้ERDDAP'สตริงสําหรับตัวละคร SQL, varchar, ตัวละครที่แตกต่างกัน, อักขระไบนารี, ช่วง, อนุกรม, mulet, xml และชนิดฐานข้อมูลอื่น ๆ ที่ไม่เข้ากับฐานข้อมูลใด ๆERDDAP™ชนิดของข้อมูล
ใช้ERDDAPข้อความของคาสเซนดร้า และข้อมูลอื่นๆ ที่ไม่เข้ากับคนอื่นERDDAP™ชนิดของข้อมูล
     
...
2. ตัวเลือก: หากคุณมีชุดข้อมูลที่จะใช้อ้างอิงไปยังแฟ้มสื่อ ให้เพิ่มไปยังERDDAP.
ตัว อย่าง เช่น คุณ อาจ มี แฟ้ม .csv ที่ มี แถว สําหรับ แต่ ละ ครั้ง ที่ มี คน เห็น ปลา วาฬ และ คอลัมน์ ซึ่ง รวม ถึง ชื่อ แฟ้ม ภาพ ที่ เกี่ยว ข้อง กับ ภาพ นั้น. หากชื่อของแฟ้มอิมเมจเป็นเพียงชื่อแฟ้ม, เช่น, IMg201410102424T2403Z, ไม่ใช่ที่อยู่ URL แบบเต็ม จากนั้นคุณจะต้องเพิ่ม[แฟ้ม accessBase แก้ไขโครงการหลัก...](#fileaccessbaseurl)คุณสมบัติของข้อมูลกํากับภาพสําหรับเรื่องนั้นdataVariableซึ่งระบุค่าฐานURL และคํากํากับชื่อแฟ้มเหล่านั้น หากคุณทําการเข้าถึงแฟ้มผ่านทาง DDTable from Files ตําแหน่ง URL จะอยู่ในแบบฟอร์ม
...
ถ้ามี.zipหรือแฟ้มตู้คอนเทนเนอร์อื่น ๆ ที่มีแฟ้มสื่อทั้งหมดที่เกี่ยวข้องกับตัวแปรข้อมูล เราขอแนะนําให้คุณทําให้แฟ้มดังกล่าวเข้าถึงผู้ใช้ได้ (ดูขั้นที่ 1) แล้วระบุมันกับ[แฟ้มAccessAchive Url](#fileaccessarchiveurl)อุปนิสัย
...
AWS รองรับความแตกต่างเล็กน้อยในการสร้างที่อยู่ URL นั้น แต่ERDDAP™ต้องการรูปแบบเฉพาะนี้:
   https://*bucketName*.s3.*region*.amazonaws.com/*key*   
มันเป็นธรรมเนียมทั่วไป เช่นตัวอย่างนี้ ที่จะสร้างชื่อสําคัญ ให้ดูเหมือนเส้นทางลําดับชั้น เนื่องจากมันเป็นธรรมดาและมีประโยชน์ERDDAP™ใช้กุญแจกับ / '/' ราวกับว่ามันเป็นเส้นทางลําดับชั้นบวกกับชื่อแฟ้ม และเอกสารนี้จะอ้างอิงไปยังมัน ถ้ากุญแจของถังไม่ได้ใช้ / ของ
ABI-Lib. 2018. 0518. 052.or/_ABI-L1b-Rad2-M3C10/_G16/s2018222475),ERDDAP™จะใช้กุญแจทั้งหมดเป็นชื่อแฟ้มยาว

ส่วนตัว vs Buckets ของสาธารณะ -- ผู้ อํานวย การ ของ ถัง S3 อาจ ทํา ถัง และ สิ่ง ของ ต่าง ๆ ของ ถัง นี้ เป็น ของ สาธารณะ หรือ ของ ส่วน ตัว. ถ้าสาธารณะ แฟ้มใด ๆ ในถังอาจจะถูกดาวน์โหลดโดยใครก็ตามที่ใช้ที่อยู่ URL สําหรับแฟ้ม อเมซอนมี[เปิดข้อมูล](https://aws.amazon.com/opendata/)โปรแกรมที่จัดการข้อมูลสาธารณะ (รวมข้อมูลจากNOAAนาซ่า และ USGS) ไม่คิดเงินใครที่จะดาวน์โหลดไฟล์จากถังเหล่านั้น หากถังเป็นส่วนบุคคล แฟ้มในถังจะเข้าถึงเฉพาะผู้ใช้ที่ได้รับอนุญาตและ AWS ค่าใช้จ่าย (ปกติจะจ่ายเงินจากเจ้าของถัง) สําหรับการดาวน์โหลดแฟ้มไปยังคอมพิวเตอร์ที่ไม่ใช่ AWS S3ERDDAP™สามารถทํางานกับข้อมูล ในถังส่วนตัวและสาธารณะ

#### อัลกอริอะฮ์{#aws-credentials} 
เพื่อให้มันดังนั้นERDDAP™สามารถอ่านเนื้อหาของถังส่วนตัวได้ คุณต้องมีใบรับรอง AWS และคุณต้องจัดเก็บเอกสารรับรองไว้ในตําแหน่งมาตรฐานERDDAP™สามารถหาข้อมูลได้ ดู AWS SDK สําหรับJavaเอกสาร 2.x:[ตั้งค่าใบรับรองปริยาย](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/setup.html#setup-credentials). (ตัวเลือกในการจัดเก็บค่าเป็นJavaพารามิเตอร์ของบรรทัดคําสั่งใน\\[แว่นขยาย\\]/ บิน/ ซีเทนฟ.ช. อาจจะเป็นตัวเลือกที่ดี) 
#### ALWS / files /{#aws-files} 
* / แฟ้ม/ ระบบ เดอะERDDAP™ [/ แฟ้ม/ ระบบ](#accessibleviafiles)อนุญาตให้ผู้ใช้ดาวน์โหลดแฟ้มต้นฉบับสําหรับชุดข้อมูลได้ เราขอแนะนําให้คุณเปิดใช้งานมัน สําหรับชุดข้อมูลทั้งหมดที่มีแฟ้มต้นทาง เพราะผู้ใช้หลายคนต้องการดาวน์โหลดแฟ้มต้นฉบับ
...
([เอกสารของ AWS](https://aws.amazon.com/documentation/gettingstarted/)เว็บไซต์เช่น[ครอบคลุมการไหล](https://stackoverflow.com/)และแบบทั่วไป
    ...
     
* มันอาจจะยากที่จะหาโครงสร้างไดเรกทอรี และชื่อแฟ้มในถัง S3ERDDAP™มีวิธีแก้ปัญหาสําหรับปัญหานี้: EDTable from Files มีชื่อพิเศษ[\\*\\*♪ จากบนฟลาย](#fromonthefly)ตัวเลือกต่าง ๆ ที่จะให้คุณได้ทํา DDTable from Februarys Name ซึ่งจะช่วยให้ผู้ใช้สามารถเรียกดูเนื้อหาจากช่อง S3 ได้ (และดาวน์โหลดแฟ้ม) ผ่านชุดข้อมูล"files"ทางเลือก มี[ตัวอย่างนี้ด้านล่าง](#viewing-the-contents-of-a-bucket).
     
...
* สําหรับกรณีทั่วไป, ที่คุณมีERDDAP™ติดตั้งบนคอมพิวเตอร์ของคุณสําหรับการทดสอบ/ การพัฒนาและที่ชุดข้อมูลมีแฟ้มข้อมูลไบนารีที่เก็บไว้เป็นวัตถุในถัง S3 วิธีหนึ่งเพื่อให้ได้ชุดข้อมูลมาERDDAP™คือ
    ...
    3. ใช้[สร้าง DatatasXml](#generatedatasetsxml)เพื่อสร้างก้อนของdatasets.xmlสําหรับข้อมูลชุดจากแฟ้มข้อมูลท้องถิ่นทั้งสอง
    4. กาเลือกที่ชุดข้อมูลที่ใช้ได้ตามที่ต้องการ[ถอดรหัส](#dasdds)และ/ หรือ ท้องถิ่นของคุณERDDAP.
    ...
    8. เพิ่ม[แคชจากUrl](#cachefromurl)แท็กที่จะใช้กําหนดชื่อถังข้อมูล และค่านําหน้า (I.E., ไดเรกทอรี) ในรายละเอียด[รูปแบบที่อยู่ URL S3 ที่ERDDAP™ต้องการ](#accessing-files-in-an-aws-s3-bucket).
...
ถ้าชุดข้อมูลเป็นชุดขนาดใหญ่ของไฟล์ข้อมูล graed ขนาดใหญ่นี้จะใช้เวลามาก และไม่สามารถทํางานได้ ในบางกรณี สําหรับไฟล์ข้อมูลที่ถูกหักหลังERDDAP™สามารถดึงข้อมูลที่จําเป็น (เช่น เวลาของข้อมูล ในแฟ้มข้อมูลแบบฝังตัว) จากชื่อแฟ้มและหลีกเลี่ยงปัญหานี้ ดู[พารามิเตอร์ ผ่าน ชื่อแฟ้ม](#aggregation-via-file-names-or-global-metadata).
        
    11. ตัวเลือก (แต่โดยเฉพาะสําหรับ EDTable จากแฟ้มข้อมูลชุด) คุณสามารถเพิ่ม[เปิด/ ปิด เสียง](#nthreads)ป้ายกํากับที่บอกข้อมูลERDDAPเพื่อใช้เธรดมากกว่าหนึ่งเธรด เมื่อมีการตอบสนองต่อการร้องขอของผู้ใช้ นี่ ทํา ให้ ผล ของ ความ ล่า ช้า น้อย ลง เมื่อERDDAP™อ่านแฟ้มข้อมูลจาก (ระยะไกล) AWS S3 ตะกร้าเข้าไปยังแคชภายในและ (บางที) กําจัดพวกมัน

#### ข้อมูลการเปิดของ AWS S3{#aws-s3-open-data} 
เป็นส่วนหนึ่งของNOAA'[โปรแกรมข้อมูลขนาดใหญ่](https://www.noaa.gov/nodd/about).NOAAมีความร่วมมือกับองค์กร 5 แห่ง รวมถึงเอดับเบิลยูเอส "เพื่อศึกษาถึงผลประโยชน์ที่อาจเกิดขึ้น ของการเก็บรวบรวมการสังเกตที่สําคัญ และผลลัพธ์จําลองในเมฆ AWS รวมชุดข้อมูลที่ได้รับจากNOAAเป็นส่วนหนึ่งของโปรแกรมที่จะนําเสนอการเข้าถึงสาธารณะเพื่อรวบรวมขนาดใหญ่ของ[เปิดข้อมูลเมื่อ AWS S3](https://registry.opendata.aws/)จากคอมพิวเตอร์ใด ๆ ไม่ว่าจะเป็นกรณีคํานวณ Amazon (คอมพิวเตอร์ ที่ เช่า ไว้) บนเครือข่าย AWS หรือเครื่องของคุณบนเครือข่ายใด ๆ ตัว อย่าง ข้าง ล่าง สมมุติ ว่า คุณ กําลัง ทํา งาน ร่วม กับ ชุด ข้อมูล ที่ เข้า ถึง ได้ ใน ที่ สาธารณะ.
...
1. เปิดบัญชีผู้ใช้แบบ AWS
    ERDDAP™ใช้[ขนาด AWS SDK สําหรับJava](https://docs.aws.amazon.com/sdk-for-java/index.html)เพื่อหาข้อมูลถังจาก AWS ดังนั้นคุณต้อง[สร้างและเปิดใช้งานบัญชีผู้ใช้ AWS](https://aws.amazon.com/premiumsupport/knowledge-center/create-and-activate-aws-account/). นั่นเป็นงานที่ใหญ่มาก มีหลายอย่างที่ต้องเรียนรู้
     
2. ใส่ AWS Cresentials ของคุณที่ERDDAP™หาพวกมันได้
ทําตามคําแนะนําที่[ตั้งค่า ARWS Cresential and ขอบเขตสําหรับการพัฒนา](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/setup.html#setup-credentials)ดังนั้นERDDAP™  (โดยเฉพาะ AWS SDK สําหรับJava) จะสามารถค้นหาและใช้เอกสารรับรอง AWS ของคุณ ถ้าERDDAP™หาใบรับรองไม่เจอ คุณจะเห็น
จาวา แลง Expodate ไม่ถูกต้อง: โพรไฟล์แฟ้มไม่สามารถผิดพลาดได้ERDDAPแฟ้มปูมบันทึก.t.
...
3. สร้างที่อยู่ URL ของถังใน[รูปแบบที่ERDDAP™ต้องการ](#accessing-files-in-an-aws-s3-bucket)อี.จี.
...
5. ถ้าคุณทําตามคําแนะนําด้านบนและโหลดข้อมูลชุดในERDDAPคุณได้สร้าง ADTable จากแฟ้มข้อมูล เพื่อเป็นตัวอย่าง และเพื่อทําให้ทุกคนสามารถเรียกดูและดาวน์โหลดไฟล์จากช่องข้อมูล AWS ได้ง่ายขึ้น เราได้สร้าง DDTable from Files (ดูรายการที่
...
ปัญหา?
ถ้า DDTable ของคุณจากแฟ้มจะไม่โหลดERDDAP™  (หรือ dads) ดูในแฟ้มปูมบันทึก.txt สําหรับข้อความผิดพลาด ถ้าคุณเห็น
จาวา แลง ส่วนเสริม Argument Expressed: แฟ้มโพรไฟล์ไม่สามารถเป็นข้อผิดพลาดว่างได้ ปัญหาอยู่ที่ว่าแฟ้ม AWS SDK สําหรับJava  (ใช้โดยERDDAP) หาแฟ้มประวัติไม่เจอ ดูคําแนะนําในเอกสาร
     
...
สําหรับตัวอย่าง EDTable จากแฟ้มNames ข้อมูลต่าง ๆ ที่คุณทําไว้ด้านบน หากคุณลองเล่นกับไดเร็กทอรีและชื่อแฟ้มในต้นไม้ไดเร็กทอรีดู จะเห็นอย่างชัดเจนถึงชื่อไดเรกทอรีระดับบนสุด (E.g., ABI-L1b-RadC) ตรงกับอะไรERDDAP™จะเรียกว่าชุดข้อมูลแยก ถัง ที่ คุณ ทํา อยู่ อาจ คล้าย กัน. คุณสามารถไล่ตามการสร้างชุดข้อมูลแยกในERDDAP™สําหรับข้อมูลแต่ละชุด โดยใช้, e.g.
 https://noaa-goes17.s3.us-east-1.amazonaws.com/ABI-L1b-RadC/   
คือ&lt;แคชจาก URI]. โชคไม่ดี, สําหรับตัวอย่างนี้, ชุดข้อมูลในถัง ดูเหมือนจะเป็นข้อมูลระดับ 1 หรือระดับ 2ERDDAP™ [ไม่ค่อยดีเท่าไหร่](#dimensions)เนื่องจากชุดข้อมูล เป็นชุดตัวแปรที่ซับซ้อนกว่า ซึ่งใช้มิติต่าง ๆ
     
    
### แฟ้ม NcML{#ncml-files} 
แฟ้ม NcML ให้คุณได้ระบุการเปลี่ยนแปลงบน-the-fly ไปยังแหล่งกําเนิดดั้งเดิมหนึ่งหรือมากกว่าNetCDF  (v3 หรือ v4)  .nc. กริบ, . บุฟ, หรือ.hdf  (v4 หรือ v5) ไฟล์ แล้วก็ERDDAP™รักษา.ncแฟ้ม mI เป็นแฟ้มต้นทางERDDAP™ชุดข้อมูลจะยอมรับ.ncแฟ้ม ml เมื่อไหร่ก็ได้.ncคาดว่าจะเป็นแฟ้ม แฟ้ม NcML ต้องมีการส่วนขยาย.ncเอ็มไอ ดู[Unidataเอกสารของ NcML](https://docs.unidata.ucar.edu/netcdf-java/current/userguide/ncml_overview.html). NcML มีประโยชน์เพราะคุณสามารถทําอะไรบางอย่างกับมันได้ (ตัวอย่างเช่น ทําการเปลี่ยนแปลงแฟ้มต่าง ๆ ในชุดสะสม รวมถึงเพิ่มมิติด้วยค่าเฉพาะไปยังแฟ้ม) ที่นายทําไม่ได้ERDDAP'datasets.xml.
...
บางครั้งแหล่งที่มา.ncแฟ้มมีมิติเวลาและเวลาอยู่แล้ว แต่ค่าของมันไม่ถูกต้อง (เพื่อวัตถุประสงค์ของคุณ) . นี่.ncแฟ้ม ml กล่าวว่า: สําหรับแฟ้มข้อมูลที่ชื่อ "1981082300-ANCI..." สําหรับตัวแปรมิติ"time"กําหนดคุณสมบัติของหน่วยเป็น 'วินาที' ตั้งแต่ 1970-01-01T00:00:00Z' และตั้งค่าเวลาเป็น 367588800.
```
    <netcdf xmlns='https://www.unidata.ucar.edu/namespaces/netcdf/ncml-2.2'
...
```
### NetCDFตัวดําเนินการ (NCO)  {#netcdf-operators-nco} 
"ผู้ดําเนินการของ NetCDF (NCO) คอมไพล์โปรแกรมพื้นฐานและบรรทัดคําสั่งโหลที่ใช้ NetCDF\\[v3 หรือ v4\\].HDF \\[v4 หรือ v5\\].\\[. กก.\\]&/ หรือDAPแฟ้มเป็นข้อมูลเข้า จากนั้นจึงประมวลผล (e.g., สืบค้นข้อมูลใหม่, คํานวณสถิติ, การพิมพ์, ความโปร่งแสง, การจัดการข้อมูลกํากับภาพ) และส่งผลลัพธ์ไปยังหน้าจอหรือแฟ้มต่าง ๆ ในรูปแบบข้อความ, แบบไบนารี หรือรูปแบบ NetCDFNCOช่วยวิเคราะห์ข้อมูลทางวิทยาศาสตร์ รูปแบบคําสั่งเชลล์ของNCOอนุญาตให้ผู้ใช้จัดการและวิเคราะห์แฟ้มแบบโต้ตอบ หรือใช้สคริปต์แสดงอารมณ์ ที่หลีกเลี่ยงการควบคุมบางอย่าง (จาก[NCO](https://nco.sourceforge.net/)หน้าหลัก) .

ทางเลือกNCOคือ[ขนาด NcML](#ncml-files). ความแตกต่างใหญ่ก็คือ NCML เป็นระบบสําหรับการเปลี่ยนแปลงเมื่อบิน (แฟ้มแหล่งไม่ได้เปลี่ยนแปลง) ในขณะที่NCOสามารถใช้ในการทําการเปลี่ยนแปลงไปยัง (หรือรุ่นใหม่ของ) ไฟล์ ทั้งคู่NCOและ NcML ยืดหยุ่นมาก และช่วยให้คุณเปลี่ยนแปลงได้เกือบทุกอย่าง สําหรับทั้งสองอย่าง อาจเป็นเรื่องยากที่จะค้นหาว่า จะทําอย่างไรดี ตรวจดูเว็บตัวอย่างที่คล้ายกัน ทั้งคู่เป็นเครื่องมือที่มีประโยชน์ในการเตรียม NetCDF และHDFแฟ้มที่จะใช้ERDDAPโดดเด่นที่จะทําการเปลี่ยนแปลงเกินสิ่งที่ERDDAPระบบควบคุมทําได้

ตัวอย่างเช่น คุณสามารถใช้NCOเพื่อให้หน่วยของตัวแปรเวลาสอดคล้องกัน ในกลุ่มแฟ้มที่มันไม่สอดคล้องกันตอนแรก หรือคุณสามารถใช้NCOปรับใช้scale\\_factorถึงadd\\_offsetในกลุ่มของแฟ้มscale\\_factorถึงadd\\_offsetมีค่าที่ต่างกันในแฟ้มแหล่งต่างๆ
 (หรือตอนนี้คุณสามารถจัดการกับปัญหาเหล่านั้นERDDAP™ผ่าน[EDDGridแฟ้มจาก Nc](#eddgridfromncfilesunpacked)ซึ่งเป็นองค์ประกอบของEDDGridจากแฟ้ม NcFiles ที่บรรจุข้อมูลและตั้งค่าเวลามาตรฐานไว้ในระดับต่ํา เพื่อจัดการกับแฟ้มชุดสะสมที่มีความแตกต่างscale\\_factors และadd\\_offsetหรือหน่วยเวลาต่างกัน.) 

NCOเป็นซอฟต์แวร์เสรีและโอเพนซอร์สที่ใช้[GPL 3. 0](https://www.gnu.org/licenses/gpl-3.0.html)ใบอนุญาติ

ตัวอย่าง #1: การสร้างหน่วยสอดคล้อง
EDDGridจากแฟ้มและ EDTable จากแฟ้มยืนยันว่าหน่วยสําหรับตัวแปรที่กําหนดจะเหมือนกันในแฟ้มทั้งหมด ถ้าบางไฟล์เป็นเรื่องเล็กน้อย (ไม่ใช้ฟังก์ชัน) แตกต่างจากคนอื่น (เช่น หน่วยเวลาของ
"วินาทีตั้งแต่ 1970-01-01 00:00 UTC" ปะทะ
"seconds since 1970-01-01T00:00:00Z"คุณสามารถใช้NCO'[หมวดหมู่](https://nco.sourceforge.net/nco.html#ncatted-netCDF-Attribute-Editor)เพื่อเปลี่ยนหน่วยในแฟ้มทั้งหมด ให้เหมือนกับ
nco/ncathed - หน่วย, เวลา, o, c'วินาทีตั้งแต่ 1970-01-01T00:00:00Z'\\*.nc  
\\[สําหรับปัญหาแบบนี้ใน EDTable จาก... ชุดข้อมูลแฟ้ม, ตอนนี้คุณสามารถใช้ได้[ขนาดมาตรฐาน อะไร](#standardizewhat)เพื่อบอกว่าERDDAPเพื่อกําหนดแฟ้มต้นฉบับตามที่อ่านเข้าไปERDDAP.\\]
...
* จํานวนสูงสุดของaxisVariables คือ ~2 พันล้าน
จํานวนสูงสุดของdataVariables คือ ~2 พันล้าน
...
เรา (โดยเฉพาะ[สร้าง DatatasXml](#generatedatasetsxml)) ขอแนะนําให้ใช้[ACD รุ่น 1. 3](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)2553 ซึ่งได้รับการรับรองเมื่อต้น ค.ศ. ก่อนถึงERDDAP™รุ่น 1. 2562 (ออก ใน เดือน มิถุนายน 2015) .ERDDAP™ใช้/ ปรับปรุงต้นฉบับ รุ่น 1. 0 ของ[NetCDFอนุ สัญญา เรื่อง การ ค้น พบ ข้อมูล](https://wiki.esipfed.org/ArchivalCopyOfVersion1)ซึ่งถูกเรียกว่า "UnidataDatetet Directy v1.0"ในการประชุมโลกและMetadata\\_Conventionsคุณสมบัติ
...
ไปยังชุดข้อมูลทั่วโลก&lt;addAttributes&gt;
     
2. ถ้า ชุด ข้อมูล มี คุณสมบัติ ใน การ ประชุม ใหญ่ ทั่ว โลก&lt;addAttributes&gt; เปลี่ยนทั้งหมด "UnidataDatetset Directory v1.0" อ้างอิงถึง "ACD-1.3".
...
ถ้าอ้างอิงเป็นรุ่นเก่าของ[ตารางชื่อมาตรฐาน CF](https://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table.html). มันอาจจะเป็นความคิดที่ดีที่จะเปลี่ยนเป็นรุ่นปัจจุบัน (65, อย่างที่เราเขียน) เนื่องจากชื่อมาตรฐานใหม่ถูกเพิ่มเข้าไปในตารางที่มีรุ่นต่อมา แต่ชื่อมาตรฐานเก่าแทบจะไม่ถูกเพิกถอนและไม่เคยถูกลบออก
     
4. แม้ว่า ACD-1.0 จะรวมคุณสมบัติระดับโลกสําหรับcreator\\_name.creator\\_email.creator\\_url.[สร้าง DatatasXml](#generatedatasetsxml)ไม่ได้บวกมันโดยอัตโนมัติ จนกระทั่งบางครั้งERDDAP™v150. นี่เป็นข้อมูลสําคัญ
...
นั่นแหละ หวังว่าคงไม่ยากนะ
     
### ซาร์{#zarr} 
รุ่น 2.25ERDDAP™ไม่สามารถอ่านข้อมูลภายในได้ เข้ารหัสแฟ้มโดยใช้[แฟ้ม DDTAB จาก Nc](#eddtablefromncfiles)ถึง[EDDGridแฟ้มจาก Nc](#eddgridfromncfiles).
...
*   [ **EDDGrid** ](#eddgrid)ชุดข้อมูลจัดการกับข้อมูลที่ถูกย่าง
    ...
    * ในEDDGridชุดข้อมูล, ตัวแปรข้อมูลทั้งหมดต้องถูกใช้ (หุ้น) ตัวแปรแกนทั้งหมด
         ([ทําไม?](#why-just-two-basic-data-structures) [แล้วถ้าไม่ล่ะ](#dimensions)) 
    * ค่าขนาดเรียงลําดับ - ทั้งหมดEDDGridชุดข้อมูล, แต่ละมิติต้องเรียงตามลําดับ (ขึ้นหรือลง) . แต่ละคนสามารถเว้นช่องว่างได้ ไม่มีสายสัมพันธ์ นี่คือเงื่อนไขของ[มาตรฐานข้อมูลกํากับภาพแบบ CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html). ถ้าค่าของมิติใด ๆ ไม่เรียงตามลําดับ ชุดข้อมูลจะไม่ถูกโหลดและERDDAP™จะระบุค่าแรกที่ยังไม่ได้เขียนในแฟ้มปูมบันทึก *เครื่อง มือ ขนาด ใหญ่* log/log.txt.
...
ค่ามิติ Unserted เกือบมักจะบอกปัญหากับชุดข้อมูลต้นฉบับ เหตุการณ์นี้มักเกิดขึ้นเมื่อแฟ้มที่มีชื่อผิดหรือไม่เหมาะสม ถูกรวมอยู่ในจดหมายเวียน ซึ่งนําไปสู่มิติเวลาที่ไม่ปรากฏ เพื่อแก้ปัญหานี้ ดูข้อความผิดพลาดในERDDAP™แฟ้มปูมบันทึก.t เพื่อค้นหาค่าของเวลาที่ขัดจังหวะ จากนั้นดูในแฟ้มต้นทางเพื่อหาแฟ้มที่สอดคล้องกัน (หรือก่อนหรือหลัง) นั่นไม่เหมาะกับรังผึ้ง
    ...
    * ดูรายละเอียดที่สมบูรณ์มากขึ้นของ[EDDGridโมเดลข้อมูล](https://coastwatch.pfeg.noaa.gov/erddap/griddap/documentation.html#dataModel).
    * เดอะEDDGridประเภทชุดข้อมูลคือ:
        *   [EDDGridจากแฟ้ม Audio](#eddfromaudiofiles)รวบรวมข้อมูลจากกลุ่มแฟ้มเสียงภายในระบบ
        *   [EDDGridจาก Dap](#eddgridfromdap)จัดการข้อมูลการแก็ดจากDAPเซิฟเวอร์
        *   [EDDGridดึงข้อมูล](#eddgridfromeddtable)ให้คุณได้แปลงชุดข้อมูลอักขระแท็บอาร์กติก ให้เป็นชุดข้อมูลแบบฝังตัว
        *   [EDDGridจาก เอลด์ดาป](#eddfromerddap)ใช้ข้อมูลจากระยะไกลERDDAP.
        *   [EDDGridจาก เอ โต โป](#eddgridfrometopo)แค่จัดการกับข้อมูลภูมิศาสตร์ของ ETOPO
        *   [EDDGridจากแฟ้ม](#eddgridfromfiles)ซูเปอร์คลาสของทั้งหมดEDDGridจากชั้นเรียนแฟ้ม
        *   [EDDGridแฟ้ม MergeIR](#eddgridfrommergeirfiles)ผนวกรวมข้อมูลจากกลุ่มของคอมไพล์ท้องถิ่น.gzแฟ้ม
        *   [EDDGridแฟ้มจาก Nc](#eddgridfromncfiles)เก็บข้อมูลจากกลุ่มผู้ใช้NetCDF  (v3 หรือ v4)  .ncและไฟล์ที่เกี่ยวข้อง
        *   [EDDGridแฟ้มจาก Nc](#eddgridfromncfilesunpacked)เป็นตัวแปรเฉพาะ หากEDDGridจากแฟ้ม Nc ซึ่งรวมข้อมูลจากกลุ่มแฟ้มภายในด้วยNetCDF  (v3 หรือ v4)  .ncและไฟล์ที่เกี่ยวข้อง ซึ่งERDDAP™ขนของที่ต่ํา
        *   [EDDGridลอน PM 180](#eddgridlonpm180)กําหนดค่าลองจิจูดของเด็กEDDGridเพื่อให้พวกเขาอยู่ในระยะ - 180 ถึง 180
        *   [EDDGridลอน0360](#eddgridlon0360)กําหนดค่าลองจิจูดของเด็กEDDGridมันเลยอยู่ในช่วง 0 ถึง 360
        *   [EDDGridด้านข้าง](#eddgridsidebyside)ประกอบสองหรือมากกว่าEDDGridชุดข้อมูลชิดกัน
        *   [EDDGridการ แบ่ง แยก](#eddgridaggregateexistingdimension)ประกอบสองหรือมากกว่าEDDGridชุดข้อมูล, แต่ละอันมีค่าต่าง ๆ กัน สําหรับมิติแรก, แต่มีค่าเหมือนกัน สําหรับมิติอื่น
        *   [EDDGridคัดลอก](#eddgridcopy)สร้างสําเนาเครื่องใหม่EDDGridข้อมูลและบริการจากสําเนาท้องถิ่น
             
    * ทั้งหมดEDDGridชุดข้อมูลรองรับการตั้งค่า nTHreads ซึ่งบอกว่าERDDAP™วิธีการหลายเธรดที่จะใช้เมื่อตอบสนองต่อการร้องขอ ดู[เปิด/ ปิด เสียง](#nthreads)เอกสารสําหรับรายละเอียด
         
...
*   [ **แก้ไขโครงการหลัก...** ](#eddtable)ชุดข้อมูลจัดการข้อมูลรอบนอก
    ...
    * ประเภทข้อมูล EDTable คือ:
        *   [DDTable from AllDatets](#eddtablefromalldatasets)เป็นชุดข้อมูลระดับสูงที่มีข้อมูลเกี่ยวกับชุดข้อมูลอื่น ๆ ทั้งหมดของคุณERDDAP.
        ...
        *   [EDTable from Ascisiservnos](#eddtablefromasciiservicenos)จัดการข้อมูลจากบางส่วนNOAAบริการ NOS.
        ...
        *   [DDTable from Depsquest](#eddtablefromdapsequence)ใช้ข้อมูลอักขระแท็บจากDAPเครื่องแม่ข่ายลําดับ
        *   [เพิ่มข้อมูล](#eddtablefromdatabase)จัดการข้อมูลรอบนอกจากตารางฐานข้อมูล
        *   [จากEDDGrid](#eddtablefromeddgrid)คุณจะสร้างชุดข้อมูล EDTable จากEDDGridชุดข้อมูล
        *   [DDTable from Edardp](#eddfromerddap)จัดการข้อมูลแท็บจากระยะไกลERDDAP.
        ...
        *   [เพิ่มข้อมูล](#eddtablefromhttpget)คือERDDAPระบบสําหรับนําเข้าข้อมูลเท่านั้น รวมถึงการส่งออกข้อมูลด้วย
        *   [จากHyraxแฟ้ม](#eddtablefromhyraxfiles)  (ขอบเขต) รวมข้อมูลจากแฟ้มที่มีตัวแปรหลายตัวที่มีมิติที่ใช้ร่วมกัน[Hyrax OPeNDAPเซิร์ฟเวอร์](https://www.opendap.org/software/hyrax-data-server).
        *   [แฟ้ม DDTA จาก InfidCra](#eddtablefrominvalidcrafiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .ncแฟ้มที่ใช้ค่าเฉพาะ, ใช้งานไม่ได้, ค่าย่อของ CFDSG แบบต่อเนื่อง Rangle Array (CRA) แฟ้ม ถึงแม้ว่าERDDAP™รองรับประเภทแฟ้มนี้ ซึ่งเป็นประเภทแฟ้มที่ไม่ควรใช้ กลุ่มที่จะใช้ประเภทแฟ้มนี้ได้ถูกสนับสนุนให้ใช้ERDDAP™เพื่อสร้างแฟ้ม CFDSG CRA และหยุดการใช้แฟ้มเหล่านี้
        *   [แฟ้ม DDTable จาก JonlCSV](#eddtablefromjsonlcsvfiles)ข้อมูลรวมจาก[เจ สัน แฟ้มข้อความ CSV](https://jsonlines.org/examples/).
        *   [แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .ncแฟ้มที่มีหลายตัวแปร ที่มีมิติร่วมกัน
        *   [แฟ้ม DDTAB จาก Nc](#eddtablefromncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .ncแฟ้มที่มีหลายตัวแปร ที่มีมิติร่วมกัน มันดีที่จะยังคงใช้ประเภทข้อมูลชุดนี้ สําหรับชุดข้อมูลที่มีอยู่ แต่สําหรับชุดข้อมูลใหม่ที่เราแนะนํา โดยใช้ DDTable from Mulitidnc Files แทน
        *   [แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .ncแฟ้มซึ่งใช้เป็นหนึ่งในรูปแบบแฟ้มที่ระบุโดย[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)การ ประชุม ภาค. แต่สําหรับไฟล์ที่ใช้เป็นหนึ่งในหลายมิติ CF DSG ตัวแปร, ใช้[แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)แทน
        ...
        *   [จากSOS](#eddtablefromsos)ใช้ข้อมูลอักขระแท็บจากSOSเซิฟเวอร์
        *   [แฟ้ม EDTIVE จาก Thredds February](#eddtablefromthreddsfiles)  (ขอบเขต) รวมข้อมูลจากแฟ้มที่มีตัวแปรหลายตัวที่มีมิติที่ใช้ร่วมกัน[(เต๋า)OPeNDAPเซิร์ฟเวอร์](https://www.unidata.ucar.edu/software/tds/).
        *   [จากWFSแฟ้ม](#eddtablefromwfsfiles)  (ขอบเขต) ทําสําเนาข้อมูลทั้งหมดจากArcGISเซิร์ฟเวอร์แผนที่WFSเซิร์ฟเวอร์ เพื่อให้ข้อมูลสามารถถูกตรวจสอบใหม่ได้อย่างรวดเร็วERDDAP™ผู้ใช้
...
### EDDGridจาก Dap{#eddgridfromdap} 
[ **EDDGridจาก Dap** ](#eddgridfromdap)ใช้ตัวแปรกริดจาก[DAP](https://www.opendap.org/)เซิฟเวอร์

* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล คุณสามารถรวบรวมข้อมูลที่คุณต้องการเพื่อปรับเปลี่ยนหรือสร้าง XML ของคุณเองสําหรับEDDGridจากฐานข้อมูล Dap โดยดูที่ DDS และ DAS ในเบราว์เซอร์ของคุณ (โดยการเพิ่ม .das และ ids กับsourceUrlยกตัวอย่างเช่น[ https://thredds1.pfeg.noaa.gov/thredds/dodsC/satellite/BA/ssta/5day.dds ](https://thredds1.pfeg.noaa.gov/thredds/dodsC/satellite/BA/ssta/5day.dds)) .
     
...
#### EDDGridจากโครงกระดูก Dap XML{#eddgridfromdap-skeleton-xml} 
 ...
 >&nbsp;&nbsp;&nbsp;&nbsp;&lt;/dataset>  
    ...
     
### EDDGridดึงข้อมูล{#eddgridfromeddtable} 
[ **EDDGridดึงข้อมูล** ](#eddgridfromeddtable)ให้คุณแปลงชุดแท็บแบบ EDTable ให้เป็นEDDGridข้อมูลปลอม จําไว้ERDDAP™ใช้ชุดข้อมูลเป็น[ชุดข้อมูลแบบฝังตัว (คลาสย่อยของEDDGrid) หรือชุดข้อมูลแท็บ (คลาสย่อยของ EDTable) ](#why-just-two-basic-data-structures).
...
* เห็นได้ชัดว่าข้อมูลใน DDTable ข้อมูลภายในจะต้อง (โดยทั่วไป) ข้อมูลที่ถูกย่าง แต่อยู่ในรูปโพลาร์ ตัว อย่าง เช่น ชุด ข้อมูล CTD แบบ EDTable อาจ มี ข้อมูล CTD: การ วัด กระแส ไฟฟ้า ทาง ตะวัน ออก และ ทาง เหนือ ที่ ความ ลึก หลาย ต่อ หลาย ครั้ง. เนื่องจากความลึกนั้นเหมือนกัน ในแต่ละครั้งEDDGridข้อมูลจาก EDDTable สามารถสร้างชุดข้อมูลแบบฝังตัวด้วยเวลาและมิติความลึก ซึ่งเข้าถึงข้อมูลได้ผ่านทางชุดข้อมูล DDTable
     
* ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ Xml -- เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล คุณ สามารถ รวบ รวม ข้อมูล ที่ คุณ จําเป็น ต้อง ได้ รับ เพื่อ ปรับ ปรุง การ ร่าง อย่าง หยาบ คาย.
     
* แหล่งของแอททริบิวต์ เหมือนกับชุดข้อมูลอื่นๆEDDGridจาก Table มีความคิดที่ว่ามีแหล่งที่มาทั่วโลก attributions และ[ทั่ว โลกaddAttributes](#global-attributes)  (ระบุในdatasets.xml) ซึ่งจะรวมกันทําให้ทั่วโลกรวมกัน แอตทริบิวต์ ซึ่งเป็นสิ่งที่ผู้ใช้เห็น สําหรับแหล่งข่าวทั่วโลกEDDGridจาก ผู้ อ่าน ของ เรา แอตทริบิวชันของชุดข้อมูล DDTable (ถ้าคุณลองคิดดูสักนาที มันก็สมเหตุสมผลดี) 
    
เช่นเดียวกัน สําหรับแต่ละคนaxisVariableและdataVariable'[addAttributes](#addattributes).EDDGridใช้ตัวแปรรวมกัน แอตทริบิวต์จากข้อมูล DDTable เป็นEDDGridแหล่งที่มาของตัวแปรจากผู้รับ (ถ้าคุณลองคิดดูสักนาที มันก็สมเหตุสมผลดี) 
...
สังเกตการใช้[ชนิดของข้อมูล](#data-types)และคําว่า "ลิสต์" และประเภทของรายการ (ยกตัวอย่างเช่น สองเท่า) ต้องตรงกับข้อมูล ชนิดของตัวแปรใน EDTable และEDDGridชุดข้อมูลจาก EDDD
...
โปรด สังเกต การ ใช้ ชนิด ข้อมูล ใน รายการ อีก ครั้ง. และประเภทของรายการ (ยกตัวอย่างเช่น สองเท่า) ต้องตรงกับข้อมูล ชนิดของตัวแปรใน EDTable และEDDGridชุดข้อมูลจาก EDDD
         
...
คุณรู้ตําแหน่งเริ่มต้นของป้าย
    4. ค้นหาdatasets.xmlสําหรับเหตุการณ์ถัดไป
```
        </att>  
```
ดังนั้นคุณรู้ตําแหน่งปลายของค่าแกน
    5. แทนที่จุดเริ่มต้นเดิม, ความคืบหน้า, หยุดค่าด้วยค่าใหม่
    6. ติดต่อ[ที่อยู่ URL ธง](/docs/server-admin/additional-information#set-dataset-flag)สําหรับข้อมูลชุดที่จะบอกว่าERDDAP™เพื่อเรียกข้อมูลใหม่
...
* ความแม่นยํา-- เมื่อEDDGridจาก EDDTIA จะตอบรับคําขอของผู้ใช้สําหรับข้อมูล, มันย้ายแถวของข้อมูล จากตารางตอบรับแบบ EDTableEDDGridระบบตอบโต้ ในการทําอย่างนี้, มันต้องหาว่า ค่า "แกน" บนแถวที่กําหนดในตาราง ตรงกับผลรวมของค่าแกนในตารางหรือเปล่า สําหรับข้อมูลประเภทจํานวนเต็ม มันง่ายกว่าที่จะตัดสินว่าค่าสองค่าเท่ากัน แต่สําหรับลอยและคู่นี้นําขึ้น ปัญหาที่น่ากลัวของจํานวนจุดลอย[ไม่ตรงกันเป๊ะ](https://randomascii.wordpress.com/2012/02/25/comparing-floating-point-numbers-2012-edition/). (ตัวอย่างเช่น, 0.2 เทียบกับ 0.19999999999999996) . ถึง (พยายาม) จัดการเรื่องนี้ซะEDDGridจากค่าที่อ่านได้ ให้คุณระบุแอตทริบิวต์ที่ถูกต้องสําหรับใด ๆaxisVariables, ซึ่งระบุจํานวนทศนิยมทั้งหมด ที่ต้องเท่ากัน
    ...
    * สําหรับaxisVariables นั่นคือ[เวลาหรือเวลา ตัวแปรสแตมป์](#timestamp-variables)ค่าเริ่มต้น: (ตรงกันเป๊ะ) .
...
นี่ไม่ใช่ข้อบกพร่องEDDGridจากเทเบิลEDDGridจาก Table ไม่สามารถแก้ปัญหานี้ได้ ปัญหามีอยู่ในการแปลงข้อมูลอักขระแท็บเป็นข้อมูล (นอกจากข้อสันนิษฐานอื่น ๆ สามารถทําได้ แต่พวกเขาไม่สามารถทําที่นี่) .
มันขึ้นอยู่กับคุณERDDAP™ผู้ดูแลระบบ, ถึง **ทดสอบEDDGridจาก ผู้ อ่าน ของ เรา** เพื่อ ให้ แน่ ใจ ว่า ค่า นิยม ที่ ถูก ต้อง แม่นยํา ถูก กําหนด ไว้ เพื่อ หลีก เลี่ยง ปัญหา ที่ อาจ เกิด ขึ้น ได้ เหล่า นี้.
        
#### ช่องว่าง{#gapthreshold} 
*   [ช่องว่าง](#gapthreshold)-- นี่เป็นชุดข้อมูลที่ผิดปกติมาก ตั้งแต่ชนิดของความต่อเนื่องที่สามารถทํา (จัดการโดย) เป็นEDDGridชุดข้อมูล (เกี่ยวข้องกับช่วงและความก้าวหน้าของaxisVariableวินาที) แตกต่างกันมากจากชนิดของ query ที่สามารถทําเพื่อ (จัดการโดย) ชุดข้อมูล EDTable (แค่เกี่ยวข้องกับเรนจ์ของตัวแปร) การแสดงของEDDGridชุดข้อมูลจาก EDDTable จะแตกต่างกันอย่างมาก ขึ้นอยู่กับการร้องขอที่แน่นอนซึ่งทํา และความเร็วของชุดข้อมูล DDTable สําหรับการร้องขอที่มีความคืบหน้าค่า &gt; 1EDDGridจาก ผู้ อ่าน ของ เรา (ราวกับว่าความคืบหน้า = 1) และผ่านผลลัพธ์ เก็บข้อมูลจากบางแถว และทิ้งข้อมูลจากคนอื่น ถ้ามันต้องผ่านข้อมูลจํานวนมาก เพื่อรับข้อมูลที่จําเป็น คําร้องขอจะใช้เวลานานขึ้น
...
สําหรับการร้องขอของผู้ใช้แต่ละคนEDDGridลายนิ้วมือที่สามารถวิเคราะห์ได้[ปูมบันทึก. txt](/docs/server-admin/additional-information#log)แฟ้ม
    
    * ถ้า&lt;logLevil) (ระดับของบล็อก) ในdatasets.xmlตั้งเป็นข้อมูลแล้ว, ลายนิ้วมือนี้เหมือนกับ
/* nouterAxes=1 จาก 4 outerRequest=22
ถ้า n outerAxes=0 ช่อง Tresold ไม่ได้เกินและคําขอเพียงหนึ่งจะทํา EDTable
ถ้า n outerAxes: 0 ช่องช่องว่าง Thresold ถูกเกิน และ n outer Requets จะทําไปยัง EDTAable, ตรงกับแต่ละชุดที่ต้องการของ nouterAxs ตัวอย่างเช่น, ถ้าชุดข้อมูลมี 4axisVariables และdataVariables เหมือนทางตะวันออก\\[เวลา\\]\\[เส้นรุ้ง\\]\\[เส้นแวง\\]\\[ความลึก\\]ทางซ้ายสุด (ก่อน) ตัวแปรแกนคือเวลา
    * ถ้า&lt;logLevil) ในdatasets.xmlมีการกําหนดไปยังทั้งหมด จะมีการเขียนข้อมูลเพิ่มเติมไปยังแฟ้ม log.txt
         
#### EDDGridโครง กระดูก ที่ ฟื้น ได้ XML{#eddgridfromeddtable-skeleton-xml} 
...
### ED* จากERDDAP {#eddfromerddap} 
 **EDDGridจาก เอลด์ดาป** ใช้ข้อมูลจากระยะไกลERDDAP™เซิฟเวอร์
 **DDTable from Edardp** จัดการข้อมูลแท็บจากระยะไกลERDDAP™เซิฟเวอร์
...
*   EDDGridจาก Eddadap และ EDTable from Edardp ประพฤติแตกต่างจากรูปแบบอื่น ๆ ทั้งหมดของชุดข้อมูลในERDDAP.
    ...
    *   EDDGridจาก เอลด์แดป และ EDTable จาก Eddep เป็นพื้นฐานสําหรับ[กริด/ ส่วนประกอบ](/docs/server-admin/scaling)ของERDDAPS ซึ่งกระจายการใช้งาน CPU อย่างมีประสิทธิภาพ (ส่วนใหญ่สําหรับการทําแผนที่) การใช้งานหน่วยความจํา คลังข้อมูล และการใช้งานแบนด์วิธของศูนย์ข้อมูลขนาดใหญ่
#### เปลี่ยนทิศทาง{#redirect} 
* ไม่เหมือนกับชุดข้อมูลประเภทอื่น เมื่อERDDAP™รับการร้องขอข้อมูลหรือภาพจากชุดข้อมูลเหล่านี้ERDDAP [เปลี่ยนทิศทาง](https://en.wikipedia.org/wiki/URL_redirection)การร้องขอไปยังระยะไกลERDDAP™เซิฟเวอร์ ผล คือ:
...
หลายสิ่งจะต้องทํางานอย่างถูกต้องเพื่อให้ระบบนี้ทํางาน เพื่อให้ชุดข้อมูลของคุณคงขึ้นถึงวันที่ ตรวจ ดู สิ่ง เหล่า นี้ แต่ ละ อย่าง เป็น ลําดับ:
    ...
    2. โดยทั่วไป (แต่ไม่เสมอไป) คุณERDDAP'&lt;และ&lt;BasehtpsUrl). ไม่จําเป็นต้องมีหมายเลขพอร์ต (E.g., :8080, :43) . ถ้าพวกเขาทํา ใช้[ตําแหน่งพร็อกซี](/docs/server-admin/deploy-install#proxypass)เพื่อเอาเรือออกจากเรืออูเรล
...
EDDGrid/ Table from Eddap languages จะเปลี่ยนข้อมูลที่เก็บเกี่ยวกับข้อมูลแต่ละแหล่งเมื่อชุดข้อมูลต้นฉบับ["โหลดใหม่"](#reloadeverynminutes)และส่วนบางส่วนของการเปลี่ยนแปลง (เช่น ตัวแปรเวลาactual\\_range) จึงทําให้เกิดการแจ้งข้อมูล ถ้าชุดข้อมูลต้นฉบับมีข้อมูลที่เปลี่ยนแปลงบ่อยครั้ง (ตัวอย่างเช่น ข้อมูลใหม่ทุกๆวินาที) และใช้["อัพเดต"](#updateeverynmillis)ระบบที่จะสังเกตเห็นการเปลี่ยนแปลงบ่อยครั้งกับข้อมูลพื้นฐาน,EDDGrid/ Table from Eddradap จะไม่ได้รับแจ้งเกี่ยวกับการเปลี่ยนแปลงที่เกิดขึ้นบ่อย ๆ นี้ จนกว่าชุดข้อมูลถัดไป "โหลด", ดังนั้นEDDGrid- Table จาก Eddadap จะไม่ได้สมบูรณ์ขึ้นสู่วันที่ คุณสามารถทําให้ปัญหานี้น้อยที่สุด โดยการเปลี่ยนข้อมูลแหล่ง&lt;โหลดรายการทุก ๆNMutchs) ไปยังค่าที่น้อยกว่า (60 หรือ 15) เพื่อให้มีการแจ้งข้อมูลเพิ่มเติมที่จะบอกว่าEDDGridสืบค้นข้อมูลแหล่งข้อมูล
...
นั่นจะนําไปสู่ข้อมูลแหล่ง ที่สมบูรณ์แบบขึ้นถึงวัน และทําให้มันสร้างการแจ้งรับข้อมูล ซึ่งจะส่งไปยังEDDGridข้อมูลจาก Eddadap นั่นจะนําEDDGridข้อมูลต่าง ๆ ของปฏิทิน (ภายใน 5 วินาที ข้อมูลใหม่จะถูกเพิ่ม) . และทุกสิ่งที่จะทําอย่างมีประสิทธิภาพ (โดยไม่ต้องใส่ข้อมูลซ้ํา) .
     
#### ไม่addAttributes.axisVariableหรือdataVariable {#no-addattributes-axisvariable-or-datavariable} 
...
2. แทนที่ EDTable from Edardp, ใช้[DDTable from Depsquest](#eddtablefromdapsequence). หรือแทนที่จะเป็นEDDGridจากเอิร์ดแดป ใช้[EDDGridจาก Dap](#eddgridfromdap). ประเภท DD เหล่านั้นช่วยให้คุณเชื่อมต่อกับข้อมูลบนระยะไกลได้มีประสิทธิภาพERDDAP™  (แต่ไม่มีการย้ายข้อมูลที่ต้องการ) และมันช่วยให้คุณรวมทั่วโลก&lt;addAttributes&gt;.&lt;axisVariable& gt; หรือ&lt;dataVariable& gt; ตอนในdatasets.xml. ความแตกต่างอีกอย่างคือ คุณจะต้องสมัครรับ ข้อมูลจากระยะไกล เพื่อให้ข้อมูลในข้อมูลERDDAP™จะถูกแจ้งให้ทราบ (ผ่าน[ที่อยู่ URL ธง](/docs/server-admin/additional-information#set-dataset-flag)) เมื่อมีการเปลี่ยนแปลงเป็นชุดข้อมูลระยะไกล ด้วย เหตุ นี้ คุณ กําลัง สร้าง ชุด ข้อมูล ใหม่ แทน ที่ จะ เชื่อม โยง กับ ข้อมูล ที่ อยู่ ห่าง ไกล.
         
#### บันทึกย่ออื่น ๆ{#other-notes} 
* ด้วยเหตุผลด้านความปลอดภัยEDDGridจาก เอลด์แดป และ EDTable จากเอเวอร์แดปไม่สนับสนุน&lt;เข้าถึง To]] (# เข้าถึงได้) แท็กและไม่สามารถใช้กับชุดข้อมูลระยะไกลที่ต้องการการบันทึก (เนื่องจากใช้ [ใน]&lt;เข้าถึง To]] (# เข้าถึงได้) ). ดูERDDAP'[ระบบรักษาความปลอดภัย](/docs/server-admin/additional-information#security)เพื่อจํากัดการเข้าถึงชุดข้อมูลบางส่วนกับผู้ใช้บางคน
     
* เริ่มด้วยERDDAP™v2.10.EDDGrid[จาก Eddadap and EDTable from Edardp สนับสนุน]&lt;Access Via Files) (แฟ้มที่อ่านได้) ป้าย ไม่เหมือนกับชุดข้อมูลประเภทอื่น, ค่าปริยายจะเป็นจริง แต่แฟ้มข้อมูลชุดนั้น จะสามารถเข้าใช้งานแฟ้ม Via ได้ก็ต่อเมื่อชุดข้อมูลต้นฉบับมี&lt;February Via Files) กําหนดให้เป็นจริง
     
* คุณสามารถใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อทําให้datasets.xmlผนวกเข้ากับชุดข้อมูลประเภทนี้ แต่คุณสามารถทําชุดข้อมูลเหล่านี้ด้วยมือได้ง่าย
     
#### EDDGridกระดูก สัน หลัง XML{#eddgridfromerddap-skeleton-xml} 
...
### EDDGridจาก เอ โต โป{#eddgridfrometopo} 
[ **EDDGridจาก เอ โต โป** ](#eddgridfrometopo)แค่เสิร์ฟ[ETOPO1 Global 1-Mitute Gried Dtaination Date](https://www.ngdc.noaa.gov/mgg/global/global.html)  (พื้นผิวน้ําแข็ง, ตารางลงทะเบียนฐานสอง,  inte: etopo1\\ ce\\_g/i2.zip) ซึ่งกระจายด้วยERDDAP.
...
### EDDGridจากแฟ้ม{#eddgridfromfiles} 
[ **EDDGridจากแฟ้ม** ](#eddgridfromfiles)ซูเปอร์คลาสของทั้งหมดEDDGridจากชั้นเรียนแฟ้ม คุณใช้ไม่ได้EDDGridจากแฟ้มโดยตรง แทนที่จะใช้คลาสย่อยของEDDGridจากแฟ้มเพื่อจัดการประเภทแฟ้มเฉพาะ:

*   [EDDGridแฟ้ม MergeIR](#eddgridfrommergeirfiles)จัดการข้อมูลจาก gried[ผนวกรวม IR.gz](https://www.cpc.ncep.noaa.gov/products/global_precip/html/README)แฟ้ม
*   [EDDGridจากแฟ้ม Audio](#eddfromaudiofiles)รวบรวมข้อมูลจากกลุ่มแฟ้มเสียงภายในระบบ
*   [EDDGridแฟ้มจาก Nc](#eddgridfromncfiles)จัดการข้อมูลจาก gried[GRIB](https://en.wikipedia.org/wiki/GRIB)แฟ้ม[HDF  (v4 หรือ v5)  .hdf](https://www.hdfgroup.org/)แฟ้ม[.ncmI](#ncml-files)แฟ้มและ[NetCDF  (v3 หรือ v4)  .nc](https://www.unidata.ucar.edu/software/netcdf/)แฟ้ม นี่อาจใช้ได้กับประเภทแฟ้มชนิดอื่น (ยกตัวอย่างเช่น BUFR) เรายังไม่เคยทดสอบมัน กรุณาส่งไฟล์ตัวอย่างมาให้เรา ถ้าคุณสนใจ
*   [EDDGridแฟ้มจาก Nc](#eddgridfromncfilesunpacked)เป็นองค์ประกอบของEDDGridจากแฟ้ม Nc ซึ่งใช้จัดการข้อมูลจาก griedNetCDF  (v3 หรือ v4)  .ncและไฟล์ที่เกี่ยวข้อง ซึ่งERDDAP™ขนของที่ต่ํา
...
* ทุกแฟ้มต้องเหมือนกันเป๊ะ[หน่วย](#units)ข้อมูลกํากับภาพทั้งหมดaxisVariables และdataVariableเอส. ถ้า นี่ เป็น ปัญหา คุณ อาจ ใช้ ได้[ขนาด NcML](#ncml-files)หรือ[NCO](#netcdf-operators-nco)เพื่อแก้ไขปัญหา
         
...
เพื่อติดตั้งหนึ่งในนี้ พลูโตในERDDAP™คุณจะนิยามมุมซ้ายใหม่ (ก่อน)  [axisVariable](#axisvariable)กับสิ่งพิเศษ ๆ&lt;sourceName&gt; ซึ่งบอกว่าERDDAP™ที่และวิธีการหาค่าสําหรับมิติใหม่จากแต่ละแฟ้ม

* รูปแบบสําหรับชุดสะสมsourceNameซึ่งจะได้ค่าจากชื่อแฟ้ม (ชื่อแฟ้ม) คือ
    \\*\\*/ *แฟ้มName* [ข้อมูล ชนิด](#data-types) *.* คลายแฟ้ม *.* จับภาพ Group section*
* รูปแบบสําหรับชุดสะสมsourceNameซึ่งจะได้ค่าจากชื่อพาธสมบูรณ์ของแฟ้ม
...
* รูปแบบสําหรับชุดสะสมsourceNameซึ่งได้รับคุณค่าจากคุณลักษณะระดับโลก
    \\*\\*/ *ทั่วไป:* คุณสมบัติ ชื่อ *.* [ข้อมูล ชนิด](#data-types) *.* คลายแฟ้ม *.* จับภาพ Group section*
* ตัวปลอมนี้sourceNameตัวเลือกทํางานแตกต่างจากตัวอื่น: แทนที่จะสร้างซ้ายสุดใหม่ (ก่อน)  axisVariableค่านี้แทนที่ค่าของปัจจุบันaxisVariableค่าที่สกัดมาจากชื่อแฟ้ม (ชื่อแฟ้ม) . รูปแบบคือ
...
*    *ข้อมูล ชนิด* -- กําหนดประเภทของข้อมูลที่จะใช้เก็บค่าต่าง ๆ ได้ที่นี่ ดูรายการมาตรฐานของ[ข้อมูล ชนิด](#data-types)นั่นERDDAP™รองรับ ยกเว้นไม่ให้ใช้สตริงที่นี่ เนื่องจากตัวแปรแกนในERDDAP™ไม่สามารถเป็นตัวแปรสตริง
    
มีรูปแบบข้อมูลเทียมเพิ่มเติม, เวลาFormat= *ข้อความ รูปแบบเวลา* ซึ่งบอกว่าERDDAP™นั่นคือค่าของเวลา[แสดงเฉพาะหมายเลขของธาตุ](#string-time-units). ในกรณีส่วนใหญ่ สตริงไทมส์ฟอร์ตที่คุณต้องการ จะถูกแปรค่าในรูปแบบใดรูปแบบหนึ่ง:
...
คุณสามารถยกเลิกเวลาพื้นฐานปริยายได้ (1970-01-01.00.00:00Z) โดยเพิ่ม[เพิ่มรายการแจกจ่าย](#addattributes)ซึ่งระบุรหัสหน่วยที่แตกต่างกัน โดยเวลาฐานต่างกัน สถานการณ์ทั่วไปคือ: มีกลุ่มไฟล์ข้อมูล แต่ละชุดมี 1 วัน เป็นชุดข้อมูลดาวเทียม (เวลากลางของแต่ละวัน) และต้องการตัวแปรlong\\_nameที่จะเป็น "เวลาแห่งความสําเร็จ" ตัวอย่างของสิ่งนี้ก็คือ
```
      <axisVariable>
        <sourceName>\\*\\*\\*fileName,timeFormat=yyyyDDD,S(\\d{7})\\.L3m.\\*,1</sourceName>
        <destinationName>time</destinationName>
...
* แน่นอน สัดส่วนการบีบอัดและความเร็วสําหรับอัลกอริทึมบีบอัดที่แตกต่างกัน แตกต่างกันกับแฟ้มต้นฉบับและการตั้งค่า (เช่น ระดับการบีบอัด) . หากคุณต้องการปรับแต่งระบบสําหรับแฟ้มของคุณ ให้ทําการทดสอบวิธีบีบอัดแบบต่าง ๆ กับแฟ้มของคุณ และด้วยช่วงของการบีบข้อมูล ถ้าเธออยากได้ของดี (ไม่จําเป็นต้องดีที่สุด) การตั้งค่าเราจะแนะนําเล็กน้อยgzip  (.gz) .gzipไม่ได้สร้างแฟ้มบีบอัดที่น้อยที่สุด (มันใกล้ทีเดียว) แต่มันบีบแฟ้มอย่างรวดเร็ว (สําคัญกว่าสําหรับERDDAP™ผู้ใช้) กําจัดแฟ้มอย่างรวดเร็ว อีกอย่างgzipซอฟต์แวร์มามาตรฐานกับทุกการติดตั้งลินุกซ์และ Mac OS และพร้อมใช้งานสําหรับวินโดวส์ผ่านทางเครื่องมือฟรี เช่น 7 Zip และ Linux เพิ่มข้อมูล เช่น Git Bash ตัวอย่างเช่น เพื่อบีบข้อมูลแหล่งที่มา.gzรุ่นของแฟ้ม (ชื่อแฟ้มเดียวกัน แต่ด้วย.gzเพิ่มเข้าไป) ใช้ (ในลินุกซ์, Mac OS, และ Git Bash)   
    gzip  *sourceName*   
เพื่อกําจัด.gzแฟ้มกลับไปยังต้นฉบับ, ใช้
ปืนพก *sourceName.gz*   
เพื่อบีบอัดแฟ้มต้นทางแต่ละแฟ้มในไดเรกทอรีและส่วนย่อยของมัน, การเรียกซ้ํา, ใช้
    gzip- อาร์ *ผู้กํากับName*   
การย่อยสลายของแต่ละ.gzแฟ้มในไดเรกทอรีและส่วนย่อยของมัน, ใช้ซ้ํา, ใช้
ปืนพก - อาร์ *ผู้กํากับName*   
     
* อย่ากดทับภายนอก (gzip) แฟ้มที่ถูกบีบอัดภายในอยู่แล้ว&#33;
มีหลายแฟ้มที่มีการบีบอัดข้อมูลภายในอยู่แล้ว ถ้าคุณgzipแฟ้มเหล่านี้ แฟ้มผลลัพธ์จะไม่มีขนาดเล็กลงมาก&lt;5%) และERDDAP™จะเสียเวลากําจัดพวกมัน เมื่อมันจําเป็นต้องอ่านมัน ตัว อย่าง เช่น:
...
เทียบกับการบีบข้อมูลภายในแฟ้มที่เสนอ.nc4 และ.hdf5 ไฟล์ERDDAPวิธีการสําหรับแฟ้มไบนารีแบบบีบอัดภายนอก มีข้อดีและข้อเสีย ข้อเสียคือ การอ่านส่วนเล็กๆของไฟล์หนึ่ง การบีบอัดภายในจะดีกว่าเพราะEDDGridจากแฟ้มจําเป็นต้องย่อยสลายไม่กี่ชิ้น (วินาที) ของไฟล์ ไม่ใช่ข้อมูลทั้งหมด แต่ERDDAPแนวทางนี้มีประโยชน์
...
ทั้งหมดEDDGridจากแฟ้มและทั้งหมด EDTable จากแฟ้มชุดข้อมูลรองรับชุดของป้ายกํากับที่บอกว่าERDDAP™เพื่อดาวน์โหลดและรักษาสําเนาของแฟ้มฐานข้อมูลทั้งหมด หรือแคชของแฟ้มไม่กี่แฟ้ม (ดาวน์โหลดเมื่อต้องการ) . มันมีประโยชน์มาก ดู[แคช เอกสารจาก Url](#cachefromurl).
...
EDDGridจาก NNc Files, EDTable from Multicalnk Files, EDTable from Nccfiles, และ EDTable fromNCF Files, สามารถ *บางครั้ง* บริการข้อมูลจาก.ncแฟ้มบนเซิร์ฟเวอร์ทางไกลและเข้าถึงผ่านทาง HTTP หากเซิร์ฟเวอร์รองรับ[การ รับ ใช้ ที่ ยาว นาน](https://en.wikipedia.org/wiki/Byte_serving)ผ่านการร้องขอในช่วง HTTP (กลไก HTTP สําหรับทํางาน) . มันเป็นไปได้เพราะ Netcdf-java (ซึ่งERDDAP™ใช้อ่าน.ncแฟ้ม) รองรับข้อมูลการอ่านจากระยะไกล.ncต้องการแฟ้มผ่านทาง HTTP

 **อย่าทําแบบนี้&#33;** มันไร้สมรรถภาพและช้า
ใช้ [แทน]&lt;แคชจาก Url&gt; ระบบ] (# คาเชฟัวร์) .

การเข้าถึงERDDAP™ชุดข้อมูลเป็นแฟ้มต่าง ๆ ที่ร้องขอมา โดยผ่านช่วงต่าง ๆ
การพลิกกลับนี้, ถ้าคุณสามารถ (ในทางทฤษฎี) คิดถึงชุดข้อมูลในERDDAP™ในฐานะยักษ์.ncแฟ้มโดยเพิ่มเข้าไป.nc"ที่ฐาน OPenDAPที่อยู่ URL สําหรับชุดข้อมูล (อี.จี. https://myserver.org/erddap/griddap/datasetID.nc และด้วยการเพิ่มข้อมูล หลังการระบุสับเซต) มันอาจจะมีเหตุผลที่จะถามว่า คุณสามารถใช้ Netcf-java,Ferretหรืออย่างอื่นNetCDFโปรแกรมลูกข่ายที่จะอ่านข้อมูลผ่านทาง ร้องขอช่วง HTTP จากERDDAP. คําตอบคือไม่ เพราะมันไม่ใหญ่มาก.nc"ไฟล์. ถ้าคุณต้องการทํา แทนที่จะเลือกแบบนี้
...
* ตารางจะถูกเก็บไว้บนดิสก์เช่นNetCDFv3.ncแฟ้มใน *เครื่อง มือ ขนาด ใหญ่* / Datatatet / *CharesOffDatataSetid ตัวสุดท้าย* / *datasetID* / ในแฟ้มชื่อ:
...
* หากแฟ้มมีโครงสร้างที่แตกต่างจากแฟ้มอื่น ๆ (ตัวอย่างเช่น, ประเภทข้อมูลที่แตกต่าง สําหรับตัวแปรตัวหนึ่ง, หรือค่าต่างๆ ของ "[หน่วย](#units)"คุณลักษณะ) .ERDDAPเพิ่มแฟ้มไปยังรายการของแฟ้ม "เลว" ข้อมูลเกี่ยวกับปัญหาที่มีแฟ้มจะถูกเขียนไปยัง *เครื่อง มือ ขนาด ใหญ่* แฟ้ม / log/log.txt
* คุณไม่จําเป็นต้องลบหรือทํางานกับแฟ้มเหล่านี้ ข้อยกเว้นหนึ่งคือ ถ้าคุณยังคงทําการเปลี่ยนแปลงชุดข้อมูลdatasets.xmlตั้งค่า คุณอาจต้องลบแฟ้มเหล่านี้เพื่อบังคับERDDAP™เพื่ออ่านแฟ้มทั้งหมดใหม่อีกครั้ง เนื่องจากแฟ้มต่าง ๆ จะถูกอ่าน/ แปลใหม่ หากคุณต้องการที่จะลบแฟ้มเหล่านี้ คุณสามารถทําได้เมื่อERDDAP™กําลังวิ่ง (แล้วตั้ง[ธง](/docs/server-admin/additional-information#set-dataset-flag)นําข้อมูลกลับมาใช้ใหม่) อย่างไรก็ตามERDDAP™มักจะสังเกตว่าdatasets.xmlข้อมูลไม่ตรงกับแฟ้ม ข้อมูลตารางและลบตารางแฟ้มโดยอัตโนมัติ
* ถ้าคุณอยากสนับสนุนERDDAP™เพื่อปรับปรุงข้อมูลชุดข้อมูลจัดเก็บ (ตัวอย่างเช่น, ถ้าคุณเพิ่งเพิ่ม, ลบ, หรือเปลี่ยนบางแฟ้มเป็นไดเรกทอรีข้อมูลชุด) ใช้[ระบบธง](/docs/server-admin/additional-information#flag)เพื่อบังคับERDDAP™เพื่อปรับปรุงข้อมูลแคชของแฟ้ม
         
...
* ชุดข้อมูลจะถูกเรียกใหม่โดยเร็วที่สุดERDDAP™ตรวจสอบว่าคุณได้เพิ่ม เอาออก[สัมผัส](https://en.wikipedia.org/wiki/Touch_(Unix)) (เพื่อเปลี่ยนแฟ้มสุดท้าย เวลาแก้ไขใหม่) หรือเปลี่ยนแฟ้มข้อมูล
...
ถ้าคุณวิ่ง[สร้าง DatatasXml](#generatedatasetsxml)หรือ[ถอดรหัส](#dasdds)หรือถ้าคุณพยายามโหลดEDDGridจาก...ERDDAP™และคุณได้ข้อความผิดพลาด "0 แฟ้ม" ระบุว่าERDDAP™พบแฟ้มที่เข้าคู่กับ 0 ในไดเรกทอรี (เมื่อคุณคิดว่า มีแฟ้มที่ตรงกับในไดเรกทอรีนั้น) .
    * ตรวจสอบว่าแฟ้มอยู่ในไดเรกทอรีดังกล่าวจริงหรือไม่
    * ตรวจสอบการสะกดชื่อไดเรกทอรี
    * ตรวจสอบแฟ้มName มันง่ายที่จะทําผิดพลาดกับรีเซกซ์ สําหรับวัตถุประสงค์การทดสอบ ลอง regex. * ซึ่งควรจะตรงกับชื่อแฟ้มทั้งหมด (ดูนี่สิ[เอกสารของ Regex](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html)ถึง[บทเรียน regax](https://www.vogella.com/tutorials/JavaRegularExpressions/article.html).) 
    * เลือกผู้ใช้ที่กําลังทํางานอยู่ของโปรแกรม (e.g., ผู้ใช้=tommat (?) สําหรับทอมแคท/ERDDAP) ได้สิทธิ์ในการอ่านไฟล์เหล่านั้น
    * ในระบบปฏิบัติการ (ยกตัวอย่างเช่น เซลีนุกซ์) และขึ้นอยู่กับการตั้งค่าของระบบ ผู้ใช้ที่เรียกใช้งานโปรแกรม จะต้องมีสิทธิ์ 'อ่าน' สําหรับห่วงโซ่ไดเรกทอรีทั้งหมดที่นํามายังไดเรกทอรีที่มีแฟ้มต่าง ๆ
         
#### EDDGridจากโครงกระดูกแฟ้ม XML{#eddgridfromfiles-skeleton-xml} 
...
ดูซูเปอร์คลาสพวกนี้สิ[EDDGridจากแฟ้ม](#eddgridfromfiles)ถึง[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลทั่วไป ว่าคลาสนี้ทํางานอย่างไร และจะใช้มันอย่างไร

เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล เนื่องจากแฟ้มเสียงไม่มีข้อมูลกํากับภาพ นอกจากข้อมูลที่เกี่ยวข้องกับการเข้ารหัสเสียง คุณจะต้องแก้ไขค่าผลลัพธ์ที่ได้จากการสร้าง Datatas Xml เพื่อให้ข้อมูลที่สําคัญ (E.g., ชื่อเรื่อง, สรุป,creator\\_name, สถาบัน, ประวัติศาสตร์) .
...
สําหรับERDDAPวัตถุประสงค์คือ กรอบมีความคล้ายคลึงกับตัวอย่าง ซึ่งเป็นข้อมูลสําหรับจุดเดียวในเวลา
คุณสมบัติในERDDAP™จะมีข้อมูลที่อธิบายข้อมูล ในไฟล์ต้นฉบับERDDAP™มักจะมีการเปลี่ยนแปลงระหว่างอ่านข้อมูล, เช่น, PCM / UNIFE, ALAW, และข้อมูลการเข้ารหัสแบบยูแอลเอW ถูกเปลี่ยนเป็น PCM\\[SIGNNDNDNDD, และ BigEDAND=ข้อมูลปลอม ถูกเปลี่ยนเป็นข้อมูล BigEANDDian=ข้อมูลจริง (ซึ่งก็คือJavaอยากอ่านมัน) . สุดท้าย ค่าข้อมูลในERDDAP™จะเป็น[รหัส PCM](https://en.wikipedia.org/wiki/Pulse-code_modulation)ค่าข้อมูล (เช่น ตัวอย่างตัวเลขของคลื่นเสียง) .
...
* เพราะว่าERDDAP™อ่านแฟ้มเสียงทั้งหมดERDDAP™ต้องเข้าถึงหน่วยความจําจํานวนมาก เพื่อทํางานกับแฟ้มเสียงขนาดใหญ่ ดู[ERDDAPตั้งค่าหน่วยความจํา](/docs/server-admin/deploy-install#memory). อีกครั้ง, ถ้านี่เป็นปัญหา, การวนรอบที่คุณสามารถใช้ได้ตอนนี้ คือการแยกไฟล์ออกเป็นก้อนเล็ก ๆ เพื่อให้ERDDAP™สามารถอ่านมันด้วยความทรงจําที่น้อยลง
...
* สําหรับEDDGridจากแฟ้ม Audio คุณควรจะใช้EDDGridจากระบบแฟ้มสําหรับ[พารามิเตอร์ ผ่าน ชื่อแฟ้ม](#aggregation-via-file-names-or-global-metadata)เกือบทุกครั้งโดยการแยกวันที่เริ่มบันทึก เวลาจากชื่อแฟ้ม ยกตัวอย่างเช่น
```
...
* สําหรับ EDTable from Audio Files คุณควรจะใช้ระบบ EDTable จากแฟ้มเกือบทุกครั้ง[\\*\\*เลเยอร์ถัดไปsourceNameวินาที](#filename-sourcenames)ดึงข้อมูลจากชื่อแฟ้ม (เกือบทุกครั้ง วันเริ่มต้น เวลาสําหรับบันทึก) และส่งเสริมให้เป็นคอลัมน์ของข้อมูล ยกตัวอย่างเช่น
```
...
ควรระบุรูปแบบของเวลาเป็นแอตทริบิวต์ของหน่วย:&lt;AT name="หน่วย"&#125;"yyymd'\'HPMS&lt;/at&gt;
     
### EDDGridแฟ้ม MergeIR{#eddgridfrommergeirfiles} 
[ **EDDGridแฟ้ม MergeIR** ](#eddgridfrommergeirfiles)รวบรวมข้อมูลจากท้องถิ่น[ผนวกรวม IR](https://www.cpc.ncep.noaa.gov/products/global_precip/html/README)ไฟล์ที่ได้มาจาก[เขต ร้อน (ตาราง) ](https://trmm.gsfc.nasa.gov)ซึ่งเป็นภารกิจร่วมระหว่าง NASA กับสํานักงานสํารวจอวกาศญี่ปุ่น (ยา) . รวม แฟ้ม IR สามารถดาวน์โหลดได้จาก[นาซา](ftp://disc2.nascom.nasa.gov/data/s4pa/TRMM_ANCILLARY/MERG/).
...
ดูคลาสนี้สิ[EDDGridจากแฟ้ม](#eddgridfromfiles)สําหรับข้อมูลทั่วไป ว่าคลาสนี้ทํางานอย่างไร และจะใช้มันอย่างไร

เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล จากนั้นคุณสามารถแก้ไขที่ปรับมัน
 
### EDDGridแฟ้มจาก Nc{#eddgridfromncfiles} 
[ **EDDGridแฟ้มจาก Nc** ](#eddgridfromncfiles)รวบรวมข้อมูลจากท้องถิ่น หักหลัง[GRIB .geb และ.gb2](https://en.wikipedia.org/wiki/GRIB)แฟ้ม[HDF  (v4 หรือ v5)  .hdf](https://www.hdfgroup.org/)แฟ้ม[.ncmI](#ncml-files)แฟ้ม[NetCDF  (v3 หรือ v4)  .nc](https://www.unidata.ucar.edu/software/netcdf/)แฟ้มและ[ซาร์](https://github.com/zarr-developers/zarr-python)แฟ้ม (รุ่น 2.25) . แฟ้ม Zar มีพฤติกรรมที่แตกต่างกันเล็กน้อย และต้องการทั้งชื่อแฟ้มRegex หรือพาธ regex เพื่อรวม "zar"
...
* ดูคลาสนี้สิ[EDDGridจากแฟ้ม](#eddgridfromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน
...
### EDDGridแฟ้มจาก Nc{#eddgridfromncfilesunpacked} 
[ **EDDGridแฟ้มจาก Nc** ](#eddgridfromncfilesunpacked)เป็นองค์ประกอบของ[EDDGridแฟ้มจาก Nc](#eddgridfromncfiles)ซึ่งรวมข้อมูลจากเครื่องในNetCDF  (v3 หรือ v4)  .ncและไฟล์ที่เกี่ยวข้อง ความแตกต่างคือ คลาสนี้ดึงข้อมูลแต่ละแฟ้มก่อนEDDGridจากแฟ้มดูแฟ้ม:

* มันแยกตัวแปรที่มี[scale\\_factor&/ หรือadd\\_offset](#scale_factor).
...
ข้อดีของคลาสนี้ก็คือ มันให้วิธีจัดการกับค่าต่างๆscale\\_factor.add\\_offset, / ฟริลวาลูmissing\\_valueหรือหน่วยเวลา ในแฟ้มแหล่งต่างๆ ในชุดสะสม ไม่อย่างนั้น, คุณต้องใช้เครื่องมืออย่างเช่น[ขนาด NcML](#ncml-files)หรือ[NCO](#netcdf-operators-nco)เพื่อแก้ไขแต่ละแฟ้ม เพื่อลบความแตกต่างเพื่อให้แฟ้มสามารถจัดการได้EDDGridจากNC Files เพื่อให้คลาสนี้ทํางานได้อย่างถูกต้อง แฟ้มต้องเป็นไปตามมาตรฐานของ CF สําหรับคุณสมบัติที่เกี่ยวข้อง

* ถ้าพยายามทําให้EDDGridแฟ้มจาก Nc ยกเลิกการแพ็คเกจจากกลุ่มของแฟ้มที่คุณเคยใช้ก่อนหน้านี้ และล้มเหลวในการใช้EDDGridแฟ้มจาก Nc, cd ถึง
     *เครื่อง มือ ขนาด ใหญ่* / Datatatet / *ผู้ให้ยืมล่าสุด 2 คน* / *datasetID* /
ที่ไหน *ผู้ให้ยืมล่าสุด 2 คน* คือ 2 ตัวอักษรสุดท้ายdatasetID.
...
### EDDGridลอน PM 180{#eddgridlonpm180} 
[ **EDDGridลอน PM 180** ](#eddgridlonpm180)กําหนดค่าลองจิจูดของเด็ก (ปิด)  EDDGridชุดข้อมูลที่มีเส้นแวงมากกว่า 180 (ตัวอย่างเช่น 0 ถึง 360) เพื่อให้อยู่ในระยะ - 180 ถึง 180 (ลองติจูดบวก หรือลบ 180, จึงมีชื่อ) .
...
* ใน[สร้าง DatatasXml](#generatedatasetsxml)มีประเภทชุดพิเศษEDDGridLon PM 180 จาก Eddepcatalog ที่ช่วยให้คุณสร้างdatasets.xmlสําหรับEDDGridLonn PM 180 ชุดข้อมูลจากแต่ละชุดEDDGridชุดข้อมูลในERDDAPที่มีค่า ลองจิจูดมากกว่า 180 นี่จะช่วยนําเสนอชุดข้อมูลสองรุ่นเหล่านี้ได้
ต้นฉบับที่มีค่า ลองจิจูดในช่วง 0 ถึง 360,
และชุดข้อมูลใหม่ที่มีค่า ลองจิจูดในช่วง - 180 ถึง 180
    
ข้อมูลเด็กในแต่ละชุดEDDGridLonnPM 1801 ชุดข้อมูลจะเป็นEDDGridชุดข้อมูลจาก Eddep ที่ชี้ไปยังชุดข้อมูลต้นฉบับ
ชุดข้อมูลชุดใหม่datasetIDจะเป็นชื่อของชุดข้อมูลต้นฉบับ บวก "/ LonpM80"
ยกตัวอย่างเช่น
```
...
#### EDDGridโครง กระดูก ของ ลอน ฟ. XML{#eddgridlonpm180-skeleton-xml} 
...
### EDDGridลอน0360{#eddgridlon0360} 
[ **EDDGridลอน0360** ](#eddgridlon0360)กําหนดค่าลองจิจูดของเด็ก (ปิด)  EDDGridชุดข้อมูลที่มีเส้นแวงน้อยกว่า 0 (ตัวอย่างเช่น -180 ถึง 180) ดังนั้นพวกมันอยู่ในระยะ 0 ถึง 360 (จึงมีชื่อ) .
...
ค่า 0 เดิมถึงเกือบ 180 ค่านั้นเปลี่ยนไม่ได้
* สําหรับชุดข้อมูลเด็กที่สแปนลอน=0 แต่ไม่ครอบคลุมโลกERDDAP™แทรกค่าที่ขาดไป เพื่อทําชุดข้อมูลครอบคลุมทั่วโลก ตัว อย่าง เช่น ข้อมูล เกี่ยว กับ เด็ก ที่ มี ค่า ลองจิจูด ของ -40 ถึง 20 จะ กลาย เป็น ชุด ข้อมูล ที่ มี ค่า ลองจิจูด 0 ถึง 360.
...
* ใน[สร้าง DatatasXml](#generatedatasetsxml)มีประเภทชุดพิเศษEDDGridLon0360 จาก ค.ศ. Erdadapcatalog ที่ช่วยให้คุณสร้างdatasets.xmlสําหรับEDDGridLon0360 ชุดข้อมูลจากแต่ละชุดEDDGridชุดข้อมูลในERDDAPที่มีค่า ลองจิจูดมากกว่า 180 นี่จะช่วยนําเสนอชุดข้อมูลสองรุ่นเหล่านี้ได้
ต้นฉบับที่มีค่า ลองจิจูดในช่วง 0 ถึง 360,
และชุดข้อมูลใหม่ที่มีค่า ลองจิจูดในช่วง - 180 ถึง 180
    
ข้อมูลเด็กในแต่ละชุดEDDGridLon0360 ชุดข้อมูลจะเป็นEDDGridชุดข้อมูลจาก Eddep ที่ชี้ไปยังชุดข้อมูลต้นฉบับ
ชุดข้อมูลชุดใหม่datasetIDจะเป็นชื่อของชุดข้อมูลต้นฉบับ บวก "/_ Lon0360"
ยกตัวอย่างเช่น
```
...
#### EDDGridลอน0360 โครงกระดูก XML{#eddgridlon0360-skeleton-xml} 
...
### EDDGridด้านข้าง{#eddgridsidebyside} 
[ **EDDGridด้านข้าง** ](#eddgridsidebyside)ประกอบสองหรือมากกว่าEDDGridชุดข้อมูล (เด็ก ๆ) เคียงข้างกัน
...
* เด็กทุกคนต้องมีแหล่งเดียวกันaxisVariableวินาที\\[1+\\]  (ตัวอย่างเช่น ละติจูด ลองติจูด) . ความแม่นยําของการทดสอบถูกกําหนดโดย[ตรงกับ Axndigit](#matchaxisndigits).
...
#### EDDGridโครง กระดูก ด้าน ข้าง XML{#eddgridsidebyside-skeleton-xml} 
...
### EDDGridการ แบ่ง แยก{#eddgridaggregateexistingdimension} 
[ **EDDGridการ แบ่ง แยก** ](#eddgridaggregateexistingdimension)ประกอบสองหรือมากกว่าEDDGridชุดข้อมูลแต่ละชุด มีช่วงของค่าต่างๆ สําหรับมิติแรก แต่มีค่าเหมือนกันสําหรับมิติอื่น
...
* เดอะ[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)สามารถทําให้ร่างหยาบของdatasets.xmlสําหรับEDDGridการแบ่งประเภทการแบ่งส่วนข้อมูล โดยอิงจากชุดแฟ้มให้บริการโดยHyraxหรือ เซิร์ฟเวอร์ THEDS ตัวอย่างเช่น ใช้ข้อมูลนี้สําหรับโปรแกรม ("/1988" ในที่อยู่ URL ทําให้ตัวอย่างทํางานเร็วขึ้น) .
    ```
...
#### EDDGridโครง กระดูก ที่ แยก ต่าง หาก XML{#eddgridaggregateexistingdimension-skeleton-xml} 
...
### EDDGridคัดลอก{#eddgridcopy} 
[ **EDDGridคัดลอก** ](#eddgridcopy)ทําและรักษาสําเนาท้องถิ่นของอื่นEDDGridข้อมูลและบริการจากสําเนาท้องถิ่น

*   EDDGridคัดลอก (และสําหรับข้อมูล[โปรแกรมคัดลอก DDTable](#eddtablecopy)) เป็นการใช้ง่ายและมีประสิทธิภาพมาก
...
*   \\[ทางเลือกEDDGridคัดลอก -
หากมีข้อมูลระยะไกลใช้ได้ผ่านทางแฟ้มที่ดาวน์โหลดได้, ไม่ใช่บริการเว็บ, ให้ใช้[แคช ตัวเลือกของ Url สําหรับEDDGridจากแฟ้ม](#cachefromurl)ซึ่งทําให้มีสําเนาแฟ้มภายในเครื่อง และป้อนข้อมูลจากแฟ้มภายในระบบ\\]
* แฟ้มภายในระบบ -- ข้อมูลแต่ละชิ้นจะเก็บไว้ในแยกNetCDFแฟ้มในส่วนย่อยของ *เครื่อง มือ ขนาด ใหญ่* / รับทราบ / *datasetID* / (ตามที่กําหนดไว้ใน[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)) . ชื่อแฟ้มที่สร้างจากค่าแกนถูกแก้ไขเพื่อให้มันปลอดภัยจากชื่อแฟ้ม (ตัวอย่างเช่น, ไฮเฟนถูกแทนที่ด้วย x2D) มันไม่มีผลกับข้อมูลจริง
     
* ข้อมูลใหม่ -- แต่ละครั้งEDDGridคัดลอกใหม่อีกครั้งตรวจสอบระยะไกล&lt;Datets) เพื่อดูว่ามีก้อนอะไรบ้าง ถ้าเอกสารสําหรับข้อมูลในส่วนนี้ยังไม่มีอยู่แล้ว การร้องขอรับเนื้อส่วนจะถูกเพิ่มเข้าไปในคิวERDDAPโพรเซส Head ของงานทั้งหมด ที่ร้องขอจากคิวสําหรับชิ้นข้อมูล, 1 คูณ 1 คุณจะเห็นสถิติของกิจกรรมของ Thead's[หน้าสถานะ](/docs/server-admin/additional-information#status-page)และใน[รายงานรายวัน](/docs/server-admin/additional-information#daily-report). (ใช่ERDDAP™สามารถกําหนดงานได้หลายอย่างในกระบวนการนี้ แต่จะใช้ข้อมูลจากระยะไกลจํานวนมากERDDAPวงดนตรี ความทรงจํา และเวลา CPU ซึ่งไม่ใช่ความคิดที่ดี) 
...
คุณบอกได้ว่าEDDGridคัดลอกเพื่อทําสําเนาของสับเซตของข้อมูลแหล่ง, แทนที่จะใช้ชุดข้อมูลแหล่งทั้งหมด, โดยการเพิ่มป้ายในรูปแบบ&lt;เฉพาะตั้งแต่ ค.ศ. *บาง ค่า* &lt;กับชุดข้อมูลdatasets.xmlชิ้น.EDDGridคัดลอกจะดาวน์โหลดเฉพาะค่าข้อมูลที่เกี่ยวข้องกับค่าของมิติแรกเท่านั้น (ปกติมิติเวลา) ที่มากกว่า *บาง ค่า* . *บาง ค่า* สามารถเป็น:
    * จํานวนครั้งที่ระบุผ่านทางnow- *ปิดการใช้งาน* .
ยกตัวอย่างเช่น&lt;เฉพาะตั้งแต่ ค.ศ.now-2 ปี&lt;เพื่อทําสําเนาของข้อมูลท้องถิ่นสําหรับข้อมูล ที่ค่าของมิติภายนอก (ค่าเวลาปกติ) อยู่ใน 2 ปีที่ผ่านมา (ซึ่งจะตรวจสอบอีกครั้งในแต่ละครั้งที่ชุดข้อมูลจะถูกโหลดอีกครั้ง ซึ่งก็คือเมื่อมันมองหาข้อมูลใหม่ที่จะคัดลอก) . ดู[now- *ปิดการใช้งาน* รายละเอียดไวยากรณ์](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/documentation.html#now). มันมีประโยชน์ถ้ามิติแรกมีข้อมูลเวลา ซึ่งมักใช้ได้
        ...
        EDDGridคัดลอกไม่ได้ลบแฟ้มภายในที่มีข้อมูลที่ เมื่อเวลาผ่านไป จะเก่ากว่าnow- *ปิดการใช้งาน* . คุณสามารถลบแฟ้มเหล่านั้นได้ทุกเวลาหากคุณเลือก ถ้าคุณทํา, เราแนะนําให้คุณตั้ง[ธง](/docs/server-admin/additional-information#flag)หลังจากที่คุณลบแฟ้มเพื่อบอกEDDGridคัดลอกไปยังรายการของแฟ้มที่มีแคช
        
    * จุดคงที่ในเวลาที่ระบุเป็นข้อความ ISO 8601yyyy-MM-ddTHH:mm:ssZ.
ยกตัวอย่างเช่น&lt;2000-01-01.00 น.&lt;บอกชุดข้อมูลเพียงเพื่อทําสําเนาของข้อมูลท้องถิ่น ที่มูลค่าของมิติแรกเป็น -&gt;=2000-0100:00Z. มันมีประโยชน์ถ้ามิติแรกมีข้อมูลเวลา ซึ่งมักใช้ได้
         
    * จํานวนจุดลอย
ยกตัวอย่างเช่น&lt;เพียง แต่ 946684800.0&lt;(ตั้งแต่นั้น) หน่วยจะเป็นหน่วยปลายทางของมิติแรก ตัวอย่างเช่น, สําหรับมิติเวลา, หน่วยในERDDAP™เสมอ"seconds since 1970-01-01T00:00:00Z". งั้น 946684800.0"seconds since 1970-01-01T00:00:00Z"เท่ากับ 2000-01-01T00:00:00Z. นี่เป็นตัวเลือกที่มีประโยชน์เสมอ, แต่มีประโยชน์ โดยเฉพาะเมื่อมิติแรกไม่มีข้อมูลเวลา

#### EDDGridทําสําเนาให้ใช้ใหม่{#eddgridcopy-recomended-use} 
1. สร้าง&lt;ชุดข้อมูล: รายการ (คนพื้นเมืองไม่EDDGridคัดลอก) สําหรับแหล่งข้อมูลระยะไกล
...
#### EDDGridคัดลอกโครงสร้าง XML{#eddgridcopy-skeleton-xml} 
...
*   ERDDAP™สามารถทํางานกับคาสเซนดร้า v2 และ v3 โดยไม่มีการเปลี่ยนแปลงหรือความแตกต่างในการจัดฉาก เราได้ทดสอบกับ[คาสเซนดร้า v2 และ v3 จาก อาปาเช](https://cassandra.apache.org/download/). มันเป็นไปได้ว่าERDDAP™สามารถทํางานกับคาสเซนดร้าดาวน์โหลดจากข้อมูล
     
...
#### คาสเซนดร้าdatasets.xml {#cassandra-datasetsxml} 
...
* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล (โดยเฉพาะอย่างยิ่ง)&lt;พาร์ติชัน KeySourceName (ชื่อส่วน:) ) คุณสามารถเก็บข้อมูลส่วนใหญ่ที่คุณต้องการในการสร้าง XML สําหรับข้อมูล EDTable from Cassandra ได้ โดยการติดต่อกับผู้ดูแลระบบแคสแซนดร้า และค้นหาในเว็บ
...
*    **ตัวเลือกการไล่ระดับสี**   
    ...
    &lt;การเชื่อมต่อ ชื่อคุณสมบัติ=" **อ่าน Times** ""-- *ตัวแก้ไข* &lt;การเชื่อมสัมพันธ์ คุณสมบัติ:
     (คาสเซนดร้าอ่านไทม์มิลลิสโดยปริยาย 12000 แต่ERDDAP™เปลี่ยนค่าปริยายเป็น 120000 ถ้า คาสเซนดร้ากําลังโยนเวลาออกไป การเพิ่มขึ้นนี้อาจไม่ช่วยอะไร เพราะบางครั้งคาสเซนดร้าก็โยนมันทิ้ง ปัญหานี้มีโอกาสที่คุณจะเก็บข้อมูลต่อพาร์ทิชันมากเกินไป รหัสกุญแจ)   
...
* คุณกําหนดรายการของพาร์ทิชันที่แยกไว้แล้วdatasets.xmlผ่าน&lt;พาร์ติชัน KeySourceName
ตัวอย่างง่ายๆ
```
...
* กุญแจแบ่งเวลา - หากหนึ่งคอลัมน์กุญแจของพาร์ทิชันเป็นคอลัมน์ของเวลาประทับที่มีรุ่นหยาบกว่าของคอลัมน์เวลาประทับอื่น ๆ กําหนดนี้ผ่าน
     *พาร์ติชันKeySourcNametime\\_precision*   
ที่ไหนtime\\_precisionคือหนึ่งใน[time\\_precision](#time_precision)ใช้ข้อความอื่นในERDDAP.
ตามรอย Z ในtime\\_precisionข้อความเป็นค่าปริยาย จึงไม่สําคัญว่าtime\\_precisionข้อความจบที่ Z หรือไม่
ยกตัวอย่างเช่นERDDAP™จะแปลวันที่/ sampletime/1970-01-01 เป็น "ข้อห้ามสําหรับวัน สามารถสร้างจากข้อจํากัดของเวลาตัวอย่างได้ โดยใช้สิ่งนี้time\\_precision"" การเปลี่ยนเงื่อนไขที่แท้จริงนั้นซับซ้อนกว่า, แต่นั่นคือภาพรวม
     **ใช้นี่เมื่อมันเกี่ยวข้อง** มันช่วยERDDAP™เพื่อทํางานกับแคสแซนดร้าอย่างมีประสิทธิภาพ ถ้าความสัมพันธ์ระหว่างคอลัมน์นี้ มีอยู่ในโต๊ะคาสเซนดร้าERDDAP™ชุดข้อมูลจะช้ามากERDDAP™และใช้ทรัพยากรคาสเซนดร้าจํานวนมาก
* เดี่ยว กุญแจแยกค่า ถ้าคุณต้องการERDDAP™ชุดข้อมูลที่จะให้ทํางานโดยใช้กุญแจพาร์ติชันเพียงตัวเดียว, ระบุ *พาร์ติชันKeySourceName* .
...
* โดยปริยายแล้ว คาสเซนดร้าและERDDAP™ทํากับชื่อคอลัมน์ในรูปแบบที่อ่อนไหว แต่ถ้าคุณตั้ง[คอลัมน์Name](#case-sensitivity)ถึง "ERDDAP™จะรักษาคอลัมน์ของคาสซานดร้า เพื่อความสบายใจ
         
...
คาสเซนดร้ายอมรับเงื่อนไขแบบ SQL บนกลุ่มคอลัมน์ ซึ่งเป็นเสาที่ประกอบเป็นส่วนที่สองของหลัก (หลังปุ่มพาร์ติชัน (วินาที) ) . ดังนั้น คุณต้องระบุคอลัมน์เหล่านี้ผ่าน&lt;Group Columnsource Names&gt;. เปิดใช้งานERDDAP™เพื่อทํางานกับแคสแซนดร้าอย่างมีประสิทธิภาพ ถ้ามีกลุ่มคอลัมน์ และคุณไม่ได้บอกERDDAPชุดข้อมูลจะช้ามากERDDAP™และใช้ทรัพยากรคาสเซนดร้าจํานวนมาก
    ...
    * โดยปริยายแล้ว คาสเซนดร้าและERDDAP™ทํากับชื่อคอลัมน์ในรูปแบบที่อ่อนไหว แต่ถ้าคุณตั้ง[คอลัมน์Name](#case-sensitivity)ถึง "ERDDAP™จะรักษาชื่อคอลัมน์ของคาสซานดร้า แบบที่อ่อนไหว
         
...
#### คาสเซนดร้าsubsetVariables {#cassandra-subsetvariables} 
เช่นชุดข้อมูล EDTable อื่น ๆ คุณสามารถระบุรายการจุลภาคของ&lt;dataVariable&gt;destinationNameS ในคุณลักษณะระดับโลกที่เรียกว่า "[subsetVariables](#subsetvariables)"เพื่อระบุตัวแปร ที่มีจํานวนค่าจํากัด ชุดข้อมูลจะมี .ubet หน้าเว็บ และแสดงรายการของตัวแปรเหล่านั้น ในรายการวางลงบนหน้าเว็บหลายหน้า
...
เพราะมันมีความคลุมเครือเกี่ยวกับ[ชนิดของข้อมูลคาสเซนดร้า](https://cassandra.apache.org/doc/latest/cql/types.html)แผนที่ที่ERDDAP™ชนิดของข้อมูล, คุณต้องระบุ [&lt;ข้อมูล (# ดาต้าชนิด) แท็กแต่ละตัว&lt;dataVariable&gt;) (เปลี่ยนตัวแปรได้) เพื่อบอกว่าERDDAP™ชนิดของข้อมูลที่จะใช้ มาตรฐานERDDAP™ข้อมูล ชนิด (และประเภทข้อมูลคาสเซนดร้า) คือ:
    
*   [บูเลียน](#boolean-data)  (บูเลียน) ซึ่งERDDAP™จากนั้นเก็บไว้เป็นไบท์
...
คาสเซนดร้า[นาฬิกา](#cassandra-timestamp-data)เป็นกรณีพิเศษ: ใช้ERDDAPข้อมูลคู่ แบบ
...
อีกทางเลือกหนึ่ง ถ้าคุณไม่ต้องการERDDAP™เพื่อขยายรายการเป็นหลายแถวในERDDAP™ตาราง, ระบุข้อความเป็นdataVariableข้อมูล พิมพ์ แล้ว รายการ ทั้ง หมด จะ ถูก แทน ด้วย เชือก เส้น เดียว ใน แถว เดียวERDDAP.
...
* ทั่วไป -
ธรรมชาติของ CQL คือความซับซ้อน[การสลายตัว](https://en.wikipedia.org/wiki/Declarative_programming). พวกเขาเพียงแค่ระบุสิ่งที่ผู้ใช้ต้องการ มันไม่ได้รวมคําจํากัดความหรือคําใบ้ว่า จะจัดการการสอบถามอย่างไร ดังนั้นไม่มีทางERDDAP™ทําให้เกิดการสอบถาม ในแบบที่จะช่วยคาสเซนดร้าให้คําสอบถาม (หรือในวิธีใด ๆ กําหนดวิธีการสอบถามที่จะจัดการ) . โดย ทั่ว ไป ขึ้น อยู่ กับ เจ้า หน้าที่ ฝ่าย บริหาร ของ คาส ซาน ที่ จะ จัด การ เรื่อง ราว ต่าง ๆ (ตัวอย่างเช่น ดัชนี) เพื่อจัดลําดับความต่อเนื่อง
     
...
*   [ข้อความวิเคราะห์ "Cassandras"](#cassandra-stats)-- สําหรับทุกERDDAP™ผู้ใช้สอบถามข้อมูลคาสเซนดร้าERDDAP™จะพิมพ์บรรทัดในแฟ้มปูมบันทึก *เครื่อง มือ ขนาด ใหญ่* logs/log.txt กับบางสถิติที่เกี่ยวข้องกับการสอบถาม ตัวอย่างเช่น
```
...
* หลังจากปรับใช้เงื่อนไขบนปุ่มพาร์ติชัน[คอลัมน์](#clustercolumnsourcenames)และ[คอลัมน์ดัชนี](#indexcolumnsourcenames)ที่ถูกส่งมาโดยERDDAP™คาสเซนดร้านําข้อมูลกลับมา 1200 แถวERDDAP™ในเซต
* ผล ที่ ตาม มา ต้องให้[ข้อมูล ประเภท= *บางประเภท* รายการ](#cassandra-datatypes)คอลัมน์ (โดย เฉลี่ย แล้ว มี รายการ ละ 10 รายการ) เพราะว่าERDDAP™ขยาย 1200 แถวจากคาสเซนดร้าเป็น 12000 แถวERDDAP.
...
 
* กรุณาอดทนด้วย อ่านข้อมูลที่นี่และในเอกสารคาสเซนดร้าอย่างระมัดระวัง การทดลอง เช็คงานของคุณสิ ถ้าคาสเซนดร้า...ERDDAP™การเชื่อมต่อยังคงช้ากว่าที่คุณคาดหวัง กรุณารวมโต๊ะคาสเซนดร้าของคุณและของคุณERDDAP™ชิ้นของdatasets.xmlและได้เห็น[ส่วนของการได้รับการสนับสนุนเพิ่มเติม](/docs/intro#support).
 
* ถ้าอย่างอื่นล้มเหลว
พิจารณาการเก็บข้อมูลในคลังเก็บNetCDFv3.ncแฟ้ม (โดยเฉพาะ.ncแฟ้มที่ใช้[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)โครง สร้าง ข้อมูล ของ อาร์เรย์ อย่าง ต่อ เนื่อง และ สามารถ จัด การ ได้ERDDAP'[แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)) . ถ้าพวกเขาถูกจัดการตามหลักเหตุผล (แต่ละด้วยข้อมูลสําหรับชิ้นส่วนของพื้นที่และเวลา) .ERDDAP™สามารถดึงข้อมูลจากพวกมันได้อย่างรวดเร็ว
         
...
[ **DDTable from Depsquest** ](#eddtablefromdapsequence)จับตัวแปรภายใน 1 และ 2 ลําดับจาก[DAP](https://www.opendap.org/)เซิร์ฟเวอร์อย่างเช่นDAPใช่ (เคยอยู่ที่ https://www.pmel.noaa.gov/epic/software/dapper/ ตอนนี้เลิกแล้ว) .

* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล จากนั้นคุณสามารถแก้ไขที่ปรับมัน คุณสามารถเก็บข้อมูลที่คุณต้องการได้ โดยการดูแฟ้ม DDS และ DASS ของแหล่งข้อมูลต่าง ๆ ในเว็บเบราว์เซอร์ของคุณ (โดยเพิ่ม .das และ inds ไปยังsourceUrl(ตัวอย่างหนึ่งอยู่ที่ https://dapper.pmel.noaa.gov/dapper/epic/tao\\_time\\_series.cdp.dds )
...
เมื่อERDDAP™นําเสนอชุดข้อมูลเป็นหนึ่งง่ายเสียศูนย์ตารางเดียวมันง่ายมากสําหรับทุกคนที่จะเข้าใจข้อมูล ผู้ใช้ส่วนมากไม่เคยได้ยินของตารางที่ปกติ และน้อยคนที่เข้าใจคีย์, กุญแจต่างประเทศ หรือตารางเข้าร่วม และพวกเขาเกือบจะแน่ใจว่าไม่ทราบรายละเอียดของชนิดที่แตกต่างกัน ของการเข้าร่วม หรือวิธีการระบุ SQL ที่จะทําเข้าร่วม (หรือหลายกลุ่ม) ถูกต้อง การใช้โต๊ะที่เสียรูปแล้ว หลีกเลี่ยงปัญหาเหล่านั้น เหตุผลนี้เพียงอย่างเดียวเพียงกําหนดการใช้งานของตารางเดียว defaulted สําหรับการนําเสนอของชุดข้อมูลเพื่อERDDAP™ผู้ใช้
     
...
แต่แม้ใน SQL ผลลัพธ์ที่ถูกส่งกลับไปยังผู้ใช้ที่ถูกลดความปกติ (เข้าร่วม) โต๊ะเดี่ยว ดังนั้นมันดูสมเหตุสมผลที่จะนําเสนอชุดข้อมูลกับผู้ใช้เป็นขนาดใหญ่ที่ถูกตัดสิทธิ์ โต๊ะเดียวที่พวกเขาสามารถร้องขอสับเซต (e.g., แสดงให้ฉันเห็นแถวของตารางที่อุณหภูมิ) 30) .
     
* คุณสามารถเปลี่ยนได้ERDDAP™โดยไม่เปลี่ยนโต๊ะ
    ERDDAP™มี ข้อ เรียก ร้อง บาง ประการ ที่ อาจ ต่าง ไป จาก วิธี ที่ คุณ ได้ จัด ตั้ง ฐานข้อมูล.
ยกตัวอย่างเช่นERDDAP™ต้องการบันทึกข้อมูลเวลาประทับไว้ใน 'เวลาประทับกับเขตเวลา'
โดยการแยกตาราง/ ดูสําหรับERDDAP™คุณสามารถทําการเปลี่ยนแปลงเหล่านี้ เมื่อคุณทําตารางที่ผิดปกติสําหรับERDDAP. ดังนั้น คุณไม่จําเป็นต้องเปลี่ยนโต๊ะของคุณ
     
*   ERDDAP™จะสร้างโครงสร้างบางส่วนของโต๊ะที่ตั้งศูนย์แล้ว
คุณสามารถกําหนดว่าคอลัมน์ใดของข้อมูลที่มาจากตาราง 'outer' และจึงมีค่าต่าง ๆ จํากัดERDDAP™จะรวบรวมค่าที่แตกต่างกันทั้งหมด ในคอลัมน์เหล่านี้ และนําเสนอให้กับผู้ใช้พิเศษ หน้าเว็บแบบสับเซตที่ช่วยผู้ใช้ให้เลือกชุดข้อมูลได้อย่างรวดเร็ว ค่าแต่ละคอลัมน์ที่แตกต่างกัน ยังจะแสดงในรายการวางลงบนข้อมูลในหน้าเว็บอื่น ๆ ด้วย
     
* ตารางที่ถูกตัดทอนข้อมูล ทําให้ข้อมูลส่งจากคุณไปยังERDDAPผู้บริหารง่าย
...
* ตัวเลือกการแบ่งเขต:
เราเชื่อมต่อกันได้ERDDAP™ไปที่ฐานข้อมูลปัจจุบันของคุณ
...
นี่เป็นตัวเลือกหากข้อมูลมีการเปลี่ยนแปลงบ่อย ๆ และคุณต้องการให้ERDDAP™ผู้ใช้สามารถเข้าถึงการเปลี่ยนแปลงเหล่านั้นในทันที อย่างไรก็ตาม มันอาจจะมีเหตุผลที่จะใช้งานตัวเลือกของแฟ้มทางด้านบนและระยะ ๆ (ทุก 30 นาที?) แทนที่แฟ้มที่มีข้อมูลปัจจุบัน
ข้อเสียของวิธีการนี้ คือERDDAP™การร้องขอของผู้ใช้อาจจะเป็นภาระที่หนักเกินทนในฐานข้อมูลของคุณและที่ERDDAP™การเชื่อมต่อคือความเสี่ยงด้านความปลอดภัย (ถึงแม้ว่าเราจะลดความเสี่ยงลง) .
...
Timetamps ไม่มีเขตเวลาข้อมูลการทํางานที่ถูกต้องในERDDAP.
...
#### ฐานข้อมูลในdatasets.xml {#database-in-datasetsxml} 
...
ใส่ลงไป *แว่นขยาย* / webapps/erdap/WEB-INF/lib หลังจากคุณติดตั้งERDDAP. แล้วในของคุณdatasets.xmlสําหรับชุดข้อมูลนี้ คุณต้องระบุค่า&lt;ไดรเวอร์Name (โชคไม่ดี) แตกต่างจากชื่อแฟ้ม ค้นหาในเว็บสําหรับไดรเวอร์ JDBC สําหรับฐานข้อมูลและไดรเวอร์ของคุณNameJavaต้องใช้มัน
...
เดอะ&lt;ไดรเวอร์Namedatasets.xml  (ดูด้านล่าง) น่าจะเป็น com.Mysql.jdbc คนขับ
    * สําหรับOracle# ลองดู[ https://www.oracle.com/database/technologies/appdev/jdbc-downloads.html ](https://www.oracle.com/database/technologies/appdev/jdbc-downloads.html).
เดอะ&lt;ไดรเวอร์Namedatasets.xml  (ดูด้านล่าง) น่าจะเป็น Acatha.jdc.driverOracleคนขับ
...
เดอะ&lt;ไดรเวอร์Namedatasets.xml  (ดูด้านล่าง) น่าจะเป็นตาข่าย. Wource Forge.jtds.jdbc. คนขับ
...
สําหรับฐานข้อมูลจํานวนมาก การใช้เครื่องหมายคําพูดใด ๆ ส่งผลให้ฐานข้อมูลทํางานกับชื่อสนาม/โคลัม ในทางที่ละเอียดอ่อน (แทนที่จะเป็นกรณีทั่วไปของฐานข้อมูล แบบไร้ความรู้สึก) . ฐานข้อมูลมักแสดงชื่อแฟ้ม/โคลัมน์เป็นอักษรตัวใหญ่ทั้งหมด เมื่อในความเป็นจริงแล้ว รูปแบบที่อ่อนไหวนั้นแตกต่างกัน ในERDDAP™กรุณารักษาชื่อคอลัมน์ฐานข้อมูล เป็นกรณีอ่อนไหว
    ...
    *   Oracleรองรับการอ้างถึงคู่ ANSI-SQL แบบมาตรฐาน[ค่าปริยาย](https://docs.oracle.com/database/121/SQLRF/sql_elements008.htm#SQLRF00223).
...
*   [ฐานข้อมูล](#database-datatype)[&lt;ข้อมูล (# ดาต้าชนิด) ป้ายกํากับ... เพราะมันมีความคลุมเครือเกี่ยวกับ[ชนิดของข้อมูลฐานข้อมูล](https://www.w3schools.com/sql/sql_datatypes_general.asp)แผนที่ที่ERDDAP™ชนิดของข้อมูล, คุณต้องระบุ [&lt;ข้อมูล (# ดาต้าชนิด) แท็กแต่ละตัว&lt;dataVariable&gt;) (เปลี่ยนตัวแปรได้) เพื่อบอกว่าERDDAP™ชนิดของข้อมูลที่จะใช้ ส่วนหนึ่งของโจทย์คือ ชุดข้อมูลต่าง ๆ ใช้เทอมต่างๆ สําหรับข้อมูลต่าง ๆ -- พยายามเทียบนิยาม, ไม่ใช่แค่ชื่อ ดูรายละเอียดของ[มาตรฐานERDDAP™ข้อมูล ชนิด](#data-types)ซึ่งรวมการอ้างอิงไปยังชนิดของข้อมูล SQL ด้วย[วันและเวลา](#database-date-time-data)เป็นกรณีพิเศษ: ใช้ERDDAPข้อมูลคู่ แบบ
     
...
ฐานข้อมูลรองรับช่องว่างในจํานวนเต็ม (Int, เล็ก, เล็ก, เล็ก) คอลัมน์ แต่ERDDAP™ไม่สนับสนุนจํานวนจริง
ฐานข้อมูลว่างจะถูกแปลงเป็นERDDAP™127 สําหรับคอลัมน์ byte 255 สําหรับคอลัมน์ ubete, 32767 สําหรับคอลัมน์แบบย่อ, 65535 สําหรับคอลัมน์แบบย่อ, 2147483647 สําหรับคอลัมน์ inte, 429496727295 สําหรับคอลัมน์ยูนนท์, 9,223,373,036,854,775,807 สําหรับคอลัมน์ยาว, หรือ 18444964640709091615 สําหรับคอลัมน์ยาว ค.ศ. หากคุณใช้ค่าปริยาย โปรดระบุมันmissing\\_values สําหรับผู้ใช้ข้อมูลในERDDAP™ด้วย
...
อีกทางเลือกหนึ่ง คุณสามารถใช้missing\\_value"แอททริบิวต์แทน "//fillvallue"
ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ เพิ่มแอตทริบิวต์ของ Xml โดยอัตโนมัติdatasets.xmlสําหรับชุดข้อมูลฐานข้อมูล

สําหรับคอลัมน์ของจุดลอยฐานข้อมูล สเปซว่างจะถูกแปลงเป็น NNs ในERDDAP.
สําหรับชนิดของข้อมูลฐานข้อมูลที่ถูกแปลงเป็นข้อความERDDAP™ช่องว่างถูกเปลี่ยนเป็นเชือกเปล่า
...
```
ERDDAP™ร้องขอโดยใช้ลวดลาย () &/ หรือorderBy ( *ตัวแปร* ) จะเพิ่มระยะตัดและ/หรือเวลาโดย *ตัวแปร* ไปยัง SQL เตรียมคําแถลง โดย ทั่ว ไป การ ทํา เช่น นี้ จะ ทํา ให้ การ ตอบ รับ จาก ข้อมูล ลด ลง อย่าง มาก.
...
PostgrsQL จะตอบสนองอย่างรวดเร็วถ้าคุณ[หมึก](https://www.postgresql.org/docs/8.3/static/sql-vacuum.html)โต๊ะ
    Oracleไม่จําเป็นต้องมีคําสั่งที่คล้ายคลึงกัน
...
### จากEDDGrid {#eddtablefromeddgrid} 
[ **จากEDDGrid** ](#eddtablefromeddgrid)ให้คุณสร้างชุดข้อมูล EDTable จากใด ๆEDDGridชุดข้อมูล
...
* ถ้าปิดEDDGridคือ[EDDGridจาก เอลด์ดาป](#eddfromerddap)และERDDAP™เหมือนกันERDDAPแล้ว EDTable จากEDDGridจะใช้ชุดข้อมูลที่อ้างถึงในปัจจุบันโดยตรงเสมอ นี่คือวิธีที่มีประสิทธิภาพมากสําหรับ EDTable จากEDDGridเพื่อเข้าถึงข้อมูลที่ถูกย่าง
...
*   [สร้าง DatatasXml](#generatedatasetsxml)มีตัวเลือกสําหรับประเภทข้อมูลประเภท=EDTableEDDGridซึ่งร้องขอที่อยู่ URL ของERDDAP  (ปกติก็เหมือนกันERDDAP)   (ลงท้ายด้วย "/dadap/") และการแสดงออกปกติ ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ XMI จะสร้าง XML สําหรับอุปกรณ์ EDTable fromEDDGridชุดข้อมูลแต่ละชุดที่ถูกฝังไว้ในERDDAP™ซึ่งมีdatasetIDซึ่งตรงกับเงื่อนไขการค้นหา (ใช้ . / * เพื่อเข้าคู่กับทั้งหมดdatasetIDs สําหรับชุดข้อมูลแบบฝังตัว) .
...
#### จากEDDGridโครงสร้าง XML{#eddtablefromeddgrid-skeleton-xml} 
...
[ **แก้ไขโครงการหลัก...** ](#eddtablefromfilenames)สร้างชุดข้อมูลต่าง ๆ จากข้อมูลเกี่ยวกับกลุ่มแฟ้มในระบบเซิร์ฟเวอร์ รวมทั้งที่อยู่ URL ของแต่ละแฟ้ม เพื่อให้ผู้ใช้สามารถดาวน์โหลดแฟ้มได้ผ่านทางERDDAP'["files"ระบบ](https://coastwatch.pfeg.noaa.gov/erddap/files/documentation.html). ไม่เหมือนกับทั้งหมด[เลือกแฟ้ม](#eddtablefromfiles)คลาสย่อย, ประเภทข้อมูลชุดนี้ ไม่มีข้อมูลจากภายในแฟ้ม
...
*   [ข้อมูลใน DDTable จากแฟ้มName](#eddtablefromfilenames-data)เป็นตารางที่ERDDAP™สร้างข้อมูลออนไลน์ด้วยข้อมูลเกี่ยวกับกลุ่มแฟ้มท้องถิ่น ในตาราง มีแถวสําหรับแต่ละไฟล์ สี่คุณลักษณะพิเศษใน[datasets.xmlสําหรับชุดข้อมูล](#eddtablefromfilenames-skeleton-xml)เลือกว่าจะให้แฟ้มใดรวมอยู่ในชุดข้อมูลนี้:
...
ยกตัวอย่างเช่น ถ้าdatasetIDแฟ้มภาพประเภท JIPMURSST
และ&lt;แฟ้ม Dir) คือ / home/data/Mur/
และไดเร็กทอรีดังกล่าวมีไฟล์ที่ชื่อ jIPMURSST201501000000.png.
...
นอกจากการใช้ไดเรกทอรีท้องถิ่นสําหรับ&lt;แฟ้ม Dir &gt; คุณสามารถกําหนดที่อยู่ URL ของหน้าเว็บทางไกลแบบไดเร็กทอรีได้ มันใช้ได้กับ
        ...
        * ชุดข้อมูลการจําแนกประเภทHyraxอี.จี.
...
[\\*\\*♪ จากบนฟลาย](#fromonthefly)-- สําหรับ S3 ถังขนาดใหญ่ (เช่น Noa-goes17 ซึ่งมี 26 ล้านไฟล์) อาจต้องใช้เวลาERDDAP™ถึง 12 ชั่วโมงที่จะดาวน์โหลดข้อมูลทั้งหมดเกี่ยวกับเนื้อหาของถัง (แล้วก็มีปัญหาอื่นอีก) . การจะอ้อมค้อมเรื่องนี้ มันมีวิธีพิเศษ&lt;Dir) ใน DDTable from Files เพื่อสร้างชุดข้อมูลและชื่อแฟ้มจากถัง AWS S3 ชุดข้อมูลจะไม่มีรายการไดเรกทอรีของ S3 ถังทั้งหมด และชื่อแฟ้มที่ผู้ใช้สามารถค้นหาได้โดยผ่านทางการร้องขอไปยังชุดข้อมูล แต่ชุดข้อมูลจะได้รับการตั้งชื่อไดเรกทอรีและแฟ้มบน-the-fly ถ้าผู้ใช้ผ่านลําดับไดเรกทอรีที่มีชุดข้อมูล"files"ทางเลือก ดังนั้น จึงอนุญาตให้ผู้ใช้สามารถเรียกดูลําดับชั้นแฟ้มและแฟ้มของ s3 ได้ โดยใช้ข้อมูลชุด"files"ระบบ เพื่อทําเช่นนี้ แทนที่จะระบุที่อยู่ URL สําหรับถัง S3 ว่าเป็น "ไดเรกทอรีเริ่มการทํางาน" (ในชุดสะสม Xml) หรือ&lt;Dir) (ในdatasets.xml) ใช้:
```
...
```
ดูเอกสารสําหรับ[ทํางานกับ S3 Buckets ในERDDAP™](#working-with-aws-s3-files)แจ้งให้ทราบถึงคําบรรยายรูปแบบที่จะใช้สําหรับที่อยู่ URL ของช่อง S3 และดู
...
##### แฟ้มRegex{#fileregex} 
*   &lt;แฟ้มRegx) -- เฉพาะชื่อแฟ้มที่ชื่อแฟ้มทั้งหมด (ไม่รวมชื่อไดเรกทอรี) ตรงกับ&lt;แฟ้มRegx) จะรวมอยู่ในชุดข้อมูลนี้ด้วย ตัวอย่างเช่น JIPMURSST. &#123;14&#125;\\.png (ดูนี่สิ[เอกสารของ Regex](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html)ถึง[บทเรียน regax](https://www.vogella.com/tutorials/JavaRegularExpressions/article.html).)   
         
##### เนื้อหาของตารางข้อมูลจากชื่อแฟ้ม{#from-file-names-data-table-contents} 
ในตารางจะมีคอลัมน์ด้วย
* url-- ตําแหน่ง URL ที่ผู้ใช้สามารถใช้ในการดาวน์โหลดแฟ้มได้ERDDAP'["files"ระบบ](https://coastwatch.pfeg.noaa.gov/erddap/files/documentation.html).
...
* เพิ่มคอลัมน์ที่นิยามโดยERDDAP™ผู้ดูแลระบบที่มีข้อมูลที่สกัดมาจากชื่อแฟ้ม (ตัวอย่างเช่น เวลาที่เกี่ยวข้องกับข้อมูลในแฟ้ม) จากคุณสมบัติสองตัวที่คุณกําหนดไว้ในข้อมูลกํากับภาพสําหรับแต่ละคอลัมน์เพิ่มเติม/dataVariable.
    ...
    * ตัวแยก regex -- นี่คือ[เงื่อนไขการค้นหา](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html)  ([การ สอน](https://www.vogella.com/tutorials/JavaRegularExpressions/article.html)) . Regex ทั้งหมดจะต้องตรงกับชื่อแฟ้มทั้งหมด (ไม่รวมชื่อไดเรกทอรี) . Regex ต้องรวมการจับภาพอย่างน้อยหนึ่งกลุ่ม (ส่วนของพจน์ปกติที่ติดวงเล็บ) ซึ่งERDDAP™ใช้พิจารณาว่าส่วนไหนของชื่อแฟ้มที่จะแยกออกมาเป็นข้อมูล
...
```
ในกรณีของตัวแปรเวลา หากแฟ้มมีชื่อ jpMURSST201501000000000.png ตัวแยก จะตรงกับชื่อแฟ้ม, สกัดตัวอักขระที่ตรงกับกลุ่มดักจับตัวแรก ("20150103000000") เป็นข้อมูลประเภท= string, แล้วใช้[แสดงเฉพาะหมายเลขของธาตุ](#string-time-units)เพื่อวิเคราะห์ข้อความต่าง ๆ ไปยังค่าของเวลา (2015-01-03.0000:00Z) .
...
* ไม่&lt;update everNMillies) (อัปเดตทุก ๆ ครั้ง) -- ชุดข้อมูลประเภทนี้ไม่ต้องการ และไม่สามารถใช้ได้&lt;update everNMillies) แท็ก เพราะว่าข้อมูลให้บริการโดย EDTable from Files นั้นสมบูรณ์แบบเสมอ เพราะERDDAP™ตรวจค้นหาระบบแฟ้ม เพื่อตอบสนองต่อการร้องขอข้อมูลแต่ละตัว ถึงแม้จะมีแฟ้มมากมาย, วิธีการนี้ ควรใช้ได้ดี การตอบรับอาจช้า ถ้ามีแฟ้มจํานวนมาก และชุดข้อมูลไม่ได้ถูกค้นหามาระยะหนึ่ง แต่ในหลายนาทีหลังจากนั้น ระบบปฏิบัติการเก็บข้อมูลไว้ในแคช ดังนั้นการตอบสนองควรจะรวดเร็วมาก
     
* คุณสามารถใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อทําให้datasets.xmlผนวกเข้ากับชุดข้อมูลประเภทนี้ คุณสามารถเพิ่ม/ ตัดกับคอลัมน์อื่น ๆ ที่มีข้อมูลที่สกัดมาจากชื่อแฟ้ม ดังที่แสดงไว้ด้านบน
     
...
*   [จาก แฟ้ม AwsXml](#eddtablefromawsxmlfiles)รวมข้อมูลจากสถานีสภาพอากาศอัตโนมัติ (ขนาด AWS) แฟ้ม XML
*   [แฟ้ม DDTable from Columnar Ascii](#eddtablefromcolumnarasciifiles)รวมข้อมูลจากแฟ้มข้อมูลอักขระ ASCII ด้วยคอลัมน์ข้อมูลที่คงที่
*   [จากHyraxแฟ้ม](#eddtablefromhyraxfiles)  (ขอบเขต) รวมข้อมูลเข้ากับตัวแปรต่างๆ แต่ละมิติมีมิติร่วมกัน (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) และรับใช้โดย[Hyrax OPeNDAPเซิร์ฟเวอร์](https://www.opendap.org/software/hyrax-data-server).
*   [แฟ้ม DDTA จาก InfidCra](#eddtablefrominvalidcrafiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .ncแฟ้มที่ใช้ค่าเฉพาะ, ใช้งานไม่ได้, ค่าย่อของ CFDSG แบบต่อเนื่อง Rangle Array (CRA) แฟ้ม ถึงแม้ว่าERDDAP™รองรับประเภทแฟ้มนี้ ซึ่งเป็นประเภทแฟ้มที่ไม่ควรใช้ กลุ่มที่จะใช้ประเภทแฟ้มนี้ได้ถูกสนับสนุนให้ใช้ERDDAP™เพื่อสร้างแฟ้ม CFDSG CRA และหยุดการใช้แฟ้มเหล่านี้
*   [แฟ้ม DDTable จาก JonlCSV](#eddtablefromjsonlcsvfiles)ข้อมูลรวมจาก[เจ สัน แฟ้มข้อความ CSV](https://jsonlines.org/examples/).
*   [แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .nc  (หรือ[.ncmI](#ncml-files)) แฟ้มที่มีหลายตัวแปร แต่ละแบบมีมิติร่วมกัน (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) .
*   [แฟ้ม DDTAB จาก Nc](#eddtablefromncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .nc  (หรือ[.ncmI](#ncml-files)) แฟ้มที่มีหลายตัวแปร แต่ละแบบมีมิติร่วมกัน (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) . มันดีที่จะยังคงใช้ประเภทข้อมูลชุดนี้ สําหรับชุดข้อมูลที่มีอยู่ แต่สําหรับชุดข้อมูลใหม่ที่เราแนะนํา โดยใช้ DDTable from Mulitidnc Files แทน
*   [แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .nc  (หรือ[.ncmI](#ncml-files)) แฟ้มซึ่งใช้เป็นหนึ่งในรูปแบบแฟ้มที่ระบุโดย[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)การ ประชุม ภาค. แต่สําหรับไฟล์ที่ใช้เป็นหนึ่งในหลายมิติ CF DSG ตัวแปร, ใช้[แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)แทน
*   [แฟ้ม DDTable จาก Nccsv](#eddtablefromnccsvfiles)ข้อมูลรวมจาก[เข้ารหัสแบบ NCSV](/docs/user/nccsv-1.00)แฟ้ม .csv ASCII
*   [เลือกจากแฟ้ม Parquet](#eddtablefromparquetfiles)จัดการกับข้อมูล[สี่เหลี่ยม](https://parquet.apache.org/).
*   [แฟ้ม EDTIVE จาก Thredds February](#eddtablefromthreddsfiles)  (ขอบเขต) รวมข้อมูลจากแฟ้มที่มีตัวแปรหลายตัวที่มีมิติที่ใช้ร่วมกัน[(เต๋า)OPeNDAPเซิร์ฟเวอร์](https://www.unidata.ucar.edu/software/tds/).
*   [จากWFSแฟ้ม](#eddtablefromwfsfiles)  (ขอบเขต) ทําสําเนาข้อมูลทั้งหมดจากArcGISเซิร์ฟเวอร์แผนที่WFSเซิร์ฟเวอร์ เพื่อให้ข้อมูลสามารถถูกตรวจสอบใหม่ได้อย่างรวดเร็วERDDAP™ผู้ใช้

ตอนนี้ยังไม่มีแฟ้มชนิดอื่นให้รองรับ แต่ ปกติ แล้ว ค่อน ข้าง ง่าย ที่ จะ เพิ่ม การ สนับสนุน ให้ มี แฟ้ม ชนิด อื่น ๆ. ติดต่อเราถ้าคุณมีคําขอ หรือถ้าข้อมูลของคุณอยู่ในรูปแบบแฟ้มเก่าที่คุณต้องการจะย้ายออกจาก เราขอแนะนําให้การแปลงแฟ้มเป็นNetCDFv3.ncแฟ้ม (โดยเฉพาะ.ncแฟ้มที่มี[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)โครงสร้างข้อมูลที่ต่อเนื่องของอาร์เรย์ERDDAP™สามารถดึงข้อมูลจากพวกมันได้อย่างรวดเร็ว) .NetCDFเป็นรูปแบบไบนารีที่รองรับอย่างกว้างขวาง อนุญาตให้เข้าถึงข้อมูลได้อย่างรวดเร็ว และรองรับแล้วโดยERDDAP.
...
คลาสนี้เก็บข้อมูลจากแฟ้มภายในระบบ แต่ละแฟ้มบรรจุ (ค่อน) ตารางข้อมูลเล็กๆ
    ...
    * ตัวแปรในแฟ้มทั้งหมด ต้องมีค่าเท่ากับ[add\\_offset](#scale_factor).[missing\\_value](#missing_value).[_ไฟ ค่า](#missing_value).[scale\\_factor](#scale_factor)และ[หน่วย](#units)คุณสมบัติ (ถ้ามี) .ERDDAP™เช็ค แต่มันคือการทดสอบที่ไม่สมบูรณ์ ถ้ามีค่าต่างกันERDDAPไม่รู้ว่าอันไหนถูกต้อง ดังนั้นแฟ้มไหนใช้งานไม่ได้ ถ้า นี่ เป็น ปัญหา คุณ อาจ ใช้ ได้[ขนาด NcML](#ncml-files)หรือ[NCO](#netcdf-operators-nco)เพื่อแก้ไขปัญหา
         
##### แฟ้มที่บีบอัด{#compressed-files} 
แฟ้มข้อมูลต้นฉบับสําหรับ EDTable จากคลาสย่อยของแฟ้ม สามารถถูกบีบอัดจากภายนอกได้ (อี.จี..tgz..tar.gz..tar.gzip..gz..gzip..zip..bz2หรือ .Z) . ดู[เอกสารที่ถูกบีบอัดจากภายนอก](#externally-compressed-files).
     
...
*    **การร้องขอของผู้บริหารระบบทางไกลและ HTTP**   (โดย ผู้ เขียน ตื่น เถิด&#33;) --
    EDDGridจาก NNc Files, EDTable from Multicalnk files, EDTable from Ncfiles, และ EDTable fromNCF Files บางครั้งสามารถให้บริการข้อมูลจาก.ncแฟ้มบนเซิร์ฟเวอร์ทางไกลและเข้าถึงผ่านทาง HTTP หากเซิร์ฟเวอร์รองรับ[การ รับ ใช้ ที่ ยาว นาน](https://en.wikipedia.org/wiki/Byte_serving)ผ่านการร้องขอในช่วง HTTP (กลไก HTTP สําหรับทํางาน) . มันเป็นไปได้เพราะ Netcdf-java (ซึ่งERDDAP™ใช้อ่าน.ncแฟ้ม) รองรับข้อมูลการอ่านจากระยะไกล.ncต้องการแฟ้มผ่านทาง HTTP
...
ทั้งหมดEDDGridจากแฟ้มและทั้งหมด EDTable จากแฟ้มชุดข้อมูลรองรับชุดของป้ายกํากับที่บอกว่าERDDAP™เพื่อดาวน์โหลดและรักษาสําเนาของแฟ้มฐานข้อมูลทั้งหมด หรือแคชของแฟ้มไม่กี่แฟ้ม (ดาวน์โหลดเมื่อต้องการ) . **นี่เป็นคุณสมบัติที่มีประโยชน์อย่างเหลือเชื่อ** 
...
ดู[ทํางานกับ S3 Buckets ในERDDAP™](#working-with-aws-s3-files).
และปกติคุณไม่จําเป็นต้องใช้แคช จาก Url กับไฟล์ในถัง S3 ถ้าไฟล์เป็นแฟ้ม ASCII (e.g., .csv) เพราะว่าERDDAP™สามารถอ่านข้อมูลจากถังโดยตรง ผ่านกระแส
...
ภายในERDDAP™ทํางานด้วยความเร็วเท่า ๆ กัน ไม่ว่าแฟ้ม n ตัว จะอยู่ในไดเรกทอรีเดียวหรือไม่ หรือกระจัดกระจายอยู่ในไดเร็กทอรีหลายอัน
     
...
* ข้อมูลบางชุดมีไฟล์แหล่งเป็นล้านERDDAP™สามารถจัดการได้ แต่ด้วยผลลัพธ์ผสม
    ...
    * สําหรับคําขออื่นERDDAP™สามารถสแกนชุดข้อมูล[ข้อมูลแฟ้มที่มีแคช](#cached-file-information)และพบว่ามีแฟ้มไม่กี่แฟ้มเท่านั้น ที่อาจจะมีข้อมูลที่เกี่ยวข้องกับคําขอนี้ และตอบโต้อย่างรวดเร็ว
...
โชคดีที่มีทางออก
    ...
    2. สร้างและประมวลผลสคริปต์ที่ต้องการชุดของ.ncแฟ้ม CF แต่ละแฟ้มมีชุดข้อมูลขนาดใหญ่ โดยปกติจะเป็นช่วงเวลา (ตัวอย่างเช่น, ข้อมูลทั้งหมดสําหรับเดือนที่กําหนด) . เลือกช่วงเวลา เพื่อให้แฟ้มผลลัพธ์ทั้งหมดน้อยกว่า 2GB (แต่หวังว่ามากกว่า 1GB) . หากชุดข้อมูลกํากับมีข้อมูลใกล้เคียงกับเวลา ให้ประมวลผลสคริปต์เพื่อสร้างแฟ้มใหม่สําหรับช่วงเวลาปัจจุบัน (อี.จี. เดือนนี้) บ่อย (ทุก 10 นาที? ทุกชั่วโมง?) . ร้องขอERDDAP™สําหรับ.ncแฟ้ม CF สร้างเป็นNetCDFv3.ncแฟ้มที่ใช้[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)ข้อมูลโครงสร้างของอาร์เรย์ต่อเนื่อง)
    3. ตั้ง[แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)ชุดข้อมูลในที่สาธารณะERDDAP™ซึ่งได้ข้อมูลจาก.nc (CF) แฟ้มERDDAP™สามารถดึงข้อมูลจากไฟล์เหล่านี้ได้อย่างรวดเร็ว และเนื่องจากตอนนี้มีหลายสิบหรือหลายร้อย (แทนที่จะเป็นล้าน) ของแฟ้ม, แม้ว่าERDDAP™ต้องเปิดไฟล์ทั้งหมด มันทําได้เร็วมาก
...
ใช่ ระบบนี้ต้องใช้เวลาและความพยายามในการจัดตั้งขึ้น แต่มันก็ได้ผลดีมาก การร้องขอข้อมูลส่วนใหญ่ สามารถจัดการได้เร็วกว่าเดิม 100 เท่า
    \\[บ๊อบรู้ว่านี่เป็นไปได้ แต่เควิน โอ'ไบรอัน เป็นคนทําครั้งแรก และแสดงให้เห็นว่ามันได้ผล ตอนนี้ Bob ใช้นี่สําหรับข้อมูล GTSP ซึ่งมีแฟ้มต้นฉบับประมาณ 18 ล้านแฟ้มERDDAP™ปัจจุบัน รับ ใช้ ผ่าน ไป ประมาณ 500 คน.nc (CF) แฟ้ม\\]
...
\\[ลักษณะนี้มีความชัดเจน กรุณาใช้[\\*\\*เลเยอร์ถัดไปsourceName](#filename-sourcenames)แทน\\]  
...
* คอลัมน์ ชื่อ For Excranget เป็นชื่อแหล่งข้อมูลสําหรับสตริงที่สกัดมา AdataVariableด้วยสิ่งนี้[sourceName](#sourcename)ต้องอยู่ในdataVariableรายชื่อ s (ด้วยชนิดของข้อมูลใด ๆ แต่ปกติสตริง) .
...
^เริ่มต้นเป็นอักขระพิเศษที่ใช้แสดงERDDAP™เพื่อหา XYZ ในตอนต้นของชื่อแฟ้ม นี่เป็นสาเหตุของ XYZ หากพบที่ตอนต้นของชื่อแฟ้ม จะถูกลบออก (ตัวอย่างเช่น ชื่อแฟ้ม XYZAble.ncกลายเป็นอํานาจ.nc) .
*   &lt;หลังจากจบการทํางานแล้ว\\.nc$&lt;&lt;i&gt;
$ ที่สุดท้ายเป็นการแสดงออกพิเศษที่ERDDAP™เพื่อหา.ncที่ท้ายชื่อแฟ้ม เนื่องจาก . เป็นตัวละครพิเศษ (ซึ่งตรงกับตัวอักษรใด ๆ) เข้ารหัสเป็น\\ ที่นี่ (เพราะ 2E คือเลขอักขระเลขฐานสิบหกสําหรับช่วงเวลา) . นี่เป็นสาเหตุ.ncหากพบที่ท้ายชื่อแฟ้ม จะลบออก (ตัวอย่างเช่น ชื่อแฟ้มบางส่วน อํานาจ.ncกลายเป็นอํานาจ) .
...
*   &lt;คอลัมน์NamestationID&lt;/ โคลัมน์Name
นี่บอกว่าERDDAP™เพื่อสร้างคอลัมน์แหล่งใหม่ ที่เรียกว่าstationIDเมื่ออ่านแฟ้มแต่ละตัว ทุกแถวของข้อมูลจากแฟ้มที่ให้ จะมีส่วนดึงข้อความออกมาจากชื่อแฟ้ม (ยกตัวอย่างเช่น อํานาจ) เป็นค่าในstationIDคอลัมน์
...
ในกรณีส่วนใหญ่ มีค่ามากมายสําหรับแท็ก สกัดเหล่านี้ที่จะส่งผลเดียวกัน การแสดงออกปกติมีความยืดหยุ่นมาก แต่ ใน บาง กรณี มี วิธี หนึ่ง ที่ จะ ได้ ผล ที่ ต้องการ.
     
##### เซโดะsourceNameวินาที{#pseudo-sourcenames} 
ทุกตัวแปรในข้อมูลERDDAP™มี a&lt;sourceName&gt;) (ชื่อแหล่ง) ซึ่งระบุชื่อแหล่งสําหรับตัวแปร สื่อ ต่าง ๆ จาก แฟ้ม ได้ รับ การ สนับสนุน บาง ส่วนsourceNames ซึ่งแยกค่าจากที่อื่น (เช่น ชื่อของแฟ้ม หรือค่าของแอททริบิวต์ทั่วไป) และส่งเสริมให้ค่านั้นเป็นคอลัมน์ของค่าคงที่ สําหรับส่วนของข้อมูล (เช่น ตารางข้อมูลของไฟล์) . สําหรับตัวแปรเหล่านี้ คุณต้องระบุชนิดของตัวแปรผ่านทาง&lt;ข้อมูล (# ดาต้าชนิด) ป้าย หากข้อมูลที่ถูกสกัดออกมา เป็นข้อความเวลา คุณกําหนดรูปแบบของข้อความเวลา ในข้อความ[คุณสมบัติของหน่วย](#string-time-units). ตัวปลอมsourceNameตัวเลือกคือ:
 
###### ทั่วไป:sourceNameวินาที{#global-sourcenames} 
...
เมื่อคุณส่งเสริมคุณลักษณะที่เป็นข้อมูลERDDAP™ลบคุณลักษณะที่คู่กันออกไป เหมาะสม เนื่องจากค่าต่าง ๆ นั้นอาจจะแตกต่างกันในทุกแฟ้ม ส่วนในชุดข้อมูลรวมERDDAP™มันจะมีค่าอย่างเดียว ถ้าคุณต้องการ คุณสามารถเพิ่มค่าใหม่ของแอททริบิวต์สําหรับชุดข้อมูลทั้งหมดได้ โดยการเพิ่ม&lt;at name=" *คุณสมบัติ ชื่อ* ""-- *สร้างใหม่ ค่า* &lt;at&gt; to the Dayet's Global [att]. สืบค้นเมื่อ 2 January 2015.&lt;addAttributes&gt;) (เพิ่มรายการแจกจ่าย) . สําหรับคุณสมบัติระดับโลกERDDAP™เพื่อ เป็น ตัว อย่าง จําเป็น ต้อง มี สถาบัน เพื่อ เพิ่ม คุณค่า ใหม่ สําหรับ คุณลักษณะ นี้.
     
###### ตัวแปร:sourceNameวินาที{#variable-sourcenames} 
...
###### แฟ้มNamesourceNameวินาที{#filename-sourcenames} 
...
เมื่อ EDTable from Files กําลังอ่านข้อมูลจากแฟ้ม มันจะทําให้แน่ใจว่าแฟ้มดังกล่าวจริง ๆName (ตัวอย่างเช่น A201807041442. scpV1..nc) ตรงกับเงื่อนไขการค้นหา ("Regex") และเราได้ปลดเปลื้องสิ่งที่ได้ระบุไว้แล้ว (ในกรณีนี้, อันแรก) กลุ่มจับ (ซึ่งเป็นส่วนหนึ่งของวงเล็บ) ยกตัวอย่างเช่น "2080807041442". (ดูนี่สิ[เอกสารของ Regex](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html)ถึง[บทเรียน regax](https://www.vogella.com/tutorials/JavaRegularExpressions/article.html).) Regex อาจกําหนดเป็นข้อความที่มีคําคมล้อมรอบหรือไม่ใช้ หากกําหนด regex เป็นข้อความที่มีข้อความอ้างถึงรอบ ๆ ก็ต้องเป็นข้อความ[ข้อความแบบ Jon- Style](https://www.json.org/json-en.html)  (มีอักขระพิเศษหนีออกมาด้วย\\ ตัวอักษร) . จํานวนกลุ่มจับมักเป็น 1 (กลุ่มที่จับได้กลุ่มแรก) แต่อาจจะเป็นหมายเลขอะไรก็ได้
     
###### พาธNamesourceNameวินาที{#pathname-sourcenames} 
...
* ถ้าคุณวิ่ง[สร้าง DatatasXml](#generatedatasetsxml)หรือ[ถอดรหัส](#dasdds)หรือถ้าคุณพยายามที่จะโหลด EDTable จาก ... ชุดข้อมูลแฟ้มในERDDAP™และคุณได้ข้อความผิดพลาด "0 แฟ้ม" ระบุว่าERDDAP™พบแฟ้มที่เข้าคู่กับ 0 ในไดเรกทอรี (เมื่อคุณคิดว่า มีแฟ้มที่ตรงกับในไดเรกทอรีนั้น) .
    * ตรวจสอบว่าแฟ้มอยู่ในไดเรกทอรีดังกล่าวจริงหรือไม่
    * ตรวจสอบการสะกดชื่อไดเรกทอรี
    * ตรวจสอบแฟ้มName มันง่ายที่จะทําผิดพลาดกับรีเซกซ์ สําหรับวัตถุประสงค์การทดสอบ ลอง regex. * ซึ่งควรจะตรงกับชื่อแฟ้มทั้งหมด (ดูนี่สิ[เอกสารของ Regex](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html)ถึง[บทเรียน regax](https://www.vogella.com/tutorials/JavaRegularExpressions/article.html).) 
    * เลือกผู้ใช้ที่กําลังทํางานอยู่ของโปรแกรม (e.g., ผู้ใช้=tommat (?) สําหรับทอมแคท/ERDDAP) ได้สิทธิ์ในการอ่านไฟล์เหล่านั้น
...
```
ใน[datasets.xmlสําหรับ EDTable จาก... ชุดข้อมูลแฟ้ม](#eddtablefromfiles-skeleton-xml)กับ&lt;ป้ายข้อมูล)
...
หากนี่ถูกเลือก และมีโอกาสที่ตัวแปรนี้scale\\_factorหรือadd\\_offset#1 ต้องถูกเลือกด้วย
     
3. ปรับใช้ข้อความmissing\\_value  
หากคอลัมน์สตริงมี /_ FullValue และ/ormissing\\_valueคุณสมบัตินี้แปลงค่าเหล่านั้นเป็น "" และลบคุณลักษณะดังกล่าวออกไป
     
4. ค้นหาตัวเลขmissing\\_value  
หากคอลัมน์ตัวเลขไม่มี /_ fullValue หรือmissing\\_valueคุณสมบัตินี้พยายามที่จะระบุตัวเลขที่นิยามไม่ได้missing\\_value  (e.g., -9999, 999, 1e37f) แล้วแปลงมันเป็นค่า "มาตรฐาน" (MAX\\ VALUE สําหรับชนิดจํานวนเต็ม และ NAN สําหรับคู่คู่และลอย) .
...
สําหรับแต่ละคอลัมน์สตริงหรือประเภทของค่าจํานวนเต็ม ให้ลองแปลงค่าเวลาข้อความแบบรวม (E.g. "2080102".) ถึง ISO 8601 เวลาวันสตริง ("2018-01-02") .
    ...
     
8. หน่วยมาตรฐาน
เป็นการพยายามกําหนดค่าข้อความของแต่ละตัวแปร ตัวอย่างเช่น "เมตรต่อวินาที" "เมตร/วินาที""m.s^-1"."m s-1"Ms-1 ทั้งหมดจะถูกเปลี่ยนเป็น Ms-1 นี่ไม่ได้เปลี่ยนค่าข้อมูล ใช้ได้กับUDUNITSหน่วยสตริง แต่อาจจะมีปัญหา กับสตริงที่ใช้ไม่ได้หรือซับซ้อน คุณสามารถจัดการกับปัญหาได้โดยระบุค่าต่าง ๆ จากคู่&lt;ขนาด ยูนิต มาตรฐาน) ในERDDAP'
...
ขนาดมาตรฐาน การตั้งค่าที่จะใช้สําหรับทุกคอลัมน์ในแฟ้มต้นฉบับ ตัวอย่างเช่น การใช้ #2048 อาจเปลี่ยนคอลัมน์ของ Cattle Times เป็น ISO 8601 สตริงไทม์ส์
     
*   datasets.xmlและสร้างชุดสะสม Xml -
เป็น เรื่อง ยาก เป็น พิเศษ ที่ จะ ปรับ ตั้ง ให้ ถูก ต้อง ในdatasets.xmlเพื่อให้ชุดข้อมูลเป็นไปตามที่คุณต้องการ วิธีที่ดีที่สุด (เหมือนทุกครั้ง) คือ
    ...
    3. ทดสอบชุดข้อมูลด้วยมือเมื่อมันอยู่ในERDDAP™เพื่อให้แน่ใจว่าตัวแปรที่ได้รับผลกระทบ จะทํางานตามที่คาดหวัง
         
* ความเสี่ยง -
ตัวเลือกที่ 256 และเหนือขึ้นไปมีความเสี่ยงมากขึ้น, เช่น, มันมีโอกาสมากกว่าERDDAP™จะเปลี่ยนแปลงสิ่งที่ไม่ควรเปลี่ยน ตัวอย่างเช่น ตัวเลือก #2048 อาจจะเปลี่ยนตัวแปรที่มีข้อความ ID ของสถานี ที่เกิดขึ้นทั้งหมดเพียงแค่ดู ISO 8601 "pact" วันที่ (E.g., 2080102.) ไปยัง ISO 8601"extended"วันที่ ("2018-01-02") .
     
* ช้าลงหลังจากเปลี่ยน
...
ตัวเลือก # 256 และด้านบนใช้ Heuristics เพื่อเปลี่ยนแปลง ถ้าคุณพบสถานการณ์ ที่พวกเฮอริคตัดสินใจผิด โปรดส่งอีเมลหาคริส จอห์นที่โนอา เพื่อที่เราจะได้พัฒนาการ Heuristics
     
* ทางเลือก --
ถ้าหนึ่งในมาตรฐานตัวเลือกใด ๆ ไม่ได้แก้ปัญหาสําหรับชุดข้อมูลที่กําหนด, คุณอาจแก้ปัญหาได้ด้วยการ[.ncแฟ้ม mI](#ncml-files)เพื่อขนานแฟ้มข้อมูลทั้งหมดและกําหนดการเปลี่ยนแปลง ของแฟ้มเพื่อให้แฟ้มสอดคล้องกัน งั้นไปบอก EDTable จาก... ข้อมูลชุดแฟ้มที่จะรวม.ncแฟ้ม mI
...
เป็นเรื่องปกติของไฟล์ข้อมูลรอบนอก ที่จะมีคอลัมน์ที่แตกต่างกัน สําหรับปี เดือน วัน ชั่วโมง นาที วินาที ก่อนERDDAP™v2.10, คําตอบเดียวคือแก้ไขแฟ้มข้อมูล เพื่อรวมคอลัมน์เหล่านั้นเป็นคอลัมน์เวลารวม ด้วยERDDAP™2.10+, คุณสามารถใช้
[&lt;sourceName&gt;= *เงื่อนไข* &lt;sourceName&gt;) (ชื่อแหล่ง) เพื่อบอกว่าERDDAP™วิธีรวมคอลัมน์ต้นฉบับ เพื่อสร้างคอลัมน์เวลารวม คุณจะได้ไม่ต้องแก้ไขแฟ้มต้นฉบับ
...
*   [EDTable from Ascisiservnos](#eddtablefromasciiservicenos)รับข้อมูลจากNOAAบริการของ NOS ASCII
...
[ **DDTable from AllDatets** ](#eddtablefromalldatasets)เป็นชุดข้อมูลระดับสูงที่มีข้อมูลเกี่ยวกับชุดข้อมูลอื่น ๆ ทั้งหมด ซึ่งปัจจุบันถูกโหลดในของคุณERDDAP. ไม่เหมือนกับชุดข้อมูลชนิดอื่น, ไม่มีการระบุallDatasetsชุดข้อมูลในdatasets.xml.ERDDAP™สร้างชุดข้อมูล EDTable from AllDatatas ชุดข้อมูลอัตโนมัติ (ด้วยdatasetID=allDatasets) . ดังนั้นallDatasetsชุดข้อมูลจะถูกสร้างในแต่ละตัวERDDAP™การติดตั้งและจะทํางานแบบเดิมในแต่ละERDDAP™การติดตั้ง
...
* ไฟล์ ASCII ใช้งานได้ง่าย แต่ไม่ได้เป็นวิธีที่มีประสิทธิภาพมากที่สุดในการเก็บ/แก้ไขข้อมูล เพื่อให้มีประสิทธิภาพมากขึ้น บันทึกแฟ้มเป็นNetCDFv3.ncแฟ้ม (กับหนึ่งมิติ, "row", แบ่งปันโดยตัวแปรทั้งหมด) แทน คุณทําได้[ใช้ERDDAP™เพื่อสร้างแฟ้มใหม่](#millions-of-files).
* ดูคลาสนี้สิ[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน
* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล เนื่อง จาก ไม่ มี ข้อมูล กํากับ ใน แฟ้ม ASCII ทั้ง หมด คุณ จึง จําเป็น ต้อง แก้ไข ผล ของ การ สร้าง DatatasetsXml เสมอ.
...
* ชนิดของแฟ้มนี้เรียบง่าย แต่เป็นวิธีที่ไม่มีประสิทธิภาพในการเก็บข้อมูล เนื่องจากแต่ละแฟ้ม ดูเหมือนจะมีข้อสังเกตจากจุดเดียว อาจมีไฟล์จํานวนมาก ถ้า คุณ ต้องการ ปรับ ปรุง ความ สามารถ ให้ ดี ขึ้น ขอ ให้ พิจารณา กลุ่ม การ สังเกต ที่ เพิ่ม ขึ้น (มีค่าแค่อาทิตย์เดียว) ในNetCDFv3.ncแฟ้ม (ดีที่สุด:.ncแฟ้มที่มี[CF แว่นขยาย (แบบ DSG) ฟอร์แมตอาร์เรย์แบบต่อเนื่อง](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)) และใช้[แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)  (หรือ[แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)) เพื่อให้บริการข้อมูล คุณทําได้[ใช้ERDDAP™เพื่อสร้างแฟ้มใหม่](#millions-of-files).
* ดูคลาสนี้สิ[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน
     
...
* ไฟล์ ASCII ใช้งานได้ง่าย แต่ไม่ได้เป็นวิธีที่มีประสิทธิภาพในการเก็บ/retrieve ข้อมูล เพื่อให้มีประสิทธิภาพมากขึ้น บันทึกแฟ้มเป็นNetCDFv3.ncแฟ้ม (กับหนึ่งมิติ, "row", แบ่งปันโดยตัวแปรทั้งหมด) แทน คุณทําได้[ใช้ERDDAP™เพื่อสร้างแฟ้มใหม่](#millions-of-files).
* ดูคลาสนี้สิ[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน
* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล เนื่อง จาก ยาก ที่ จะ กําหนด ตําแหน่ง เริ่ม และ จบ สําหรับ แต่ ละ คอลัมน์ ข้อมูล และ การ ขาด ข้อมูล กํากับ ทั้ง หมด ใน แฟ้ม ASCII คุณ จึง จําเป็น ต้อง แก้ไข ผล จาก การ สร้าง เครื่อง XM.
     
### เพิ่มข้อมูล{#eddtablefromhttpget} 
แก้ไขโครงการหลัก... จาก htppget จะแตกต่างจากชุดข้อมูลชนิดอื่นทั้งหมดERDDAP™เนื่องจากมันมีระบบที่สามารถเพิ่มข้อมูล, แก้ไขข้อมูล, หรือลบข้อมูลจากชุดข้อมูล โดยปกติHTTP GETหรือ[POST](#http-post)ต้องการจากโปรแกรมคอมพิวเตอร์, สคริปต์หรือเบราว์เซอร์ ชุดข้อมูลสามารถสอบถามได้โดยผู้ใช้ในวิธีเดียวกันกับชุดข้อมูล EDTable อื่น ๆ ทั้งหมดสามารถสอบถามได้ERDDAP. ดูรายละเอียดของชนชั้นนี้[เลือกแฟ้ม](#eddtablefromfiles)เพื่ออ่านเกี่ยวกับคุณสมบัติ ที่สืบทอดมาจากซูเปอร์คลาส
...
* ข้อมูลเวลาจริง -
เป้าหมายคือการอนุญาตให้ผู้เขียน (เช่น เซ็นเซอร์ สคริปต์ QC อัตโนมัติ หรือมนุษย์โดยเฉพาะ) เพื่อเปลี่ยนข้อมูลชุดข้อมูล (ผ่าน[คําสั่ง .insert หรือ .delete](#insert-and-delete)) และทําให้การเปลี่ยนแปลงที่เข้าถึงได้ERDDAP™ผู้ใช้ทั้งหมดในเวลาน้อยกว่า 1 วินาที และอาจจะเร็วกว่ามาก ส่วนใหญ่แล้ว 1 วินาทีคือเวลาของเครือข่ายERDDAP™สามารถประมวลผลการร้องขอภายใน 1 มิลลิวินาที และข้อมูลสามารถเข้าใช้ผู้ใช้ได้ทันที นี่คือ[เร็ว](#httpget-speed).[หนัก](#robust)และ[ระบบที่เชื่อถือได้](#system-reliability).
...
*   [รุ่น](#versioning)/[วิทยาศาสตร์ ที่ สลับ ซับ ซ้อน](https://en.wikipedia.org/wiki/Reproducibility)/DOIเอส
สถานการณ์ที่คุณจําเป็นต้องทําการเปลี่ยนแปลงข้อมูล (เช่น เปลี่ยนแฟล็กควบคุมคุณภาพ) ทราบว่าผู้เขียนทําการเปลี่ยนแปลงแต่ละครั้ง รู้เวลาของเวลาที่ผู้เขียนทําการเปลี่ยนแปลง และ (เมื่อร้องขอ) สามารถดูข้อมูลต้นฉบับได้ ก่อนที่จะมีการเปลี่ยนแปลง ดังนั้น ชุดข้อมูลเหล่านี้จึงเหมาะสม[DOIวินาที](https://en.wikipedia.org/wiki/Digital_object_identifier). เพราะพวกเขาได้พบกับDOIความต้องการที่ชุดข้อมูลจะไม่ถูกเปลี่ยนแปลง ยกเว้นโดยตราสาร โดยทั่วไปแล้ว ข้อมูลเวลาจริง ไม่เหมาะกับDOIs เพราะข้อมูลมักเปลี่ยนแปลงแบบย้อนกลับ (e.g. สําหรับ QA/QC วัตถุประสงค์) .
     
...
บอกERDDAP™เพื่อเพิ่มหรือเปลี่ยนแปลงข้อมูลstationID=46088 สําหรับเวลาที่กําหนดไว้
    ...
    * ที่อยู่ URL ต้องรวมค่าที่ถูกต้องด้วย (ไม่ขาดค่า) สําหรับทั้งหมด[httpรับ การ ปรับ ตั้ง ใหม่](#httpgetrequiredvariables-global-attribute)
    * ถ้าค่าของhttpได้ รับ การ ตอบ รับ ตัวแปรในการร้องขอ (อี.จี.stationID& เวลา) ตรงกับค่าในแถวที่มีอยู่แล้วในชุดข้อมูล ค่าใหม่จะเขียนทับค่าเก่าได้อย่างมีประสิทธิภาพ (แม้ว่าค่าเก่าจะยังเข้าถึงได้ หากผู้ใช้ร้องขอข้อมูลจากข้อมูลก่อนหน้า[รุ่น](#versioning)ของชุดข้อมูล) .
...
บอกERDDAP™เพื่อลบข้อมูลstationID=46088 ณ เวลาที่กําหนดไว้
        * ผู้เขียนการเปลี่ยนแปลงนี้ คือ จอห์น สมิท และกุญแจคือ บางเคย์ 1
        * ที่อยู่ URL ต้องกําหนดที่อยู่ URL[httpรับ การ ปรับ ตั้ง ใหม่](#httpgetrequiredvariables-global-attribute)ในคําขอ (อี.จี.stationID& เวลา) . หากค่าเหล่านั้นตรงกับค่าในแถวอยู่แล้วในชุดข้อมูล (ซึ่งพวกเขาก็ปรารถนา) ค่าเก่าถูกลบออกไป (แม้ว่าค่าเก่าจะยังเข้าถึงได้ หากผู้ใช้ร้องขอข้อมูลจากข้อมูลก่อนหน้า[รุ่น](#versioning)ของชุดข้อมูล) .
...
รายละเอียด:
    ...
    * .insert และ .dete ร้องขอจะต้องรวม[httpรับ การ ปรับ ตั้ง ใหม่](#httpgetrequiredvariables-global-attribute)อี.จี.stationIDและเวลา สําหรับการร้องขอแบบ .nsert, ตัวแปรที่ไม่ได้ระบุไว้ในการร้องขอ สันนิษฐานว่าเป็นค่าที่ขาดหายไป (MAX\\ VALUE สําหรับตัวแปรจํานวนเต็ม, NN สําหรับตัวแปรลอยและคู่, และข้อความว่างสําหรับตัวแปรสตริง) . สําหรับการร้องขอ.delete, ค่าสําหรับที่ไม่ใช่ htttpget requare ตัวแปร (นอกจากผู้เขียน ซึ่งจําเป็น) ถูกละเลย
    * สืบค้นเมื่อ: *ผู้เขียน/ คีย์* เป็นพารามิเตอร์สุดท้ายในการร้องขอ การขอนี่เพื่อยืนยันครั้งสุดท้าย ว่าคําขอทั้งหมดได้รับการตอบรับERDDAP. ผู้เขียนเท่านั้น (ไม่ใช่กุญแจ) จะถูกเก็บไว้ในแฟ้มข้อมูล คุณต้องกําหนดรายการที่อนุญาต *ผู้เขียน/ คีย์* 'ผ่านคุณสมบัติระดับโลก'[httpGetKeys](#httpgetkeys)
    * ตัวแปร .nset และ .dete พารามิเตอร์ อาจจะเป็นสเกลาร์ (เดี่ยว) ค่าหรืออาร์เรย์ของความยาวใด ๆ ในรูปแบบ\\[ค่า 1, ค่า 2, ค่า 3,..., ค่าN\\]. สําหรับการร้องขอที่กําหนด ตัวแปรทั้งหมดที่มีอาร์เรย์ จะต้องมีอาร์เรย์ที่มีจํานวนเท่ากัน (อื่น ๆ มีข้อผิดพลาด) . หากคําขอมีค่าสเกลาร์และลําดับ, ค่าสเกลาร์จะถูกทําซ้ําเป็นอาร์เรย์ที่มีความยาวเท่ากับอาร์เรย์ที่ระบุ, เช่นstationID=46088 อาจถือว่าเป็น &stationID=\\[46088,46088,46088\\]. อาร์เรย์คือกุญแจสําคัญ[ปรับความดังสูง](#httpget-speed). ถ้าไม่มีอาร์เรย์ มันจะท้าทายกับ .insert หรือ .delete มากกว่า 8 แถวของข้อมูล จากผู้เขียนระยะไกล (เพราะเหนือหัวของเครือข่าย) . ด้วยอาร์เรย์ มันจะง่ายต่อ .insert หรือ .delete มากกว่า 1000 แถวของข้อมูลต่อวินาที จากเซ็นเซอร์ระยะไกล
...
เมื่อ EDTable fromhtppget รับคําสั่ง .insert และ.delete มันจะทําการเพิ่มข้อมูลไปยังแฟ้มที่เกี่ยวข้องในชุดแฟ้มปูมบันทึก ซึ่งแต่ละตัวคือตารางที่ถูกเก็บไว้ใน[เจ สัน แฟ้มข้อความ CSV](https://jsonlines.org/examples/). เมื่อผู้ใช้ร้องขอข้อมูลERDDAP™อ่านแฟ้มปูมบันทึกที่เกี่ยวข้องอย่างรวดเร็ว นําการเปลี่ยนแปลงไปใช้กับชุดข้อมูลลําดับที่แฟ้มถูกสร้างมา แล้วกรองคําขอผ่านเงื่อนไขของผู้ใช้ เช่น อื่น ๆERDDAP™ขอข้อมูล พาร์ทิชันของข้อมูลไปไว้ในแฟ้มปูมบันทึกต่าง ๆ, การเก็บข้อมูลต่าง ๆ (เช่น เวลาประทับของคําสั่ง และคําสั่งเป็น .insert หรือ .delete) และด้านต่าง ๆ ของการตั้งค่าชุดข้อมูล ทั้งหมดทําให้มันเป็นไปได้สําหรับERDDAPเพื่อเก็บข้อมูลและดึงข้อมูลจากชุดข้อมูลนี้ อย่างรวดเร็วและมีประสิทธิภาพมาก
     
#### ความปลอดภัยและผู้เขียน{#security-and-author} 
คําสั่ง .insert และ.delete จะต้องรวม & ผู้เขียน= *ผู้เขียน/ คีย์* เป็นพารามิเตอร์ตัวสุดท้าย ที่ผู้เขียน/ key ถูกคอมไพล์มาจากผู้เขียน (คุณเลือก: ชื่อ, ชื่อย่อ, นามแฝง, ตัวเลข) อุปทาน และกุญแจลับ เดอะERDDAP™ผู้ดูแลระบบจะทํางานร่วมกับผู้เขียน เพื่อสร้างรายการของผู้เขียน/ _key ที่ใช้ได้ ซึ่งสามารถเปลี่ยนแปลงได้ทุกเวลา
เมื่อ EDTable fromhtpppget ได้รับคําสั่ง .insert หรือ .delete มันจะทําให้แน่ใจว่า ผู้เขียน - key เป็นพารามิเตอร์สุดท้ายและใช้ได้ เนื่องจากเป็นพารามิเตอร์สุดท้าย, มันบ่งชี้ว่าบรรทัดคําสั่งทั้งหมดถึงERDDAP™และไม่ได้ถูกตัดสิทธิ์ กุญแจลับทําให้แน่ใจว่า เฉพาะผู้เขียนที่ระบุเท่านั้น ที่สามารถแทรกหรือลบข้อมูลในชุดข้อมูลได้ERDDAP™แล้วแยกหมายเลขผู้เขียนออกมา และบันทึกมันไว้ในตัวแปรของผู้เขียน เพื่อที่ทุกคนจะได้เห็นว่าใครเป็นผู้รับผิดชอบ ต่อการเปลี่ยนแปลงข้อมูลที่ได้รับมา
คําสั่ง .insert และ.delete เท่านั้นที่สามารถทําผ่านhttps:  (ปลอดภัย)  ERDDAP™ที่อยู่ URL นี่ ทํา ให้ แน่ ใจ ว่า ข้อมูล ที่ ได้ รับ การ ถ่ายทอด ถูก เก็บ ไว้ เป็น ความ ลับ ระหว่าง การ ส่ง.
     
#### นาฬิกา{#timestamp} 
...
สั่งการโดยคําสั่งstationID=46088 และเวลา =2016-06-23T19: 533:00Z (และค่าอื่นสําหรับตัวแปรอื่น) .
...
* หากมีข้อมูลสําหรับสถานีดังกล่าวและเวลาดังกล่าวแล้ว ลูกเล่นจะถูกแทนที่แถวของข้อมูลที่มีอยู่ ด้วยข้อมูลใหม่นี้ (แน่นอน ตั้งแต่ERDDAP™เก็บปูมบันทึกของทุกคําสั่งที่ได้รับ ข้อมูลเดิมยังอยู่ในปูมบันทึก หากผู้ใช้ร้องขอข้อมูลจากรุ่นของชุดข้อมูลก่อนที่จะมีการเปลี่ยนแปลงนี้ พวกเขาจะเห็นข้อมูลที่เก่ากว่า)   
         
#### httpหาตัวแบ่งส่วน{#httpgetdirectorystructure} 
*   [httpรับ การ ตรวจ สอบ โครงสร้างแอททริบิวต์ทั่วไป (บันทึกการทํางาน) ชื่อแฟ้ม](#httpgetdirectorystructure)  
...
1. ทําไดเรกทอรีหลักเพื่อเก็บข้อมูลของชุดข้อมูลนี้ สําหรับตัวอย่างนี้ ลองใช้ / datata/estestet/ ผู้ใช้ที่กําลังทํางานสร้าง DatatasetsXml และผู้ใช้กําลังทํางานอยู่ERDDAP™ต้องมีสิทธิ์ในการอ่านและเขียนไปยังไดเรกทอรีนี้
     
2. ใช้เครื่องมือแก้ไขข้อความเพื่อสร้างตัวอย่าง.jsonแฟ้ม lCSV ด้วยส่วนขยาย.jsonL ในไดเรกทอรีนั่น
ชื่อไม่สําคัญ ตัวอย่างเช่น, คุณอาจเรียกมันว่าตัวอย่าง.jsonI
สร้าง 2 บรรทัด.jsonแฟ้ม lCSV, มีชื่อคอลัมน์ในบรรทัดแรก และชื่อจําลอง/ ภาพ (ชนิดของข้อมูลที่ถูกต้อง) ในบรรทัดที่สอง นี่เป็นแฟ้มตัวอย่างที่เหมาะสําหรับชุดสะสมfeatureType=ข้อมูลเวลาที่วัดอุณหภูมิอากาศและน้ํา
    \\[สําหรับfeatureType(เพลง)stationIDเพื่อเป็นวิถีกระสุน\\]  
...
หมายเหตุ:
    ...
    * จะมักจะมีตัวแปรชื่อเวลา ซึ่งบันทึกเวลาที่สังเกตทํา มันอาจจะเป็นข้อมูลที่ผูกติดกับ[แสดงเฉพาะหมายเลขของธาตุ](#string-time-units)  (อี.จี.yyyy-MM-ddT'HH:M:s.SSZ) หรือข้อมูล พิมพ์คู่คู่ด้วย[หน่วยที่เหมาะสําหรับเวลาเศษ](#time-units)  (e.g., วินาทีตั้งแต่ 1970-01-01.00.00:00Z หรือเวลาพื้นฐานอื่น ๆ) .
    ...
    * ผู้เขียนคอลัมน์ที่มีข้อมูลกํากับ จะถูกใช้เพื่อบันทึกว่า ผู้เขียนที่ได้รับอนุญาต ให้แสดงข้อมูลบรรทัดนี้หรือไม่ ผู้เขียนที่ได้รับอนุญาต ได้รับการระบุโดย[httpGetKeys แอตทริบิวต์ทั่วโลก](#httpgetkeys). แม้ว่ากุญแจจะถูกระบุเป็น *ผู้เขียน/ คีย์* และอยู่ในที่อยู่ URL "request" ในรูปแบบนั้น ส่วนของผู้เขียนเท่านั้นที่ถูกบันทึกไว้ในแฟ้มข้อมูล
...
3. ประมวลผลแฟ้ม Datata XMI และบอกมัน
    ...
    4. เดอะhttpได้ รับ การ ตอบ รับ ตัวแปรคือ (สําหรับตัวอย่างนี้)  stationIDเวลา ดูคําอธิบายของ[httpรับ การ ปรับ ตั้ง ใหม่](#httpgetrequiredvariables-global-attribute)ข้างล่าง
    5. ถ้าเก็บข้อมูลทุก 5 นาทีhttpหาตัวแบ่งประเภทสําหรับตัวอย่างนี้stationID2 เดือน ดูคําอธิบายของ[httpหาตัวแบ่งส่วน](#httpgetdirectorystructure)ข้างล่าง
    6. เดอะ[httpGetKeys](#httpgetkeys)
    
เพิ่มการแสดงผล (ชิ้นส่วนของdatasets.xmlสําหรับชุดข้อมูล) ถึงdatasets.xml.
     
4. แก้ไขdatasets.xmlชิ้นส่วนสําหรับชุดข้อมูลนี้ เพื่อให้มันถูกต้องและเสร็จสมบูรณ์
...
5. สําหรับ&lt;Name=ไดเรกทอรีName
    ...
    * ตั้งเป็นเท็จ (ค่าปริยาย) หากชุดข้อมูลจะได้รับใน .insert และ/หรือ .delete ต้องการ (e.g. น้อยกว่าหนึ่งครั้งทุกๆ 10 วินาที) .
         
6. หมายเหตุ: มันเป็นไปได้ที่จะใช้&lt;แคชจากUrl&gt; และการตั้งค่าที่เกี่ยวข้องในdatasets.xmlสําหรับ EDTable จาก Http รับชุดข้อมูลเป็นวิธีทําและรักษาสําเนาท้องถิ่นของ IDTable fromhtpget ข้อมูลบนอื่นERDDAP. อย่างไรก็ตาม ในกรณีนี้ ชุดข้อมูลท้องถิ่นนี้ จะปฏิเสธการร้องขอ .insert และ .delete
...
* ผู้ใช้ยังสามารถทําการร้องขอที่เฉพาะกับ EDTable fromhtppetsets:
    * ถ้าคําขอนี้รวม&lt;หรือ&lt;=ข้อจํากัดของคอลัมน์เวลา, แล้วERDDAP™ประมวลผลแถวของแฟ้มปูมบันทึกขึ้นจนถึงวันที่ระบุ ผลก็คือ นี่ลบการเปลี่ยนแปลงทั้งหมด ที่เกิดขึ้นกับชุดข้อมูลชั่วคราว ตั้งแต่ค่าของเวลานั้น สําหรับข้อมูลเพิ่มเติม ดู[รุ่น](#versioning).
...
สําหรับการร้องขอ .insert หรือ .dete (ไม่นับhttpลบทั้งเธรด) ค่าความไวของ .insert หรือ .delete
...
* มีประโยชน์อย่างมากในการใช้แฟ้มมาตรฐานที่ใช้ได้ของ admin (เทียบกับฐานข้อมูลความสัมพันธ์ หรือคาสเซนดร้า หรือซอฟท์แวร์อื่น) . ไม่มีซอฟต์แวร์อื่น ๆ ที่จะต้องบํารุงรักษา และจะต้องดําเนินการเพื่อเก็บหรือรับข้อมูล และมันง่ายที่จะสํารองข้อมูลแฟ้มมาตรฐานที่เวลาใด ๆ และในการเพิ่มขึ้นเพราะข้อมูลอยู่ในชิ้น (เมื่อเวลาผ่านไป เพียงแฟ้มเวลาปัจจุบันของแต่ละสถานีจะมีการเปลี่ยนแปลง) . ใน ทาง ตรง กัน ข้าม ต้อง ใช้ ความ พยายาม และ ระบบ อย่าง มาก เพื่อ จะ มี สํารอง ข้อมูล จาก ฐานข้อมูล และ จาก คาส ซาน.
         
##### ความจุของระบบ{#system-reliability} 
มันสมเหตุสมผลที่จะคาดหวังว่าหนึ่งเซิร์ฟเวอร์ด้วยERDDAP™เพื่อให้เวลาเพิ่มขึ้น 99.9% -- นั่นคือเวลาลดลงประมาณ 9 ชั่วโมงต่อปี (แม้ ว่า คุณ จะ ใช้ มัน ได้ ใน คืน ที่ ไม่ ดี คืน เดียว&#33;) .
...
แดเนียลส์ เอ็ม. ดี., เคอร์เคซ, บี. จันทราเซก้า, วี., เกรฟส์, เอส., สตรอมส์, เอส., มาร์ติน, ซี., ดี., เอ็ม. (2014) . บริการข้อมูลเรียลไทม์ของจีโอไซลด์ (CHORDS) ซอฟท์แวร์ UCAR/NCAR - ห้องควบคุมโรคโลก[ https://doi.org/10.5065/d6v1236q ](https://doi.org/10.5065/d6v1236q)  
     
### จากHyraxแฟ้ม{#eddtablefromhyraxfiles} 
[ **จากHyraxแฟ้ม** ](#eddtablefromhyraxfiles)  (หมดอายุแล้ว) ผนวกรวมแฟ้มข้อมูลด้วยตัวแปรหลายค่า แต่ละแฟ้มมีหนึ่งมิติที่ใช้ร่วมกันหรือมากกว่า (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) และรับใช้โดย[Hyrax OPeNDAPเซิร์ฟเวอร์](https://www.opendap.org/software/hyrax-data-server).
...
* สําหรับ&lt;sourceUrl&gt; ใช้ที่อยู่ URL ของไดเรกทอรีพื้นฐานของชุดข้อมูลในHyraxตัวอย่างเช่น แม่ข่าย
...
เดอะsourceUrlหน้าเว็บปกติจะมี "OPeNDAPดัชนีของแม่ข่าย\\[ไดเร็กทอรีName\\]"ที่ด้านบน
...
[ **แฟ้ม DDTable จาก Multiidnc** ](#eddtablefrommultidimncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .nc  (หรือ[.ncmI](#ncml-files)) ไฟล์ที่มีตัวแปรหลายอย่าง แต่ละแบบมีมิติร่วมกันหรือมากกว่า แฟ้มอาจมีตัวแปรที่มี หรือไม่มีมิติเพิ่มเติม (ยกตัวอย่างเช่น STRING14) . ดูคลาสนี้สิ[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน

* ถ้าแฟ้มเป็นแฟ้มแบบหลายมิติแบบ CF DSG, ให้ใช้ประเภทชุดข้อมูลนี้แทน[แฟ้ม DDTable จาก NCF](#eddtablefromncfiles).
     
* สําหรับข้อมูลแท็บใหม่.ncแฟ้ม, ใช้ตัวเลือกนี้ก่อนที่จะลองใหม่[แฟ้ม DDTAB จาก Nc](#eddtablefromncfiles). ข้อดีของคลาสนี้คือ
...
ข้อความ "DimessionCSV" กลายเป็น&lt;ขนาด CSV&gt; ในdatasets.xmlข้อมูลชุดข้อมูล
...
[ **แฟ้ม DDTAB จาก Nc** ](#eddtablefromncfiles)ข้อมูลรวมจากNetCDF  (v3 หรือ v4)  .nc  (หรือ[.ncmI](#ncml-files)) แฟ้มและ[ซาร์](https://github.com/zarr-developers/zarr-python)แฟ้ม (รุ่น 2.25) กับตัวแปรหลาย, แต่ละที่มีหนึ่งมิติร่วมกัน (ยกตัวอย่างเช่น เวลา) หรือมากกว่าหนึ่งมิติที่ใช้ร่วมกัน (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) . แฟ้มต้องมีชื่อมิติเดียวกัน แฟ้มที่กําหนดให้อาจมีหลายขนาดของแต่ละมิติ และค่าต่าง ๆ กันในแฟ้มต้นฉบับ แฟ้มอาจมีตัวแปรที่มีมิติเพิ่มเติม (ยกตัวอย่างเช่น STRING14) . ดูคลาสนี้สิ[เลือกแฟ้ม](#eddtablefromfiles)สําหรับข้อมูลการทํางานของคลาสนี้ และวิธีการใช้มัน

แฟ้ม Zar มีพฤติกรรมที่แตกต่างกันเล็กน้อย และต้องการทั้งชื่อแฟ้มRegex หรือพาธ regex เพื่อรวม "zar"

* ถ้า.ncแฟ้มใช้เป็นหนึ่งในแฟ้ม[CF แว่นขยาย (แบบ DSG) ](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)รูปแบบแฟ้ม, ลองใช้[แฟ้ม DDTable จาก NCF](#eddtablefromncfiles)ก่อนที่จะลองนี่
     
* สําหรับข้อมูลแท็บใหม่.ncแฟ้ม, ลองใหม่[แฟ้ม DDTable จาก Multiidnc](#eddtablefrommultidimncfiles)ก่อน
     
* เราขอแนะนําให้ใช้[ตั้งค่าชุดภาพไอคอนสื่ออารมณ์ โปรแกรม Xml](#generatedatasetsxml)เพื่อให้ฉบับหยาบของdatasets.xmlรวมเข้ากับชุดข้อมูล จากนั้นคุณสามารถแก้ไขที่ปรับมัน
...
[ **อิเล็กทรอนิกส์** ](#eddtablefromnos)  (ขอบเขต) จับข้อมูลจากNOAA [โหนด](https://opendap.co-ops.nos.noaa.gov/axis/)แหล่งที่มาซึ่งใช้[SOAP+XML](https://www.w3schools.com/xml/xml_soap.asp)สําหรับคําขอและการตอบสนอง มันเจาะจงมากกับNOAANOS ของ XML ดูตัวอย่าง EDTable fromNOS ในชุดข้อมูล 2xml
 
...
* (คําเตือน: เมื่อใด)ERDDAP™อ่านแฟ้มข้อมูล Parquett หากมันพบข้อผิดพลาดบนบรรทัดที่กําหนด (เช่น, จํานวนรายการไม่ถูกต้อง) มันบันทึกข้อความเตือน ("WARINNG: บรรทัดที่ไม่ดี (วินาที) ของข้อมูล"... กับรายการของบรรทัดที่ไม่ดีในบรรทัดหลัง) ถึง[แฟ้มปูมบันทึก.txt](/docs/server-admin/additional-information#log)แล้วก็อ่านข้อมูลที่เหลือต่อไป ดัง นั้น คุณ ต้อง ดู แล เป็น ระยะ ๆ (หรือเขียนสคริปต์เพื่อทําเช่นนั้น) สําหรับข้อความในบันทึก txt เพื่อให้คุณสามารถแก้ปัญหาในแฟ้มข้อมูลได้ERDDAP™มีการตั้งค่าแบบนี้เพื่อให้ผู้ใช้สามารถอ่านข้อมูลทั้งหมดที่ถูกต้องได้ แม้ว่าบางบรรทัดของแฟ้มจะมีข้อบกพร่องก็ตาม
     
### จากSOS {#eddtablefromsos} 
[ **จากSOS** ](#eddtablefromsos)จัดการข้อมูลจากบริการ Observation (เอสอี/[SOS](https://www.ogc.org/standards/sos)) เซิฟเวอร์
...
*   SOSภาพรวม:
    * แบบ SWE (เปิดใช้งานเว็บตัวตรวจจับ) ถึงSOS  (บริการ Observation ของตัวตรวจจับ) คือ[มาตรฐาน OpenGISH](https://www.ogc.org/standards). เว็บไซต์นั้นมีเอกสารมาตรฐาน
...
*   SOSเครื่องแม่ข่ายปกติจะแสดง[หน่วย](#units)กับ[UCUM](https://unitsofmeasure.org/ucum.html)ระบบ มากERDDAP™หน่วยด่วนเซิร์ฟเวอร์พร้อมบริการ[UDUNITS](https://www.unidata.ucar.edu/software/udunits/)ระบบ ถ้าคุณต้องการที่จะแปลงระหว่างระบบทั้งสอง คุณสามารถใช้[ERDDAPบริการเว็บที่จะแปลงหน่วย UCUM เป็น/ จากUDUNITS](https://coastwatch.pfeg.noaa.gov/erddap/convert/units.html).
#### จากSOSโครงสร้าง XML{#eddtablefromsos-skeleton-xml} 
...
[ **แฟ้ม EDTIVE จาก Thredds February** ](#eddtablefromthreddsfiles)  (หมดอายุแล้ว) ผนวกรวมแฟ้มข้อมูลด้วยตัวแปรหลายค่า แต่ละแฟ้มมีหนึ่งมิติที่ใช้ร่วมกันหรือมากกว่า (ยกตัวอย่างเช่น เวลา ความสูง (ความลึก) ละติจูด ลองติจูด) และรับใช้โดย[(เต๋า)OPeNDAPเซิร์ฟเวอร์](https://www.unidata.ucar.edu/software/tds/).
...
* ดูตัวอย่าง 1D, 2D, 3D และ 4 มิติ[แฟ้ม DDTAB จาก Nc](#eddtablefromncfiles).
     
### จากWFSแฟ้ม{#eddtablefromwfsfiles} 
[ **จากWFSแฟ้ม** ](#eddtablefromwfsfiles)  (ขอบเขต) ทําสําเนาข้อมูลทั้งหมดจากArcGISเซิร์ฟเวอร์แผนที่WFSเซิร์ฟเวอร์ เพื่อให้ข้อมูลสามารถถูกตรวจสอบใหม่ได้อย่างรวดเร็วERDDAP™ผู้ใช้
...
* เด็กแต่ละคน&lt;ชุดข้อมูลที่ระบุไว้: ต้องเป็นชุดข้อมูลที่สมบูรณ์ ราวกับว่ามันเป็นชุดข้อมูลมาตรฐาน แต่ละคนต้องมีเหมือนกัน[dataVariableวินาที](#datavariable)ในลําดับเดียวกัน กับ[destinationNameวินาที](#destinationname).[ข้อมูล ชนิด](#datatype).[missing\\_valueวินาที](#missing_value).[_สัญญาอนุญาต](#missing_value)และ[หน่วย](#units). ข้อมูลกํากับภาพของแต่ละตัวแปรสําหรับ EDTABEAGGERows languages มาจากตัวแปรต่าง ๆ ในชุดข้อมูลเด็กชุดแรก แต่ EDTAGGGEGRows จะปรับปรุงข้อมูล[actual\\_range](#actual_range)ข้อมูลกํากับภาพเป็นช่วงของเด็กๆทุกคน
...
* โปรแกรมคัดลอก DDTable (และสําหรับข้อมูลกริด[EDDGridคัดลอก](#eddgridcopy)) เป็นการใช้ง่ายและมีประสิทธิภาพมาก **คําตอบของปัญหาที่ใหญ่ที่สุดบางข้อ กับการให้บริการข้อมูลจากแหล่งข้อมูลระยะไกล** 
    * การเข้าถึงข้อมูลจากแหล่งข้อมูลระยะไกล อาจช้า
...
ข้อมูลแต่ละชิ้นจะเก็บไว้ในแยกNetCDFแฟ้มในส่วนย่อยของ *เครื่อง มือ ขนาด ใหญ่* / รับทราบ / *datasetID* / (ตามที่กําหนดไว้ใน[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)) . มีระดับทิศทางย่อยหนึ่งสําหรับทั้งหมด แต่มีการสกัดตัวสุดท้ายName ตัวอย่างเช่น ข้อมูลสําหรับ Tig17+1017 จะถูกเก็บไว้
...
* เปลี่ยนข้อมูลกํากับภาพ -- ถ้าคุณต้องการเปลี่ยนaddAttributesหรือเปลี่ยนลําดับตัวแปรที่เกี่ยวข้องกับชุดข้อมูลแหล่ง:
    ...
    4. แฟ้มที่ถูกลบออกไปจะถูกสร้างขึ้นมาใหม่โดยใช้ข้อมูลกํากับภาพตัวใหม่ หากไม่มีชุดข้อมูลแหล่งข้อมูล DDTable ชุดข้อมูลการคัดลอก (DDTable Datrap) จะรับข้อมูลกํากับจากแฟ้มที่มีการสร้างขึ้นใหม่ เนื่องจากเป็นแฟ้มที่อายุน้อยที่สุด
         
*   [EDDGridคัดลอก](#eddgridcopy)คล้ายกันมากกับการคัดลอกแบบ EDTable แต่ทํางานกับชุดข้อมูลปลอม
...
* [ ** &lt;AngularDience University) ** ) (หน่วยองศาเชิงกราน) เป็นป้ายที่ใช้ได้น้อยมาก&lt;Dadapdatatets) ป้ายในdatasets.xmlซึ่งมีรายการของสายหน่วยที่แยกเครื่องหมายจุลภาคERDDAP™ควรดูแลเป็นหน่วยองศาเชิงกราน ถ้าตัวแปรหนึ่งมีหนึ่งหน่วยtabledap'orderByMeanกรองจะคํานวณค่าเฉลี่ยในรูปแบบพิเศษ แล้วรายงานค่าเฉลี่ยเป็นค่าจาก - 180 ถึง 180 ดูERDDAPแฟ้มต้นฉบับของ EDStic.java สําหรับรายการปริยายปัจจุบัน การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag).
### &lt;AngularDescription University&gt;{#angulardegreetrueunits} 
* [ ** &lt;มุม: (สัญญา) ** ) (ความเป็น จริง) เป็นป้ายที่ใช้ได้น้อยมาก&lt;Dadapdatatets) ป้ายในdatasets.xmlซึ่งมีรายการของสายหน่วยที่แยกเครื่องหมายจุลภาคERDDAP™ควรปฏิบัติต่อหน่วยที่เป็นมุมจริง ถ้าตัวแปรหนึ่งมีหนึ่งหน่วยtabledap'orderByMeanกรองจะคํานวณค่าเฉลี่ยในรูปแบบพิเศษ แล้วรายงานค่าเฉลี่ยเป็นค่าจาก 0 ถึง 360 ดูERDDAPแฟ้มต้นฉบับของ EDStic.java สําหรับรายการปริยายปัจจุบัน การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag).
     
### &lt;Standard ทั่วไปName{#commonstandardnames} 
* [ ** &lt;Standard ทั่วไปName ** ) (ชื่อแฟ้มมาตรฐาน) เป็นป้ายที่ใช้ได้น้อยมาก&lt;Dadapdatatets) ป้ายในdatasets.xmlเพื่อกําหนดรายการของเครื่องหมายจุลภาค[ชื่อมาตรฐานของ CF](https://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table.html). อี.จี.
```
...
ถ้าคุณต้องการให้ข้อมูลนี้datasets.xmlเริ่มการทํางานโดยคัดลอกรายการปริยายปัจจุบัน&lt;DEFALULL\\_คอมมอน สแตนดาร์ดName ในERDDAP'
...
```
โดยทั่วไปแล้ว จะมีแต่แฟ้มภาพเท่านั้น (เพราะภาพเดียวกันนี้มักจะถูกร้องขอซ้ํา) ถึง.ncแฟ้ม (เนื่องจากมันต้องถูกสร้างอย่างเต็มที่ก่อนจะทําการส่งไปยังผู้ใช้) มีแคช แม้ว่ามันดูเหมือนว่าคําขอที่ให้ ควรตอบกลับเหมือนเดิม, นั่นไม่จริง ยกตัวอย่างเช่นtabledapร้องขอที่รวมถึงเวลา) *บาง เวลา* จะเปลี่ยนเมื่อข้อมูลใหม่มาถึง สําหรับชุดข้อมูล และคําขอแบบตารางดาป ซึ่งรวมไปถึง\\[สุดท้าย\\]สําหรับมิติเวลาจะมีการเปลี่ยนแปลง เมื่อข้อมูลใหม่มาถึงสําหรับชุดข้อมูล การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). ก่อนERDDAP™v2.00, นี่เป็นการกําหนดในการตั้งค่า.xml, ซึ่งยังคงอนุญาตให้ใช้ได้ แต่ท้อแท้

### &lt;มวล{#cacheclearminutes} 
* [ ** &lt;แคช ** ) (# Cachox minute #) เป็นป้ายที่ใช้ได้น้อยมาก&lt;Dadapdatatets) ป้ายในdatasets.xmlเพื่อกําหนดความถี่ในการตรวจสอบแฟ้มแคชและลบแฟ้มเก่า (นาที)   (ค่าปริยาย=15) . อี.จี.
```
    <cacheClearMinutes>15</cacheClearMinutes>  
```
เมื่อเซิร์ฟเวอร์เสร็จสิ้น การจัดการการร้องขอ จะตรวจสอบว่า แคชล่าสุดชัดเจนแค่ไหน ถ้ามันนานเกินไป มันก็จะกําหนดภารกิจบนเนื้อเรื่อง การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). คุณสามารถระบุมันได้ในการตั้งค่า. xml, แต่เกิดความท้อแท้
     
...
ไปสู่สาธารณะชนsourceUrl  ( https://oceanwatch.pfeg.noaa.gov/thredds/dodsC/satellite/BA/ssta/5day ) .
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag).

แต่ด้วยเหตุผลด้านความปลอดภัย และเหตุผลที่เกี่ยวข้องกับระบบการสมัครสมาชิก **อย่าใช้กล่องนี้&#33;**   
แต่ใช้ชื่อโดเมนแทนเสมอ&lt;sourceUrlแท็ก &gt; และใช้[ตาราง/ เครื่อง](https://linux.die.net/man/5/hosts)บนเซิร์ฟเวอร์ของคุณ เพื่อแปลงชื่อโดเมนท้องถิ่นเป็นหมายเลข IP โดยไม่ต้องใช้เซิร์ฟเวอร์ DNS คุณสามารถทดสอบว่าชื่อโดเมนถูกแปลงเป็นหมายเลข IP ได้อย่างถูกต้องโดยการใช้:
...
*   [ **drawLandMask** ](#drawlandmask)กําหนดค่าที่ตั้งปริยายสําหรับควบคุมว่าจะให้วาดพื้นที่หน้าจอเมื่อไรและอย่างไร เมื่อใดERDDAP™วาดแผนที่ สามารถระบุได้ 3 ตําแหน่งdatasets.xml  (เรียงลําดับจากต่ําที่สุดถึงระดับความสําคัญสูงสุด) .
    ...
    ```
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAPอ่านdatasets.xml.
หากป้ายนี้ยังไม่ปรากฏ ค่าโดยปริยายจะต่ํากว่า
         
    2. ถ้าdrawLandMaskได้รับการกําหนดเป็นแอททริบิวต์ทั่วไปของชุดข้อมูลที่กําหนด จากนั้นมันจะใช้กําหนดค่าปริยายของdrawLandMaskสําหรับตัวแปรทั้งหมดในชุดข้อมูลนั้น, แทนค่าลําดับความสําคัญที่ต่ํากว่า ยกตัวอย่างเช่น
    ```
        <att name="drawLandMask">under</att>  
    ```
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™โหลดชุดข้อมูลนั้นใหม่
         
    3. ถ้าdrawLandMaskระบุเป็นแอททริบิวต์ของตัวแปรในชุดข้อมูลที่กําหนด, แล้วมันได้กําหนดค่าปริยายของdrawLandMaskสําหรับตัวแปรนั้นในชุดข้อมูลนั้น, แทนค่าลําดับความสําคัญที่ต่ํากว่า ยกตัวอย่างเช่น
    ```
        <att name="drawLandMask">under</att>  
    ```
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™โหลดชุดข้อมูลนั้นใหม่
...
```
โดยทั่วไปแล้ว, นี่ควรกําหนดเป็นอย่างน้อยสองครั้ง ตราบใดที่คุณคิดอย่างมีเหตุผล ว่าการโหลดชุดข้อมูลทั้งหมดใหม่ (สะสม) น่าจะใช้เวลา (เนื่องจากคอมพิวเตอร์และเครือข่าย บางครั้งช้ากว่าที่คาดไว้) นี่น่าจะนานกว่าการโหลด Datatets Mindumuts ค่าปริยายคือ 60 นาที บางคนจะทําให้เรื่องนี้มันยาวขึ้น การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). ก่อนERDDAP™v2.00, นี่เป็นการกําหนดในการตั้งค่า.xml, ซึ่งยังคงอนุญาตให้ใช้ได้ แต่ท้อแท้
     
### &lt;LodLevil&gt;{#loglevel} 
...
```
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). ก่อนERDDAP™v2.00, นี่เป็นการกําหนดในการตั้งค่า.xml, ซึ่งยังคงอนุญาตให้ใช้ได้ แต่ท้อแท้
     
...
คุณสามารถยกเลิกขนาดท่อนโดยปริยายได้ โดยการนิยามป้ายกํากับเหล่านี้datasets.xml  (ด้วยค่าที่แตกต่างจากที่แสดงไว้ที่นี่) .
...
การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). ก่อนERDDAP™v2.00, สิ่งเหล่านี้ถูกระบุในการตั้ง.xml, ซึ่งยังคงอนุญาตให้ทําได้ แต่ท้อแท้
     
### &lt;ร้องขอ Blacklist&gt;{#requestblacklist} 
* [ ** &lt;ร้องขอ Blacklist) ** ) (# ชื่อเล่นคือ แบล็คลิสต์)  [เป็นป้ายกํากับ](/docs/server-admin/additional-information#frequent-crashes-or-freezes)ภายใน&lt;Dadapdatatets) ป้ายในdatasets.xmlซึ่งมีรายการหมายเลข IP ที่แยกมาจากเครื่องหมายจุลภาค ซึ่งจะถูกใช้แทนชื่อ การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag).
    ...
    * ผู้ใช้ที่มีปัญหา -- ถ้าERDDAP™การ ทํา เช่น นี้ อาจ ทํา ให้ เกิด ปัญหา ได้. ดูใน[ปูมบันทึก. txt](/docs/server-admin/additional-information#log)เพื่อดูว่านี่เป็นจริงหรือไม่ และเพื่อค้นหาหมายเลขไอพีที่อยู่ของผู้ใช้ที่มีปัญหา ถ้านี่คือปัญหา คุณควรจะให้ผู้ใช้คนนั้นเป็นแบล็คลิสต์
...
ผู้ ใช้ มัก เพียง แต่ ไม่ รู้ ว่า คํา ขอ ของ เขา เป็น ปัญหา. พวก เขา มัก จะ ไม่ รู้ จัก แมลง, ความ ไม่ เหมาะ สม, หรือ ปัญหา อื่น ๆ ที่ เกี่ยว ข้อง กับ สคริปต์ ของ ตน. พวกเขามักจะคิดแบบนั้นเพราะERDDAP™เสนอข้อมูลให้ฟรี โดยพวกเขาสามารถขอข้อมูลได้มากเท่าที่ต้องการ เช่น โดยการเรียกใช้งานสคริปต์หลายๆ ตัว หรือใช้หลายเธรดพร้อมกัน
    ...
    * ยกตัวอย่างเช่น
    ...
    ```
    * คุณไม่ต้องเริ่มใหม่ก็ได้ERDDAP™สําหรับการเปลี่ยนแปลงไปยัง&lt;ขอให้แบล็คลิสต์) มีผลกระทบ จะตรวจพบการเปลี่ยนแปลงในครั้งต่อไปERDDAP™ตรวจสอบว่าชุดข้อมูลใด ๆ ที่ต้องการจะเรียกใหม่อีกครั้งหรือไม่ หรือ คุณสามารถเร่งกระบวนการได้โดยไปเยี่ยม[ตั้งค่า ที่อยู่ URL ธง](/docs/server-admin/additional-information#set-dataset-flag)สําหรับชุดข้อมูลใดๆ
...
* [ ** &lt;สมาชิก อีเมล Blacklist) ** ) (# รองรับสคริปต์) เป็นป้ายที่ใช้ได้น้อยมาก&lt;Dadapdatatets) ป้ายในdatasets.xmlซึ่งมีรายการที่อยู่อีเมลที่แยกด้วยเครื่องหมายจุลภาค ซึ่งอยู่ในรายการสีดําทันที[ระบบบอกรับ](https://coastwatch.pfeg.noaa.gov/erddap/subscriptions)ตัวอย่างเช่น
    ```
    <subscriptionEmailBlacklist>bob@badguy.com, john@badguy.com</subscriptionEmailBlacklist>  
    ```
นี่เป็นระบบที่ไวต่อสถานการณ์ หากอีเมลถูกเพิ่มเข้าไปในรายการนี้ หากที่อยู่อีเมลดังกล่าวมีการบอกรับ สมาชิกก็จะถูกยกเลิก หากที่อยู่อีเมลในรายการพยายามสมัครสมาชิก การร้องขอจะถูกปฏิเสธ การเปลี่ยนแปลงใด ๆ ของค่าป้ายกํากับนี้จะมีผลในครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag).
     
### ข้อความมาตรฐาน{#standard-text} 
*   [ **ข้อความมาตรฐาน** ](#standard-text)-- มีป้ายกํากับหลายแบบ (ส่วนใหญ่มักไม่ได้ใช้) ภายใน&lt;Dadapdatatets) ป้ายในdatasets.xmlแสดงเฉพาะหมายเลขที่ปรากฏในตําแหน่งต่าง ๆERDDAP. หากคุณต้องการเปลี่ยนข้อความปริยาย โปรดคัดลอกค่าเดิมจากป้ายชื่อเดียวกัน
     *แว่นขยาย* / webapps/erdep/WEB-INF/คลาสต่างๆ/gov/noa/pfel/erdep/util. exml ถึงdatasets.xmlแล้วแก้ไขเนื้อหา ข้อได้เปรียบของการมีมันdatasets.xmlคือคุณสามารถกําหนดค่าใหม่ได้ทุกเวลา แม้เมื่อERDDAP™กําลังวิ่ง การเปลี่ยนแปลงต่าง ๆ ของป้ายกํากับเหล่านี้จะมีผลกับครั้งต่อไปERDDAP™อ่านdatasets.xmlร่วมกับชุดข้อมูล[ธง](/docs/server-admin/additional-information#flag). ชื่อป้ายบรรยายวัตถุประสงค์ของมัน แต่ดูที่เนื้อหาปริยายในจดหมาย.xml เพื่อความเข้าใจที่ลึกกว่า
...
* [ ** &lt;ผู้ใช้: ** ) (ผู้ใช้) เป็นเครื่องหมายการค้าภายใน&lt;Dadapdatatets) ป้ายในdatasets.xmlที่ระบุชื่อผู้ใช้, รหัสผ่านของผู้ใช้ (หากการตรวจสอบสิทธิ์=ชื่อแฟ้ม) และบทบาท (รายการแยกจุลภาค) . การใช้ชื่อผู้ใช้และรหัสผ่านต่าง ๆ จากค่าเล็กน้อย&lt;การตรวจสอบสิทธิ์) (/ docs/ server-admin/Adctional Indactation#) ในของคุณERDDAPกําลังตั้งค่าแฟ้ม.xml
    * นี่เป็นส่วนหนึ่งของERDDAP'[ระบบรักษาความปลอดภัย](/docs/server-admin/additional-information#security)เพื่อจํากัดการเข้าถึงชุดข้อมูลบางส่วนกับผู้ใช้บางคน
...
สําหรับการตรวจสอบสิทธิ์=google, ชื่อผู้ใช้คือที่อยู่อีเมลเต็มของผู้ใช้ นี่รวมถึงบัญชีแบบ Google-manage เช่น@noaa.govบัญชี
...
ขนาด md5 - djtit:ERDDAPคํานําหน้าของฉัน
...
สมมุติว่าเรามีโครงสร้างไดเรกทอรีต่อไปนี้:
    ...
    ...  
    ```
และไดเรกทอรีที่ระบุคือ /foo/bar/ และเราแค่ต้องการ.ncแฟ้มใน D\\[0- 9\\]2561. สืบค้นเมื่อ 22 พฤษภาคม พ.ศ.
...
* [ ** &lt;ชุดข้อมูล: ** ) (# ดาตาเซต) (คือ) บรรดาผู้บันทึก (แต่ใช้เสมอ) แท็กภายใน&lt;Dadapdatatets) ป้ายในdatasets.xmlที่ (ถ้าคุณรวมข้อมูลทั้งหมดระหว่าง&lt;และ&lt;/ datatets) อธิบายข้อมูลได้สมบูรณ์เพียงชุดเดียว ยกตัวอย่างเช่น
    ...
    ```
อาจมีป้ายกํากับชุดข้อมูลอยู่ในของคุณdatasets.xmlแฟ้ม
สามคุณลักษณะที่ปรากฏภายใน&lt;แท็ก:
     
    *    **ประเภท=" *คือ ชนิด* "** ในนั้นมีสิ่งก่อสร้าง&lt;แท็กในdatasets.xmlซึ่งระบุประเภทชุดข้อมูล (ตัวอย่างเช่น ไม่ว่าจะเป็นEDDGridข้อมูลต่าง ๆ ของกล้อง) และที่มาของข้อมูล (ตัวอย่างเช่น ฐานข้อมูล แฟ้ม หรือระยะไกลOPeNDAPเซิร์ฟเวอร์) . ดู[ **รายการชนิดชุดข้อมูล** ](#list-of-types-datasets).
         
...
หลังจากโหลดครั้งถัดไป ชุดข้อมูล คุณสามารถลบป้ายกํากับดังกล่าวหลังจากที่ชุดข้อมูลเก่าไม่ได้ใช้งาน
                 
#### ทํางาน{#active} 
*   [ **ใช้งานอยู่=" *บูเลียน* "** ](#active)ในดังกล่าวนั้นเป็นการสาบาน สําหรับผู้ที่มีปัญญามิใช่หรือ?&lt;แท็กในdatasets.xmlซึ่งแสดงว่าชุดข้อมูลทํางาน (คุณสมบัติที่จะใช้ในERDDAP) หรือไม่
    ...
    * ในการลบชุดข้อมูลออกจากERDDAP™ดู[บังคับให้เอาชุดข้อมูลออก](/docs/server-admin/additional-information#removing-datasets).
         
...
* [ ** &lt;เข้าถึงแฟ้ม Via) ** ) (แฟ้มที่อ่านได้) เป็นเครื่องหมายการค้าภายใน&lt;แท็กในdatasets.xmlสําหรับ[EDDGridการ แบ่ง แยก](#eddgridaggregateexistingdimension).[EDDGridคัดลอก](#eddgridcopy).[EDDGridดึงข้อมูล](#eddgridfromeddtable).[EDDGridจาก เอลด์ดาป](#eddfromerddap).[EDDGridจาก เอ โต โป](#eddgridfrometopo).[EDDGridจากแฟ้ม](#eddgridfromfiles)  (รวมทั้งคลาสย่อยทั้งหมด) .[EDDGridด้านข้าง](#eddgridsidebyside).[โปรแกรมคัดลอก DDTable](#eddtablecopy) [DDTable from Edardp](#eddfromerddap).[จากEDDGrid](#eddtablefromeddgrid)และ[เลือกแฟ้ม](#eddtablefromfiles)  (รวมทั้งคลาสย่อยทั้งหมด) ชุดข้อมูล อาจ มี คุณค่า ของ ความ จริง หรือ ความ เท็จ. ยกตัวอย่างเช่น
    ```
    <accessibleViaFiles>true</accessibleViaFiles>  
    ```
ถ้าค่านั้นเป็นจริงERDDAP™จะทําให้ผู้ใช้สามารถเรียกดูและดาวน์โหลดข้อมูลจากฐานข้อมูลได้ผ่านทางERDDAP'["files"ระบบ](https://coastwatch.pfeg.noaa.gov/erddap/files/). ดู"files"ระบบ[เอกสาร](https://coastwatch.pfeg.noaa.gov/erddap/files/documentation.html)เพื่อข้อมูลเพิ่มเติม
...
คําแนะนํา-- เราขอแนะนําให้ทําชุดข้อมูลที่เกี่ยวข้องทั้งหมด เข้าถึงผ่านทางระบบแฟ้มโดยการตั้งค่า&lt;ค่าปริยาย AccessableViaFiles) เพื่อเป็นค่าจริงในการตั้งค่า.xml เนื่องจากมีกลุ่มผู้ใช้ซึ่งนี่เป็นวิธีมากกว่าในการรับข้อมูล หนึ่งในเหตุผลอื่น ๆ ที่"files"ระบบทําให้ผู้ใช้เห็นว่ามีแฟ้มไหนอยู่ และได้เปลี่ยนแปลงล่าสุด ทําให้ผู้ใช้สามารถรักษาสําเนาข้อมูลทั้งหมดของตัวเองได้ง่ายขึ้น หากคุณไม่ต้องการสร้างชุดข้อมูลต่าง ๆ ผ่านทางระบบแฟ้ม ให้ตั้งค่า&lt;เป็นเท็จ ไม่ว่ากรณีใด ใช้&lt;AccessVia Files) สําหรับชุดข้อมูลไม่กี่ชุด ซึ่งยกเว้นนโยบายทั่วไปที่ตั้งขึ้นโดย&lt;ตัวกรองปริยาย: (ตัวอย่างเช่น เมื่อชุดข้อมูลใช้[.ncmI](#ncml-files)แฟ้มซึ่งใช้ไม่ได้กับผู้ใช้) .
     
#### &lt;เข้าใช้ได้ ผ่านWMS& gt;{#accessibleviawms} 
* [ ** &lt;เข้าใช้ได้ ผ่านWMS&gt; ** ) (สัตว์ป่าที่ควบคุมได้) เป็นเครื่องหมายการค้าภายใน&lt;แท็กในdatasets.xmlสําหรับทุกคน[EDDGrid](#eddgrid)ชั้นประถมศึกษา มันมีค่าเป็นจํานวนจริง (ค่าปริยาย) หรือไม่จริง ยกตัวอย่างเช่น
    ```
    <accessibleViaWMS>true</accessibleViaWMS>  
    ```
ถ้ามูลค่าเป็นเท็จERDDAP'WMSจะไม่สามารถใช้งานแม่ข่ายสําหรับชุดข้อมูลนี้ได้ นี้มักใช้กับชุดข้อมูลที่มีเส้นแวงมากกว่า 180 (ซึ่งทางเทคนิคแล้ว ใช้งานไม่ได้สําหรับWMSบริการ) และเพื่อให้คุณกําลังนําเสนอความต่อเนื่องของชุดข้อมูลที่มีค่า ลองจิจูดทั้งหมดในช่วง - 180 ถึง 180 ผ่าน[EDDGridลอน PM 180](#eddgridlonpm180).
ถ้าค่านั้นเป็นจริงERDDAP™จะพยายามสร้างชุดข้อมูลให้ใช้ได้ผ่านทางERDDAP'WMSเซิฟเวอร์ แต่ถ้าชุดข้อมูลนั้นไม่เหมาะสมกับWMS  (เช่น มันไม่มีข้อมูล ลองติจูดหรือละติจูด) แล้วชุดข้อมูลจะไม่ได้ผ่านERDDAP'WMSเซิร์ฟเวอร์ ไม่คํานึงถึงการตั้งค่านี้
     
...
ตัว อย่าง เช่น:
    ...
    * สําหรับtabledapชุดข้อมูล ซึ่งใช้กันบ่อยที่สุด คือระบุช่วงเวลาต่าง ๆ ของเวลา (สัมพันธ์กับค่ามากที่สุด (เวลา) ตัวอย่างเช่น & เวลา)=max (เวลา) -1 วัน หรือเมื่อเทียบกับตอนนี้ ตัวอย่างเช่น & เวลา) =now-1 วัน) .
...
หรือคุณสามารถใส่&lt;เท็จ&lt;ในการตั้งค่า.xml เพื่อบอกERDDAP™ไม่เสนอข้อมูลกํากับภาพแบบ FGDC สําหรับชุดข้อมูลใดๆ
     
...
หรือคุณสามารถใส่&lt;ไอโซ19115 แอ็คชั่น) เท็จ&lt;ในการตั้งค่า.xml เพื่อบอกERDDAP™ไม่เสนอข้อมูล ISO 19115 สําหรับข้อมูลใด ๆ
     
#### &lt;จับคู่ให้ตรงกัน NDigits&gt;{#matchaxisndigits} 
* [ ** &lt;ตรงกับ Axndigits) ** ) (เลขคู่) เป็นเครื่องหมายการค้าภายในEDDGrid &lt;แท็กสําหรับEDDGridชุดข้อมูลที่เป็นหัวเรื่อง, เช่น, เป็นกลุ่มของแฟ้ม ทุกครั้งที่ชุดข้อมูลถูกโหลดใหม่ERDDAP™ตรวจดูว่าค่าแกนของแต่ละองค์ประกอบ ของเครื่องหมายอาร์คโคไซน์เท่ากัน ความแม่นยําของการทดสอบถูกกําหนดโดย[ตรงกับ Axndigit](#matchaxisndigits)ซึ่งระบุจํานวนตัวเลขทั้งหมด ที่ต้องตรงกันเมื่อทดสอบค่าแกนคู่ (ค่าปริยาย) . เมื่อทดสอบค่าแกนลอย, การทดสอบจะทํากับ accessNdigits/2 หลัก ค่า 18 หรือมากกว่าที่บอกEDDGridเพื่อทําการทดสอบที่แน่นอน ค่า 0 บอกEDDGridอย่า ทํา การ ทดลอง ใดๆ ซึ่ง ไม่ มี การ แนะนํา เว้นแต่ ที่ พรรณนา ไว้ ข้าง ล่าง นี้
...
มีกรณีหนึ่งที่ใช้กับ accessNDigits\\=0 ที่มีประโยชน์ และขอแนะนําให้ใช้: ด้วยกลุ่มย่อยของแฟ้มทางไกล เช่น ข้อมูลในถัง S3 ในกรณีนี้ หากชุดข้อมูลใช้แคชจาก Url, แคช SizeGB, ตรงกับ AxisNDigits\\=0 และEDDGridจากระบบแฟ้มสําหรับ[พารามิเตอร์ ผ่าน ชื่อแฟ้ม](#aggregation-via-file-names-or-global-metadata)งั้นEDDGridไม่ต้องอ่านทุกไฟล์ เพื่อทําจดหมาย นี่ช่วยให้ชุดข้อมูลที่ถูกสร้างจากข้อมูลในถัง S3 ได้โหลดอย่างรวดเร็ว (แทนที่จะช้าอย่างไร้สาระEDDGridต้องดาวน์โหลดและอ่านแฟ้มทั้งหมด) .
...
สําหรับชุดข้อมูลส่วนใหญ่ในระบบส่วนใหญ่ ให้ใช้ nTheads=1, ค่าปริยาย ถ้าคุณมีคอมพิวเตอร์ที่มีประสิทธิภาพ (ศูนย์ CPU จํานวนมาก ความทรงจําจํานวนมาก) ถ้า เช่น นั้น ขอ ให้ คิด ถึง การ กําหนด ขอบ เขต 2, 3, 4 หรือ สูง กว่า นั้น (แต่ไม่มากไปกว่าจํานวนแกน CPU ในคอมพิวเตอร์) สําหรับชุดข้อมูล ที่อาจมีประโยชน์:
        ...
        * ชุดข้อมูลซึ่งบางสิ่งทําให้เกิดความล้าหลัง ก่อนที่จะสามารถประมวลผลข้อมูลชิ้นเล็ก ๆ ได้จริงๆ จะเป็นประโยชน์ ตัวอย่างเช่น:
            * ข้อมูลต่าง ๆ กับ[ถูกกดทับภายนอก (อี.จี..gz) ](#externally-compressed-files)ไบนารี (อี.จี..nc) แฟ้ม เพราะว่าERDDAP™ต้องย่อยสลายแฟ้มทั้งหมดก่อนที่จะเริ่มอ่านแฟ้มได้
            * ชุดข้อมูลที่จะใช้[แคชSizeGB](#cachefromurl)เพราะว่าERDDAP™บ่อยครั้งที่ต้องดาวน์โหลดแฟ้มก่อนที่จะอ่านได้
...
คําเตือน: เมื่อใช้ nTHeads&gt; 1, จับตาดูไว้ERDDAPการใช้หน่วยความจํา, การใช้ด้าย, และการตอบสนองโดยรวม (ดู[ERDDAPหน้าสถานะ](/docs/server-admin/additional-information#status-page)) . ดู ความ เห็น เกี่ยว กับ ประเด็น เหล่า นี้ ข้าง ล่าง นี้.
         
...
สําหรับ[เดิมERDDAP™การติดตั้ง](https://coastwatch.pfeg.noaa.gov/erddap/index.html)เราใช้
...
ไม่ว่าอะไรก็ตามที่การตั้งค่า ntheads, จับตาดูสถิติการใช้งานหน่วยความจําของคุณ[ERDDAPหน้าสถานะ](/docs/server-admin/additional-information#status-page). คุณไม่ควรจะเข้าใกล้การทําให้การใช้งานหน่วยความจําสูงสุดในERDDAPมิ ฉะนั้น จะ เกิด ความ ผิด พลาด ร้าย แรง และ ความ ล้ม เหลว.
...
อย่างไรก็ตาม มันมีวิธีที่จะทําให้ERDDAP™สัดส่วนที่จะจัดการกับจํานวนมากของการร้องขอได้อย่างรวดเร็วโดยการตั้งค่า[กริด/ ตัวประกอบ/ ปริมาตรERDDAPวินาที](/docs/server-admin/scaling).
         
#### &lt;แผ่นใส{#palettes} 
* เริ่มด้วยERDDAP™รุ่น 2.12datasets.xmlอาจรวม&lt;ป้ายกํากับ (in)&lt;ซึ่งแทนที่&lt;ค่าของจานสี: (หรือกลับไปใช้ค่า xml ของจดหมายหากป้ายเข้ามาdatasets.xmlว่าง) . ให้คุณได้เปลี่ยนรายการของจานสีที่มีERDDAP™กําลังวิ่ง นอกจากนี้ยังช่วยให้คุณทําการเปลี่ยนแปลงและมีมันอย่างต่อเนื่องเมื่อคุณติดตั้งรุ่นใหม่ของERDDAP.
(คําเตือน: แผ่นจานเสียงในรายการในdatasets.xmlต้องเป็นชุดของจานสีในรายการในจดหมาย.xml; ไม่เช่นนั้นERDDAP™จะโยนข้อยกเว้นและหยุดการประมวลผลdatasets.xml. นี่รับประกันได้ว่าERDDAP™การติดตั้งอย่างน้อยสนับสนุนจานสีหลักเดียวกัน
(คําเตือน:ERDDAP™ตรวจสอบว่าแฟ้มจานสีที่ระบุในจดหมายมีอยู่หรือไม่ แต่มันไม่ได้ตรวจสอบแฟ้มจานสีในรายการdatasets.xml. มันเป็นความรับผิดชอบของคุณที่จะแน่ใจว่าไฟล์มีอยู่
...
ตัวอย่างเช่นstationID&gt; = "41004"
กลายเป็นstationID% 3 =% 241004% 2
ต้องการการเข้ารหัส Percent เมื่อคุณใช้งานERDDAPผ่านซอฟต์แวร์อื่นที่ไม่ใช่เบราว์เซอร์ เว็บเบราว์เซอร์มักจะใช้รหัสเปอร์เซ็นต์สําหรับคุณ
ในบางสถานการณ์ คุณต้องเข้ารหัสตัวอักษรทุกตัว นอกจาก A-Za-z0-9/_-~ ' () ** แต่ยังคงไม่เข้ารหัสเริ่มต้น '&' หรือหลัก'='ในเงื่อนไข
การเขียนโปรแกรมภาษามีเครื่องมือในการทําเช่นนี้ (ตัวอย่างเช่น ดูJava'[java.net.URLEncoder](https://docs.oracle.com/javase/8/docs/api/java/net/URLEncoder.html)ถึงJavaสคริปต์encodeURIComponent()) ( https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent ) และยังมี
...
ตัว อย่าง เช่น:&lt;OnChangle~. mailto: hohn. mith@ Company.com&lt;/ ออนแชง ถ้าคุณมีเหตุผลที่ดีERDDAP™เพื่อสนับสนุนการกระทําอื่น ๆ ให้ส่งอีเมลมาอธิบายสิ่งที่คุณต้องการ
...
* [ ** &lt;โหลดใหม่ ทุก ๆ สิ่ง ** ) (โหลดทุกนาที) เป็นเครื่องหมายการค้าภายใน&lt;แท็กในdatasets.xmlของประเภทชุดข้อมูลเกือบทั้งหมด ที่ระบุว่าควรโหลดข้อมูลใหม่บ่อยแค่ไหน ยกตัวอย่างเช่น
    ...
    * โดยไม่คํานึงถึงการโหลดทุก Minuts คุณสามารถบอกด้วยตนเองERDDAP™เพื่อเรียกข้อมูลต่าง ๆ ที่กําหนดโดยเร็วที่สุด[แฟ้มธง](/docs/server-admin/additional-information#flag).
...
หลังจาก&lt;โหลดทุก ๆ Nminuts) ป้ายข้อมูลในdatasets.xml. จํานวนมิลลิวินาทีที่คุณระบุ สามารถมีขนาดเล็กเท่า 1 (เพื่อให้แน่ใจว่าชุดข้อมูลมีขึ้นตลอดเวลา) . ค่า 0 (ค่าปริยาย) หรือเลขลบปิดระบบ
    ...
    * สําหรับชุดข้อมูลที่จะใช้คลาสย่อยของ[EDDGridจากแฟ้ม](#eddgridfromfiles)ถึง[เลือกแฟ้ม](#eddtablefromfiles).
        *    **(คําเตือน:** เมื่อคุณเพิ่มแฟ้มข้อมูลใหม่ไปยังชุดข้อมูล โดยการคัดลอกมันไปยังไดเรกทอรีที่ERDDAP™ดูสิ มันอันตรายERDDAP™จะแจ้งให้ทราบแฟ้มที่เขียนบางส่วน; ลองอ่านดู แต่ล้มเหลวเพราะแฟ้มยังไม่สมบูรณ์; ประกาศแฟ้มที่ "แย่" แล้วลบแฟ้มออก (ชั่วคราว) จากชุดข้อมูล
เพื่อหลีกเลี่ยงเรื่องนี้ พวกเรา **โดย ผู้ เขียน ตื่น เถิด&#33;** คุณสามารถคัดลอกแฟ้มใหม่ไปยังไดเรกทอรีด้วยชื่อชั่วคราว (ตัวอย่างเช่น 20150226.ncเทือก) ที่ไม่ตรงกับแฟ้มข้อมูลชุด ชื่อregex (_*\\.nc) ชื่อแฟ้มโดยใช้ชื่อที่ถูกต้อง (ตัวอย่างเช่น 20150226.nc) . ถ้าคุณใช้วิธีนี้ERDDAP™จะไม่สนใจแฟ้มชั่วคราว และจะสังเกตเห็นเฉพาะแฟ้มที่มีชื่อที่ถูกต้องเมื่อมันสมบูรณ์และพร้อมใช้งาน
        * หากคุณทําการแก้ไขแฟ้มข้อมูลที่มีอยู่ในตําแหน่ง (ตัวอย่างเช่น, เพื่อเพิ่มข้อมูลใหม่) .&lt;update everNMillys) จะทํางานได้ดีถ้าการเปลี่ยนแปลงปรากฏเป็นอะตอม (ในไม่ช้า) และแฟ้มจะเป็นแฟ้มที่ถูกต้องเสมอ ยกตัวอย่างเช่น ห้องสมุด Netcdf-java อนุญาตให้เพิ่มเติมได้ ในมิติที่ไร้ขีดจํากัดของ "คลาสสิก".ncv3 แฟ้มที่จะทําแบบอะตอม
...
หรือ ใช้ เลข ที่ สูง กว่า หาก ปัญหา ยัง มี อยู่. ค่าปริยายของนาฬิกาคือ 8192 ค่าปริยายของกรณีคือ 128
    ...
    * สําหรับโปรแกรมที่อยากรู้อยากเห็น -- การปรับปรุงเพิ่มขึ้นเหล่านี้ไม่เหมือนERDDAPเต็มแล้ว[โหลดรายการต่าง ๆ ทุกรายการ](#reloadeverynminutes)ระบบ โดยจะเกิดขึ้นภายในเธรดที่ผู้ใช้ร้องขอ ดังนั้น ชุดข้อมูลจํานวนใด ๆ สามารถปรับปรุงได้พร้อมกัน มีรหัส (และล็อค) เพื่อให้แน่ใจว่ามีเพียงเธรดเดียว ที่กําลังทํางานเกี่ยวกับการปรับปรุงสําหรับชุดข้อมูลใด ๆ ที่ให้มาในช่วงเวลาใด ๆ การอนุญาตให้ปรับปรุงหลาย ๆ ครั้งพร้อมกันเป็นเรื่องง่าย การอนุญาตให้โหลดเต็มจํานวนมากพร้อมกันจะยาก
         
...
ไม่มี|บางส่วนERDDAP™จะเรียงลําดับ "" ในจุดเริ่มต้นของผลลัพธ์ (ก่อนสตริงที่ไม่ใช่ "") .
หากใช่ ฐานข้อมูลอาจจะ (Postgrest จะ) เรียงลําดับ "" ในตอนจบของผลลัพธ์ (หลังจากสตริงที่ไม่ใช่ "") .
ผมเดาได้ว่านี่จะมีผลต่อ การเรียงลําดับของคําสั้น ๆ กับคําที่ยาวขึ้น ยกตัวอย่างเช่นERDDAP™จะเรียงลําดับ "Sign" ก่อน "simps".
    * ตัวอย่างก็คือ
```
...
ไม่มี|บางส่วนERDDAP™จะเรียงลําดับ "" ในจุดเริ่มต้นของผลลัพธ์ (ก่อนสตริงที่ไม่ใช่ "") .
หากใช่ ฐานข้อมูลอาจจะ (Postgrest จะ) เรียงลําดับ "" ในตอนจบของผลลัพธ์ (หลังจากสตริงที่ไม่ใช่ "") .
ทั้ง นี้ อาจ มี ผล กระทบ ต่อ การ เรียง คํา สั้น ๆ กับ คํา ที่ ยาว กว่า ซึ่ง เริ่ม ด้วย คํา สั้น ๆ. ยกตัวอย่างเช่นERDDAP™จะเรียงตัว "Sign" ก่อนคําว่า "simission" แต่ผมไม่แน่ใจว่าฐานข้อมูลจะเรียงมันอย่างไร
    * ตัวอย่างก็คือ
```
...
* [ ** &lt;sourceUrl&gt; ** ) (# ผู้สนับสนุน) เป็นข้อมูลทั่วไปภายในข้อมูล&lt;addAttributes&gt; ป้ายกํากับที่ใช้กําหนดที่อยู่ URL ที่เป็นแหล่งข้อมูลของข้อมูล
    ...
    * สําหรับชุดข้อมูลบางส่วนแยก&lt;sourceUrlไม่อนุญาตให้ใช้ป้าย &gt; แทนที่คุณจะให้sourceUrl"[คุณสมบัติทั่วไป](#global-attributes)สืบค้นเมื่อ:addAttributes&lt;. หากไม่มีที่อยู่ URL ต้นทางจริง (ตัวอย่างเช่น ถ้าข้อมูลถูกเก็บไว้ในแฟ้มภายในระบบ) อุปสงค์นี้มักจะมีค่าเป็นหลัก&lt;at name=" name"&gt; (แฟ้มภายในระบบ) &lt;(at).
...
ในบางกรณีsourceUrlคุณอาจจะใช้นามแฝงของชื่อโดเมนบนใบรับรอง ยกตัวอย่างเช่น
...
แอตทริบิวต์ cwdf / กลับหน้า สาเหตุของแอตทริบิวต์ cwodf /_ กลับหน้า (ถ้ามี) ที่จะถูกลบออกจากรอบสุดท้าย รวมของคุณสมบัติ
    * การ จัด เตรียม ข้อมูล นี้ ช่วย ได้ERDDAP™ทํางานที่ดีขึ้น และช่วยผู้ใช้ ให้เข้าใจชุดข้อมูล
...
สําหรับชุดข้อมูลเหล่านี้ทั้งหมด ในการประชุมและMetadata\\_Conventionsคุณสมบัติระดับโลก โปรดอ้างอิงถึง CF-1.6 (ไม่ใช่ CF-1.0, 1.1, 1.1, 1.4, หรือ 1.5) เนื่องจาก CF-1.6 เป็นเวอร์ชันแรก ที่รวมการเปลี่ยนแปลงที่เกี่ยวข้องกับ เรขาคณิต Sampling (แบบ DSG) การ ประชุม ภาค.
        ...
        *   ERDDAP™ทําการทดสอบความถูกต้องมากมาย เมื่อมันโหลดข้อมูล หากชุดข้อมูลที่มี cdm /_ data\\_ประเภท (หรือfeatureType) แอตทริบิวต์โหลดเรียบร้อยแล้วERDDAP™งั้นERDDAP™กําลังบอกว่า ชุดข้อมูลตรงกับความต้องการ DSG (ไม่อย่างนั้นERDDAP™จะโยนข้อยกเว้นอธิบายปัญหาแรกที่มันพบ) .
คําเตือน: ชุดข้อมูลที่ถูกโหลดสําเร็จแล้ว ดูจะตรงกับความต้องการ DSG (มันมีส่วนผสมที่ถูกต้องของคุณลักษณะ) แต่อาจจะยังตั้งค่าไม่ถูกต้อง นําไปสู่ผลลัพธ์ที่ผิดใน.ncCF และ.ncแฟ้มการตอบสนองของ CFMA (ซอฟต์แวร์นั้นฉลาดในบางแง่ และไร้ปัญญาในตัวอื่นๆ) 
        ...
        * หากผู้ใช้ร้องขอสับเซตของชุดข้อมูลใน.ncCF (เป็น.ncแฟ้มในรูปแบบแฟ้ม DSG แบบต่อเนื่อง) หรือ.ncแฟ้ม CFMA (คือ.ncแฟ้มในรูปแบบแฟ้ม Rray ในรูปแบบมัลติดิสก์ของ DSG) แฟ้มดังกล่าวจะเป็นแฟ้ม CFDSG ที่ถูกต้อง
...
* สําหรับเป๋าฮื้อบางชนิด ข้อมูลชุดหนึ่งอาจมี ตัวแปรละติจูดและลองติจูด 2 ชุด
    ...
    2. กําหนดค่าละติจูดและลองติจูด ที่เกี่ยวข้องกับแต่ละข้อสังเกต ในERDDAP™ให้ตัวแปรเหล่านี้แตกต่างกันdestinationNameวินาที (E.g., แม่นยําและแม่นยํา ลอน) และไม่ต้องรวมตัวแปรเหล่านี้ในรายการของ cdm\\_timesory\\_sivables
...
การ ประชุม ใหญ่ ที่ มี การ ใช้ คํา ศัพท์ ทั่ว ไปERDDAP™คือ:
    ...
    *   [COARDSการ ประชุม ภาค](https://ferret.pmel.noaa.gov/noaa_coop/coop_cdf_profile.html)เป็นตัวแทนของซีเอฟ
    *   [สภาพ อากาศ และ การ คาด การณ์ ล่วง หน้า (CF) การ ประชุม ภาค](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)เป็นที่มาของคุณลักษณะที่แนะนําและจําเป็นหลายอย่างERDDAP. CF รุ่นปัจจุบันระบุว่าเป็น "CF-1.6"
    * เดอะNetCDFอนุ สัญญา เรื่อง การ ค้น พบ ข้อมูล (หมายเลข ACD) เป็นที่มาของคุณลักษณะที่แนะนําและจําเป็นหลายอย่างERDDAP. ACD รุ่นดั้งเดิม (ผลงานชิ้นเอกของ อีธาน เดวิส) ระบุว่า[UnidataDirectory รุ่น v1. 0](https://wiki.esipfed.org/ArchivalCopyOfVersion1)ปัจจุบัน (เริ่มตั้งแต่ 2015) 1. 3 รุ่นของ ACDD ถูกระบุว่าเป็น[ACD-1.3](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3). ถ้าชุดข้อมูลของคุณถูกใช้UnidataDatetet Directy v1.0 เราแนะนําให้คุณ[สลับชุดข้อมูลที่จะใช้ ACD-1.3](#switch-to-acdd-13).
...
*   [ **coverage\\_content\\_type** ](#coverage_content_type)  (จาก[ISO 19115](https://en.wikipedia.org/wiki/Geospatial_metadata)มาตรฐานข้อมูลกํากับภาพ) เป็นวิธีวิเคราะห์ชนิดของข้อมูล (ในEDDGridชุดข้อมูล) . ยกตัวอย่างเช่น
    ```
...
*   [ **date\\_issued** ](#date_issued)  (จาก[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็นวิธี REMMMENE ที่ระบุวันที่ซึ่งข้อมูลได้ถูกทําขึ้นครั้งแรกให้กับคนอื่น ๆ ในรูปแบบ ISO 8601 ตัวอย่างเช่น 2012-03-15. ยกตัวอย่างเช่น
    ...
    ```
ตัวอย่างเช่น ชุดข้อมูลอาจมี[date\\_created](#date_created)2010-01-30 แต่ได้รับการเปิดให้บริการเฉพาะสาธารณชน 2010-07-30.date\\_issuedมีการใช้ทั่วไปน้อยกว่าdate\\_createdถึงdate\\_modified. ถ้าdate\\_issuedการตัดทิ้ง, สันนิษฐานว่าเหมือนกับdate\\_created.
###### ทั่ว โลกdrawLandMask {#global-drawlandmask} 
*   [ **drawLandMask** ](#global-drawlandmask)-- นี่คือคุณลักษณะสากลที่ใช้โดยERDDAP™  (และไม่มีมาตรฐานกํากับ) ซึ่งกําหนดค่าปริยายสําหรับตัวเลือก "การวาดแผ่น" บนชุดข้อมูล สร้างกราฟ ( *datasetID* ลายเซ็น) และสําหรับพารามิเตอร์ต่าง ๆ ในที่อยู่ URL ที่ร้องขอแผนที่ข้อมูล ยกตัวอย่างเช่น
    ```
    <att name="drawLandMask">over</att>  
    ```
ดู[drawLandMaskภาพรวม](#drawlandmask).
###### featureType {#featuretype} 
*   [ **featureType** ](#featuretype)  (จาก[CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)มาตรฐานข้อมูลกํากับภาพ) คือความไม่สงบและ/หรือการตอบโต้ ถ้าชุดข้อมูล[cdm\\_data\\_ชนิด](#cdm_data_type)เหมาะสมดีERDDAP™จะใช้มันอัตโนมัติในการสร้างfeatureTypeอุปนิสัย ดังนั้น คุณไม่จําเป็นต้องบวกมัน
    
อย่างไรก็ตาม ถ้าคุณกําลังใช้[แฟ้ม DDTable จาก NCF](#eddtablefromnccffiles)เพื่อสร้างชุดข้อมูลต่าง ๆ จากแฟ้มที่ทําตาม[CF แว่นขยาย (แบบ DSG) มาตรฐาน](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html#discrete-sampling-geometries)ไฟล์ทั้งหมดfeatureTypeนิยามได้ถูกต้อง ดังนั้นERDDAP™สามารถอ่านแฟ้มได้อย่างถูกต้อง นั่นคือส่วนของความต้องการแบบ CF DSG สําหรับชนิดของแฟ้ม
     
###### ประวัติ{#history} 
...
การเขียนโปรแกรมภาษามีเครื่องมือในการทําเช่นนี้ (ตัวอย่างเช่น ดูJava'[java.net.URLEncoder](https://docs.oracle.com/javase/8/docs/api/java/net/URLEncoder.html)  
ถึงJavaสคริปต์encodeURIComponent()) ( https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent ) และยังมี
        [เว็บไซต์ที่มีการเข้ารหัส/ถอดรหัสให้คุณ](https://www.url-encode-decode.com/).
    * ตั้งแต่datasets.xmlเป็นแฟ้ม XML นอกจากนี้คุณยังต้องกําหนด &-encode ทั้งหมด '&' ด้วย&lt;'และ'&gt;' ในที่อยู่ URL ที่เรียกว่า '&amp;'&lt;'และ '&gt;'หลังจากเปอร์เซ็นต์การเข้ารหัส
...
*   [ **สถาบัน** ](#institution)  (จาก[CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)ถึง[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็นคุณสมบัติระดับโลกที่มีชื่อย่อของสถาบัน ซึ่งเป็นที่มาของข้อมูลนี้ (ปกติจะเป็นชื่อย่อ)&lt;20 ตัวอักษร). ยกตัวอย่างเช่น
    ...
    * ถ้าคุณเพิ่มสถาบันเข้าไปในรายการ&lt;categoryAttributes&gt; ในERDDAP'[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)แฟ้ม, ผู้ใช้สามารถหาชุดข้อมูลจากสถาบันเดียวกันได้ง่ายERDDAP"ค้นหาข้อมูลตามหมวดหมู่" ในโฮมเพจ
###### คําค้น:{#keywords} 
*   [ **คําค้น:** ](#keywords)  (จาก[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็นรายการคําและวลีสั้นที่รวมเครื่องหมายจุลภาค (ยกตัวอย่างเช่น[แบบ GCMD คํา ศัพท์ วิทยาศาสตร์](https://wiki.earthdata.nasa.gov/display/CMR/GCMD+Keyword+Access)) ที่อธิบายชุดข้อมูลโดยทั่วไป และไม่สมมุติความรู้อื่น ๆ ของชุดข้อมูล (ยกตัวอย่างเช่น สําหรับข้อมูลทางทะเล รวมถึงมหาสมุทรด้วย) . ยกตัวอย่างเช่น
    ...
    ```
ตั้งแต่datasets.xmlเป็นเอกสาร XML, อักขระต่าง ๆ&lt;และ &gt; ในคุณลักษณะเช่นคําสําคัญ (E.g., the &gt; อักขระในกุญแจวิทยาศาสตร์ GCMD) ต้องเข้ารหัสเป็น & amp;&lt;& gt; ตามด้วย
เมื่อมีการโหลดชุดข้อมูลERDDAP.
...
*   [ **สัญญาอนุญาตสิทธิ์** ](#license)  (จาก[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็นคุณสมบัติระดับโลกที่มีใบอนุญาตและ/หรือข้อห้ามการใช้งาน ยกตัวอย่างเช่น
    ...
    ```
    * ถ้า\\[มาตรฐาน\\]"เกิดขึ้นในค่าคุณลักษณะ, มันจะแทนที่ด้วยมาตรฐานERDDAP™ใบอนุญาตจาก&lt;ป้ายมาตรฐาน)ERDDAP'
        \\[แว่นขยาย\\]แฟ้ม / webapps/ erdep/WEB-INF/คลาสต่างๆ/gov/noa/pfel/erdep/util/xmlfile
         
###### Metadata\\_Conventions {#metadata_conventions} 
*   [ **Metadata\\_Conventions** ](#metadata_conventions)มาจากยุคโบราณ[ACD 1. 0](https://wiki.esipfed.org/ArchivalCopyOfVersion1)  (ซึ่งระบุในMetadata\\_Conventionsว่าUnidataDatetset Directory v1. 0") มาตรฐานข้อมูล ค่าของแอททริบิวต์เป็นรายการของการประชุมข้อมูลกํากับที่ถูกใช้โดยชุดข้อมูลนี้
ถ้าชุดข้อมูลใช้ ACD 1.0 แอตทริบิวต์นี้จะถูกรีคอมมิวสิกชั่น
    ...
    ```
แต่ERDDAP™ตอนนี้แนะนํา ACD-1.3 ถ้าคุณมี[เปลี่ยนชุดข้อมูลที่จะใช้ ACD-1.3](#switch-to-acdd-13)ใช้Metadata\\_Conventions(ไม่ต่อเนื่อง)&lt;การประชุม) (# ความร่วมมือ) แทน
...
*   [ **publisher\\_name** ](#publisher_name)  (จาก[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็น วิธี ที่ ได้ รับ การ สนับสนุน ให้ ระบุ ตัว บุคคล, องค์การ, หรือ โครงการ ซึ่ง กําลัง จัด พิมพ์ ข้อมูล ชุด นี้. ยกตัวอย่างเช่น
    ...
    ```
ตัวอย่างเช่น คุณเป็นผู้เผยแพร่ ถ้ามีคนอื่นหรือกลุ่ม[สร้าง](#creator_name)ชุดข้อมูลและคุณเพียงแค่กลับมาตอบสนองมันผ่านERDDAP. ถ้า "ผู้ตีพิมพ์" ไม่ใช้กับชุดข้อมูลจริง ก็ยกเลิกคุณสมบัตินี้ เทียบกับ[creator\\_name](#creator_name)สํานักพิมพ์อาจจะไม่ได้แก้ไขหรือประมวลข้อมูลในนัยสําคัญ ผู้จัดทําเพียงแค่ทําให้ข้อมูลดังกล่าวใช้ได้ ในสถานที่ใหม่
...
*   [ **real\\_time** ](#real_time)เป็นคุณสมบัติของสตริงทั่วโลก (ไม่จากมาตรฐานใด ๆ) แสดงว่านี่คือข้อมูลเวลาจริง ยกตัวอย่างเช่น
    ...
    ```
ถ้านี่เป็นเท็จ (ค่าปริยาย) .ERDDAP™จะทําการบันทึกการตอบสนองไปยังการร้องขอประเภทแฟ้มที่แฟ้มทั้งหมดจะต้องถูกสร้างขึ้นก่อนERDDAP™สามารถเริ่มส่งการตอบสนองไปยังผู้ใช้และใช้อีกครั้งได้ถึงประมาณ 15 นาที (อี.จี..nc.png) .
ถ้านี่เป็นจริงERDDAP™จะไม่จัดเก็บแฟ้มที่ตอบรับ และจะคืนแฟ้มที่ถูกสร้างใหม่เสมอ
###### sourceUrlคุณสมบัติ{#sourceurl-attribute} 
...
ถ้าเป็นไปได้[สร้าง DatatasXml](#generatedatasetsxml)เพิ่มtestOutOfDateคําพูดของโลกaddAttributesของชุดข้อมูล ค่านี้เป็นคําแนะนําที่อิงจากข้อมูลที่มีให้สร้าง DatatatetsXml ถ้าค่ามันไม่เหมาะสม ก็เปลี่ยนมันซะ
...
สําหรับEDDGridชุดข้อมูล หนึ่งหรือมากกว่าaxisVariableแท็กมีการผลิตและทั้งหมด[dataVariableวินาที](#datavariable)แบ่งปัน/ใช้ตัวแปรแกนทั้งหมด ([ทําไม?](#why-just-two-basic-data-structures) [แล้วถ้าไม่ล่ะ](#dimensions))   
...
* [&lt;destinationName–&gt; (ชื่อที่ใช้แทน) เป็นชื่อของตัวแปรที่จะแสดงและใช้โดยERDDAP™ผู้ใช้
    ...
    * ในEDDGridชุดข้อมูล,[ลองจิจูด, ละติจูด, ระดับความสูง, ความลึก, และเวลา](#destinationname)ตัวแปรแกนนั้นพิเศษ
         
...
ถ้าตัวแปร[ผู้ส่งที่มา](#variable-addattributes)หรือ&lt;addAttributes&gt; รวม[scale\\_factor&/ หรือadd\\_offset](#scale_factor)คุณสมบัติ, ค่าของพวกเขาจะถูกใช้ในการแตกข้อมูลออกจากแหล่งข้อมูล ก่อนที่จะจัดจําหน่ายไปยังไคลเอนต์
     (ผล ค่า =แหล่ง ค่า / *scale\\_factor+add\\_offset) . ตัวแปรที่ถูกถอดออกจากกล่อง จะอยู่ในประเภทเดียวกันของข้อมูล (ยกตัวอย่างเช่น ลอย) คือscale\\_factorถึงadd\\_offsetค่า
         
...
DDTable from Februarys can use some excoded-signsourceNames เพื่อกําหนดตัวแปรข้อมูลใหม่ เช่น เพื่อส่งเสริมให้คุณลักษณะระดับโลกเป็นตัวแปรข้อมูล ดู[เอกสารนี้](#pseudo-sourcenames).
###### HDFโครงสร้าง{#hdf-structures} 
เริ่มด้วยERDDAP™v2.12EDDGridแฟ้มจาก Nc และEDDGridแฟ้มจาก Nc ไม่สามารถอ่านข้อมูลจาก "รูปแบบ" ในได้.nc4 และ.hdf4 แฟ้ม เพื่อระบุตัวแปรที่มาจากโครงสร้าง&lt;sourceName&gt; ต้องใช้รูปแบบ: *การจัดเต็มName* | *สมาชิกName* ตัวอย่างเช่นกลุ่ม 1/MyStrucht|เม็มเบอร์
...
เริ่มด้วยERDDAP™v2.10, ใน[เลือกแฟ้ม](#eddtablefromfiles).[เพิ่มข้อมูล](#eddtablefromdatabase)หรือ[แก้ไขโครงการหลัก...](#eddtablefromfilenames)ชุดข้อมูล,&lt;sourceName&gt; สามารถ
...
```
ERDDAP™ขึ้นอยู่กับ[โครงการ Apache](https://www.apache.org/) [Javaนิพจน์ภาษา (JEXL) ](https://commons.apache.org/proper/commons-jexl/)  (ใบอนุญาต:[อาปาเช](https://www.apache.org/licenses/LICENSE-2.0)) เพื่อประเมินนิพจน์และประมวลผลสคริปต์
การคํานวณสําหรับตัวแปรใหม่ที่กําหนด จะทําภายในแถวเดียวของผลลัพธ์ หลายครั้งสําหรับทุกแถว
นิพจน์และสคริปต์ที่ใช้Java- และJavaไวยากรณ์เหมือนสคริปต์และสามารถใช้ใด ๆ ของ
...
* สายซึ่งช่วยให้คุณเข้าถึงวิธีการทั้งหมดคงที่สตริงที่เกี่ยวข้องใน[จาวา แลง ข้อความ](https://docs.oracle.com/javase/8/docs/api/java/lang/String). วัตถุสตริงในERDDAP™นิพจน์และสคริปต์สามารถใช้ค่าใด ๆ ที่สัมพันธ์กันJavaวิธีที่อธิบายในจาวา แลง เอกสาร ยกตัวอย่างเช่น สตริง คุณค่าของ (_) จะแปลงค่าคู่ d เป็นข้อความ (แม้ว่าคุณสามารถใช้ ""+d) .
*   [สตริง2](/docs/dokka/-e-r-d-d-a-p/com.cohort.util/-script-string2)ซึ่งเป็นห่อสําหรับส่วนใหญ่ของ คงที่, สาย และวิธีการเกี่ยวกับอาร์เรย์ใน com.coort.util.String2 ([สัญญาอนุญาตสิทธิ์](/acknowledgements#cohort-software)) . ยกตัวอย่างเช่น สาย2.zลวดลาย ( *ตัวเลข, ตัวเลข* ) จะบวก 0 ทางซ้ายของเส้นจํานวน เพื่อให้จํานวนหลักทั้งหมดเป็น ndigits (e.g., สตริง2.zลวดลาย ("6", 2) จะกลับมา "06") .
...
ในทางเทคนิคแล้ว คุณไม่จําเป็นต้องใช้ ปฏิทิน2. parase topoch sesetcords () ในการแปลงวันที่รวม+เวลารวมเป็นลําดับชั้น ๆ คุณสามารถเพียงแค่ผ่านวันที่ + เวลาสตริงERDDAP™และระบุรูปแบบ (เช่น)
...
```
###### แฟล็กแข็ง
หากคุณเปลี่ยนนิพจน์หรือสคริปต์ที่ถูกนิยามไว้sourceNameคุณต้องตั้ง[แฟล็กแข็ง](/docs/server-admin/additional-information#hard-flag)สําหรับชุดข้อมูลERDDAP™ลบข้อมูลทั้งหมดที่มีแคชสําหรับชุดข้อมูล และอ่านข้อมูลใหม่ทุกแฟ้ม (ใช้นิพจน์หรือสคริปต์ตัวใหม่) ครั้งต่อไป มันจะโหลดข้อมูล อีกทางเลือกหนึ่ง คุณสามารถใช้[ถอดรหัส](#dasdds)ซึ่งเทียบเท่าการตั้งธงแข็ง
...
ถ้าตัวแปร[ผู้ส่งที่มา](#variable-addattributes)หรือ&lt;addAttributes&gt; รวม[scale\\_factor&/ หรือadd\\_offset](#scale_factor)คุณสมบัติ, ค่าของพวกเขาจะถูกใช้ในการแยกข้อมูลจากแหล่ง ก่อนที่จะแจกจ่ายไปยังไคลเอนต์ ตัวแปรที่ถูกถอดออกจากกล่อง จะอยู่ในประเภทเดียวกันของข้อมูล (ยกตัวอย่างเช่น ลอย) คือscale\\_factorถึงadd\\_offsetค่า
...
* คุณสมบัตินี้มาจาก[CDCCOARDS](https://ferret.pmel.noaa.gov/noaa_coop/coop_cdf_profile.html)ถึง[CF 1. 7+](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)มาตรฐานข้อมูล
...
* ถ้าข้อมูลถูกบรรจุด้วย[scale\\_factor&/ หรือadd\\_offset](#scale_factor).actual\\_rangeต้องเก็บค่าต่าง ๆ และเก็บข้อมูลประเภทเดียวกับ ค่าที่ถูกถอดออกจากกระเป๋า
...
* ถ้าactual\\_rangeเป็นที่ทราบกันว่า ผู้ใช้สามารถใช้[นาที () และสูงสุด () ฟังก์ชัน](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/documentation.html#min)ในการร้องขอ ซึ่งมักมีประโยชน์มาก
...
* ดูเพิ่มเติม[data\\_minถึงdata\\_max](#data_min-and-data_max)ซึ่งเป็นวิธีอื่นที่จะระบุactual\\_range. อย่างไรก็ตาม, ตอนนี้พวกนี้กําลังเสื่อมถอยactual\\_rangeนิยามโดย CF 1.7+
         
...
* คุณสมบัติที่เกี่ยวข้องกับแถบสีคือ:
    ...
    * ถ้าข้อมูลถูกบรรจุด้วย[scale\\_factor&/ หรือadd\\_offset](#scale_factor)_ระบุcolorBarMinimumเป็นมูลค่าที่ห่อมา
...
###### data\\_minถึงdata\\_max {#data_min-and-data_max} 
*   [ **data\\_min** ถึง **data\\_max** ](#data_min-and-data_max)-- สิ่งเหล่านี้เป็นคุณลักษณะตัวแปรที่เสื่อมโทรม ซึ่งถูกนิยามไว้ในมหาสมุทรโลก (อินโดนีเซีย) คําบรรยายข้อมูล ยกตัวอย่างเช่น
    ...
    * เราแนะนําให้คุณใช้[actual\\_range](#actual_range)แทนdata\\_minถึงdata\\_maxเพราะว่าactual\\_rangeตอนนี้นิยามตามข้อกําหนดของ CF
    ...
    * ถ้าข้อมูลถูกบรรจุด้วย[scale\\_factor&/ หรือadd\\_offset](#scale_factor).data\\_minถึงdata\\_maxต้องทําการแตกรายการต่าง ๆ โดยใช้ชนิดของข้อมูลที่แตกออกมา
         
###### ตัวแปรdrawLandMask {#variable-drawlandmask} 
*   [ **drawLandMask** ](#variable-drawlandmask)-- นี่คือคุณลักษณะตัวแปรที่ถูกใช้โดยERDDAP™  (และไม่มีมาตรฐานกํากับ) ซึ่งกําหนดค่าปริยายสําหรับตัวเลือก "การวาดแผ่น" บนชุดข้อมูล สร้างกราฟ ( *datasetID* ลายเซ็น) และสําหรับพารามิเตอร์ต่าง ๆ ในที่อยู่ URL ที่ร้องขอแผนที่ข้อมูล ยกตัวอย่างเช่น
    ```
        <att name="drawLandMask">under</att>  
    ```
ดู[drawLandMaskภาพรวม](#drawlandmask).
...
*   [ **_หมายเหตุ** ](#encoding)
    ...
    * คุณสมบัตินี้มาจาก[NetCDFทิศทางของผู้ใช้ (กัญชา) ](https://docs.unidata.ucar.edu/netcdf-java/current/userguide/index.html).
    * ภายในERDDAP™สตริงเป็นลําดับของตัวอักษร 2 ไบต์ที่ใช้[ชุดรหัสอักขระ UCS-2](https://en.wikipedia.org/wiki/UTF-16).
    ...
    * ในทางทฤษฎีแล้วERDDAP™รองรับ\\_บันทึกการแสดงตัวจาก[รายการ ISA](https://www.iana.org/assignments/character-sets/character-sets.xhtml)แต่ในระหว่างการฝึกERDDAP™ปัจจุบันเพียงแค่รองรับ
...
เพื่อกําหนดส่วนกํากับที่จะใช้เพิ่มชื่อชื่อแฟ้มไปยังที่อยู่ URL ที่สมบูรณ์ งั้นก็เพื่อ.htmlTableการตอบรับERDDAP™จะแสดงชื่อแฟ้มเป็นตําแหน่งเชื่อมโยงไปยังที่อยู่ URL แบบเต็ม (ฐาน เพิ่มชื่อแฟ้ม) .

ถ้าคุณต้องการERDDAP™เพื่อให้บริการแฟ้มที่เกี่ยวข้อง ทําให้แยก[แก้ไขโครงการหลัก...](#eddtablefromfilenames)ชุดข้อมูลสําหรับแฟ้มเหล่านั้น (มันอาจจะเป็นชุดข้อมูลส่วนตัว) .
...
ถ้าคุณต้องการERDDAP™เพื่อรับใช้แฟ้มสํารองข้อมูล ให้ทําการแยกต่างหาก[แก้ไขโครงการหลัก...](#eddtablefromfilenames)ชุดข้อมูลสําหรับแฟ้มนั้น (มันอาจจะเป็นชุดข้อมูลส่วนตัว) .
...
หมวดหมู่มาจาก[NOAAระบบสํารวจมหาสมุทร (ไอโอเอส) ](https://ioos.noaa.gov/).
    ...
    * ถ้าคุณเพิ่มioos\\_categoryไปยังรายการ&lt;categoryAttributes&gt; ในERDDAP'[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)แฟ้ม, ผู้ใช้สามารถหาชุดข้อมูลที่มีข้อมูลคล้ายกันได้โดยง่ายERDDAP"ค้นหาข้อมูลตามหมวดหมู่" ในโฮมเพจ
        [ลองใช้ดูioos\\_categoryเพื่อค้นหาชุดข้อมูลที่น่าสนใจ](https://coastwatch.pfeg.noaa.gov/erddap/categorize/ioos_category/index.html?page=1&itemsPerPage=1000)
    * มี[การสนทนาเกี่ยวกับERDDAP™ถึงioos\\_categoryในERDDAP™กูเกิ้ลกรุ๊ป](https://groups.google.com/forum/#!topic/erddap/TnwbgzpSS0w)
...
คุณ อาจ ถูก ล่อ ใจ ให้ ทํา เช่น นั้น&lt;ตัวแปร Musst Haiscategry) เพื่อเท็จเพื่อให้คุณสมบัตินี้ไม่จําเป็น ("พัฟต์&#33; แล้วมันเกี่ยวอะไรกับฉัน?") เหตุ ผล บาง ประการ ที่ ทํา ให้ เรื่อง นี้ เป็น ความ จริง (ค่าปริยาย) และใช้ioos\\_categoryคือ:
    ...
    * หากตั้งค่า.xml ของ&lt;ตัวแปร Musest Haiscategory: ) ว่ากันว่า[สร้าง DatatasXml](#generatedatasetsxml)มักจะสร้าง/ chucksioos\\_categoryแต่ละตัวแปรในแต่ละชุดข้อมูล แล้วทําไมไม่ทิ้งมันไว้ข้างในล่ะ
    *   ERDDAP™ให้ผู้ใช้ทําการค้นหาข้อมูลต่าง ๆ ตามหมวดหมู่ioos\\_categoryเป็นหมวดหมู่การค้นหาที่มีประโยชน์มาก เนื่องจาก ious/_ positions (ยกตัวอย่างเช่น อุณหภูมิ) ก็ค่อนข้างกว้างนะ นี่ทําให้ioos\\_categoryดีสําหรับวัตถุประสงค์นี้มากกว่าตัวอย่างที่ดีมากขึ้น CFstandard\\_nameวินาที (ซึ่งมันไม่ดีต่อวัตถุประสงค์นี้ เนื่องจาก synonomys และการเปลี่ยนแปลงเล็กน้อย ตัวอย่างเช่น ทะเล/ surface/ tempority continuation to น้ําทะเล/ น้ํา/ ความจุ) .
(พ.ศ.ioos\\_categoryสําหรับวัตถุประสงค์นี้จะถูกควบคุมโดย&lt;categoryAttributes&gt; ในแฟ้มตั้งค่า.xml ของคุณ)
        [ลองใช้ดูioos\\_categoryเพื่อค้นหาชุดข้อมูลที่น่าสนใจ](https://coastwatch.pfeg.noaa.gov/erddap/categorize/ioos_category/index.html?page=1&itemsPerPage=1000)
    * ประเภทเหล่านี้มาจาก[NOAAระบบสํารวจมหาสมุทร (ไอโอเอส) ](https://ioos.noaa.gov/). หมวดหมู่เหล่านี้เป็นพื้นฐานในการอธิบายของ ไอโอส ภารกิจของ ไอโอส ถ้าคุณอยู่ในNOAAการสนับสนุนioos\\_categoryดี หนึ่ง...NOAAสิ่งที่ต้องทํา (ดูนี่[หนึ่งNOAAวิดีโอ](https://www.youtube.com/watch?v=nBnCsMYm2yQ)และได้รับแรงบันดาลใจ) ถ้าคุณอยู่ในบางหน่วยงานสหรัฐหรือต่างประเทศอื่น ๆ หรือทํางานร่วมกับหน่วยงานราชการ หรือทํางานกับระบบสังเกตมหาสมุทรอื่น ๆ ไม่ได้เป็นความคิดที่ดีที่จะให้ความร่วมมือกับสํานักงาน IOOS สหรัฐอเมริกา?
    * ไม่ ช้า ก็ เร็ว คุณ อาจ ต้องการ คน อื่นERDDAP™เพื่อเชื่อมโยงไปยังชุดข้อมูลต่าง ๆ ของคุณผ่านทาง[EDDGridจาก เอลด์ดาป](#eddfromerddap)ถึง[DDTable from Edardp](#eddfromerddap). ถ้าอีกคนERDDAP™ต้องการioos\\_categoryชุดข้อมูลของคุณต้องมีioos\\_categoryตามลําดับEDDGridจากเอ็ดแดปและอีดีทีเบิล จากเอเวอร์แดปไปทํางาน
...
*   [ **long\\_name** ](#long_name)  ([COARDS](https://ferret.pmel.noaa.gov/noaa_coop/coop_cdf_profile.html).[CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)ถึง[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) เป็นคุณลักษณะตัวแปรที่ซ้ํากันERDDAP. ยกตัวอย่างเช่น
    ```
        <att name="long\\_name">Eastward Sea Water Velocity</att>
    ```
    *   ERDDAP™ใช้long\\_nameสําหรับระบุแกนบนกราฟ
    * การปฏิบัติที่ดีที่สุด: ทําให้คําในlong\\_nameเสมือนหนึ่งพวกนางเป็นกรณีหนึ่ง (ใช้คําแรกเป็นตัวพิมพ์ใหญ่ และคําที่ไม่ใช่วงกลมทั้งหมด) . อย่ารวมหน่วยในlong\\_name. ชื่อยาวไม่ควรจะยาว (ปกติแล้วชื่อนี้มักจะยาว)&lt;20 ตัวอักษร) แต่ควรจะปรากฏมากกว่า[destinationName](#destinationname)ซึ่งมักจะมีความรวดเร็วมาก
    * ถ้าlong\\_name"ไม่ได้นิยามในตัวแปร[ผู้ส่งที่มา](#variable-addattributes)หรือ&lt;addAttributes&gt;ERDDAP™จะสร้างโดยทําความสะอาด[standard\\_name](#standard_name)  (ถ้าอยู่) หรือdestinationName.
         
...
* ถ้าข้อมูลถูกบรรจุด้วย[scale\\_factor&/ หรือadd\\_offset](#scale_factor).missing\\_valueควรบรรจุค่า_ fillvalue ไว้ด้วย เช่นเดียวกัน สําหรับคอลัมน์ที่มีค่าวันที่/ เวลาของสตริงที่ใช้ท้องถิ่น[time\\_zone](#time_zone).missing\\_valueและค่า_FullValue ควรจะใช้พื้นที่เวลาท้องถิ่น
* ถ้าตัวแปรใช้ค่าพิเศษนี้missing\\_valueและ/หรือ/หรือคุณสมบัติ//////N-FillValue is rectIED
* สําหรับ[ตัวแปรเวลาและเวลา](#time-units)  (แหล่งที่มาคือสตริงหรือตัวเลข) .missing\\_values และ _Fullvulues ปรากฎเป็น "" (ข้อความเปล่า) เมื่อเวลาที่ถูกเขียนเป็นสตริงและเป็น NaN เมื่อเวลาที่ถูกเขียนเป็นคู่ ค่าแหล่งสําหรับmissing\\_valueและ /_Fillvalue จะไม่ปรากฏอยู่ในข้อมูลกํากับภาพของตัวแปร
...
* สําหรับตัวแปรตัวเลข:
เดอะmissing\\_valueและ /_Fillvalue จะปรากฏในข้อมูลกํากับภาพของตัวแปร
สําหรับรูปแบบข้อมูลผลลัพธ์ERDDAP™จะปล่อยให้หมายเลขพิเศษนี้ยังคงที่ เช่น คุณจะเห็น -9999.
...
* หากตัวแปรจํานวนเต็มมีค่าหายไป (ตัวอย่างเช่น ตําแหน่งว่างในแฟ้ม .csv) .ERDDAP™จะตีความค่าเป็นนิยามmissing\\_valueหรือ\\_Fillvalue สําหรับตัวแปรนั้น ถ้าไม่มีคําจํากัดความERDDAP™จะแปลค่าเป็นค่าปริยายที่ขาดหายไปสําหรับชนิดของข้อมูลดังกล่าว ซึ่งเป็นค่าสูงสุดของค่าซึ่งสามารถเก็บได้โดยชนิดของข้อมูล:
...
*   [ADD \\_FillValue ATTRIBUTES?](#add-_fillvalue-attributes)  
แต่ละครั้งERDDAP™โหลดชุดข้อมูลมา, มันจะตรวจสอบว่าตัวแปรที่มีแหล่งข้อมูลจํานวนเต็มถูกนิยามไว้missing\\_valueหรือลักษณะเฉพาะของ _fillvalue ถ้าตัวแปรไม่,ERDDAP™พิมพ์จดหมายไปยังปูมบันทึก (เริ่มด้วย "Addd\\_failValue continuation?") แนะนําว่าERDDAP™ผู้ดูแลระบบทําการเพิ่ม\\_ fire แอททริบิวต์ค่าสําหรับตัวแปรนี้ในdatasets.xml. มันมีประโยชน์มากสําหรับทุกตัวแปรที่มี\\_FullValue หรือmissing\\_valueเนื่องจากค่าที่สูญหายเป็นไปได้เสมอ, เช่น, ถ้าแฟ้มที่มีอยู่ในชุดข้อมูลไม่มีตัวแปรERDDAP™ต้องสามารถนําเสนอตัวแปรนั้น ว่ามีค่าที่หายไปสําหรับตัวแปรนั้น หากคุณเลือกตัวแปรที่ไม่ควรมีแอตทริบิวต์_ fullValue คุณสามารถเพิ่ม
    &lt;ชื่อ AT= "/_FillValue"&lt;ซึ่งจะระงับข้อความสําหรับที่datasetIDการรวมกันของตัวแปรในอนาคต
...
* _FullValue และmissing\\_valueค่าต่าง ๆ ของตัวแปรต่าง ๆ ในแฟ้มต้นฉบับ จะต้องสม่ําเสมอ ไม่เช่นนั้นERDDAP™จะยอมรับแฟ้มที่มีชุดของค่า และปฏิเสธแฟ้มอื่น ๆ ทั้งหมดเป็น "แฟ้ม bad" เพื่อแก้ไขปัญหา
    * หากแฟ้มถูกสลับ.ncแฟ้ม, คุณสามารถใช้ได้[EDDGridแฟ้มจาก Nc](#eddgridfromncfilesunpacked).
    * ถ้าแฟ้มเป็นแฟ้มข้อมูลอักขระแท็บ คุณสามารถใช้ DEDTable from... files '[ขนาดมาตรฐาน อะไร](#standardizewhat)เพื่อบอกว่าERDDAPเพื่อกําหนดแฟ้มต้นฉบับตามที่อ่านเข้าไปERDDAP.
...
ตัว อย่าง เช่น แหล่ง ข้อมูล อาจ เก็บ ค่า ข้อมูล ลอย ไว้ โดย มี หลัก ทศนิยม หนึ่ง หลัก ที่ บรรจุ ไว้ ใน ช่อง สั้น (Int16) ใช้scale\\_factorเท่ากับ 0.1 และadd\\_offsetเช่น
...
ในตัวอย่างนี้ERDDAP™จะทําการแตกข้อมูลและนําเสนอไปยังผู้ใช้เป็นข้อมูลแบบลอย
    * ถ้าวันนี้ERDDAP™จะทําการแยกเอาค่าออกจากแอตทริบิวต์เหล่านี้, ลบแอตทริบิวต์ และดึงข้อมูลให้ผู้ใช้โดยอัตโนมัติ:
...
*   [ **standard\\_name** ](#standard_name)  (จาก[CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)มาตรฐานข้อมูลกํากับภาพ) เป็นคุณลักษณะตัวแปรที่ซ้ํากันERDDAP. CF คงรายการที่อนุญาต[ชื่อมาตรฐานของ CF](https://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table.html). ยกตัวอย่างเช่น
    ```
        <att name="standard\\_name">eastward\\_sea\\_water\\_velocity</att>
    ```
    * ถ้าคุณเพิ่มstandard\\_nameไปยังคุณสมบัติของตัวแปรและเพิ่มstandard\\_nameไปยังรายการ&lt;categoryAttributes&gt; ในERDDAP'[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)แฟ้ม, ผู้ใช้สามารถหาชุดข้อมูลที่มีข้อมูลคล้ายกันได้โดยง่ายERDDAP"ค้นหาข้อมูลตามหมวดหมู่" ในโฮมเพจ
...
*   time\\_precisionเป็นคุณสมบัติเฉพาะที่ใช้โดยERDDAP™  (และไม่มีมาตรฐานกํากับ) สําหรับ[ตัวแปรเวลาและเวลา](#time-units)ซึ่งอาจจะอยู่ในชุดข้อมูลแบบ gried หรือแท็บลูกชุด และในaxisVariables หรือdataVariableเอส. ยกตัวอย่างเช่น
    ```
...
* ที่นี่ เช่นเดียวกับในส่วนอื่น ๆERDDAP™( ราย ละเอียด ดู จาก วารสาร) ตัวอย่างเช่น 1985-07, 1985-07-01, 1985-07-01T00Z, 1985-07-07-01.00 น. และ 1985-07-01.00 น. Z ล้วนเทียบเท่ากัน แม้ว่าจะมีความแม่นยําต่างกัน นี่ตรงกับ[ISO 8601: 2004"extended"รูปแบบเวลากําหนด](https://www.iso.org/iso/date_and_time_format).
...
*   [ **หน่วย** ](#units)  ([COARDS](https://ferret.pmel.noaa.gov/noaa_coop/coop_cdf_profile.html).[CF](https://cfconventions.org/Data/cf-conventions/cf-conventions-1.8/cf-conventions.html)ถึง[หมายเลข ACD](https://wiki.esipfed.org/Attribute_Convention_for_Data_Discovery_1-3)มาตรฐานข้อมูลกํากับภาพ) กําหนดหน่วยของค่าข้อมูล ยกตัวอย่างเช่น
    ...
    * เราแนะนําให้คุณใช้หน่วยมาตรฐานหนึ่งหน่วย สําหรับชุดข้อมูลทั้งหมดในERDDAP. คุณควรจะบอกว่าERDDAP™มาตรฐานไหนที่คุณใช้อยู่&lt;หน่วย /_มาตรฐาน) ในเครื่องของคุณ[ตั้งค่า. xml](/docs/server-admin/deploy-install#setupxml)แฟ้ม
...
"deg/C" หรือ
"knots" กับ "m/s" คุณต้องหาวิธีมาตรฐานของหน่วย ไม่อย่างนั้นERDDAP™จะโหลดเฉพาะสับเซตของแฟ้มเท่านั้น ลองคิดดู ถ้าแฟ้มหนึ่งมีหน่วย windSpeed=knots และอีกตัวมีหน่วย windSpeed=m/s, แล้วค่าจากแฟ้มทั้งสองไม่ควรรวมอยู่ในชุดข้อมูลเดียวกัน
        * หากแฟ้มถูกสลับ.ncแฟ้ม, ในหลายสถานการณ์ที่คุณสามารถใช้ได้[EDDGridแฟ้มจาก Nc](#eddgridfromncfilesunpacked).
        * ถ้าแฟ้มเป็นแฟ้มข้อมูลแท็บมองล ในหลายสถานการณ์ คุณสามารถใช้ EDTable from... files '[ขนาดมาตรฐาน อะไร](#standardizewhat)เพื่อบอกว่าERDDAPเพื่อกําหนดแฟ้มต้นฉบับตามที่อ่านเข้าไปERDDAP.
        * สําหรับปัญหาที่ยากขึ้น, คุณสามารถใช้[ขนาด NcML](#ncml-files)หรือ[NCO](#netcdf-operators-nco)เพื่อแก้ไขปัญหา
    * CF ส่วนมาตรฐาน 8.1 กล่าวว่าถ้าข้อมูลตัวแปรถูกแพ็คผ่านทาง[scale\\_factor&/ หรือadd\\_offset](#scale_factor)"หน่วยของตัวแปร ควรเป็นตัวแทนของข้อมูลที่ถูกถอดออกมา"
...
คุณสามารถทดสอบERDDAPความสามารถในการจัดการกับบางอย่าง *หน่วย* ตั้งแต่ *เวลาพื้นฐาน* ด้วยERDDAP'[ตัวแปลงเวลา](https://coastwatch.pfeg.noaa.gov/erddap/convert/time.html). หวังว่าคุณคงใส่เลขลงไป (ค่าแรกจากแหล่งข้อมูล?) และสตริงหน่วย คลิกบนการแปลง และERDDAP™จะสามารถแปลงเป็น ISO 8601:2004 (E) ปรับเทียบเวลา ตัวแปลง จะคืนค่ากลับมาเป็นข้อความผิดพลาด หากข้อความของหน่วยยังไม่ปรากฏ
...
สําหรับรูปแบบเวลาอื่น ๆ ทั้งหมด คุณต้องระบุวันที่ที่เข้ากันได้กับรูปแบบเวลาที่ถูกต้อง เหมือนyyyy-MM-dd'T'HH:M:sZ, สตริงรูปแบบเหล่านี้ถูกสร้างขึ้นจากตัวอักษร ซึ่งระบุชนิดของข้อมูลเฉพาะจากสตริงเวลา, เช่น, m หมายถึง นาที-ชั่วโมง ถ้าคุณทําซ้ํารูปแบบตัวอักษรบาง ครั้ง มันจะปรับความหมายเพิ่มเติม เช่น m หมายถึง ค่านั้นอาจจะกําหนดโดยจํานวนตัวเลขใด ๆ mm หมายความว่า ค่านั้นจะต้องกําหนดโดย 2 หลัก เดอะJavaเอกสาร สําหรับ วัน เวลา นี่คือรายการของรูปแบบรูปแบบรูปแบบ และความหมายของมันภายในERDDAP™  (ซึ่งบางครั้งก็แตกต่างจากJavaวันและเวลา) .
    ...
    |---|---|---|
    |u, y, Y|–4712, 0, 1, 10, 100, 2018|จํานวนปี จํานวนตัวเลขERDDAP™ใช้ค่า y (ปี-รา) และ Y (หนึ่งสัปดาห์, เพราะนี่มักใช้ผิด) เป็น u,[จํานวนปีดาราศาสตร์](https://en.wikipedia.org/wiki/Astronomical_year_numbering). ปีดาราศาสตร์เป็นจํานวนเต็มบวกหรือลบที่ไม่ได้ใช้ BC (BC) หรือ CE (ค่า) 2=2 ซีซี, 1 = 1 ซีซี, 0 = 1BCE, -1=2BCE, -2=3BCE.|
    ...
    |N|0,00000000000000, 866999999999999.|นาโนวินาทีต่อวัน จํานวนตัวเลข ในERDDAP™นี่ถูกพันกับนิมิลลิส|
    |NNNNNNNNNNNNNNNN|8000000000000, 866999999999999.|นาโนวินาทีของวัน 14 หลัก ในERDDAP™มันถูกพันกับนิมิลลิส|
    |n|0,00000000, 5999999999|นาโนวินาที จํานวนตัวเลข ในERDDAP™มันถูกพันกับนิมิลลิส|
    |นินน์น์|100000000000, 599999999999|นาโนวินาที 11 หลัก ในERDDAP™มันถูกพันกับนิมิลลิส|
    ...
    |G, L, Q, c, V,z, O, p|     |อักขระการฟอร์แมตเหล่านี้ถูกรองรับโดยJavaวันและเวลา แต่ปัจจุบันยังไม่รองรับERDDAP. ถ้าคุณต้องการความช่วยเหลือ อีเมล์ชื่อคริส จอห์นที่โนอา โกฟ|
...
หรือ
    ...
    *   ERDDAP™ไม่สามารถใช้valid\\_range. พูดอีกอย่างคือERDDAP™ไม่แปลงค่าข้อมูลภายนอกvalid\\_rangeไปยัง / help ค่าหรือmissing\\_value.ERDDAP™แค่ส่งต่อข้อมูลกํากับนี้ และทิ้งโปรแกรมไว้ให้คุณ
ทําไม? นี่คือข้อมูลกํากับ ถ้าผู้ให้บริการข้อมูลต้องการ ผู้ให้บริการข้อมูลอาจจะแปลงค่าข้อมูลนอกvalid\\_rangeให้เป็น /N/FillvaluesERDDAP™ไม่ได้เดาจากผู้จัดหาข้อมูล วิธี นี้ ปลอด ภัย กว่า: ถ้า ภาย หลัง แสดง ว่าvalid\\_rangeมันแคบหรือไม่ถูกต้องERDDAP™จะไม่ลบข้อมูล
    * ถ้าข้อมูลถูกบรรจุด้วย[scale\\_factor&/ หรือadd\\_offset](#scale_factor).valid\\_range.valid\\_minถึงvalid\\_maxควรเป็นชนิดของข้อมูลและค่าของแพ็ค ตั้งแต่ERDDAP™ใช้ได้scale\\_factorถึงadd\\_offsetเมื่อมันโหลดข้อมูลERDDAP™จะรื้อถอนvalid\\_range.valid\\_minถึงvalid\\_maxตั้งค่าข้อมูลกํากับภาพปลายทาง (แสดงไปยังผู้ใช้) จะบอกชนิดของข้อมูลที่แตกและช่วง
หรือ ถ้าของที่ห่อมา\\_valid\\_rangeแอททริบิวต์มีอยู่, จะถูกเปลี่ยนชื่อvalid\\_rangeเมื่อERDDAP™ขนชุดข้อมูล
##### &lt;ลบรายการออก{#removemvrows} 
* [ ** &lt;เอาMVrows ออก&#33; ** ) (# ย้าย #) เป็นป้ายกํากับภายในแท็กdatasets.xmlสําหรับ DDTA จากแฟ้ม (รวมทั้งคลาสย่อยทั้งหมด) ชุดข้อมูล แม้ว่ามันจะถูกใช้เฉพาะกับ DEDTable from Multiidimc Files อาจ มี คุณค่า ของ ความ จริง หรือ ความ เท็จ. ตัวอย่างเช่น จริง
นี่ลบบล็อกของแถวใด ๆ ที่ปลายกลุ่มที่ค่าทั้งหมดmissing\\_value, / volvulue หรือ Cohert ... ค่าพื้นเมืองที่หายไป (หรือชาร์=#32 สําหรับชาอาเรย์) . นี่เป็นแฟ้มประเภท CF DSG แบบหลายมิติอาร์เรย์และแฟ้มที่คล้ายกัน ถ้าเป็นจริง นี้ไม่ทดสอบที่เหมาะสมและเพื่อให้เสมอโหลดตัวแปรสลัวสูงสุดเพื่อให้มันใช้เวลาพิเศษ
ค่าปริยายเป็นเท็จ
