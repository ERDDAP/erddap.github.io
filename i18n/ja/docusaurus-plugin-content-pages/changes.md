...
*    **お知らせ ERDDAP™ 管理者は知っておく必要があります。** 
    * 新機能、MQTT対応 詳細はこちら 読むことをお勧めします [新着情報](/docs/server-admin/mqtt-integration.md) MQTTメッセージからデータセットを作成したり、データセットの変更時にMQTTメッセージを公開したりすることができます。 デフォルトではオフなので、使用したい場合は有効にする必要があります。
...
お問い合わせ [セスシャンペンギンNRL](https://github.com/ERDDAP/erddap/pull/373) 
    ...
    * 安全であるべき最適化のコレクションがありますが、必要に応じて古い動作に戻すフラグがあります。 任意のフラグを設定する必要がある場合は、バグを提出してください。 これらのほとんどが将来的に新しい挙動のデフォルトで削除される問題がない場合。 お問い合わせ [機能フラグに関する新しいページ](/docs/server-admin/feature-flags.md) これらのその他のフラグについて読むことができます。
