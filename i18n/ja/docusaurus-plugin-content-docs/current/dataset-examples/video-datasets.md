...
このドキュメントでは、動画の表示方法を示します。 ERDDAP™ 、また
ビデオメタデータを検索するためのデータセットを作成する
選択した検索条件を満たす動画を見つけます。

この設定に必要な重要なコンポーネントは、最初に、ビデオ
Webブラウザで表示できる形式( ERDDAP™ それ自体
csv は、ビデオを表示せず、リンクのみを提供します。
ファイル (または寄木のようなテーブルベースのフォーマット) 含まれているもの
ビデオに関連するメタデータ。

適切なビデオフォーマットを持つキーです。 下の例では
もともとはWindowsブラウザでも古いウィンドウのフォーマットだった
ボックスから表示できず、プラグインを追加する必要があります。 古い投稿
フォーマットは非常に大きいファイルです。 ハンドブレーキアプリケーションは、
動画を変換するために使用されます。

## セットアップ

最初のステップは「EDDTableFromFileNames」のデータセットを作成することです。
各ビデオに関連したURLがあります。
お問い合わせ ERDDAP™ "files" コマンド。 このデータセットの例が与えられている間
以下では、動画のこのxmlを生成するための最良の方法は、
生成データセット Xml.sh とそれをビデオでディレクトリに置きます (in)
一般的な新しいデータセットを追加する最良の方法 ERDDAP™ 利用する
生成データセット そこに道のほとんどを得るXml.sh、
その後、最終結果の編集を行います。

2番目のステップは、EDDTableFromAsciiFilesを作成することです (または類似) 
ビデオで使用するメタデータを含むデータセット
検索する。 以下に例を示します。 この作業のために、一つ一つ必要
ビデオへのリンクを含む列、例えばそれが
変数「videoFile」。 この変数は、完全な URL を含むことができます。
最初のステップで作成されますが、ファイルがポータブルではありません。
1台の機械でテストし、別の機械で実際に役立つことを望みます
マシン。

"videoFile" のエントリを作ることでファイルをポータブルにすることができます
そのようなパスの下の例では、ビデオへの相対パスは次のとおりです。

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

すると、その名前を付ける「fileAccessBaseUrl」属性を追加できます。
列のパスへのURL。 例:
...
すると、サーバーを切り替えると、その1つの属性だけを変更する必要があります。
xmlスニペットは、csvファイルを編集する必要はありません。

完了したら、ファイルとしてアクセス可能なビデオのリストがあります。
...
「EDDTableFromFileNames」のデータセットによってセットアップされる簡単な検索:
...
最後の URL のデフォルトオプションは URL を生成します。

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black ロックフィッシュ](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 
...
応答の各エントリは、ビデオへのリンクを提供します。
最初にマウスをオンにすると、次のようになります。
...
html でクリックすると あなたのビデオディスプレイを表します
ブラウザ。
