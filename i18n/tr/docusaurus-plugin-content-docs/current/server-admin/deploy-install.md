...
ücretsiz ücretsiz ücretsiz ücretsiz (Bira ve konuşma gibi) versiyonu Java 25, Long Term Support (Uzun yıllar boyunca ücretsiz yükseltmeler, ilk sürüm) .
Güvenlik sebepleri için lütfen güncellemek ERDDAP 's version of' Java periyodik olarak yeni versiyonları olarak Java 25 Buyium'dan kullanılabilir.

    ERDDAP™ 25, diğer versiyonlarla kapsamlı olarak test edildi ve yaygın olarak kullanıldı. Çeşitli nedenlerle, başka versiyonlarını test etmiyoruz veya desteklemiyoruz Java .
     
## Tomcat{#tomcat} 
...
Dağıtım bir Windows installer ise, muhtemelen Tomcat'ı örnek olarak koyacak. `/Program Files /apache-tomcat-10.0.23` .
             
...
*  [server.xml](#serverxml) - In the `tomcat/conf /server.xml` Dosya, her iki değişiklik yapmanız gereken iki değişiklik var ` <Connector> ` etiketler
   ...
   1.  (Önerilen önerilen önerilen önerilen önerilen) Arttırmayı artırın `bağlantı bağlantısı Timeout` parametre değeri, belki 300000 (milisans, 5 dakika) .
   2.  (Önerilen önerilen önerilen önerilen önerilen) Yeni bir parametre ekleyin: `RahatQueryChars="[] | " " ""` . Bu Seçmeli ve biraz daha az güvenli,
...
* Linux bilgisayarlarda, Apache zamanout ayarlarını değiştirin, böylece zaman alıcı kullanıcı istekleri zaman zamanlamayın
  ...
  * Apache'yi Değiştirin ` http d.conf` Dosya dosyası (Genellikle genellikle içeride `/ http d/conf /` ) :
    * Mevcut değişikliği değiştir ` <Timeout> ` ayar ayar ayar ayarı ayar ayar ayar ayar ayarı ayar ayar ayar ayar ayar ayar ayarı (veya dosyanın sonunda bir tane ekleyin) 3600 (saniye saniye saniye saniye saniye) Ancak varsayılan 60 veya 120 saniye yerine.
...
Böylece, sadece süper kullanıcı kullanıcı kullanıcı olarak hareket etmeyi değiştirebilir `tomcat` . Bu, hackerların sunucunuza kullanıcı olarak giriş yapmak için imkansız hale getiriyor `tomcat` .
...
Bu dosya tarafından kullanılacak `tomcat/bin/startup.sh` ve `kapatma. sh` . Dosya gibi bir şey içermelidir:
  ...
  ```
   ...
   (Daha önce ayarlarsanız `JRE_HOME` Ama bunu kaldırabilirsiniz.) 
...
Eğer sunucunuz 2GB'den daha az hafızaya sahipse, azalır `-Xmx` değer değeri değeri ('M'egaBytes) Bilgisayarın fiziksel hafızasının 1/2'ine.
...
Ya da içinde `BüyükParentYönetmen /loglar / [Giriş.txt](/docs/server-admin/additional-information#log) ` Dosya dosyası ( `Büyük Parent Yönetmeny` belirtilmiştir. [Kurulum.xml](#setupxml) ) .
...
koleksiyon çünkü Eden Odadan çıktı (Bölüm Java Çok genç nesneler için) . Genellikle mesaj sizi gösterir
   `memoryUse before-&gt;memoryUse After-&gt;memoryUse After-&gt;` . Bu iki sayı birbirine yakınsa, çöp toplamanın üretken olmadığı anlamına gelir.
...
DejaVu fontlarını kullanmamayı tercih ederseniz, fontFamily kurulumda ayarlamanız gerekir.xml to use the DejaVu fonts, you need to change the fontFamily setting in installation.xml to order.xml to order.xml to use the DejaVu fonts ` <fontFamily> SansSerif </fontFamily> ` ,
...
Ve mevcut fontların listesini yazdıracak `Giriş.txt` Dosya. Bu fontlardan birini kullanmalısınız.

DejaVu fontlarını kullanmayı seçerseniz lütfen emin olun ` <fontFamily> ` Kurulumda ayarlanır.xml is setting in installation.xml is setting in setting.xml.xml is setting in setting.xml. ` <fontFamily> DejaVu Sans </fontFamily> ` .
...
Böyle bir şeyle bir çizgi geri dönmeli:
  ...
  ```
   ...
   (nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede nerede? `# # # #` Bazı sayısal) Bunun üzerine yemin ederim ki `java` süreç süreci (Muhtemelen Tomcat) "tcp" trafiği için port "80" üzerinde dinliyor.
...
4. Linux, Mac ve Windows, __download [Heddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.30.0/ERDDAP-2.30.0.war) __ into __ into `tomcat /webapps` :

__Version 2.30.0, 706,939,121 bytes, MD5=CDC4B3D82A20B33A6623B85312F6DC21, 2026-04-06__
...
Eğer eğer ERDDAP™ Diğer bir yolda servis edilir `/erddap` Ayrıca set the `X-Forwarded-Prefix` Başlara
...
Bir dakika veya iki dakika sürebilir ERDDAP™ Tamamen kapat. Sabırlı olun. Ya da kendi başına durmayacak gibi görünüyorsa, kullanın:
   `Kill -9 <processID> ` 
* Tomcat'a başlayın (Linux veya Mac OS)   `tomcat/bin/startup.sh` veya (Windows'da)   `tomcat\bin'in başlangıcı.bat` 
...
Tomcat log dosyalarında ( `tomcat/logs/catalina.out` Ve bu dizideki diğer dosyalar) .
...
Eğer öyleyse `Giriş.txt` Dosya eskidir, bir işarettir ki ERDDAP™ Son zamanlarda yeniden başlamadı. ERDDAP™ Bir buffer için log bilgileri yazın
...
içine ` datasets.xml ` Ve onu düzenler.) Örneğin, yapabilirsiniz:
...
See See See See See https://en.wikipedia.org/wiki/Favicon.
