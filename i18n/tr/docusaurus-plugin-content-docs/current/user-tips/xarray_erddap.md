...
Bunun bir hata yapmasının nedeni ‘xarray’ kullanımıdır. OPeNDAP   ( https://www.opendap.org ) Uzak erişim için protokolü olarak ve iken ERDDAP™ Gazetecilik, sözcülüğe dayanmaktadır OPeNDAP Kelimeler ve bir antropolog ERDDAP™ sunucusu da bir an olarak hareket edebilir OPeNDAP sunucu, bu iki hizmet için nasıl yapıldığı konusunda farklılıklar var. (Örneğin bakınız https://coastwatch.pfeg.noaa.gov/erddap/griddap/documentation.html#opendapLibraries ) .
