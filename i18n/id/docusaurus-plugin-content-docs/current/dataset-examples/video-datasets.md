...
# Mencari dan Menampilkan Video di ERDDAP™ 

Dokumen ini menunjukkan cara menampilkan video di ERDDAP™ Sitemap
membuat dataset yang menyediakan untuk mencari metadata video untuk
menemukan video yang memenuhi kriteria pencarian yang dipilih.

Komponen penting yang diperlukan untuk mengatur up ini pertama, video yang
dalam format yang dapat ditampilkan di browser web ( ERDDAP™ Login
tidak menampilkan video, hanya menyediakan tautan), dan kedua, csv
Login (atau format berbasis tabel serupa seperti parket) yang mengandung
metadata relevan dengan video.

Memiliki format video yang sesuai adalah kunci. Dalam contoh di bawah
video awalnya adalah format jendela lama yang bahkan browser Windows
tidak dapat menampilkan dari kotak, dan diperlukan menambahkan plugin. Login
format sering adalah file yang sangat besar. Aplikasi Handbrake dapat
digunakan untuk mengkonversi video, yang apa yang dilakukan.

## Login

Langkah pertama adalah untuk membuat "EDDTableDariFileNames" dataset sehingga
ada URL yang terkait dengan setiap video yang dapat diakses melalui
Login ERDDAP™ "file" perintah. Sementara contoh dataset ini diberikan
di bawah ini, cara terbaik untuk menghasilkan xml ini untuk video Anda adalah untuk digunakan
Login Xml.sh dan titik ke direktori dengan video (dalam
umum cara terbaik untuk menambahkan dataset baru untuk ERDDAP™ Sitemap
Login Xml.sh yang akan mendapatkan Anda sebagian besar jalan di sana, dan
kemudian mengedit bahwa untuk hasil akhir).

Langkah kedua adalah untuk membuat EDDTableDariAsciiFiles (atau serupa) 
dataset yang mengandung metadata yang akan digunakan dalam video
Sitemap Contoh diberikan di bawah ini. Untuk pekerjaan ini harus ada satu Meme it
kolom yang berisi link ke video, dalam contoh itu adalah
variabel "File". Variabel ini hanya dapat berisi URL penuh
dibuat dalam langkah pertama, tetapi kemudian file tidak portabel, seperti jika
Anda ingin menguji satu mesin dan kemudian benar-benar melayani di lain
mesin.

Anda dapat membuat file portabel dengan membuat entri di "videoFile" menjadi
jalan relatif ke video, dalam contoh di bawah satu jalan tersebut adalah:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Kemudian Anda dapat menambahkan atribut “fileAccessBaseUrl” yang menandakan
URL ke jalur di kolom. Dalam contoh:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Kemudian jika Anda beralih server Anda hanya perlu mengubah atribut satu di
snippet xml, Anda tidak perlu mengedit file csv.

Setelah selesai, Anda memiliki daftar video yang dapat diakses sebagai file:
...
pencarian sederhana yang ditetapkan oleh "EDDTableDariFileNames" dataset:
...
Opsi default di URL terakhir menghasilkan URL:

 [ https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time,CommonName,latitude,longitude,depth,videoFile&time%3E%3D2022-09-06&CommonName=“black Login](https://coastwatch.pfeg.noaa.gov/erddap/tabledap/fed_HAGE_VisualSurvey_Query.htmlTable?time%2CCommonName%2Clatitude%2Clongitude%2Cdepth%2CvideoFile%26time%3C%3D2022-09-06%26CommonName%3D%E2%80%9Cblack%20rockfish%E2%80%9D) 

dan itu menghasilkan meja dengan kolom terakhir:
...
Setiap entri dalam respons menyediakan tautan ke video, seperti
jika Anda mouse atas yang pertama yang Anda dapatkan:
...
dan jika Anda klik di html Tabel tampilan video di Anda
Login
