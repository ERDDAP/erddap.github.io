...
1.  [Untuk ERDDAP™ v2.29.0 +, set up Java 25.](#java) 
Untuk alasan keamanan, hampir selalu terbaik untuk menggunakan versi terbaru Java 25.
Silakan unduh dan instal versi terbaru dari
    [Adoptium 's OpenJDK (Temarin) 25 (LTS) ](https://adoptium.net/temurin/releases/?version=25) .
Untuk memverifikasi instalasi, jalankan `/ javajreBinDirectory / java -versi` , misalnya
    `/ usr / local / jdk-25.0.1 + 8 / jre / bin / java -versi` .
...
bebas (seperti dalam bir dan pidato) versi Java 25 yang menawarkan Dukungan Jangka Panjang (upgrade bebas selama bertahun-tahun melewati rilis awal) .
Untuk alasan keamanan, tolong perbarui ERDDAP versi dari Java berkala sebagai versi baru dari Java 25 tersedia dari Adoptium.

    ERDDAP™ telah diuji dan digunakan secara ekstensif dengan 25, bukan versi lain. Untuk berbagai alasan, kita tidak menguji dengan atau mendukung versi lain dari Java .
     
## Tomcat{#tomcat} 
...
yang dirancang untuk bekerja dengan Java 25 atau lebih baru. Di bawah ini, direktori Tomcat akan disebut sebagai `tomcat` .
...
Kami menyarankan membongkar dalam `/ usr / local` .
     * Pada Mac, Tomcat mungkin sudah dipasang di `/ Pustaka / Tomcat` , tapi harus memperbarui ke versi terbaru Tomcat 10.
Jika kau mengunduhnya, [unduh "Core" "tar .gz "Sumbangan Tomcat](https://tomcat.apache.org/download-10.cgi) Dan membongkar dalam `/ Pustaka / Tomcat` .
     * Pada Windows, Anda dapat [unduh distribusi "Core" "zip" Tomcat](https://tomcat.apache.org/download-10.cgi) 
        ...
        (Untuk pengembangan, kami menggunakan distribusi "Core" "zip". Kami membuat `/ program` direktori dan membongkar di sana.) 
Atau Anda dapat mengunduh distribusi "Core" "64-bit Windows zip", yang memuat lebih banyak fitur.
Jika distribusi adalah Windows installer, mungkin akan menempatkan Tomcat, misalnya, `/ Program Berkas / apache -tomcat- 10.0.23` .
             
### server.xml{#serverxml} 

*  [server.xml](#serverxml) - Di `tomcat / conf / server.xml` berkas, ada dua perubahan yang harus Anda buat untuk masing-masing dari dua ` <Connector> ` tag
   (satu untuk `Port &lt; Konektor = "8080"` dan satu untuk `Port &lt; Conektor = "8443"` ) .
   1.  (Disarankan) Meningkatkan `koneksi Waktu habis` nilai parameter, mungkin untuk 300.000 (milidetik, yaitu 5 menit) .
   2.  (Disarankan) Tambah parameter baru: `relaxed QueryChars = "[] | "` . Ini opsional dan sedikit kurang aman,
...
* kontekt.xml -- Cache Sumber Daya-Masuk `tomcat / conf / context.xml` , Tepat sebelum ` </Context> ` tag, ubah tag Sumber Daya
...
* Pada komputer Linux, ubah pengaturan waktu habis Apache sehingga waktu memakan permintaan pengguna tidak habis waktu
  ...
  * Ubah Apache ` http d.conf` berkas (biasanya masuk `/ dll / http d / conf /` ) :
    * Mengubah yang ada ` <Timeout> ` pengaturan (atau tambahkan satu di akhir berkas) ke 3600 (detik) , daripada baku 60 atau 120 detik.
  ...
  * Memulai ulang Apache: `/ usr / sbin / apachectl -k anggun`   (tetapi kadang-kadang dalam direktori yang berbeda) .
...
instalasi Tomcat Anda, terutama untuk server publik.
         
* Untuk publik ERDDAP™ instalasi pada Linux dan Mac, adalah yang terbaik untuk mengatur Tomcat (program) sebagai milik pengguna `tomcat` 
...
Dengan demikian, hanya pengguna super yang dapat menjadi pengguna `tomcat` . Hal ini membuat mustahil bagi hacker untuk masuk ke server Anda sebagai pengguna `tomcat` .
Dan dalam hal apapun, Anda harus membuatnya sehingga `tomcat` pengguna memiliki hak akses yang sangat terbatas pada sistem berkas server (baca + write + executive executive
untuk `apache-tomcat` pohon direktori dan ` <bigParentDirectory> ` dan hak baca-saja untuk direktori dengan data yang ERDDAP™ membutuhkan akses ke).
  * Anda dapat membuat `tomcat` akun pengguna (yang tidak memiliki password) dengan menggunakan perintah:
    ```
    sudo useradd tomcat -s /bin/bash -p '*'
    ```
  * Anda dapat beralih ke bekerja sebagai pengguna `tomcat` dengan menggunakan perintah
    ```
    sudo su - tomcat
    ```
    ...
    * Anda dapat berhenti bekerja sebagai tomcat pengguna dengan menggunakan perintah
    ...
    exit
    ````
    * Apakah sebagian besar dari sisa Tomcat dan ERDDAP™ setup instruksi sebagai pengguna `tomcat` . Kemudian, jalankan `startup.sh` dan `Matikan. sh` skrip sebagai pengguna `tomcat` 
...
* Pada Linux dan Mac:
Membuat berkas `tomcat / bin / setenv.sh`   (atau di Red Hat Enterprise Linux \\[ RHEL \\] , edit `~ tomcat / conf / tomcat10.conf` ) untuk mengatur variabel lingkungan Tomcat.
Berkas ini akan digunakan oleh `tomcat / bin / startup.sh` dan `Matikan. sh` . Berkas harus berisi sesuatu seperti:
  ```
  ...
  ```
   ...
   (Jika Anda sebelumnya diset `JRE _ HOME` , Anda dapat menghapus itu.) 
Pada Mac, Anda mungkin tidak perlu mengatur `JAVA _ HOME` .

* Pada Windows:
Membuat berkas `tomcat\\ bin\\ setenv.bat` untuk mengatur variabel lingkungan Tomcat.
...
The `-Xmx` dan `-Xms` pengaturan memori sangat penting karena ERDDAP™ Bekerja lebih baik dengan lebih banyak memori.
Selalu diset `-Xms` ke nilai yang sama seperti `-Xmx` .
...
Jika server Anda memiliki kurang dari 2GB memori, mengurangi `-Xmx` nilai (dalam 'M' egaBytes) sampai 1 / 2 memori fisik komputer.
...
Dengan 64 bit Java , Tomcat dan Java dapat menggunakan sangat tinggi `-Xmx` dan `-Xms` pengaturan. Semakin banyak memori fisik di server semakin baik.
Sebagai saran sederhana: kami sarankan Anda mengatur `-Xmx` dan `-Xms` ke (dalam 'M' egaBytes) ke 1 / 2 (atau kurang) memori fisik komputer.
...
atau di `big ParentDirektori / log / [log.txt](/docs/server-admin/additional-information#log) ` berkas ( `Larang Direktori` ditentukan pada [setup.xml](#setupxml) ) .
...
koleksi karena kehabisan ruang di Eden (bagian dari Java stack untuk sangat muda objek) . Biasanya pesan menunjukkan Anda
   `remember UseBefore- &gt; remember UseAfter` . Jika kedua angka itu berdekatan, artinya koleksi sampah tidak produktif.
...
* Jika Anda melihat `Java.lang.OutOf MemoryError` s in ERDDAP™ ' [log.txt](/docs/server-admin/additional-information#log) berkas,
...
*  [Pada Linux dan Mac, ubah perizinan](#permissions) dari semua `* .sh` berkas dalam `tomcat / bin /` untuk dieksekusi oleh pemilik:
  ```
  chmod +x *.sh
  ```
...
Jika Anda memilih untuk tidak menggunakan fonta DejaVu, Anda perlu mengubah pengaturan keluarga dalam semp.xml ke ` <fontFamily> SansSerif </fontFamily> ` ,
yang tersedia dengan semua Java Penyebaran. Jika Anda mengatur ` <fontFamily> ` dengan nama fonta yang tidak tersedia, ERDDAP™ tidak akan memuat
dan akan mencetak daftar fonta yang tersedia di `log.txt` file. Anda harus menggunakan salah satu font.

Jika Anda memilih untuk menggunakan font DejaVu, pastikan ` <fontFamily> ` konfigurasi dalam setup.xml adalah ` <fontFamily> DejaVu Sans Name </fontFamily> ` .
...
dan lepaskan berkas fonta ke direktori sementara.
  ...
  * Pada Linux:
    ...
    * Dengan yang lain Java distribusi: Sebagai `tomcat` pengguna, salin berkas fonta kedalam `$JAVA _ HOME / lib / font` jadi Java dapat menemukan font.
...
Itu harus kembali satu baris dengan sesuatu seperti:
  ...
  ```
   ...
   (dimana `#` adalah beberapa digit) , menunjukkan bahwa `java` proses (Mungkin Tomcat) adalah mendengarkan di pelabuhan "8080" untuk "tcp" lalu lintas.
...
* Lihat berkas log Tomcat `tomcat / log / catalina.out` . Tomcat masalah dan beberapa ERDDAP™ Masalah awal hampir selalu ditunjukkan di sana.
...
3.   [Mengatur `tomcat / content / erddap` berkas konfigurasi.](#erddap-content) 
Pada Linux, Mac, dan Windows, unduh [erddapContent .zip ](https://github.com/ERDDAP/erddapContent/releases/download/content1.0.1/erddapContent.zip) 
dan membukanya ke dalam `tomcat` direktori, membuat `tomcat / content / erddap` .

_ Versi 1.0.1, 20683 bytes, MD5 = 989E767459fe35e9c967b5, tanggal 20259-06-02 _
...
unzip `erddapContent .zip ` ke direktori yang diinginkan (ke mana hanya `tomcat` pengguna memiliki akses) dan mengatur ` erddapContentDirectory ` properti sistem
 (mis. ` erddapContentDirectory  =~tomcat/content/erddap ` ) jadi ERDDAP™ dapat menemukan direktori isi baru ini.

### setup.xml{#setupxml} 

*  [Baca komentar di `tomcat / content / erddap / setup.xml` ](#setupxml) dan membuat perubahan yang diminta. setup.xml adalah berkas dengan semua pengaturan yang menentukan bagaimana Anda ERDDAP™ Berperilaku.
...
Ketika Anda membuat Big ParentDirectory, dari direktori induk dari BigParentDirektori:
    ...
    * Membuat `tomcat` pengguna pemilik dari `Larang Direktori` :
      ```
...
untuk pertama kalinya (biasanya hanya dengan data baku) , kamu akan memodifikasi XML `tomcat / content / erddap / datasets.xml ` 
...
*  (Tidak mungkin) Sekarang atau (sedikit lebih mungkin) di masa depan, jika anda ingin memodifikasi berkas CSS erddap, salin
   `tomcat / content / erddap / image / erdlapStart2.css` ke `tomcat / content / erddap / image / erddap2.css` dan kemudian membuat perubahan untuk itu.
Perubahan ke `erddap2.css` hanya mengambil efek ketika ERDDAP™ dijalankan ulang dan sering juga memerlukan pengguna untuk menghapus berkas cache peramban.
     
...
Beberapa versi sebelumnya juga tersedia.
   ...
   *  [2.28.1](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war)   (622.676,238 bytes, MD5 = 48b422265f950c88c 69E9221b9bc9, tanggal 20252260609-05) 
   *  [2.29,0](https://github.com/ERDDAP/erddap/releases/download/v2.29.0/erddap.war)   (706,788.135 bytes, MD5 = A5EDCC8D46C27640EB8CE48560, tanggal 2025- 12- 15) 
...
Jika ERDDAP™ dilayani di jalan lain selain `/ erddap` , juga mengatur `X- Forwarded- Prefix` tajuk ke
...
3. Kemudian restart Apache: `/ usr / sbin / apachectl -k anggun`   (tetapi kadang-kadang dalam direktori yang berbeda) .
         
#### NGINX{#nginx} 
...
Untuk mendapatkan NGINX dan ERDDAP™ bekerja dengan benar https , Anda perlu menempatkan snippet berikut di dalam server.xml Tomcat ` <Host> ` blok:
```
...
* Jika Tomcat sudah berjalan, menutup Tomcat dengan (dalam Linux atau Mac OS)   `tomcat / bin / shutdown.sh` 
atau (dalam Windows)   `tomcat\\ bin\\ shutdown.bat ` 

Pada Linux, gunakan `ps -ef | grep tomcat` sebelum dan sesudah `Matikan. sh` untuk memastikan proses tomcat telah berhenti.
...
Gunakan peramban untuk mencoba melihat http://www.YourServer.org/erddap/status.html.
...
dan pesan kesalahan dari ERDDAP , di ERDDAP™   ` <bigParentDirectory> / log / log.txt` file.
...
* Kapan ERDDAP™ start up, it renames the old ERDDAP™ berkas log.txt ( `Log ArchivedAt <CurrentTime> .txt` ) dan membuat berkas log.txt baru.
Jadi jika `log.txt` berkas sudah tua, itu adalah tanda bahwa ERDDAP™ tidak baru-baru ini dimulai kembali. ERDDAP™ menulis info log ke buffer
...
Tomcat harus melakukan banyak pekerjaan pertama kalinya aplikasi seperti ERDDAP™ dimulai; terutama, harus membongkar `erddap.war` berkas
...
ke ` datasets.xml ` dan mengeditnya.) Sebagai contoh, Anda bisa:
...
* Mengubah informasi pada sisi kiri halaman rumah dengan mengedit ` <theShortDescriptionHtml> ` tag dalam Anda ` datasets.xml ` file.
(Jika tidak ada, salin baku dari ERDDAP™ ' `tomcat / webapps / erddap / WEB-INF / classes / gov / noaa / pfel / erddap / util / messages.xml` berkas
ke ` datasets.xml ` dan mengeditnya.) Sebagai contoh, Anda bisa:
  * Jelaskan apa yang organisasi Anda dan / atau kelompok tidak.
  * Jelaskan jenis data apa ini ERDDAP™ telah.
  * Untuk mengubah ikon yang muncul pada tab peramban, letakkan favicon organisasi Anda. Meksiko di `tomcat / content / erddap / image /` .
Lihat https://en.wikipedia.org/wiki/Favicon.
