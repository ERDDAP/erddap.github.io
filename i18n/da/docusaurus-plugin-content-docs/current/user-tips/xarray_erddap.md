...
The The The The The The The Python pakke 'xarray' er blevet meget populær til at få adgang til, undersetting og visualisere gitterde data i en række formater. 'xarray' arbejder fint med ERDDAP™ Når du forstår, hvordan du bruger det ordentligt. Jeg ville påpege, at det Python pakke 'erddapy '' ( https://github.com/ioos/erddapy ) kan få adgang til data fra ERDDAP™ servere ved hjælp af både 'griddap' og ' tabledap ', mens 'xarray' er begrænset til gitterded data, og 'erddapy' kan eksportere dataene til 'xarray'. Men hvis du er vant til at bruge "xarray" og har arbejdsgange ved hjælp af pakken, så kan det være ønskeligt at bare arbejde inden for den enkelte pakke.
...
Årsagen til, at "xarray" bruger OPeNDAP   ( https://www.opendap.org ) som dens protokol til fjernadgang, og mens protokollen ERDDAP™ Syntaksen er baseret på OPeNDAP Syntaksen og en ERDDAP™ serveren kan også fungere som en OPeNDAP server, der er forskelle i, hvordan dette sker for de to tjenester. (Se for eksempel https://coastwatch.pfeg.noaa.gov/erddap/griddap/documentation.html#opendapLibraries ) .
