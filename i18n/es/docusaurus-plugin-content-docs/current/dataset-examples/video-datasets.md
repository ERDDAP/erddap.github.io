...
# Búsqueda y Mostrando videos en ERDDAP™ 

Este documento muestra cómo mostrar vídeos en ERDDAP™ , así como
crear un conjunto de datos que permita buscar los metadatos de vídeo
encontrar los videos que cumplen los criterios de búsqueda seleccionados.

Los componentes esenciales necesarios para configurarlo son los primeros vídeos que
están en un formato que se puede mostrar en un navegador web ( ERDDAP™ en sí misma
no muestra el vídeo, sólo proporciona un enlace), y segundo, un csv
archivo (o un formato similar basado en tablas como el parquet) que contienen
metadatos relevantes para los videos.

Tener un formato de vídeo adecuado es clave. En el ejemplo a continuación
videos originalmente eran un formato antiguo de ventana que incluso los navegadores de Windows
no podía mostrar fuera de la caja, y requería añadir un plugin. Viejo
formatos a menudo son archivos muy grandes. La aplicación Handbrake puede ser
utilizado para convertir los videos, que es lo que se hizo.

## Configuración

El primer paso es crear un conjunto de datos “EDDTableDesdeFileNames” para que
hay una URL asociada con cada vídeo que es accesible
el ERDDAP™ comando “files”. Si bien se da un ejemplo de este conjunto de datos
abajo, la mejor manera de generar este xml para tus vídeos es usar
GenerarDatasets Xml.sh y señalarlo al directorio con los vídeos (en
general la mejor manera de añadir nuevos conjuntos de datos a ERDDAP™ es utilizar
GenerarDatasets Xml.sh que te llevará la mayor parte del camino allí, y
entonces editar eso para el resultado final).

El segundo paso es crear un EDDTableDesdeAsciiFiles (o similares) 
dataset que contiene los metadatos que se utilizarán en el vídeo
búsqueda. A continuación se presenta un ejemplo. Para que esto funcione debe haber uno
columna que contiene un enlace al vídeo, en el ejemplo es el
variable “videoFile”. Esta variable sólo puede contener la URL completa
creado en el primer paso, pero entonces el archivo no es portátil, como si
quieres probar en una máquina y luego servir en otra
máquina.

Usted puede hacer el archivo portátil haciendo la entrada en “videoFile” para ser
el camino relativo al video, en el ejemplo debajo de uno tal camino es:

2010_Piggy_Bank_audio_stripped/0001_20100627_144631.mp4

Luego puede agregar el atributo “fileAccessBaseUrl” que anexa eso
URL al camino en la columna. En el ejemplo:

```
<att name="fileAccessBaseUrl"><https://coastwatch.pfeg.noaa.gov/erddap/files/fed_HAGE_VisualSurvey/></att>
```

Entonces si cambia los servidores sólo necesita cambiar ese atributo en
el snippet xml, usted no necesita para editar el archivo csv.

Una vez hecho, tiene una lista de los vídeos accesibles como archivos:
...
una simple búsqueda establecida por el conjunto de datos “EDDTableDesdeFileNames”:
...
Las opciones predeterminadas en la última URL producen la URL:
...
y que produce una tabla con la última columna:
...
Cada una de las entradas en la respuesta proporciona un enlace a un vídeo, como
si usted ratón sobre el primero usted consigue:
...
y si hace clic en él en el html Mesa las pantallas de vídeo en su
Navegador.
