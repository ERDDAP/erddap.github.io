...
*  [server.xml](#serverxml) - En el archivo `tomcat/conf/server.xml`, hay dos cambios que usted debe hacer a cada uno de los dos ` <Connector> Etiquetas
   (uno para ` port connector='80'' y otro para ``) .
...
* En las computadoras Linux, cambia la configuración de tiempo de Apache para que las solicitudes de usuario que consumen tiempo no tengan tiempo
  ...
  * Modificar el Apache http d. archivo (generalmente en `/etc/ http d/conf/ `) :
    * Cambio de la <Timeout> ` (o añadir uno al final del archivo) a 3600 (segundos) , en lugar de los 60 o 120 segundos predeterminados.
    * Cambio de la <ProxyTimeout> ` (o añadir uno al final del archivo) a 3600 (segundos) , en lugar de los 60 o 120 segundos predeterminados.
...
para el árbol del directorio 'apache-tomcat' y ` <bigParentDirectory> &quot; y privilegios sólo leídos para los directorios con datos que ERDDAP™ necesidad de acceso a).
...
Con 32 bits Java , incluso con abundante memoria física, Tomcat y Java no funcionará si tratas de poner `-Xmx` mucho por encima de 1500M (1200M en algunas computadoras) .
...
que está disponible con todos Java distribuciones. Si pones <fontFamily> ` al nombre de una fuente que no está disponible, ERDDAP™ no cargará
e imprimirá una lista de fuentes disponibles en el archivo `log.txt. Debe utilizar una de esas fuentes.

Si eliges usar las fuentes DejaVu, asegúrate de: <fontFamily> ` setting in setup.xml is ` <fontFamily> DejaVu Sans </fontFamily> `.
...
__Versión 1.0.0, 20333 bytes, MD5=2B8D2A5AE5ED73E3A42B529C168C60B5, fechada 2024-10-14__
...
Cuando creas el directorio de BigParent, desde el directorio principal de BigParentDirectory:
    ...
    * Hacer que el usuario 'tomcat' sea el propietario de la 'bigParentDirectory':
      ```
...
llamado ERDDAP _valueName` antes de correr ERDDAP™ . Por ejemplo, use ` ERDDAP _baseUrl` invalida el ` <baseUrl> Valor.
...
4. En Linux, Mac y Windows, __download [Erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.0/erddap.war) __ into `tomcat/webapps`:

__Version 2.28.0, 620,824,288 bytes, MD5=f948b2ba603f65a83ac67af43da9e4c2, de fecha 2025-08-29__
...
Utilice un navegador para intentar verhttp://www.YourServer.org/erddap/status.html.
...
* Cuando ERDDAP™ comienza, renombra el viejo ERDDAP™ log.txt file (Archivado At <CurrentTime> .txt) y crea un nuevo archivo log.txt.
Así que si el archivo `log.txt` es viejo, es un signo que ERDDAP™ no ha vuelto a empezar. ERDDAP™ escribe información de registro a un buffer
