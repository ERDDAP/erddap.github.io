...
* Su computer Linux, modificare le impostazioni di timeout di Apache in modo che le richieste degli utenti che richiedono tempo non timeout
  ...
  * Modificare il Apache ` http file d.conf` (di solito in `/etc/ http d/conf/ #) :
    * Cambiare il ` esistente <Timeout> Impostazione (o aggiungere uno alla fine del file) a 3600 (secondi) , invece dei 60 o 120 secondi di default.
    * Cambiare il ` esistente <ProxyTimeout> Impostazione (o aggiungere uno alla fine del file) a 3600 (secondi) , invece dei 60 o 120 secondi di default.
...
`memoryUseBefore-&gt;memoryUseAfter`. Se quei due numeri sono vicini insieme, significa che la raccolta di rifiuti non era produttiva.
...
e deselezionare i file del carattere in una directory temporanea.
  ...
  * Su Linux:
    ...
    * Con altri Java distribuzioni: Come utente `tomcat`, copiare i file del carattere in `$JAVA_HOME/lib/fonts` in modo Java può trovare i caratteri.
...
e disinserirlo nella directory `tomcat`, creando `tomcat/content/erddap`.
...
4. Su Linux, Mac e Windows, __download [erddap.war](https://github.com/ERDDAP/erddap/releases/download/v2.28.1/erddap.war) __ in `tomcat/webapps`:
...
2. Modificare il ` esistente <VirtualHost> Articolo (se c'è uno) , o aggiungere uno alla fine del file:
```
<VirtualHost *:80>
...
(Se non ce n'è uno, copia il default da ERDDAP™ 's `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` file
...
* Modificare le informazioni sul lato sinistro della pagina iniziale modificando il ` <theShortDescriptionHtml> ` tag nel tuo ` datasets.xml File.
(Se non ce n'è uno, copia il default da ERDDAP™ 's `tomcat/webapps/erddap/WEB-INF/classes/gov/noaa/pfel/erddap/util/messages.xml` file
in ` datasets.xml ` e modificarlo.) Per esempio, si potrebbe:
